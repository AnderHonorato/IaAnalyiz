...
import { 
  ...
  X, Send, Paperclip, Image as ImageIcon, Film, Sparkles, MessageSquare, Minus, ShoppingBag, ShoppingCart
} from 'lucide-react';
...
export default function App() {
  ...
  return (
    <div className="h-screen w-full bg-[#f8fafc] flex flex-col font-sans text-slate-900 overflow-hidden relative" style={{ zoom: zoom }}>
      ...
      {/* --- BARRA SUPERIOR DE MARKETPLACES --- */}
      <div className="bg-slate-950 border-b border-slate-900 px-6 py-2.5 flex items-center gap-6 text-[10px] font-black uppercase tracking-[0.2em] shrink-0 z-30">
        <span className="text-slate-500 mr-2">Plataforma:</span>
        <button onClick={() => setActiveMarketplace('ml')} className={`flex items-center gap-1.5 transition-colors ${activeMarketplace === 'ml' ? 'text-[#FFE600]' : 'text-slate-400 hover:text-white'}`}>
          <ShoppingBag className="w-3.5 h-3.5" /> Mercado Livre
        </button>
        <button onClick={() => setActiveMarketplace('shopee')} className={`flex items-center gap-1.5 transition-colors ${activeMarketplace === 'shopee' ? 'text-[#EE4D2D]' : 'text-slate-400 hover:text-white'}`}>
          <ShoppingCart className="w-3.5 h-3.5" /> Shopee
        </button>
        <button onClick={() => setActiveMarketplace('amazon')} className={`flex items-center gap-1.5 transition-colors ${activeMarketplace === 'amazon' ? 'text-[#FF9900]' : 'text-slate-400 hover:text-white'}`}>
          <Box className="w-3.5 h-3.5" /> Amazon
        </button>
      </div>
      ...
      {/* --- IA CHAT (IA Analyiz) COM CSS DE ALTURA CORRIGIDO --- */}
