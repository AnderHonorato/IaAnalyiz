...
export default function App() {
  ...
  // --- ESTADOS DO MENU DROPDOWN (LÓGICA MAINLAYOUT) ---
  const [isPlatformMenuOpen, setIsPlatformMenuOpen] = useState(false);
  const platformMenuRef = useRef(null);
  const platformHoverTimer = useRef(null);
  ...
  // Funções de Hover do Menu
  const handlePlatformEnter = () => {
    clearTimeout(platformHoverTimer.current);
    setIsPlatformMenuOpen(true);
  };

  const handlePlatformLeave = () => {
    platformHoverTimer.current = setTimeout(() => setIsPlatformMenuOpen(false), 300);
  };
        ...
        <div className="w-full max-w-7xl mx-auto flex justify-between items-center text-white">
          ...
          <div className="flex items-center gap-4">
            ...
            >
              ...
              {/* Menu Flutuante (Renderização Condicional + Animação) */}
              {isPlatformMenuOpen && (
