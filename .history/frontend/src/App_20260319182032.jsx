...
import { 
  ...
  X, Send, Paperclip, Image as ImageIcon, Film, Sparkles, MessageSquare, Minus
} from 'lucide-react';
...
export default function App() {
  ...
  const [chatMessages, setChatMessages] = useState([
    { role: 'ia', content: 'Olá Master Admin! Eu sou a **IA Analyiz**, sua interface de inteligência do Mercado Livre. O que devemos analisar hoje?' }
  ]);
  ...
  const [logs, setLogs] = useState([{ msg: 'KERNEL_READY: Sistema inicializado com sucesso.', type: 'info' }]);
  ...
  // --- LÓGICA DO CHAT DA IA ANALYIZ ---
  const handleSendChat = async () => {
    if (!chatInput.trim()) return;
    
    const userMessage = chatInput;
    // Salva histórico atual para mandar pra API
  ...
  // Formatador Markdown para negrito
  ...
  // Ícone do Robô usando GIF animado (Flat Style)
  const RobotGif = ({ size = "md" }) => {
    const gifUrl = "https://i.ibb.co/3pY8D0n/robot-chat.gif";
    return (
      <div className={`${size === "sm" ? 'h-10 w-10' : 'h-16 w-16'} rounded-full flex items-center justify-center relative shadow-inner shrink-0 overflow-hidden`}>
        <img src={gifUrl} alt="Robot AI" className="w-full h-full object-cover" />
      </div>
    );
  };
  ...
  return (
    <div className="h-screen w-full bg-[#f8fafc] flex flex-col font-sans text-slate-900 overflow-hidden relative" style={{ zoom: zoom }}>
            ...
            <section className="col-span-2 bg-white border border-slate-200 rounded-xl shadow-2xl flex flex-col h-full overflow-hidden">
              <div className="p-5 flex flex-col flex-1 min-h-0">
                ...
                <div ref={terminalRef} className="flex-1 bg-slate-950 rounded-lg p-4 font-mono text-[10px] overflow-y-auto custom-scrollbar border border-slate-800 flex flex-col gap-2">
                  {logs.map((log, i) => (<div key={i} className={log.type === 'warn' ? 'text-amber-500 font-bold' : log.type === 'success' ? 'text-emerald-500 font-bold' : 'text-slate-400'}>{log.msg}</div>))}
                </div>
              </div>
            </section>

            <section className="col-span-3 bg-white border border-slate-200 rounded-xl shadow-xl flex flex-col overflow-hidden">
                <div className="p-5 border-b border-slate-100 font-black text-xs uppercase text-slate-400 flex justify-between items-center"><span className="flex items-center gap-2"><AlertTriangle className="text-amber-500 w-4 h-4" /> Inconsistências Detectadas</span> <span className="opacity-50">Live_Database</span></div>
      ...
      <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end max-w-[calc(100vw-48px)]">
        ...
        <div className={`chat-anim w-[420px] max-w-full h-[600px] max-h-[85vh] bg-white rounded-3xl shadow-[0_20px_60px_rgba(0,0,0,0.2)] border border-slate-100 flex flex-col mb-4 overflow-hidden origin-bottom-right ${isChatOpen ? 'scale-100 opacity-100' : 'scale-0 opacity-0 pointer-events-none'}`}>
          
          <div className="bg-[#1e293b] p-6 flex justify-between items-center text-white shrink-0">
            <div className="flex items-center gap-4">
              <RobotGif size="sm" />
              <div><p className="text-sm font-black uppercase tracking-widest leading-none">IA Analyiz</p><p className="text-[10px] text-emerald-400 font-bold uppercase tracking-widest mt-1">Sistemas Online</p></div>
            </div>
          ...
          <div ref={chatScrollRef} className="flex-1 bg-slate-50 p-6 overflow-y-auto space-y-5 custom-scrollbar text-xs text-slate-700 leading-relaxed italic">
            ...
            {chatMessages.map((msg, index) => (
              <div key={index} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                ...
                <div className={`p-4 rounded-2xl shadow-sm max-w-[85%] whitespace-pre-wrap ${msg.role === 'user' ? 'bg-slate-800 text-white rounded-tr-none not-italic' : 'bg-white border border-slate-200 rounded-tl-none'}`}>
          ...
          <div className="p-4 border-t border-slate-100 bg-white shrink-0">
            <div className="flex items-center gap-4 mb-3 px-1 text-slate-400">
              <button className="hover:text-blue-600 transition-colors"><ImageIcon className="w-5 h-5" /></button>
              <button className="hover:text-blue-600 transition-colors"><Film className="w-5 h-5" /></button>
              <button className="hover:text-blue-600 transition-colors"><Paperclip className="w-5 h-5" /></button>
            </div>
            <div className="flex gap-2">
              ...
              />
              <button onClick={handleSendChat} disabled={isChatLoading} className="bg-blue-600 text-white p-3 rounded-xl hover:bg-blue-700 active:scale-95 transition-all disabled:opacity-50 flex items-center justify-center">
        ...
        <div className="relative group">
            ...
            >
                ...
                {!isChatOpen && (
                    ...
                    <span className="relative rounded-full h-5 w-5 bg-blue-500 border-2 border-white text-[8px] flex items-center justify-center text-white font-black">!</span>
                    </span>
                )}
            </button>
      ...
      <footer className="bg-white border-t border-slate-200 p-2 px-6 shrink-0 z-20 flex justify-between items-center text-[9px] font-black uppercase text-slate-400 tracking-[0.4em] italic">
          <span>IA_KERNEL_3.0_ONLINE</span><span>Neural ML Bot</span>
      </footer>
    </div>
  );
}