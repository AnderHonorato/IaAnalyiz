...
export default function MainLayout() {
  ...
  // ── ESTADO DOS PENDENTES RESTAURADO ──
  ...
  // ── BUSCA DE USUÁRIOS BLOQUEADOS RESTAURADA ──
  useEffect(() => {
    if (user?.role === 'OWNER') {
                  ...
                  <div className="grid grid-cols-2 gap-1.5">
                    {TEMAS.map(t => (
                      <button key={t.id} onClick={() => handleSelectTheme(t.id)}
                        ...
                        <span className="w-4 h-4 rounded-full flex-shrink-0 border border-slate-200" style={{ background: t.header }}></span>
                        {t.label}
            ...
            <div className="relative" ref={userMenuRef}>
              ...
              {isMenuOpen && (
                <div className="absolute right-0 top-full mt-2 w-44 bg-white rounded-xl shadow-xl border border-slate-200 z-[999999] overflow-hidden text-slate-700">
                  {/* ── BOTÃO DE IR PARA USUÁRIOS RESTAURADO ── */}
                  {user?.role === 'OWNER' && (
