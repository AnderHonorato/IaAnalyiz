...
// ── COMPONENTE BREADCRUMBS (Sem fundo) ──
function Breadcrumbs() {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 pt-4 pb-0 flex items-center gap-1.5 overflow-x-auto custom-scrollbar shrink-0 z-10 relative">
      <Link to="/" className="flex items-center gap-1 text-[9px] font-black uppercase tracking-widest text-slate-400 hover:text-blue-600 transition-colors whitespace-nowrap">
      ...
      {pathnames.map((value, index) => {
        ...
        return (
          <React.Fragment key={to}>
            ...
            {isLast ? (
              <span className="text-[9px] font-black uppercase tracking-widest text-slate-700 whitespace-nowrap">{label}</span>
            ) : (
              <Link to={to} className="text-[9px] font-black uppercase tracking-widest text-slate-400 hover:text-blue-600 transition-colors whitespace-nowrap">{label}</Link>
            )}
          </React.Fragment>
        );
      })}
