// MainLayout.jsx — adiciona controle de zoom global vinculado ao usuário
// Apenas as partes alteradas estão marcadas com // ← ZOOM

import React, { useState, useEffect, useRef } from 'react';
import { Outlet, useNavigate, useLocation, Link } from 'react-router-dom';
import { Activity, User, LogOut, ChevronDown, Bell, Lock, CheckCircle2, Palette, Home, ChevronRight, ZoomIn, ZoomOut } from 'lucide-react';
...
const getSafeUser = () => {
  try {
    ...
    return {
      ...
      zoom: parsed?.zoom || 100, // ← ZOOM
    };
  } catch {
    return { nome: 'Usuário', role: 'BLOQUEADO', solicitouDesbloqueio: false, tema: 'dark', zoom: 100 };
  }
};
...
function Breadcrumbs() {
  ...
  return (
    <div className="w-full px-4 py-2 flex items-center gap-1.5 overflow-x-auto shrink-0 relative z-10" style={{ background: 'transparent' }}>
...
export default function MainLayout() {
  ...
  // ← ZOOM: aplica zoom no conteúdo
  useEffect(() => {
    const zoom = user.zoom || 100;
    document.documentElement.style.setProperty('--content-zoom', `${zoom / 100}`);
  }, [user.zoom]);
  ...
  // ← ZOOM: handlers
  const handleZoomIn = () => {
    const currentIdx = ZOOM_LEVELS.indexOf(user.zoom || 100);
    if (currentIdx < ZOOM_LEVELS.length - 1) {
      ...
      // Persiste no servidor sem bloquear
      fetch(`${API_BASE_URL}/api/auth/profile`, {
        method: 'PUT', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: user.id, nome: user.nome, avatar: user.avatar, tema: user.tema }),
      }).catch(() => {});
    }
  };
  ...
  const currentZoom   = user.zoom || 100; // ← ZOOM
    ...
    <div className="h-screen w-full flex flex-col font-sans overflow-hidden relative"
      ...
      <style>{`
        :root {
          ...
          --theme-sidebar: #f1f5f9; --content-zoom: 1;
        }
      ...
      <header className="border-b border-white/10 px-4 py-2.5 shadow-md shrink-0 z-20 relative text-white" style={{ background: 'var(--theme-header)' }}>
        <div className="w-full max-w-7xl mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-2 relative">
            ...
            <div className="flex items-center gap-0.5 bg-white/10 border border-white/10 rounded-full px-1.5 py-1">
              <button onClick={handleZoomOut} disabled={currentZoom <= ZOOM_LEVELS[0]}
                title="Diminuir texto"
                className="p-1 hover:bg-white/20 rounded-full disabled:opacity-30 transition-all">
                <ZoomOut className="w-3.5 h-3.5 text-white" />
              </button>
              <button onClick={handleZoomReset}
                title="Redefinir zoom"
                className="text-[9px] font-black text-white/70 hover:text-white px-1.5 min-w-[36px] text-center transition-all">
                {currentZoom}%
              </button>
              <button onClick={handleZoomIn} disabled={currentZoom >= ZOOM_LEVELS[ZOOM_LEVELS.length - 1]}
            ...
            <div className="relative" ref={themeRef}>
              ...
              {isThemeOpen && (
                <div className="absolute right-0 top-full mt-2 bg-white rounded-2xl shadow-2xl border border-slate-200 z-[200] overflow-hidden" style={{ width: '290px' }}>
                  <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest px-3 pt-3 pb-2">Aparência</p>
            ...
            <div className="relative" ref={userMenuRef}>
              ...
              {isMenuOpen && (
                <div className="absolute right-0 top-full mt-2 w-44 bg-white rounded-xl shadow-xl border border-slate-200 z-[100] overflow-hidden text-slate-700">
                  {user?.role === 'OWNER' && (
                    <button onClick={() => { navigate('/usuarios'); setIsMenuOpen(false); }}
      ...
      {/* ← ZOOM: wrapper de conteúdo com zoom aplicado */}
        ...
        <ProfileModal user={user} onClose={() => setIsProfileModalOpen(false)}
          onSave={async (nome, avatar) => {
            ...
            try {
              ...
              if (res.ok && data?.user) {
                const updated = { ...data.user, zoom: user.zoom };
                setUser(updated);
                localStorage.setItem('analyiz_user', JSON.stringify(updated));
