...
// Dicionário para traduzir os caminhos da URL para nomes bonitos no Breadcrumb
const ROUTE_NAMES = {
  'ml': 'Mercado Livre',
...
// ── COMPONENTE BREADCRUMBS (Caminho de Navegação) ──
function Breadcrumbs() {
  const location = useLocation();
  const pathnames = location.pathname.split('/').filter((x) => x);

  // Não mostra o breadcrumb na Home principal
  if (pathnames.length === 0) return null;

  return (
    <div className="w-full bg-white/50 border-b border-slate-200/60 backdrop-blur-sm px-4 py-2.5 flex items-center gap-1.5 overflow-x-auto custom-scrollbar shrink-0 z-10 relative">
      <Link to="/" className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-blue-600 transition-colors whitespace-nowrap">
      ...
      {pathnames.map((value, index) => {
        ...
        const label = ROUTE_NAMES[value] || value; // Usa o dicionário ou o próprio valor da URL
        ...
        return (
          <React.Fragment key={to}>
            ...
            {isLast ? (
              ...
              <Link to={to} className="text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-blue-600 transition-colors whitespace-nowrap">
                {label}
              </Link>
            )}
          </React.Fragment>
...
export default function MainLayout() {
  ...
  return (
    <div className="h-screen w-full flex flex-col font-sans overflow-hidden relative"
      ...
      {/* HEADER PRINCIPAL */}
      <header className="border-b border-white/10 px-4 py-2.5 shadow-md shrink-0 z-20 relative text-white"
      ...
      {/* COMPONENTE BREADCRUMBS (Caminho para voltar) */}
      <Breadcrumbs />

      {/* CONTEÚDO PRINCIPAL z-index ajustado para 10 */}
