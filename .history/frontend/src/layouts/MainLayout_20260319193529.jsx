...
export default function MainLayout() {
  ...
  const handleRequestUnblock = async () => {
    ...
    } catch (e) { console.error("Erro ao solicitar acesso:", e); } finally { setIsRequestingUnblock(false); }
  };

  // Proteção contra quebra no processamento do nome
      ...
      <header className="bg-[#1e293b] border-b border-slate-700 px-6 py-3 shadow-md shrink-0 z-30 relative text-white">
        <div className="w-full max-w-7xl mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-4 relative">
            ...
            {user?.role === 'BLOQUEADO' && (
              <button onClick={handleRequestUnblock} disabled={user?.solicitouDesbloqueio || isRequestingUnblock} className={`flex items-center gap-2 border px-3 py-1.5 rounded-full text-[10px] font-black uppercase transition-all ${user?.solicitouDesbloqueio ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400' : 'bg-red-500/10 border-red-500/30 text-red-400 hover:bg-red-500/20'}`}>
                {user?.solicitouDesbloqueio ? 'Acesso Solicitado' : 'Solicitar Acesso'}
              </button>
            )}
            ...
            <div className="relative" ref={userMenuRef}>
              ...
              {isMenuOpen && (
                <div className="absolute right-0 top-full mt-2 w-48 bg-white rounded-xl shadow-xl border border-slate-200 z-[100] overflow-hidden text-slate-700">
                  {user?.role === 'OWNER' && (
                    <button onClick={() => { navigate('/usuarios'); setIsMenuOpen(false); }} className="w-full flex items-center gap-2.5 px-4 py-3 hover:bg-slate-50 text-xs font-bold border-b border-slate-100">
                      <Activity className="w-4 h-4 text-emerald-600" /> Gestão Usuários
                    </button>
                  )}
                  <button onClick={() => { setIsProfileModalOpen(true); setIsMenuOpen(false); }} className="w-full flex items-center gap-2.5 px-4 py-3 hover:bg-slate-50 text-xs font-bold border-b border-slate-100">
                    <User className="w-4 h-4 text-blue-600" /> Editar Perfil
                  </button>
                  <button onClick={handleLogout} className="w-full flex items-center gap-2.5 px-4 py-3 hover:bg-red-50 text-xs font-bold text-red-600">
        ...
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-sm z-[1000] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden flex flex-col">
            <div className="bg-slate-900 p-4 flex justify-between items-center text-white">
              <h3 className="font-black text-xs uppercase">Configurações</h3>
              <button onClick={() => setIsProfileModalOpen(false)}><X className="w-4 h-4" /></button>
            </div>
            <div className="p-5 space-y-5">
              <div className="flex flex-col items-center gap-2">
                <div className="relative group cursor-pointer w-20 h-20 rounded-full overflow-hidden border-4 border-slate-100 shadow-sm flex items-center justify-center">
                  {editAvatar ? <img src={editAvatar} className="w-full h-full object-cover" /> : <User className="w-8 h-8 text-slate-300" />}
              ...
              <div>
                <label className="text-[9px] font-bold text-slate-500 uppercase block mb-1">Nome</label>
                <input type="text" value={editName} onChange={(e) => setEditName(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl py-2 px-3 text-sm" />
              </div>
              <button onClick={handleSaveProfile} disabled={isSavingProfile} className="w-full bg-blue-600 text-white font-black text-[11px] py-3 rounded-xl shadow-md">
                {isSavingProfile ? <Loader2 className="w-4 h-4 animate-spin mx-auto" /> : "Salvar Alterações"}
              </button>
            </div>
          </div>
        </div>
