// MainLayout.jsx — nav links + zoom/tema dentro do ícone de config
...
const NAV_LINKS = [
  { to: '/ml',     label: 'Mercado Livre', icon: ShoppingBag, color: '#FFE600', textColor: '#1e293b' },
...
export default function MainLayout() {
  ...
  const [isSettingsOpen, setIsSettingsOpen]         = useState(false); // ← zoom+tema unificado
  ...
  const handleZoomIn = () => {
    const idx = ZOOM_LEVELS.indexOf(user.zoom || 100);
    if (idx < ZOOM_LEVELS.length - 1) handleZoomChange(ZOOM_LEVELS[idx + 1]);
  };
  const handleZoomOut = () => {
    const idx = ZOOM_LEVELS.indexOf(user.zoom || 100);
    if (idx > 0) handleZoomChange(ZOOM_LEVELS[idx - 1]);
  };
      ...
      <header className="border-b border-white/10 px-4 py-2.5 shadow-md shrink-0 z-20 relative text-white" style={{ background: 'var(--theme-header)' }}>
        <div className="w-full max-w-7xl mx-auto flex justify-between items-center gap-3">
          ...
          <div className="flex items-center gap-4">
            ...
            {/* Nav Links — ML, Shopee, Amazon */}
            <nav className="hidden sm:flex items-center gap-1">
              {NAV_LINKS.map(({ to, label, icon: Icon, color, textColor }) => {
                ...
                return (
                  <Link
                    ...
                    style={{
                      ...
                      color: isActive ? textColor : 'rgba(255,255,255,0.65)',
          ...
          <div className="flex items-center gap-2 relative">
            ...
            {/* ── BOTÃO DE CONFIGURAÇÕES (Zoom + Tema unificados) ───────────── */}
            <div className="relative" ref={settingsRef}>
              <button
                onClick={() => setIsSettingsOpen(v => !v)}
                title="Aparência e Zoom"
                  ...
                  <div className="px-3 pt-3 pb-2">
                    ...
                    <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-xl px-3 py-2">
                      <button
                        ...
                        className="p-1 hover:bg-slate-200 rounded-lg disabled:opacity-30 transition-all text-slate-600"
                        title="Diminuir"
                      >
                        <ZoomOut className="w-3.5 h-3.5" />
                      </button>
                      {/* Slider visual por botões */}
                      <div className="flex-1 flex items-center gap-0.5">
                        {ZOOM_LEVELS.map(z => (
                          <button
                            ...
                            style={{ background: z <= currentZoom ? 'var(--theme-accent, #3b82f6)' : '#e2e8f0' }}
                            title={`${z}%`}
                          />
                        ))}
                      </div>
                      <button
                        ...
                        disabled={currentZoom >= ZOOM_LEVELS[ZOOM_LEVELS.length - 1]}
                        className="p-1 hover:bg-slate-200 rounded-lg disabled:opacity-30 transition-all text-slate-600"
                        title="Aumentar"
                      >
                      ...
                      <button
                        ...
                        className="text-[9px] font-black text-slate-500 hover:text-slate-800 min-w-[32px] text-center transition-all"
                        title="Redefinir"
                      >
                        {currentZoom}%
                      </button>
