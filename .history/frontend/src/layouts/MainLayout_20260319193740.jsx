...
// Função para obter o usuário do cache sem risco de erro fatal
const getSafeUser = () => {
  try {
...
export default function MainLayout() {
  ...
  // Busca notificações de bloqueio se for OWNER
  useEffect(() => {
    if (user?.role === 'OWNER') {
  ...
  // Proteção contra processamento de strings nulas
      ...
      <header className="bg-[#1e293b] border-b border-slate-700 px-6 py-3 shadow-md shrink-0 z-30 relative text-white">
        <div className="w-full max-w-7xl mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-4 relative">
            ...
            {/* BOTÃO DE SOLICITAR DESBLOQUEIO */}
            {user?.role === 'BLOQUEADO' && (
              <button 
                onClick={handleRequestUnblock} 
                disabled={user?.solicitouDesbloqueio || isRequestingUnblock} 
                className={`flex items-center gap-2 border px-3 py-1.5 rounded-full text-[10px] font-black uppercase transition-all ${user?.solicitouDesbloqueio ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400 cursor-not-allowed' : 'bg-red-500/10 border-red-500/30 text-red-400 hover:bg-red-500/20'}`}
              >
            ...
            <div className="relative" ref={userMenuRef}>
              <div 
                className="flex items-center gap-2.5 bg-slate-800 border border-slate-700 px-2.5 py-1.5 rounded-full cursor-pointer hover:bg-slate-700 transition-all shadow-inner" 
