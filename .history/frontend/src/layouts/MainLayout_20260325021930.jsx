...
function applyTheme(temaId) {
  ...
  if (tema.bgGrad) root.style.setProperty('--theme-bg', tema.bgGrad);
  else root.style.setProperty('--theme-bg', tema.bg);
  ...
  if (tema.cardGrad) root.style.setProperty('--theme-card', tema.cardGrad);
  else root.style.setProperty('--theme-card', tema.card);
          ...
          <div className="flex items-center gap-4">
            ...
            <nav className="hidden sm:flex items-center gap-1">
              ...
              <Link to="/"
                ...
                style={{ 
                  background: location.pathname === '/' ? '#10b981' : 'rgba(255,255,255,0.07)', 
                  color: location.pathname === '/' ? '#ffffff' : 'rgba(255,255,255,0.65)', 
                  border: location.pathname === '/' ? `1px solid #10b981` : '1px solid rgba(255,255,255,0.08)' 
                }}
                onMouseEnter={e => { if (location.pathname !== '/') { e.currentTarget.style.background='rgba(255,255,255,0.14)'; e.currentTarget.style.color='#fff'; } }}
                onMouseLeave={e => { if (location.pathname !== '/') { e.currentTarget.style.background='rgba(255,255,255,0.07)'; e.currentTarget.style.color='rgba(255,255,255,0.65)'; } }}
              >
                <Home className="w-3.5 h-3.5 shrink-0" />
                <span className="hidden md:inline">Início</span>
              </Link>

              {/* Menu 2: PLATAFORMAS (Dropdown) */}
              <div className="relative group flex items-center h-full">
                <button
                  className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider transition-all"
                  style={{ 
                    background: ['/ml', '/shopee', '/amazon'].some(p => location.pathname.startsWith(p)) ? '#8b5cf6' : 'rgba(255,255,255,0.07)', 
                    color: ['/ml', '/shopee', '/amazon'].some(p => location.pathname.startsWith(p)) ? '#ffffff' : 'rgba(255,255,255,0.65)', 
                    border: ['/ml', '/shopee', '/amazon'].some(p => location.pathname.startsWith(p)) ? `1px solid #8b5cf6` : '1px solid rgba(255,255,255,0.08)' 
                  }}
                  onMouseEnter={e => { if (!['/ml', '/shopee', '/amazon'].some(p => location.pathname.startsWith(p))) { e.currentTarget.style.background='rgba(255,255,255,0.14)'; e.currentTarget.style.color='#fff'; } }}
                  onMouseLeave={e => { if (!['/ml', '/shopee', '/amazon'].some(p => location.pathname.startsWith(p))) { e.currentTarget.style.background='rgba(255,255,255,0.07)'; e.currentTarget.style.color='rgba(255,255,255,0.65)'; } }}
                >
                  <LayoutGrid className="w-3.5 h-3.5 shrink-0" />
                  <span className="hidden md:inline">Plataformas</span>
                </button>

                {/* Conteúdo do Dropdown de Plataformas */}
                <div className="absolute top-full left-0 pt-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-[200]">
                  <div className="w-52 bg-white rounded-xl shadow-xl border border-slate-200 overflow-hidden text-slate-700 flex flex-col">
                    
                    <div className="px-4 py-2 text-[9px] font-black text-slate-400 uppercase tracking-widest bg-slate-50 flex items-center gap-2">
                      <ShoppingBag className="w-3 h-3 text-yellow-500" /> Mercado Livre
                    </div>
                    <Link to="/ml" className="flex items-center gap-2 px-4 py-2.5 hover:bg-slate-50 text-[11px] font-bold border-b border-slate-100 transition-colors">
                      <ChevronRight className="w-3 h-3 text-slate-400" />Dashboard ML
                    </Link>
                    ...
                    <Link to="/ml/pesquisa" className="flex items-center gap-2 px-4 py-2.5 hover:bg-slate-50 text-[11px] font-bold transition-colors border-b border-slate-100">
                      <ChevronRight className="w-3 h-3 text-slate-400" />Pesquisa de Anúncios
                    </Link>

                    <div className="px-4 py-2 text-[9px] font-black text-slate-400 uppercase tracking-widest bg-slate-50 flex items-center gap-2">
                      <ShoppingCart className="w-3 h-3 text-orange-500" /> Shopee
                    </div>
                    <Link to="/shopee" className="flex items-center gap-2 px-4 py-2.5 hover:bg-slate-50 text-[11px] font-bold transition-colors border-b border-slate-100">
                      <ChevronRight className="w-3 h-3 text-slate-400" />Acessar Shopee
                    </Link>

                    <div className="px-4 py-2 text-[9px] font-black text-slate-400 uppercase tracking-widest bg-slate-50 flex items-center gap-2">
                      <Box className="w-3 h-3 text-yellow-600" /> Amazon
                    </div>
                    <Link to="/amazon" className="flex items-center gap-2 px-4 py-2.5 hover:bg-slate-50 text-[11px] font-bold transition-colors">
                      <ChevronRight className="w-3 h-3 text-slate-400" />Acessar Amazon
                    </Link>
              ...
              {/* Botão Central: Falar com IA */}
              <button 
                onClick={() => setIsChatOpen(true)}
                className="ml-2 flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider transition-all bg-gradient-to-r from-indigo-500 to-purple-500 hover:from-indigo-600 hover:to-purple-600 text-white border border-indigo-400/50 shadow-md"
              >
