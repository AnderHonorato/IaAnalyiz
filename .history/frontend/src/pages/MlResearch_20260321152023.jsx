                        ...
                        return (
                          <div key={i} style={{ background:eMenorGeral?'#f0fdf4':abaixoPrincipal?'#fef2f2':'var(--theme-sidebar)',border:`1px solid ${eMenorGeral?'#86efac':abaixoPrincipal?'#fca5a5':'var(--theme-card-border)'}`,borderRadius:'12px',padding:'10px 14px',display:'flex',alignItems:'center',gap:'10px' }}>
                            ...
                            <div style={{ textAlign:'right',flexShrink:0,minWidth:'100px' }}>
                              ...
                              {c.link && (
                                <a href={c.link} target="_blank" rel="noopener noreferrer" style={{ display:'flex',alignItems:'center',gap:'3px',justifyContent:'flex-end',fontSize:'10px',color:'#3b82f6',textDecoration:'none',marginTop:'4px' }}>
                                  Ver <ExternalLink style={{ width:'10px',height:'10px' }} />
                                </a>
                              )}
                            </div>
                          </div>
...
function ResumoGeral({ itens }) {
  ...
  return (
    <div style={{ background:'linear-gradient(135deg,#0f172a,#1e293b)',borderRadius:'12px',padding:'12px 16px',color:'#fff',marginBottom:'8px' }}>
      <p style={{ fontSize:'9px',fontWeight:900,textTransform:'uppercase',color:'rgba(255,255,255,0.3)',letterSpacing:'0.1em',marginBottom:'8px' }}>Resumo da sessão</p>
      <div style={{ display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'12px' }}>
        {[
          ...
          <div key={l}><p style={{ fontSize:'16px',fontWeight:900,color:c,lineHeight:1,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap' }}>{v}</p><p style={{ fontSize:'9px',textTransform:'uppercase',color:'rgba(255,255,255,0.3)',marginTop:'3px' }}>{l}</p></div>
        ))}
      </div>
    </div>
  );
...
export default function MlResearch() {
  ...
  const buscarAnuncio = useCallback(async (mlbId, url) => {
    ...
    const res = await fetch(`${API_BASE_URL}/api/ml/research/${mlbId}?${params}`, { signal: AbortSignal.timeout(30000) });
  ...
  const executarFila = useCallback(async (ids) => {
    ...
    for (let i = 0; i < ids.length; i++) {
      ...
      try {
        ...
        addLog(`✅ ${mlbId} · ${dados.titulo?.substring(0,20)||''} · ${numVend} vendedor(es)`,'success');
    ...
    if (retentar.length && !abortRef.current) {
      ...
      for (const { mlbId, url } of retentar) {
        ...
        try { const dados = await buscarAnuncio(mlbId, url); setItens(prev => prev.map(it => it.mlbId===mlbId?{...it,status:'concluido',dados}:it)); addLog(`✅ ${mlbId}`,'success'); }
    ...
    <div style={{ maxWidth:'1280px',margin:'0 auto',padding:'8px 12px',minHeight:'100vh',display:'flex',flexDirection:'column',gap:'8px',color:'var(--theme-text)' }}>
      ...
      <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:'8px' }}>
        ...
        <div style={{ display:'flex',alignItems:'center',gap:'6px',flexWrap:'wrap' }}>
          ...
          {aba==='pesquisa' && <button onClick={() => setMostrarInput(v=>!v)} style={{ ...btnBase,background:'var(--theme-header)',color:'#FFE600',border:'none',padding:'6px 12px' }}><Plus style={{ width:'13px',height:'13px' }} />Adicionar</button>}
        </div>
      </div>
      ...
      {aba === 'pesquisa' && (<>
        {mostrarInput && (
          <div style={{ background:'var(--theme-card)',border:'1px solid var(--theme-card-border)',borderRadius:'14px',padding:'14px' }}>
            <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'8px' }}>
              <p style={{ fontSize:'11px',fontWeight:900,textTransform:'uppercase',display:'flex',alignItems:'center',gap:'6px',color:'var(--theme-text)' }}>
            ...
            <textarea value={inputTexto} onChange={e=>setInputTexto(e.target.value)}
              ...
              style={{ width:'100%',border:'1px solid var(--theme-card-border)',borderRadius:'10px',padding:'10px 12px',fontSize:'12px',fontFamily:'monospace',outline:'none',resize:'none',background:'var(--theme-sidebar)',color:'var(--theme-text)',boxSizing:'border-box' }} rows={4}
            ...
            <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',marginTop:'8px' }}>
              ...
              <button onClick={processarInput} disabled={!inputTexto.trim()} style={{ ...btnBase,background:inputTexto.trim()?'var(--theme-accent)':'var(--theme-sidebar)',color:inputTexto.trim()?'#fff':'var(--theme-text)',opacity:inputTexto.trim()?1:0.4,padding:'7px 16px',fontSize:'11px',border:'none' }}>
        ...
        {itens.length > 0 && (
          <div style={{ display:'flex',gap:'12px',flex:1,minHeight:0 }}>
            {/* Terminal */}
            <div style={{ width:'210px',flexShrink:0,background:'#020617',borderRadius:'14px',display:'flex',flexDirection:'column',maxHeight:'76vh' }}>
              ...
              <div style={{ background:'var(--theme-card)',border:'1px solid var(--theme-card-border)',borderRadius:'12px',padding:'8px 12px',display:'flex',alignItems:'center',gap:'5px',flexWrap:'wrap' }}>
                ...
                {selecionados.size>0 && (
                  <div style={{ marginLeft:'auto',display:'flex',alignItems:'center',gap:'6px',background:'#eff6ff',border:'1px solid #bfdbfe',borderRadius:'8px',padding:'4px 10px' }}>
                    ...
                    <button onClick={()=>{[...selecionados].forEach(id=>removerItem(id));}} style={{ padding:'2px',background:'none',border:'none',cursor:'pointer',color:'#ef4444',display:'flex' }}><Trash2 style={{ width:'12px',height:'12px' }}/></button>
                  </div>
                )}
              </div>
      ...
      {(aba === 'historico' || aba === 'arquivados') && (
        <div style={{ display:'flex',flexDirection:'column',gap:'8px' }}>
          <div style={{ background:'var(--theme-card)',border:'1px solid var(--theme-card-border)',borderRadius:'12px',padding:'8px 12px',display:'flex',alignItems:'center',gap:'8px',flexWrap:'wrap' }}>
            ...
            {selHist.size > 0 && (
              <div style={{ marginLeft:'auto',display:'flex',gap:'6px',alignItems:'center',background:'#eff6ff',border:'1px solid #bfdbfe',borderRadius:'8px',padding:'4px 10px' }}>
                <span style={{ fontSize:'9px',fontWeight:900,color:'#1d4ed8' }}>{selHist.size} sel.</span>
                {aba==='historico' && <button onClick={() => acaoLoteHist('arquivar')} style={{ ...btnBase,padding:'3px 8px' }}><Archive style={{ width:'11px',height:'11px' }}/>Arquivar</button>}
                <button onClick={() => acaoLoteHist('excluir')} style={{ ...btnBase,padding:'3px 8px',color:'#ef4444',borderColor:'#fca5a5',background:'#fee2e2' }}><Trash2 style={{ width:'11px',height:'11px' }}/>Excluir</button>
              </div>
            )}
          </div>
          {loadingHist
