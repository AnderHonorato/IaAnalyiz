...
export default function MlResearch() {
  ...
  // ── Estados da Pesquisa de Mercado IA ──────────────────────────────────────
  ...
  const limparTudo = () => {
    if (rodandoRef.current) return;
    if (!window.confirm('Limpar todos os anúncios da sessão atual?\n\nAtenção: o histórico de pesquisas no banco de dados será mantido — você pode acessá-lo pela aba Histórico.')) return;
    setItens([]); setLog([]); setSelecionados(new Set());
  };
  ...
  // ── Pesquisa de Mercado Profunda (IA) ─────────────────────────────────────
  const dispararPesquisaMercado = async () => {
    const selecionadosData = itens.filter(i => selecionados.has(i.id) && i.status === 'concluido');
    if (!selecionadosData.length) return;
    setGerandoPesquisa(true);
    try {
      const res = await fetch(`${API_BASE_URL}/api/ml/research/deep-market`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId, itens: selecionadosData.map(i => i.dados) }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || `HTTP ${res.status}`);
      setPesquisaIA(data);
    } catch (e) {
      addLog(`❌ Pesquisa de Mercado falhou: ${e.message}`, 'warn');
      alert('Erro na pesquisa profunda: ' + e.message);
    } finally {
  ...
  const salvarPesquisaNoHistorico = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      addLog('💾 Análise de mercado salva no histórico!', 'success');
      alert('Análise salva no histórico!');
      setPesquisaIA(null);
    } catch (e) {
      addLog(`❌ Falha ao salvar análise: ${e.message}`, 'warn');
    }
  };
  ...
  return (
    <div style={{ maxWidth:'1280px',margin:'0 auto',padding:'8px 12px',minHeight:'100vh',display:'flex',flexDirection:'column',gap:'8px',color:'var(--theme-text)' }}>
      <style>{`
        ...
        .ia-scroll::-webkit-scrollbar { width: 6px; }
        .ia-scroll::-webkit-scrollbar-track { background: transparent; }
        .ia-scroll::-webkit-scrollbar-thumb { background: rgba(255,255,255,0.15); border-radius: 3px; }
      `}</style>
            ...
            <div style={{ display:'flex',gap:'6px',alignItems:'center' }}>
              ...
              <div style={{ position:'relative' }}>
                <button
                  ...
                  title="Dicas de uso"
                  style={{ ...btnBase,padding:'6px',background:showDica?'#fef9c3':'var(--theme-sidebar)',color:showDica?'#854d0e':'var(--theme-text)',borderColor:showDica?'#fde047':'var(--theme-card-border)' }}>
                  <HelpCircle style={{ width:'14px',height:'14px' }}/>
                </button>
                {showDica && (
                  <Portal>
                    <div onClick={() => setShowDica(false)} style={{ position:'fixed',inset:0,zIndex:99990 }}>
                      <div onClick={e=>e.stopPropagation()} style={{ position:'fixed',top:'60px',right:'16px',zIndex:99991,width:'320px',background:'#1e293b',border:'1px solid #334155',borderRadius:'14px',padding:'14px',boxShadow:'0 20px 60px rgba(0,0,0,0.5)',animation:'slideDown 0.15s ease' }}>
                        <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'10px' }}>
                          <p style={{ fontSize:'11px',fontWeight:900,textTransform:'uppercase',color:'#fbbf24',display:'flex',alignItems:'center',gap:'6px' }}><HelpCircle style={{ width:'13px',height:'13px' }}/>Dicas de uso</p>
                          <button onClick={() => setShowDica(false)} style={{ background:'none',border:'none',cursor:'pointer',color:'rgba(255,255,255,0.3)' }}><X style={{ width:'14px',height:'14px' }}/></button>
                        </div>
                        {[
                          { emoji:'🔗', titulo:'Cole links ou IDs',       texto:'Cole links do Mercado Livre ou IDs (MLB...) — um por linha. O sistema detecta automaticamente.' },
                          { emoji:'⚡', titulo:'Análise automática',       texto:'Após adicionar, clique em "Analisar" no terminal. O sistema tenta 3 estratégias por anúncio.' },
                          { emoji:'📦', titulo:'Catálogo',                 texto:'Anúncios de catálogo buscam todas as opções de compra disponíveis no ML.' },
                          { emoji:'📊', titulo:'Comparador',               texto:'Selecione 2+ anúncios concluídos e clique "Comparar" para ver vantagem competitiva.' },
                          { emoji:'🤖', titulo:'Pesquisa de Mercado IA',   texto:'Selecione anúncios concluídos e clique "Pesquisa de Mercado" para gerar análise inteligente.' },
                          { emoji:'📂', titulo:'Histórico automático',     texto:'Toda pesquisa é salva automaticamente. Acesse pela aba Histórico para recarregar.' },
                          { emoji:'🗃️', titulo:'Arquivar',                 texto:'Selecione anúncios concluídos e arquive-os no histórico para manter a lista limpa.' },
                        ].map(({ emoji, titulo, texto }) => (
                          <div key={titulo} style={{ display:'flex',gap:'8px',padding:'8px 0',borderBottom:'1px solid rgba(255,255,255,0.07)' }}>
                            <span style={{ fontSize:'16px',flexShrink:0 }}>{emoji}</span>
                            <div>
                              <p style={{ fontSize:'11px',fontWeight:900,color:'#fff',marginBottom:'2px' }}>{titulo}</p>
                              <p style={{ fontSize:'10px',color:'rgba(255,255,255,0.45)',lineHeight:1.5 }}>{texto}</p>
                            </div>
                          </div>
                        ))}
                      </div>
              ...
              <div style={{ background:'var(--theme-card)',border:'1px solid var(--theme-card-border)',borderRadius:'12px',padding:'8px 12px',display:'flex',alignItems:'center',gap:'5px',flexWrap:'wrap' }}>
                ...
                {selecionados.size>0 && (
                  <div style={{ marginLeft:'auto',display:'flex',alignItems:'center',gap:'5px',background:'#eff6ff',border:'1px solid #bfdbfe',borderRadius:'8px',padding:'4px 10px' }}>
                    ...
                    {/* Arquivar selecionados no histórico (só itens concluídos) */}
                    {[...selecionados].some(id => itens.find(i=>i.id===id&&i.status==='concluido')) && (
                      <button
                        onClick={async () => {
                          ...
                          for (const it of concluidos) {
                            try {
                              // Busca o id no histórico pelo mlbId
                              const res = await fetch(`${API_BASE_URL}/api/ml/research/historico?userId=${userId}&arquivado=false`);
                              const hist = await res.json();
                              const entrada = Array.isArray(hist) ? hist.find(h => h.mlbId === it.mlbId) : null;
                              if (entrada) await fetch(`${API_BASE_URL}/api/ml/research/historico/${entrada.id}/arquivar`, { method: 'PUT' });
                            } catch {}
                          }
                          addLog(`📦 ${concluidos.length} item(s) arquivado(s) no histórico`, 'success');
                          setSelecionados(new Set());
                        }}
                        style={{ display:'flex',alignItems:'center',gap:'3px',padding:'3px 8px',borderRadius:'6px',background:'var(--theme-sidebar)',border:'1px solid var(--theme-card-border)',color:'var(--theme-text)',cursor:'pointer',fontSize:'9px',fontWeight:900,textTransform:'uppercase' }}>
                    ...
                    {/* Pesquisa de Mercado IA */}
                    {[...selecionados].some(id => itens.find(i=>i.id===id&&i.status==='concluido')) && (
                      <button
                        disabled={gerandoPesquisa}
                        onClick={dispararPesquisaMercado}
                        style={{ display:'flex',alignItems:'center',gap:'3px',padding:'3px 8px',borderRadius:'6px',background:gerandoPesquisa?'rgba(79,70,229,0.5)':'linear-gradient(90deg, #4f46e5, #9333ea)',color:'#fff',border:'none',cursor:gerandoPesquisa?'not-allowed':'pointer',fontSize:'9px',fontWeight:900,textTransform:'uppercase',opacity:gerandoPesquisa?0.7:1 }}>
                        {gerandoPesquisa
                          ? <Loader2 style={{ width:'11px',height:'11px',animation:'spin 1s linear infinite' }}/>
                          : <Sparkles style={{ width:'11px',height:'11px' }}/>}
                        {gerandoPesquisa ? 'Gerando...' : 'Pesquisa de Mercado'}
                      </button>
                    )}
      ...
      {/* ── Modal Pesquisa de Mercado IA ───────────────────────────────────── */}
      {pesquisaIA && (
        <Portal>
          <div
            onClick={() => setPesquisaIA(null)}
            style={{ position:'fixed',inset:0,zIndex:999999,background:'rgba(0,0,0,0.75)',backdropFilter:'blur(4px)',display:'flex',alignItems:'center',justifyContent:'center',padding:'20px' }}>
            <div
              onClick={e => e.stopPropagation()}
              style={{ width:'640px',maxWidth:'96vw',maxHeight:'90vh',background:'var(--theme-card)',border:'1px solid var(--theme-card-border)',borderRadius:'20px',overflow:'hidden',display:'flex',flexDirection:'column',boxShadow:'0 30px 80px rgba(0,0,0,0.5)',color:'var(--theme-text)' }}>

              {/* Header */}
              <div style={{ padding:'16px 20px',background:'var(--theme-header)',display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0 }}>
                <div style={{ display:'flex',alignItems:'center',gap:'10px' }}>
                  <div style={{ width:'32px',height:'32px',borderRadius:'10px',background:'linear-gradient(135deg,#4f46e5,#9333ea)',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0 }}>
                    <Sparkles style={{ width:'16px',height:'16px',color:'#fff' }}/>
                  </div>
                  <div>
                    <p style={{ fontSize:'14px',fontWeight:900,color:'#fff' }}>Inteligência de Mercado</p>
                    <p style={{ fontSize:'10px',color:'rgba(255,255,255,0.4)' }}>Análise gerada por IA</p>
                  </div>
                </div>
                <button onClick={() => setPesquisaIA(null)} style={{ background:'none',border:'none',cursor:'pointer',color:'rgba(255,255,255,0.4)',padding:'4px' }}>
              ...
              <div
                className="ia-scroll"
                style={{ padding:'20px',flex:1,overflowY:'auto',fontSize:'13px',lineHeight:'1.7' }}
                dangerouslySetInnerHTML={{ __html: pesquisaIA.conteudoHtml }}
              />

              {/* Footer com ações */}
              <div style={{ padding:'12px 16px',borderTop:'1px solid var(--theme-card-border)',display:'flex',gap:'10px',flexShrink:0 }}>
                <button
                  onClick={salvarPesquisaNoHistorico}
                  style={{ flex:1,padding:'10px',borderRadius:'10px',background:'linear-gradient(90deg,#059669,#10b981)',color:'#fff',fontWeight:900,fontSize:'11px',border:'none',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:'6px',textTransform:'uppercase' }}>
                  💾 Salvar no histórico
                </button>
                <button
                  onClick={() => setPesquisaIA(null)}
                  style={{ flex:1,padding:'10px',borderRadius:'10px',background:'var(--theme-sidebar)',border:'1px solid var(--theme-card-border)',color:'var(--theme-text)',fontWeight:900,fontSize:'11px',cursor:'pointer',textTransform:'uppercase' }}>
                  Fechar
                </button>
              </div>
            </div>
