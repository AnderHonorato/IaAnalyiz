 ...
 * 
 * Propósito:
 * Página de recuperação de senha com fluxo de dois passos.
 * Etapa 1: Utilizador insere email e recebe código
 * Etapa 2: Insere código e define nova senha
 * 
 * Responsabilidades:
 * - Solicitar código de recuperação (/api/auth/forgot-password)
 * - Validar código e definir nova senha (/api/auth/reset-password)
 * - Gerenciar transição entre visualizações (forgot/reset)
 * - Exibir avisos sobre etapas em desenvolvimento
 * 
 * Estado:
 *   - view: 'forgot' (etapa 1) ou 'reset' (etapa 2)
 *   - formData: { email, codigo, novaSenha }
 *   - error: Mensagem de erro
 *   - msg: Mensagem de sucesso
 *   - loading: Flag de carregamento
 * 
 * Notas:
 * - OBSERVAÇÃO: Integração SMTP em desenvolvimento
 * - Códigos gerados aparecem nos logs do backend
 * 
...
export default function Recovery() {
  ...
  const handleForgot = async (e) => {
    ...
    try {
      ...
      setMsg('Código enviado! (Ver logs do backend)');
