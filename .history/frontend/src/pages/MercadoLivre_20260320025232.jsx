...
// ── COMPONENTE: Máquina de Escrever (Typewriter) ──
...
// ── COMPONENTE: Insight IA Discreto ──
function AiInsightHeader({ insight }) {
  if (!insight) return null;
  const cleanInsight = insight.replace(/Tenho novos dados para você:/i, '').trim();
  return (
    <div className="flex items-start gap-1.5 ml-4 max-h-[42px] overflow-y-auto custom-scrollbar pr-2 max-w-sm">
      <Sparkles className="w-3.5 h-3.5 text-blue-500 animate-pulse mt-0.5 flex-shrink-0" />
      <p className="text-[11px] font-medium text-slate-500 leading-snug" dangerouslySetInnerHTML={{ __html: cleanInsight }} />
    </div>
  );
}
...
function QuickViewPopover({ item, onClose, onOpenFull }) {
  if (!item) return null;
  return (
    <div className="fixed z-[9999] bg-slate-900 text-white p-3 rounded-xl shadow-[0_10px_40px_rgba(0,0,0,0.4)] border border-slate-700 w-64 right-10 top-40 animate-in fade-in slide-in-from-right-8 pointer-events-none">
      ...
      <p className="text-[9px] text-blue-400 font-black uppercase tracking-widest text-center">Clique no olho para a ficha técnica</p>
    </div>
  );
}
...
export default function MercadoLivre() {
  ...
  // MODO TELA CHEIA (FULLSCREEN OVERLAY GERAL)
  ...
  useEffect(() => { 
    if (terminalRef.current) {
      setTimeout(() => { terminalRef.current.scrollTop = terminalRef.current.scrollHeight; }, 150);
    }
  }, [logs]);
  ...
  const excluirDivergencia = async (id) => {
    if (!await confirm({ title: 'Excluir', message: 'Excluir permanentemente?', confirmLabel: 'Excluir', danger: true })) return;
    try { await fetch(`${API_BASE_URL}/api/divergencias/${id}`, { method: 'DELETE' }); await buscarDivergencias(); await buscarDivStats(); } catch {}
  };
  ...
  const acaoLoteNaoVinculados = async (acaoStr) => {
    ...
    if (!await confirm({ title: 'Ação Lote', message: `Aplicar em ${selectedNaoVinc.size} anúncio(s)?`, confirmLabel: 'Confirmar' })) return;
  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-4 h-full flex flex-col animate-in fade-in duration-300 relative">
      ...
      {!fullScreenMode && (
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 shrink-0 gap-3">
          <div className="flex items-center">
            <button onClick={() => navigate('/ml')} className="p-1.5 mr-2 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-500 transition-colors" title="Voltar ao Painel ML">
              <ArrowLeft className="w-4 h-4" />
            </button>
            <h2 className="text-lg font-black tracking-tight text-slate-800 flex items-center gap-1.5 whitespace-nowrap">
              Gestão ML
            </h2>
      ...
      {/* ── ABA SCANNER ── */}
      {activeTab === 'bot' && (
        <div className={`grid grid-cols-1 ${fullScreenMode ? 'fixed inset-0 z-[99999] bg-slate-100 p-4 gap-4 overflow-hidden' : 'lg:grid-cols-5 gap-4'} flex-1 min-h-0 transition-all`}>
          {!fullScreenMode && (
            <section className="col-span-2 bg-white border border-slate-200 rounded-2xl shadow-sm flex flex-col h-full overflow-hidden">
              <div className="p-3 border-b border-slate-100">
                <button onClick={iniciarBot} disabled={isBotRunning || !mlConectado}
                  ...
                  {isBotRunning ? <span className="flex items-center justify-center gap-1.5"><Loader2 className="w-3.5 h-3.5 animate-spin" /> Auditando ML...</span> : !mlConectado ? '🔒 Conecte o ML' : '🔍 Iniciar Varredura'}
                </button>
              ...
              <div ref={terminalRef} className="flex-1 bg-slate-950 m-2 rounded-xl p-4 overflow-y-auto custom-scrollbar flex flex-col gap-2">
              ...
              <div className="flex justify-between items-center gap-2 flex-wrap">
                <div className="flex gap-1.5 flex-wrap">
                  {['PENDENTE', 'REINCIDENTE', 'CORRIGIDO', 'IGNORADO', 'TODOS'].map(s => {
                    ...
                    return (
                      <button key={s} onClick={() => setFiltroStatus(s)} className={`flex items-center gap-1 px-2.5 py-1 rounded-lg text-[9px] font-black uppercase border transition-all ${filtroStatus === s ? `${cfg.bg} ${cfg.color} ${cfg.border} shadow-sm` : 'bg-white text-slate-500 border-slate-200 hover:bg-slate-50'}`}>
                ...
                <table className="w-full text-left">
                  <thead className="sticky top-0 bg-slate-50 border-b border-slate-200 shadow-sm z-10">
                    <tr className="text-[9px] font-black uppercase tracking-widest text-slate-500">
                      <th className="px-3 py-2.5 w-8"><button onClick={() => toggleSelectAll(divsFiltradas, selectedDivs, setSelectedDivs, 'id')} className="text-slate-400 hover:text-slate-600">{selectedDivs.size === divsFiltradas.length && divsFiltradas.length > 0 ? <CheckSquare className="w-3.5 h-3.5" /> : <Square className="w-3.5 h-3.5" />}</button></th>
                      <th className="px-3 py-2.5">Anúncio</th><th className="px-3 py-2.5">Divergência</th><th className="px-3 py-2.5 text-right">Ações</th>
                    </tr>
                  </thead>
                    ...
                    {divsFiltradas.map((div) => {
                      ...
                      return (
                        <tr key={div.id} className={`hover:bg-slate-50/80 ${sel ? 'bg-blue-50/40' : ''}`}>
                          ...
                          <td className="py-2.5 px-3"><p className="text-slate-600 text-[10px] font-medium leading-relaxed">{div.motivo}</p></td>
      ...
      {/* ── ABA NÃO VINCULADOS ── */}
      {activeTab === 'pendentes' && (
        <section className={`bg-white border border-slate-200 rounded-2xl shadow-sm flex flex-col flex-1 min-h-0 overflow-hidden ${fullScreenMode ? 'fixed inset-0 z-[99999] bg-slate-100 p-4' : ''}`}>
            ...
            {nVincFiltrados.length === 0 ? (
              ...
              <table className="w-full text-left">
                <thead className="sticky top-0 bg-white border-b border-slate-100 shadow-sm z-10">
                  <tr className="text-[9px] font-black uppercase tracking-widest text-slate-400"><th className="px-4 py-2.5 w-8"><button onClick={() => toggleSelectAll(nVincFiltrados, selectedNaoVinc, setSelectedNaoVinc, 'id')} className="text-slate-400">{selectedNaoVinc.size === nVincFiltrados.length ? <CheckSquare className="w-3.5 h-3.5" /> : <Square className="w-3.5 h-3.5" />}</button></th><th className="px-4 py-2.5">Produto ML</th><th className="px-4 py-2.5 text-right">Ação</th></tr>
                </thead>
                  ...
                  {nVincFiltrados.map(prod => {
                    ...
                    return (
                      <tr key={prod.id} className={`hover:bg-slate-50/60 transition-colors ${sel ? 'bg-amber-50/30' : ''}`}>
                        <td className="px-4 py-3"><button onClick={() => { const n = new Set(selectedNaoVinc); sel ? n.delete(prod.id) : n.add(prod.id); setSelectedNaoVinc(n); }} className="text-slate-400 hover:text-amber-500">{sel ? <CheckSquare className="w-3.5 h-3.5 text-amber-500" /> : <Square className="w-3.5 h-3.5" />}</button></td>
      ...
      {/* ── ABA CATÁLOGO ── */}
      {activeTab === 'produtos' && (
        <div className={`grid grid-cols-1 ${fullScreenMode ? 'fixed inset-0 z-[99999] bg-slate-100 p-4 gap-4' : 'lg:grid-cols-5 gap-4'} flex-1 min-h-0 transition-all`}>
          {!fullScreenMode && (
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar">
              <table className="w-full text-left">
                <thead className="bg-slate-50 sticky top-0 z-10 border-b border-slate-100">
                  <tr className="text-[9px] font-black uppercase tracking-widest text-slate-400"><th className="px-3 py-2.5 w-8"><button onClick={() => toggleSelectAll(produtosFiltrados, selectedProds, setSelectedProds)} className="text-slate-400">{selectedProds.size === produtosFiltrados.length && produtosFiltrados.length > 0 ? <CheckSquare className="w-3.5 h-3.5" /> : <Square className="w-3.5 h-3.5" />}</button></th><th className="px-3 py-2.5">Produto</th><th className="px-3 py-2.5">Peso</th><th className="px-3 py-2.5 text-right">Ação</th></tr>
                </thead>
                <tbody className="divide-y divide-slate-50 text-[11px]">
                      ...
                      <tr key={prod.id} className={`hover:bg-slate-50/80 transition-colors ${sel ? 'bg-blue-50/30' : ''}`}>
                        ...
                        <td className="px-3 py-2.5">
                          <div className="flex items-center gap-2.5">
                            {prod.thumbnail ? <img src={prod.thumbnail} className="w-8 h-8 rounded-md object-cover shadow-sm border border-slate-100" /> : <div className="w-8 h-8 bg-slate-100 rounded-md flex items-center justify-center"><ImageIcon className="w-3 h-3 text-slate-300"/></div>}
                            <div>
        ...
        <div className="fixed inset-0 z-[99999] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-4 border-b border-slate-100 bg-white flex justify-between items-center z-10 shadow-sm">
              <h3 className="font-black text-slate-800 uppercase text-[11px] tracking-tight flex items-center gap-2"><Link2 className="w-4 h-4 text-emerald-600"/> Vincular Anúncio</h3>
            ...
            <div className="flex-1 overflow-y-auto p-5 bg-slate-50 custom-scrollbar">
              <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-sm mb-5 flex gap-3 items-center">
                ...
                <div>
                  <span className="text-[9px] font-mono text-emerald-700 bg-emerald-50 border border-emerald-100 px-1.5 py-0.5 rounded font-black">{vincularAnuncio.mlItemId}</span>
        ...
        <div className="fixed inset-0 z-[99999] bg-slate-900/70 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
            <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-blue-600 text-white">
              <h3 className="font-black uppercase tracking-widest text-[11px] flex items-center gap-2"><HelpCircle className="w-4 h-4"/> Dicas do Sistema</h3>
            ...
            <div className="p-6 space-y-4 text-[11px] text-slate-600 leading-relaxed">
              ...
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-100"><h4 className="font-black text-slate-800 mb-1 flex items-center gap-1.5"><Package className="w-3.5 h-3.5 text-blue-500"/> Base vs Anúncios</h4><p><b>Base:</b> Peça física (ex: Controle).<br/><b>Anúncio:</b> O pacote final (ex: Kit com 2 Controles).</p></div>
            </div>
          </div>
        </div>
      )}
