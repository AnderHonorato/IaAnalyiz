...
import {
  ...
  Wifi, WifiOff, FileSearch, History, Play, Pause, Download, TrendingUp, AlertCircle, BarChart2, Calendar
} from 'lucide-react';
  ...
  return (
    <div
      style={{
        ...
        borderRadius: '16px', padding: '12px', width: '220px', zIndex: 50,
        boxShadow: '0 8px 30px rgba(0,0,0,0.12)', pointerEvents: 'none',
      }}
    >
      {item.thumbnail && (
        <img src={item.thumbnail} alt="" style={{ width: '100%', height: '100px', objectFit: 'cover', borderRadius: '8px', marginBottom: '8px' }} />
      )}
      <p style={{ fontSize: '11px', fontWeight: 700, color: '#1e293b', lineHeight: 1.4, marginBottom: '4px' }}>{item.titulo || item.nome}</p>
      {item.mlItemId && <p style={{ fontSize: '10px', color: '#94a3b8', fontFamily: 'monospace' }}>{item.mlItemId}</p>}
      {item.motivo && <p style={{ fontSize: '10px', color: '#ef4444', marginTop: '6px', lineHeight: 1.4 }}>{item.motivo}</p>}
    </div>
  );
}
...
function downloadLogs(logs) {
  ...
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url; a.download = `varredura_${new Date().toISOString().slice(0,10)}.txt`; a.click();
  URL.revokeObjectURL(url);
}
...
export default function MercadoLivre() {
  ...
  const [botStats, setBotStats] = useState(null); // stats finais do bot
  ...
  const conectarML = async () => {
    ...
    } catch { alert({ title: 'Erro', message: 'Falha ao buscar URL de autenticação.' }); }
  };

  const desconectarML = async () => {
    if (!await confirm({ title: 'Desconectar ML', message: 'Deseja desconectar sua conta do Mercado Livre?', confirmLabel: 'Desconectar', danger: true })) return;
  ...
  const gerarResumoLogistico = async () => {
    ...
    try {
      const dadosStr = `Total de divergências: ${divStats.total}. Pendentes: ${divStats.pendente}. Reincidentes: ${divStats.reincidente}. Corrigidas: ${divStats.corrigido}. Ignoradas: ${divStats.ignorado}. Anúncios não vinculados: ${naoVinculados.length}. Produtos no catálogo: ${produtos.length}.`;
    ...
    } catch { setResumoIA('<b>Diagnóstico Geral</b><br>Erro ao processar resumo. Tente novamente.'); }
  ...
  const buscarDivergencias = async () => {
    setLoadingDiv(true);
    try {
      // Busca com o filtro correto — TODOS não passa status
      const statusParam = filtroStatus === 'TODOS' ? '' : `&status=${filtroStatus}`;
      const data = await apiGet(`/api/divergencias?plataforma=Mercado%20Livre&userId=${userId}${statusParam}`);
  ...
  const acaoLoteDivs = async (tipo) => {
    ...
    if (tipo === 'excluir' && !await confirm({ title: 'Excluir divergências', message: `Remover ${selectedDivs.size} registro(s) selecionado(s)?`, confirmLabel: 'Excluir', danger: true })) return;
  ...
  const acaoLoteNaoVinculados = async (acaoStr) => {
    ...
    if (acaoStr === 'excluir' && !await confirm({ title: 'Excluir anúncios', message: `Excluir ${selectedNaoVinc.size} anúncio(s) selecionado(s)?`, confirmLabel: 'Excluir', danger: true })) return;
  ...
  const excluirProduto = async (id) => {
    if (!await confirm({ title: 'Excluir produto', message: 'Remover este produto do catálogo?', confirmLabel: 'Excluir', danger: true })) return;
    try { await fetch(`${API_BASE_URL}/api/produtos/${id}`, { method: 'DELETE' }); await buscarProdutos(); } catch {}
  };

  const desvincularProduto = async (id) => {
    if (!await confirm({ title: 'Desvincular anúncio', message: 'O anúncio voltará para Não Vinculados. Continuar?', confirmLabel: 'Desvincular' })) return;
  ...
  const excluirLoteProdutos = async () => {
    ...
    if (!await confirm({ title: 'Excluir produtos', message: `Excluir ${selectedProds.size} produto(s)?`, confirmLabel: 'Excluir', danger: true })) return;
  ...
  const abrirModalVincular = (prod) => {
    setVincularAnuncio(prod);
    setComposicaoKit([]);
    setBuscaBase('');
    setPesoManual('');
    setUnidadeManual('g');
  ...
  // ── Varredura com progresso realista e dramático ───────────────────────────
  const iniciarBot = () => {
    if (!userId) return;
    ...
    eventSource.onmessage = (e) => {
      try {
        ...
        if (data.type === 'done') {
          ...
          if (data.stats) setBotStats(data.stats);
          eventSource.close();
          buscarDivergencias(); buscarDivStats(); buscarNaoVinculados(); buscarInsightIA();
        }
      } catch (_) {}
    };
  ...
  // ── Filtros ────────────────────────────────────────────────────────────────
  const divsFiltradas = divergencias.filter(d => {
    const matchBusca = !buscaDiv || (d.titulo || '').toLowerCase().includes(buscaDiv.toLowerCase()) || (d.mlItemId || '').toLowerCase().includes(buscaDiv.toLowerCase());
    return matchBusca;
  });
  ...
  // ── Métricas rápidas ───────────────────────────────────────────────────────
  const taxaReincidencia = divStats.total > 0 ? ((divStats.reincidente / divStats.total) * 100).toFixed(0) : 0;
  const taxaCorrecao = divStats.total > 0 ? ((divStats.corrigido / divStats.total) * 100).toFixed(0) : 0;
  const produtosVinculados = produtos.filter(p => p.mlItemId && p.plataforma !== 'ML_PENDENTE' && p.plataforma !== 'ML_IGNORADO').length;
  const produtosBase = produtos.filter(p => !p.mlItemId && p.plataforma !== 'ML_PENDENTE').length;
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 py-2 h-full flex flex-col relative animate-in fade-in duration-300">
        ...
        <div className="flex flex-col xl:flex-row justify-between xl:items-start mb-4 shrink-0 gap-3 relative z-10">
          <div className="flex items-start gap-3">
            ...
            <div className="hidden md:flex flex-col max-h-[50px] overflow-y-auto custom-scrollbar max-w-md pl-2">
              {aiInsight && <span className="text-[11px] font-semibold text-slate-600 leading-snug break-words" dangerouslySetInnerHTML={{ __html: aiInsight }} />}
            </div>
          </div>
          ...
          <div className="flex items-center flex-wrap gap-2 justify-end">
            ...
            <div className="flex gap-1.5">
              <button onClick={() => { buscarHistoricoResumos(); setShowResumoModal(true); }} className="flex items-center gap-1 px-2 py-1.5 bg-indigo-50 text-indigo-600 rounded-xl hover:bg-indigo-100 shadow-sm border border-indigo-100 text-[10px] font-black uppercase" title="Resumo IA">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4 shrink-0">
          <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-sm">
            <div className="flex items-center justify-between mb-2">
              <span className="text-[9px] font-black uppercase tracking-widest text-slate-400">Divergências</span>
              <AlertTriangle className="w-4 h-4 text-amber-500" />
            </div>
            <p className="text-2xl font-black text-slate-800">{divStats.pendente + divStats.reincidente}</p>
            <p className="text-[10px] text-slate-500 mt-1">{divStats.reincidente} reincidentes</p>
            <div className="h-1 bg-slate-100 rounded-full mt-2 overflow-hidden">
              <div className="h-full bg-amber-400 rounded-full" style={{ width: `${Math.min((divStats.pendente / Math.max(divStats.total, 1)) * 100, 100)}%` }}></div>
            </div>
          </div>
          <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-sm">
            <div className="flex items-center justify-between mb-2">
              <span className="text-[9px] font-black uppercase tracking-widest text-slate-400">Taxa de Correção</span>
              <TrendingUp className="w-4 h-4 text-emerald-500" />
            </div>
            <p className="text-2xl font-black text-emerald-600">{taxaCorrecao}%</p>
            <p className="text-[10px] text-slate-500 mt-1">{divStats.corrigido} corrigidas</p>
            <div className="h-1 bg-slate-100 rounded-full mt-2 overflow-hidden">
              <div className="h-full bg-emerald-400 rounded-full" style={{ width: `${taxaCorrecao}%` }}></div>
            </div>
          </div>
          <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-sm">
            <div className="flex items-center justify-between mb-2">
              <span className="text-[9px] font-black uppercase tracking-widest text-slate-400">Anúncios</span>
              <ShoppingBag className="w-4 h-4 text-blue-500" />
            </div>
            <p className="text-2xl font-black text-slate-800">{produtosVinculados}</p>
            <p className="text-[10px] text-slate-500 mt-1">{naoVinculados.length} aguardando vínculo</p>
            <div className="h-1 bg-slate-100 rounded-full mt-2 overflow-hidden">
              <div className="h-full bg-blue-400 rounded-full" style={{ width: `${produtosVinculados > 0 ? Math.min((produtosVinculados / (produtosVinculados + naoVinculados.length)) * 100, 100) : 0}%` }}></div>
            </div>
          </div>
          <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-sm">
            <div className="flex items-center justify-between mb-2">
              <span className="text-[9px] font-black uppercase tracking-widest text-slate-400">Catálogo Base</span>
              <Package className="w-4 h-4 text-slate-500" />
            </div>
            <p className="text-2xl font-black text-slate-800">{produtosBase}</p>
            <p className="text-[10px] text-slate-500 mt-1">{produtos.filter(p => p.eKit).length} kits cadastrados</p>
            <div className="h-1 bg-slate-100 rounded-full mt-2 overflow-hidden">
              <div className="h-full bg-slate-400 rounded-full" style={{ width: `${produtosBase > 0 ? 70 : 0}%` }}></div>
            </div>
          </div>
        </div>
      )}
      ...
      {/* ── CONTEÚDO PRINCIPAL ── */}
      <div className={`relative flex-1 min-h-0 transition-all duration-300 ${fullScreenMode ? 'fixed inset-0 z-[999999] bg-[#f8fafc] p-3 sm:p-5 flex flex-col' : ''}`}>

        {/* ── ABA SCANNER ── */}
        {activeTab === 'bot' && (
          <div className={`grid grid-cols-1 xl:grid-cols-5 gap-4 ${fullScreenMode ? 'flex-1 min-h-0' : 'h-full'}`}>

            {/* Painel esquerdo: controles + terminal */}
            <section className="xl:col-span-2 bg-white border border-slate-200 rounded-3xl shadow-md flex flex-col overflow-hidden h-full">
              <div className="p-4 border-b border-slate-100 space-y-3">
                <button
                  ...
                  className={`w-full py-3 rounded-xl font-black text-[11px] uppercase tracking-widest transition-all shadow-sm ${isBotRunning || !mlConectado ? 'bg-slate-100 text-slate-400' : 'bg-blue-600 text-white hover:bg-blue-700'}`}
                >
                  {isBotRunning
                    ? <span className="flex items-center justify-center gap-2"><Loader2 className="w-3.5 h-3.5 animate-spin" /> Auditando ML...</span>
                    : !mlConectado
                      ? '🔒 Conecte o ML nas Configurações'
                ...
                <div className="bg-slate-900 rounded-xl p-3 border border-slate-800">
                  <div className="flex justify-between items-center text-[9px] font-black uppercase tracking-widest mb-2">
                    <span className="text-slate-400 flex items-center gap-1.5">
                      {isBotRunning ? <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse inline-block"></span> : null}
                  ...
                  <div className="h-2 bg-slate-800 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${progress}%`,
                        background: progress < 30 ? '#3b82f6' : progress < 60 ? '#f59e0b' : progress < 90 ? '#8b5cf6' : '#10b981'
                      }}
                    ></div>
                  </div>
                  {isBotRunning && (
                    <p className="text-[9px] text-slate-500 mt-1.5 text-center animate-pulse">
                      {progress < 5 ? 'Autenticando...' : progress < 20 ? 'Mapeando anúncios...' : progress < 25 ? 'Carregando catálogo...' : progress < 90 ? 'Auditando divergências...' : 'Finalizando relatório...'}
                    </p>
                  )}
                </div>

                {/* Resumo pós-varredura */}
                {botStats && !isBotRunning && (
                  <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-3 space-y-1.5">
                    <p className="text-[10px] font-black text-emerald-700 uppercase tracking-widest mb-2 flex items-center gap-1.5">
                      <CheckCircle2 className="w-3.5 h-3.5" /> Resultado da Varredura
                    </p>
                    <div className="grid grid-cols-2 gap-1.5 text-[10px]">
                      <div className="bg-white rounded-lg p-2 border border-emerald-100">
                        <p className="text-slate-400 font-bold">Auditados</p>
                        <p className="text-slate-800 font-black text-base">{botStats.totalAnuncios}</p>
                      </div>
                      <div className="bg-white rounded-lg p-2 border border-emerald-100">
                        <p className="text-slate-400 font-bold">Sem divergência</p>
                        <p className="text-emerald-600 font-black text-base">{botStats.anunciosOk}</p>
                      </div>
                      <div className="bg-white rounded-lg p-2 border border-amber-100">
                        <p className="text-slate-400 font-bold">Novas divergências</p>
                        <p className="text-amber-600 font-black text-base">{botStats.novasDiv}</p>
                      </div>
                      <div className="bg-white rounded-lg p-2 border border-purple-100">
                        <p className="text-slate-400 font-bold">Reincidentes</p>
                        <p className="text-purple-600 font-black text-base">{botStats.reincidentes}</p>
                      </div>
                      <div className="bg-white rounded-lg p-2 border border-emerald-100">
                        <p className="text-slate-400 font-bold">Resolvidos auto</p>
                        <p className="text-emerald-600 font-black text-base">{botStats.resolvidas}</p>
                      </div>
                      <div className="bg-white rounded-lg p-2 border border-blue-100">
                        <p className="text-slate-400 font-bold">Sem vínculo</p>
                        <p className="text-blue-600 font-black text-base">{botStats.pendentesCriados}</p>
                      </div>
                    </div>
                    {botStats.maioresDivergencias?.length > 0 && (
                      <div className="mt-2 pt-2 border-t border-emerald-100">
                        <p className="text-[9px] font-black text-red-600 uppercase mb-1.5">Top impacto no frete:</p>
                        {botStats.maioresDivergencias.slice(0, 3).map((d, i) => (
                          <div key={i} className="flex items-center gap-1.5 text-[9px] text-slate-600 mb-1">
                            <span className="w-4 h-4 rounded bg-red-100 text-red-600 font-black flex items-center justify-center flex-shrink-0">{i+1}</span>
                            <span className="truncate flex-1">{d.titulo}</span>
                            <span className="text-red-500 font-black">{d.diff}g</span>
                          </div>
                        ))}
                      </div>
                    )}
                    <button onClick={() => downloadLogs(logs)} className="w-full mt-1 py-1.5 rounded-lg bg-white border border-emerald-200 text-emerald-700 text-[9px] font-black uppercase flex items-center justify-center gap-1">
                      <Download className="w-3 h-3" /> Baixar Log Completo
                    </button>
                  </div>
                )}
              </div>
              ...
              {/* Terminal */}
              <div ref={terminalRef} className="flex-1 bg-slate-950 m-3 mt-0 rounded-xl p-3 overflow-y-auto custom-scrollbar flex flex-col gap-1 min-h-0">
                {logs.map((log, i) => (
                ...
                {isBotRunning && (
                  <div className="flex items-center gap-2 text-[10px] text-blue-400 font-mono mt-1">
                  ...
                  <div className="flex items-center gap-1.5">
                    {selectedDivs.size > 0 && (
                      <div className="flex items-center gap-1 bg-blue-50 px-2 py-1 rounded-lg border border-blue-200 shadow-sm">
                        ...
                        <button onClick={() => acaoLoteDivs('corrigido')} className="p-1 text-emerald-600 hover:bg-white rounded" title="Marcar corrigidas"><Check className="w-3.5 h-3.5" /></button>
                        <button onClick={() => acaoLoteDivs('ignorado')} className="p-1 text-slate-500 hover:bg-white rounded" title="Ignorar"><EyeOff className="w-3.5 h-3.5" /></button>
                        <button onClick={() => acaoLoteDivs('excluir')} className="p-1 text-red-500 hover:bg-white rounded" title="Excluir"><Trash2 className="w-3.5 h-3.5" /></button>
                      </div>
                    )}
                  ...
                  <div className="flex gap-1 flex-wrap">
                    {['PENDENTE', 'REINCIDENTE', 'CORRIGIDO', 'IGNORADO', 'TODOS'].map(s => {
                      ...
                      return (
                        <button
                          key={s}
                          onClick={() => { setFiltroStatus(s); setSelectedDivs(new Set()); }}
                          className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[9px] font-black uppercase border transition-all ${filtroStatus === s ? `${cfg.bg} ${cfg.color} ${cfg.border} shadow-sm` : 'bg-white text-slate-500 border-slate-200 hover:bg-slate-50'}`}
                        >
                          {cfg.label} <span className={`px-1 rounded text-[9px] ${filtroStatus === s ? 'bg-white/60' : 'bg-slate-100'}`}>{count}</span>
                        </button>
                      );
                    })}
                  </div>
                ...
                {loadingDiv ? (
                  ...
                  <div className="py-16 flex flex-col items-center gap-3">
                    ...
                    <p className="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em]">
                      {filtroStatus === 'TODOS' ? 'Nenhuma divergência encontrada' : `Nenhuma divergência ${STATUS_CONFIG[filtroStatus]?.label?.toLowerCase() || ''}`}
                    </p>
                  </div>
                ) : (
                  <table className="w-full text-left">
                    <thead className="sticky top-0 bg-slate-50 border-b border-slate-200 shadow-sm z-10">
                      <tr className="text-[9px] font-black uppercase tracking-widest text-slate-500">
                        ...
                        <th className="px-4 py-2.5">Anúncio ML</th>
                        ...
                        return (
                          <tr key={div.id} className={`hover:bg-slate-50/80 transition-colors ${sel ? 'bg-blue-50/30' : ''}`}>
                            <td className="px-4 py-2.5">
                              <button onClick={() => { const n = new Set(selectedDivs); sel ? n.delete(div.id) : n.add(div.id); setSelectedDivs(n); }} className="text-slate-400">
                                {sel ? <CheckSquare className="w-3.5 h-3.5 text-blue-600" /> : <Square className="w-3.5 h-3.5" />}
                              </button>
                            </td>
                            <td className="py-2.5 px-4">
                              <span className={`text-[8px] font-black px-1.5 py-0.5 rounded-sm ${cfg.bg} ${cfg.color} flex items-center gap-1 w-fit mb-1 border ${cfg.border}`}>
                                <cfg.icon className="w-2.5 h-2.5" />{cfg.label}
                              </span>
                              <p className="font-bold text-slate-800 text-[11px] truncate max-w-[200px]">{div.titulo || div.mlItemId}</p>
                              <a
                                href={`https://produto.mercadolivre.com.br/MLB-${(div.mlItemId||'').replace(/^MLB/i,'')}`}
                                target="_blank"
                                rel="noopener noreferrer"
                                className="text-blue-500 hover:text-blue-700 text-[9px] font-mono mt-0.5 flex items-center gap-0.5"
                              >
                                {div.mlItemId} <ExternalLink className="w-2.5 h-2.5" />
                              </a>
                            </td>
                            <td className="py-2.5 px-4">
                              <p className="text-slate-600 text-[10px] font-medium leading-relaxed max-w-[250px]">{div.motivo}</p>
                              {div.pesoMl > 0 && div.pesoLocal > 0 && (
                                <div className="flex items-center gap-2 mt-1">
                                  <span className="text-[9px] bg-blue-50 text-blue-600 px-1.5 py-0.5 rounded font-bold border border-blue-100">ML: {div.pesoMl}g</span>
                                  <span className="text-[9px] text-slate-400">→</span>
                                  <span className="text-[9px] bg-emerald-50 text-emerald-600 px-1.5 py-0.5 rounded font-bold border border-emerald-100">Real: {div.pesoLocal}g</span>
                                </div>
                              )}
                            </td>
                            <td className="py-2.5 px-4 text-right">
                              <div className="flex items-center justify-end gap-1 relative">
                                ...
                                {div.status !== 'CORRIGIDO' && <button onClick={() => acaoDiv(div.id, 'corrigido')} className="p-1.5 rounded-lg bg-emerald-50 text-emerald-600 hover:bg-emerald-100" title="Marcar corrigido"><Check className="w-3.5 h-3.5" /></button>}
                                {div.status !== 'IGNORADO' && <button onClick={() => acaoDiv(div.id, 'ignorado')} className="p-1.5 rounded-lg bg-slate-100 text-slate-500 hover:bg-slate-200" title="Ignorar"><EyeOff className="w-3.5 h-3.5" /></button>}
                              </div>
                            </td>
                          </tr>
                        );
        ...
        {activeTab === 'pendentes' && (
          <section className="bg-white border border-slate-200 rounded-3xl shadow-md flex flex-col h-full overflow-hidden">
            <div className="p-5 border-b border-slate-100 bg-amber-50/30 flex justify-between items-center flex-wrap gap-3">
              <div>
                <h3 className="text-[14px] font-black text-amber-800 uppercase flex items-center gap-2"><Box className="w-5 h-5" /> Anúncios Desconhecidos ({nVincFiltrados.length})</h3>
                <p className="text-[10px] text-amber-600 mt-0.5">Anúncios do ML sem produto vinculado no catálogo local</p>
              </div>
              <div className="flex items-center gap-2">
                <div className="relative w-48">
                  <Search className="absolute left-2.5 top-2 w-3.5 h-3.5 text-amber-600/50"/>
                  <input value={buscaNaoVinc} onChange={e => setBuscaNaoVinc(e.target.value)} placeholder="Pesquisar..." className="w-full pl-7 pr-2 py-1.5 bg-white border border-amber-200 rounded-lg outline-none focus:border-amber-400 text-[10px]" />
                </div>
                {selectedNaoVinc.size > 0 && (
                  <div className="flex items-center gap-1 bg-white border border-amber-200 rounded-lg px-2 py-1 shadow-sm">
                    ...
                    <button onClick={() => acaoLoteNaoVinculados('ignorado')} className="p-1 text-slate-500 hover:bg-slate-100 rounded" title="Ignorar selecionados"><EyeOff className="w-3.5 h-3.5"/></button>
                    <button onClick={() => acaoLoteNaoVinculados('excluir')} className="p-1 text-red-500 hover:bg-red-50 rounded" title="Excluir selecionados"><Trash2 className="w-3.5 h-3.5"/></button>
                  </div>
                )}
                  ...
                  <thead className="sticky top-0 bg-white border-b border-slate-100 shadow-sm z-10">
                    <tr className="text-[10px] font-black uppercase tracking-widest text-slate-400">
                      <th className="px-5 py-3 w-10">
                        <button onClick={() => toggleSelectAll(nVincFiltrados, selectedNaoVinc, setSelectedNaoVinc, 'id')} className="text-slate-400">
                          {selectedNaoVinc.size === nVincFiltrados.length && nVincFiltrados.length > 0 ? <CheckSquare className="w-4 h-4" /> : <Square className="w-4 h-4" />}
                        </button>
                      </th>
                      <th className="px-5 py-3">Produto ML</th>
                      ...
                      return (
                        <tr key={prod.id} className={`hover:bg-slate-50/60 transition-colors ${sel ? 'bg-amber-50/30' : ''}`}>
                          <td className="px-5 py-3">
                            <button onClick={() => { const n = new Set(selectedNaoVinc); sel ? n.delete(prod.id) : n.add(prod.id); setSelectedNaoVinc(n); }} className="text-slate-400 hover:text-amber-500">
                              {sel ? <CheckSquare className="w-4 h-4 text-amber-500" /> : <Square className="w-4 h-4" />}
                            </button>
                          </td>
                          <td className="px-5 py-3 flex items-center gap-3">
                            {prod.thumbnail ? <img src={prod.thumbnail} className="w-10 h-10 object-cover rounded-lg border border-slate-100" alt="" /> : <div className="w-10 h-10 bg-slate-100 rounded-lg flex items-center justify-center"><ImageIcon className="w-4 h-4 text-slate-300"/></div>}
                            <div>
                              <p className="font-bold text-slate-800 max-w-[350px] truncate">{prod.nome}</p>
                              <a href={`https://produto.mercadolivre.com.br/MLB-${(prod.mlItemId||'').replace(/^MLB/i,'')}`} target="_blank" rel="noopener noreferrer" className="text-[9px] font-mono text-blue-500 hover:text-blue-700 bg-blue-50 px-1.5 py-0.5 rounded mt-1 inline-flex items-center gap-0.5 border border-blue-100">
                          ...
                          <td className="px-5 py-3 text-right">
                            <div className="flex items-center justify-end gap-1.5 relative">
                              ...
                              <button onClick={() => acaoNaoVinculadoBasica(prod.id, 'ignorado')} className="p-1.5 rounded-lg bg-slate-50 text-slate-500 hover:bg-slate-100" title="Ignorar"><EyeOff className="w-3.5 h-3.5" /></button>
                            </div>
                          </td>
                        </tr>
                      );
            ...
            <section className="col-span-2 bg-white border border-slate-200 rounded-3xl shadow-md flex flex-col overflow-hidden h-full">
              ...
              <div className="flex-1 overflow-y-auto p-5 custom-scrollbar">
                <form onSubmit={handleSubmitProduto} className="space-y-4 text-[10px] font-semibold text-slate-600">
                  ...
                  {editandoId && <button type="button" onClick={() => { setFormProd(FORM_INICIAL); setEditandoId(null); }} className="w-full py-2 bg-slate-100 text-slate-600 font-black text-[10px] uppercase rounded-xl">Cancelar Edição</button>}
                </form>
              </div>
            </section>
            ...
            <section className="col-span-3 bg-white border border-slate-200 rounded-3xl shadow-md flex flex-col h-full overflow-hidden">
              <div className="p-4 border-b border-slate-100 flex justify-between items-center flex-wrap gap-3">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  {selectedProds.size > 0 && <button onClick={excluirLoteProdutos} className="p-1.5 rounded-lg bg-red-50 text-red-600 hover:bg-red-100" title="Excluir selecionados"><Trash2 className="w-3.5 h-3.5" /></button>}
                  ...
                  <thead className="bg-slate-50 sticky top-0 z-10 border-b border-slate-100 shadow-sm">
                    <tr className="text-[9px] font-black uppercase tracking-widest text-slate-400">
                      <th className="px-4 py-3 w-8">
                        <button onClick={() => toggleSelectAll(produtosFiltrados, selectedProds, setSelectedProds)} className="text-slate-400">
                      ...
                      <th className="px-4 py-3">Item</th><th className="px-4 py-3">Peso</th><th className="px-4 py-3 text-right">Ação</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-50 text-[11px]">
                        ...
                        <tr key={prod.id} className={`hover:bg-slate-50/80 transition-colors ${sel ? 'bg-blue-50/30' : ''}`}>
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-3">
                              {prod.thumbnail ? <img src={prod.thumbnail} className="w-10 h-10 rounded-lg object-cover shadow-sm border border-slate-100" alt="" /> : <div className="w-10 h-10 bg-slate-100 rounded-lg flex items-center justify-center"><ImageIcon className="w-4 h-4 text-slate-300"/></div>}
                              <div>
                                <div className="flex flex-wrap items-center gap-1.5 mb-1">
                                  ...
                                  {prod.mlItemId && (
                                    <a href={`https://produto.mercadolivre.com.br/MLB-${(prod.mlItemId||'').replace(/^MLB/i,'')}`} target="_blank" rel="noopener noreferrer" className="text-[8px] font-bold text-emerald-700 bg-emerald-100 px-1.5 py-0.5 rounded flex items-center gap-0.5 hover:bg-emerald-200">
                                      <Check className="w-2.5 h-2.5"/> {prod.mlItemId} <ExternalLink className="w-2 h-2" />
                                    </a>
                                  )}
                          ...
                          <td className="px-4 py-3 text-right">
                            <div className="flex justify-end gap-1.5 relative">
                              ...
                              <button onClick={() => { setFormProd({ sku: prod.sku, nome: prod.nome, preco: prod.preco, mlItemId: prod.mlItemId || '', peso: prod.pesoGramas, unidadePeso: 'g', alturaCm: prod.alturaCm || '', larguraCm: prod.larguraCm || '', comprimentoCm: prod.comprimentoCm || '', eKit: prod.eKit, categoria: prod.categoria || '', plataforma: prod.plataforma }); setEditandoId(prod.id); }} className="p-1.5 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-100" title="Editar"><Settings className="w-4 h-4" /></button>
                              ...
                              <button onClick={() => excluirProduto(prod.id)} className="p-1.5 rounded-lg bg-red-50 text-red-500 hover:bg-red-100" title="Excluir"><Trash2 className="w-4 h-4" /></button>
                            </div>
                          </td>
                        </tr>
                      );
      ...
      {/* ══ MODAIS — z-index [999999] para ficar acima do MainLayout ══════════ */}
        ...
        <div className="fixed inset-0 z-[999999] bg-slate-900/80 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-5xl flex flex-col max-h-[90vh]">
            <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-white rounded-t-3xl">
              <h3 className="font-black text-slate-800 uppercase tracking-widest flex items-center gap-2"><Box className="w-4 h-4 text-blue-600"/> Detalhes do Anúncio — Mercado Livre</h3>
            ...
            <div className="flex-1 overflow-y-auto bg-slate-50/50 p-6 custom-scrollbar rounded-b-3xl">
              {loadingFullData ? (
                <div className="flex flex-col items-center justify-center py-20 text-blue-500"><Loader2 className="w-8 h-8 animate-spin mb-3"/><p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Buscando dados do ML...</p></div>
              ) : fullItemData ? (
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                  <div className="lg:col-span-5 space-y-4">
                    <div className="bg-white rounded-2xl p-2 border border-slate-200 flex items-center justify-center h-64 shadow-sm">
                      {fullItemData.pictures?.length > 0 ? <img src={fullItemData.pictures[0].url} className="max-h-full max-w-full object-contain rounded-lg" alt="" /> : <ImageIcon className="w-12 h-12 text-slate-200"/>}
                    </div>
                    {fullItemData.pictures?.length > 1 && (
                      <div className="flex gap-2 overflow-x-auto pb-2 custom-scrollbar">
                        {fullItemData.pictures.slice(1, 5).map((pic, idx) => (
                          <img key={idx} src={pic.url} className="w-16 h-16 object-cover rounded-xl border border-slate-200 shadow-sm flex-shrink-0" alt="" />
                        ))}
                      </div>
                    )}
                    <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
                      <a href={`https://produto.mercadolivre.com.br/MLB-${(fullItemData.id||'').replace(/^MLB/i,'')}`} target="_blank" rel="noopener noreferrer" className="text-[10px] font-mono bg-blue-50 text-blue-700 px-2 py-1 rounded font-black border border-blue-100 hover:bg-blue-100 inline-flex items-center gap-1">
                        {fullItemData.id} <ExternalLink className="w-3 h-3" />
                      </a>
                      ...
                      <a href={fullItemData.permalink} target="_blank" rel="noopener noreferrer" className="mt-4 flex items-center justify-center gap-1.5 w-full py-3 bg-[#FFE600] text-slate-900 rounded-xl text-[11px] font-black uppercase tracking-widest shadow-sm hover:bg-[#facc15]">
                ...
                <div className="text-center py-20 text-slate-400 text-[11px] uppercase font-black">
                  {fullModalItem.mlItemId ? 'Falha ao obter dados do ML' : 'Este produto não possui ID do ML vinculado'}
                </div>
              )}
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-[999999] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-5 border-b border-slate-100 bg-white flex justify-between items-center z-10 shadow-sm">
              <h3 className="font-black text-slate-800 uppercase text-[12px] tracking-tight flex items-center gap-2"><Link2 className="w-5 h-5 text-emerald-600"/> Vincular Anúncio ao Catálogo</h3>
            ...
            <div className="flex-1 overflow-y-auto p-6 bg-slate-50 custom-scrollbar">
              <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm mb-6 flex gap-4 items-center">
                {vincularAnuncio.thumbnail ? <img src={vincularAnuncio.thumbnail} className="w-14 h-14 object-cover rounded-xl border border-slate-100" alt="" /> : <div className="w-14 h-14 bg-slate-100 rounded-xl flex items-center justify-center"><ImageIcon className="w-5 h-5 text-slate-300"/></div>}
                <div>
                  <a href={`https://produto.mercadolivre.com.br/MLB-${(vincularAnuncio.mlItemId||'').replace(/^MLB/i,'')}`} target="_blank" rel="noopener noreferrer" className="text-[10px] font-mono text-emerald-700 bg-emerald-50 border border-emerald-100 px-2 py-0.5 rounded font-black hover:bg-emerald-100 inline-flex items-center gap-1">
                    {vincularAnuncio.mlItemId} <ExternalLink className="w-2.5 h-2.5" />
                  </a>
                  <p className="font-bold text-[12px] text-slate-800 mt-1 leading-tight line-clamp-2">{vincularAnuncio.nome}</p>
                </div>
              </div>
              <div className="mb-6">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-2 block">1. Montar Kit (buscar produtos base)</label>
                <div className="relative mb-2">
                  <Search className="absolute left-3 top-3 w-4 h-4 text-slate-400"/>
                  <input value={buscaBase} onChange={e => setBuscaBase(e.target.value)} placeholder="Nome do produto base..." className="w-full pl-9 pr-3 py-2.5 bg-white border border-slate-200 rounded-xl outline-none focus:border-blue-500 text-[12px]" />
                </div>
                {buscaBase && (
                  <div className="mb-3 border border-slate-200 rounded-xl bg-white shadow-lg overflow-hidden">
                    {produtosBaseParaBusca.length === 0 && <p className="p-3 text-[11px] text-slate-400">Nenhum produto encontrado</p>}
                    {produtosBaseParaBusca.map(p => (
                      <div key={p.id} className="p-3 border-b border-slate-50 flex justify-between items-center hover:bg-slate-50">
                        ...
                        <button onClick={() => { if (!composicaoKit.find(c => c.produto.id === p.id)) setComposicaoKit([...composicaoKit, {produto: p, quantidade: 1}]); setBuscaBase(''); }} className="text-[10px] font-black uppercase bg-blue-50 text-blue-600 px-3 py-1.5 rounded-lg hover:bg-blue-100">Add</button>
                      </div>
                    ))}
                  </div>
                )}
                    ...
                    <div key={item.produto.id} className="flex justify-between items-center p-3 bg-white border border-slate-200 rounded-xl shadow-sm">
                      ...
                      <div className="flex items-center gap-3">
                        <div className="flex items-center gap-1.5 bg-slate-50 border border-slate-200 rounded-lg p-1">
                          <button onClick={() => { const q = item.quantidade - 1; if(q<=0) setComposicaoKit(composicaoKit.filter(c=>c.produto.id!==item.produto.id)); else setComposicaoKit(composicaoKit.map(c=>c.produto.id===item.produto.id?{...c,quantidade:q}:c)); }} className="text-slate-500 hover:text-red-500 bg-white shadow-sm p-0.5 rounded">
                            <span className="w-4 h-4 flex items-center justify-center font-black text-sm">−</span>
                          </button>
                          <span className="w-6 text-center font-black text-[12px]">{item.quantidade}</span>
                          <button onClick={() => setComposicaoKit(composicaoKit.map(c=>c.produto.id===item.produto.id?{...c,quantidade:item.quantidade+1}:c))} className="text-slate-500 hover:text-blue-500 bg-white shadow-sm p-0.5 rounded">
                            <span className="w-4 h-4 flex items-center justify-center font-black text-sm">+</span>
                          </button>
                        </div>
                ...
                {composicaoKit.length > 0 && (
                  <div className="mt-3 p-3 bg-blue-50 rounded-xl border border-blue-100 text-[11px] font-bold text-blue-700 flex justify-between">
                    <span>Peso total do kit:</span>
                    <span className="font-black">{composicaoKit.reduce((s, c) => s + c.produto.pesoGramas * c.quantidade, 0)}g</span>
                  </div>
                )}
              </div>
              {composicaoKit.length === 0 && (
                <div>
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-2 block">2. Ou informar Peso Fixo</label>
                  <div className="flex gap-2">
                    ...
                    <select value={unidadeManual} onChange={e => setUnidadeManual(e.target.value)} className="bg-white border border-slate-200 rounded-xl px-3 text-[11px] font-black text-slate-600 outline-none"><option value="g">Gramas (g)</option><option value="kg">Quilos (kg)</option></select>
                  </div>
                </div>
              )}
            </div>
            <div className="p-5 border-t border-slate-100 bg-white flex justify-end gap-3 z-10">
              ...
              <button onClick={salvarVinculacao} disabled={composicaoKit.length === 0 && !pesoManual} className="px-8 py-2.5 rounded-xl text-[11px] font-black uppercase tracking-widest text-white bg-emerald-500 hover:bg-emerald-600 disabled:bg-slate-200 disabled:text-slate-400 shadow-md">Confirmar Vínculo</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[999999] bg-slate-900/70 backdrop-blur-sm flex justify-end animate-in fade-in duration-200">
          <div className="bg-white shadow-2xl w-full max-w-md h-full flex flex-col animate-in slide-in-from-right-8">
            <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-indigo-600 text-white">
              <h3 className="font-black uppercase tracking-widest text-[12px] flex items-center gap-2"><Sparkles className="w-4 h-4"/> Resumo Executivo</h3>
            ...
            <div className="flex-1 overflow-y-auto p-6 bg-slate-50 custom-scrollbar">
              {loadingResumo ? (
                <div className="flex flex-col items-center justify-center py-20 text-indigo-500">
                  ...
                  <p className="text-[11px] font-black uppercase tracking-widest text-indigo-400">Gerando insight...</p>
                </div>
              ) : resumoIA ? (
                <>
                  <div className="text-[12px] text-slate-700 leading-relaxed bg-white p-5 rounded-2xl shadow-sm border border-slate-200" dangerouslySetInnerHTML={{ __html: resumoIA }} />
                  <button
                    onClick={() => downloadTxt(resumoIA, `resumo_ml_${new Date().toISOString().slice(0,10)}.txt`)}
                    className="mt-3 w-full flex items-center justify-center gap-2 py-2.5 bg-white border border-slate-200 text-slate-600 rounded-xl text-[10px] font-black uppercase hover:bg-slate-50 shadow-sm"
                  >
                    <Download className="w-3.5 h-3.5" /> Baixar como TXT
                  </button>
                </>
              ) : (
                <div className="text-center py-16 text-slate-400 text-[12px]">Clique em "Gerar Resumo" para criar o relatório.</div>
              )}

              {historicoResumos.length > 0 && (
                <div className="mt-8 pt-6 border-t border-slate-200">
                  <h4 className="text-[10px] font-black uppercase text-slate-400 mb-4 flex items-center gap-1.5"><History className="w-3.5 h-3.5"/> Histórico de Resumos</h4>
            ...
            <div className="p-5 border-t border-slate-100 bg-white">
              <button onClick={gerarResumoLogistico} disabled={loadingResumo} className="w-full py-3.5 bg-indigo-600 text-white rounded-xl text-[11px] font-black uppercase shadow-md hover:bg-indigo-700 disabled:opacity-50">
                {loadingResumo ? 'Gerando...' : 'Gerar Novo Resumo'}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-[999999] bg-slate-900/70 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden">
            <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-blue-600 text-white">
              <h3 className="font-black uppercase tracking-widest text-[12px] flex items-center gap-2"><HelpCircle className="w-5 h-5"/> Como funciona</h3>
              <button onClick={() => setShowDicas(false)} className="text-blue-200 hover:text-white"><XCircle className="w-6 h-6"/></button>
            </div>
            <div className="p-8 space-y-5 text-[13px] text-slate-600 leading-relaxed">
              <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100">
                <h4 className="font-black text-slate-800 mb-2 flex items-center gap-2"><Box className="w-4 h-4 text-amber-500"/> Não Vinculados</h4>
                <p>Anúncios encontrados no ML que ainda não têm peso cadastrado. Clique em "Vincular" para montar o kit ou informar o peso fixo.</p>
              </div>
              <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100">
                <h4 className="font-black text-slate-800 mb-2 flex items-center gap-2"><Package className="w-4 h-4 text-blue-500"/> Base vs Anúncios</h4>
                <p><b>Produtos Base:</b> peça física unitária com peso cadastrado.<br/><b>Anúncios Vinculados:</b> o pacote final vendido no ML (pode ser um kit com soma de pesos).</p>
              </div>
              <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100">
                <h4 className="font-black text-slate-800 mb-2 flex items-center gap-2"><AlertTriangle className="w-4 h-4 text-purple-500"/> Reincidentes</h4>
                <p>Divergências que foram marcadas como corrigidas mas voltaram a aparecer na varredura. Indica que o ML atualizou o peso novamente.</p>
              </div>
              <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100">
                <h4 className="font-black text-slate-800 mb-2 flex items-center gap-2"><Calendar className="w-4 h-4 text-emerald-500"/> Agendador</h4>
                <p>Configure a varredura automática nas Configurações. O sistema roda o scanner no intervalo definido sem precisar abrir o sistema.</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-[999999] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in">
          <div className="bg-white rounded-3xl w-full max-w-sm shadow-2xl overflow-hidden">
            ...
            <div className="p-6 space-y-6">
              ...
              {/* Conexão ML */}
              <div>
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 block mb-3">Conexão Mercado Livre</label>
                {!mlConectado ? (
                  <button onClick={conectarML} className="w-full flex items-center justify-center gap-2 bg-[#FFE600] text-slate-900 hover:bg-[#facc15] py-3 rounded-xl text-[11px] font-black uppercase shadow-sm">
                    <ShoppingBag className="w-4 h-4" /> Conectar Conta
                  </button>
                ) : (
                  <div className="flex justify-between items-center bg-emerald-50 border border-emerald-100 p-3 rounded-xl">
                    <span className="text-[11px] font-bold text-emerald-700 flex items-center gap-2"><CheckCircle2 className="w-4 h-4"/> {mlNickname || 'Conta Vinculada'}</span>
              ...
              <div className="pt-4 border-t border-slate-100">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 block mb-3">Varredura Automática</label>

                {/* Toggle ativar/desativar */}
                <div
                  className="flex items-center justify-between bg-slate-50 border border-slate-200 p-4 rounded-2xl cursor-pointer hover:bg-slate-100 transition-colors mb-3"
                  onClick={() => {
                    const novoAtivo = !(agendador?.ativo);
                    salvarAgendador(novoAtivo, intervalo);
                  }}
                >
                  <div>
                    <p className="text-[11px] font-black text-slate-800 uppercase">
                      {agendador?.ativo ? '✅ Ativa' : 'Ativar Varredura Automática'}
                    </p>
                    <p className="text-[10px] text-slate-500 mt-0.5">
                      {agendador?.ativo
                        ? `Próxima: ${formatarProxima(agendador?.proximaExecucao)}`
                        : 'O scanner rodará automaticamente no intervalo definido'}
                    </p>
                  </div>
                  <div className={`w-10 h-5 rounded-full relative transition-colors ${agendador?.ativo ? 'bg-emerald-500' : 'bg-slate-300'}`}>
                    <div className={`absolute top-1 bg-white w-3 h-3 rounded-full transition-all shadow-sm ${agendador?.ativo ? 'left-6' : 'left-1'}`}></div>
                  </div>
                </div>

                {/* Seletor de intervalo */}
                <div className="space-y-2">
                  <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Intervalo entre Varreduras</p>
                  <div className="grid grid-cols-3 gap-1.5">
                    {INTERVALOS.map(opt => (
                      <button
                        key={opt.value}
                        onClick={() => {
                          setIntervalo(opt.value);
                          if (agendador?.ativo) salvarAgendador(true, opt.value);
                        }}
                        className={`py-2 px-2 rounded-xl text-[10px] font-black uppercase border transition-all ${intervalo === opt.value ? 'bg-blue-600 text-white border-blue-600 shadow-sm' : 'bg-white text-slate-600 border-slate-200 hover:border-blue-300 hover:bg-blue-50'}`}
                      >
                        {opt.label}
                      </button>
                    ))}
                  ...
                  {!agendador?.ativo && (
                    <button
                      ...
                      disabled={savingAgendador}
                      className="w-full py-2.5 mt-1 bg-emerald-500 text-white rounded-xl text-[11px] font-black uppercase hover:bg-emerald-600 disabled:opacity-50"
                    >
                      {savingAgendador ? 'Salvando...' : `Ativar (${INTERVALOS.find(i => i.value === intervalo)?.label || intervalo + 'min'})`}
                    </button>
                  )}
                </div>
              </div>
              ...
              <div className="pt-4 border-t border-slate-100">
                ...
                <div className="flex items-center justify-between bg-slate-50 border border-slate-200 p-4 rounded-2xl cursor-pointer hover:bg-slate-100 transition-colors" onClick={() => setModoLento(!modoLento)}>
                  <div>
                    <p className="text-[11px] font-black text-slate-800 uppercase">Standby Anti-Timeout</p>
                    <p className="text-[10px] text-slate-500 mt-1 max-w-[200px] leading-snug">Pausas entre buscas para evitar bloqueio pelo ML.</p>
                  </div>
                  <div className={`w-10 h-5 rounded-full relative transition-colors ${modoLento ? 'bg-emerald-500' : 'bg-slate-300'}`}>
                    <div className={`absolute top-1 bg-white w-3 h-3 rounded-full transition-all shadow-sm ${modoLento ? 'left-6' : 'left-1'}`}></div>
                  </div>
                </div>
              </div>
            </div>
