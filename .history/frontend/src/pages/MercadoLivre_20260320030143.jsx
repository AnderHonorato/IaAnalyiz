...
function TypewriterLog({ msg, type }) {
  ...
  return <div className={`text-[10px] font-mono leading-relaxed ${colorClass}`}><span className="mr-2 text-slate-600 select-none">❯</span>{text}</div>;
}
...
function AiInsightHeader({ insight }) {
  ...
  return (
    <div className="flex items-start gap-1.5 ml-3 max-h-[36px] overflow-y-auto custom-scrollbar pr-2 max-w-md">
      <Sparkles className="w-3.5 h-3.5 text-blue-500 animate-pulse mt-0.5 flex-shrink-0" />
      <p className="text-[10px] font-medium text-slate-500 leading-snug" dangerouslySetInnerHTML={{ __html: cleanInsight }} />
    </div>
  );
}
...
function QuickViewPopover({ item, onClose, onOpenFull }) {
  if (!item) return null;
  return (
    <div className="fixed z-[9999] bg-slate-900 text-white p-3 rounded-xl shadow-2xl border border-slate-700 w-72 right-10 top-40 animate-in fade-in slide-in-from-right-8 pointer-events-none">
...
export default function MercadoLivre() {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 py-2 h-full flex flex-col animate-in fade-in duration-300 relative">
      <QuickViewPopover item={quickViewItem} onClose={() => setQuickViewItem(null)} onOpenFull={abrirFullModal} />

      {/* Header Ultracompacto */}
      {!fullScreenMode && (
        <div className="flex justify-between items-end mb-3 shrink-0 gap-3">
          <div className="flex flex-col">
            <div className="flex items-center gap-2">
              <button onClick={() => navigate('/ml')} className="p-1 rounded bg-slate-100 hover:bg-slate-200 text-slate-500 transition-colors" title="Voltar ao Painel ML">
            ...
            <nav className="flex items-center gap-1 bg-white p-1 rounded-xl border border-slate-200 shadow-sm overflow-x-auto">
              {[['bot', 'Scanner ML'], ['pendentes', `Não Vinculados (${naoVinculados.length})`], ['produtos', 'Catálogo Local']].map(([k, l]) => (
                <button key={k} onClick={() => setActiveTab(k)}
                  className={`px-3 py-1.5 rounded-lg text-[9px] font-black uppercase transition-all whitespace-nowrap ${activeTab === k ? 'bg-slate-900 text-[#FFE600] shadow-sm' : 'text-slate-500 hover:bg-slate-100'}`}>
                  {l}
                </button>
              ))}
            </nav>
        ...
        <div className={`grid grid-cols-1 ${fullScreenMode ? 'fixed inset-0 z-[99999] bg-slate-100 p-4 gap-4 overflow-hidden' : 'lg:grid-cols-5 gap-4'} flex-1 min-h-0 transition-all`}>
          {!fullScreenMode && (
            <section className="col-span-2 bg-white border border-slate-200 rounded-2xl shadow-sm flex flex-col h-full overflow-hidden">
              ...
              <div ref={terminalRef} className="flex-1 bg-slate-950 m-2 rounded-xl p-3 overflow-y-auto custom-scrollbar flex flex-col gap-1.5">
              ...
              <div className="flex justify-between items-center gap-2 flex-wrap">
                <div className="flex gap-1.5 flex-wrap">
                  {['PENDENTE', 'REINCIDENTE', 'CORRIGIDO', 'IGNORADO', 'TODOS'].map(s => {
                    ...
                    return (
                      <button key={s} onClick={() => setFiltroStatus(s)} className={`flex items-center gap-1 px-2 py-1 rounded-lg text-[9px] font-black uppercase border transition-all ${filtroStatus === s ? `${cfg.bg} ${cfg.color} ${cfg.border} shadow-sm ring-1 ring-offset-1 ring-${cfg.color.split('-')[1]}-200` : 'bg-white text-slate-500 border-slate-200 hover:bg-slate-50'}`}>
      ...
      {/* ── ABA NÃO VINCULADOS ── */}
      {activeTab === 'pendentes' && (
        <section className={`bg-white border border-slate-200 rounded-2xl shadow-sm flex flex-col flex-1 min-h-0 overflow-hidden ${fullScreenMode ? 'fixed inset-0 z-[99999] bg-slate-100/95 p-4' : ''}`}>
