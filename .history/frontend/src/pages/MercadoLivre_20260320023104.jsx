...
function AiInsightHeader({ insight }) {
  ...
  return (
    <div className="flex items-start gap-2.5 bg-indigo-50 border border-indigo-100 px-4 py-2.5 rounded-xl shadow-sm animate-in fade-in slide-in-from-top-2 max-w-2xl">
      ...
      <p className="text-[12px] font-medium text-slate-700 leading-snug" dangerouslySetInnerHTML={{ __html: cleanInsight || insight }} />
    </div>
  );
}
...
function QuickViewPopover({ item, onClose, onOpenFull }) {
  if (!item) return null;
  return (
    <div className="fixed z-[100] bg-slate-900 text-white p-4 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] border border-slate-700 w-80 right-8 top-32 animate-in fade-in slide-in-from-right-8 pointer-events-none">
      ...
      <div className="space-y-1.5 text-[11px] mb-2 bg-slate-800 p-2.5 rounded-xl border border-slate-700">
...
export default function MercadoLivre() {
  ...
  const [logs, setLogs] = useState([{ msg: 'IA_Analyiz_Ready: Módulo Analítico Carregado.', type: 'info' }]);
  ...
  const acaoLoteNaoVinculados = async (acaoStr) => {
    ...
    if (!await confirm({ title: 'Ação em Lote', message: `Aplicar ação em ${selectedNaoVinc.size} anúncio(s)?`, confirmLabel: 'Confirmar' })) return;
  ...
  const desvincularProduto = async (id) => {
    if (!await confirm({ title: 'Desvincular', message: 'Aviso: Este anúncio perderá as regras de peso e voltará para "Não Vinculados".', confirmLabel: 'Desvincular' })) return;
  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-4 h-full flex flex-col animate-in fade-in duration-500 relative">
      ...
      {!fullScreenMode && (
        <div className="flex flex-col md:flex-row justify-between md:items-center mb-6 shrink-0 gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setShowDicas(true)} className="p-3 bg-blue-50 text-blue-600 rounded-xl hover:bg-blue-100 transition-colors shadow-sm" title="Ajuda e Dicas"><HelpCircle className="w-5 h-5" /></button>
          </div>
        </div>
      )}
      ...
      {activeTab === 'bot' && (
        <div className={`grid grid-cols-1 ${fullScreenMode ? 'fixed inset-4 z-50 bg-slate-100 p-4 gap-4 rounded-3xl' : 'lg:grid-cols-5 gap-5'} flex-1 min-h-0 transition-all`}>
          {!fullScreenMode && (
            <section className="col-span-2 bg-white border border-slate-200 rounded-3xl shadow-lg flex flex-col h-full overflow-hidden">
              <div className="p-5 border-b border-slate-100">
                <button onClick={iniciarBot} disabled={isBotRunning || !mlConectado}
                  className={`w-full py-4 rounded-2xl font-black text-[12px] uppercase tracking-widest mb-4 transition-all ${isBotRunning || !mlConectado ? 'bg-slate-100 text-slate-400' : 'bg-blue-600 text-white shadow-[0_8px_16px_rgba(37,99,235,0.2)] hover:bg-blue-700 active:scale-95'}`}>
                ...
                <div className="bg-slate-900 rounded-2xl p-4 border border-slate-800">
                  <div className="flex justify-between text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2"><span>Progresso da IA</span><span className="text-emerald-400">{progress}%</span></div>
                  <div className="h-2 bg-slate-800 rounded-full overflow-hidden"><div className="h-full bg-emerald-500 transition-all duration-300 shadow-[0_0_10px_#10b981]" style={{ width: `${progress}%` }}></div></div>
                </div>
              </div>
              <div ref={terminalRef} className="flex-1 bg-slate-950 m-4 rounded-2xl p-5 font-mono text-[11px] overflow-y-auto custom-scrollbar flex flex-col gap-2">
                {logs.map((log, i) => (<div key={i} className={log.type === 'warn' ? 'text-amber-400 font-bold' : log.type === 'success' ? 'text-emerald-400 font-bold' : 'text-slate-400'}><span className="mr-2 text-slate-600 select-none">❯</span>{log.msg}</div>))}
              </div>
            </section>
          )}

          <section className={`${fullScreenMode ? 'col-span-1' : 'col-span-3'} bg-white border border-slate-200 rounded-3xl shadow-lg flex flex-col overflow-hidden`}>
            <div className="p-4 border-b border-slate-100">
              <div className="flex justify-between items-center mb-3">
                ...
                <div className="flex items-center gap-2">
                  {selectedDivs.size > 0 && (
                    <div className="flex items-center gap-1.5 bg-blue-50 px-2 py-1.5 rounded-xl border border-blue-200">
                      <span className="text-[10px] font-black text-blue-700 px-2">{selectedDivs.size} selecionados</span>
                      <button onClick={() => acaoLoteDivs('corrigido')} className="p-1 text-emerald-600 hover:bg-white rounded" title="Corrigir Todos"><Check className="w-4 h-4" /></button>
                      <button onClick={() => acaoLoteDivs('ignorado')} className="p-1 text-slate-500 hover:bg-white rounded" title="Ignorar Todos"><EyeOff className="w-4 h-4" /></button>
                      <button onClick={() => acaoLoteDivs('excluir')} className="p-1 text-red-500 hover:bg-white rounded" title="Excluir Lote"><Trash2 className="w-4 h-4" /></button>
                    </div>
                  )}
                  {divStats.corrigido > 0 && <button onClick={limparCorrigidas} className="p-2 rounded-xl text-red-500 hover:bg-red-50" title="Limpar Corrigidas"><Trash2 className="w-4 h-4" /></button>}
                  ...
                  <button onClick={() => setFullScreenMode(!fullScreenMode)} className="p-2 rounded-xl bg-slate-50 hover:bg-slate-100 border border-slate-200 shadow-sm" title="Tela Cheia">
              ...
              <div className="flex justify-between items-center gap-2 flex-wrap">
                ...
                <div className="relative w-full sm:w-64 mt-2 sm:mt-0">
                  ...
                  <input value={buscaDiv} onChange={e => setBuscaDiv(e.target.value)} placeholder="Pesquisar divergência..." className="w-full pl-9 pr-3 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-blue-500 text-[11px]" />
                </div>
              </div>
            </div>
                ...
                <table className="w-full text-left">
                  <thead className="sticky top-0 bg-slate-50 border-b border-slate-200 shadow-sm z-10">
                    <tr className="text-[10px] font-black uppercase tracking-widest text-slate-500">
                      <th className="px-5 py-3 w-8"><button onClick={() => toggleSelectAll(divsFiltradas, selectedDivs, setSelectedDivs, 'id')} className="text-slate-400 hover:text-slate-600">{selectedDivs.size === divsFiltradas.length && divsFiltradas.length > 0 ? <CheckSquare className="w-4 h-4" /> : <Square className="w-4 h-4" />}</button></th>
                      <th className="px-5 py-3">Anúncio</th><th className="px-5 py-3">Divergência</th><th className="px-5 py-3 text-right">Ações</th>
                    </tr>
                  </thead>
                    ...
                    {divsFiltradas.map((div) => {
                      ...
                      return (
                        <tr key={div.id} className={`hover:bg-slate-50/80 transition-colors ${sel ? 'bg-blue-50/40' : ''}`}>
                          <td className="px-5 py-4"><button onClick={() => { const n = new Set(selectedDivs); sel ? n.delete(div.id) : n.add(div.id); setSelectedDivs(n); }} className="text-slate-400 hover:text-blue-600">{sel ? <CheckSquare className="w-4 h-4 text-blue-600" /> : <Square className="w-4 h-4" />}</button></td>
                          ...
                          <td className="py-4 px-5"><p className="text-slate-700 text-[11px] font-medium leading-relaxed">{div.motivo}</p></td>
                          <td className="py-4 px-5 text-right">
                            <div className="flex items-center justify-end gap-1.5">
                              <div onMouseEnter={() => setQuickViewItem(div)} onMouseLeave={() => setQuickViewItem(null)}>
                                <button onClick={() => abrirFullModal(div)} className="p-2 rounded-xl bg-slate-100 text-slate-600 hover:bg-blue-100 hover:text-blue-600 hover:scale-105 transition-all shadow-sm"><Eye className="w-4 h-4" /></button>
                              </div>
      ...
      {/* ── ABA NÃO VINCULADOS ── */}
      {activeTab === 'pendentes' && (
        <section className={`bg-white border border-slate-200 rounded-3xl shadow-lg flex flex-col flex-1 min-h-0 overflow-hidden ${fullScreenMode ? 'fixed inset-4 z-50' : ''}`}>
          <div className="p-5 border-b border-slate-100 bg-amber-50 flex justify-between items-center flex-wrap gap-4">
            <div>
            ...
            <div className="flex items-center gap-3">
              <div className="relative w-64">
                ...
                <input value={buscaNaoVinc} onChange={e => setBuscaNaoVinc(e.target.value)} placeholder="Pesquisar anúncio fantasma..." className="w-full pl-9 pr-3 py-2 bg-white/50 border border-amber-200 rounded-xl outline-none focus:border-amber-400 text-[11px] placeholder:text-amber-600/50 text-amber-900" />
              </div>
              {selectedNaoVinc.size > 0 && (
                <div className="flex items-center gap-1.5 bg-white border border-amber-200 rounded-xl px-2.5 py-1.5 shadow-sm">
                  ...
                  <button onClick={() => acaoLoteNaoVinculados('ignorado')} className="flex items-center gap-1 text-[10px] font-black uppercase bg-slate-100 text-slate-600 px-2 py-1 rounded hover:bg-slate-200"><EyeOff className="w-3 h-3"/> Ignorar</button>
              ...
              <table className="w-full text-left">
                <thead className="sticky top-0 bg-white border-b border-slate-100 shadow-sm z-10">
                  <tr className="text-[10px] font-black uppercase tracking-widest text-slate-400">
                    <th className="px-5 py-4 w-10"><button onClick={() => toggleSelectAll(nVincFiltrados, selectedNaoVinc, setSelectedNaoVinc, 'id')} className="text-slate-400">{selectedNaoVinc.size === nVincFiltrados.length ? <CheckSquare className="w-4 h-4" /> : <Square className="w-4 h-4" />}</button></th>
                    <th className="px-5 py-4">Produto ML</th><th className="px-5 py-4 text-right">Ação</th>
                  </tr>
                </thead>
                        ...
                        <td className="px-5 py-4 text-right">
                          <div className="flex items-center justify-end gap-2 relative">
                            <div onMouseEnter={() => setQuickViewItem(prod)} onMouseLeave={() => setQuickViewItem(null)}>
                              <button onClick={() => abrirFullModal(prod)} className="p-2 rounded-xl bg-slate-100 text-slate-600 hover:bg-blue-100 hover:text-blue-600 hover:scale-105 transition-all shadow-sm"><Eye className="w-4 h-4" /></button>
                            </div>
                            ...
                            <button onClick={() => acaoNaoVinculadoBasica(prod.id, 'ignorado')} className="p-2 rounded-xl bg-slate-50 text-slate-500 hover:bg-slate-200"><EyeOff className="w-4 h-4" /></button>
                          </div>
                        </td>
                      </tr>
                    );
      ...
      {activeTab === 'produtos' && (
        <div className={`grid grid-cols-1 ${fullScreenMode ? 'fixed inset-4 z-50 bg-slate-100 p-4 gap-4 rounded-3xl' : 'lg:grid-cols-5 gap-5'} flex-1 min-h-0 transition-all`}>
          ...
          <section className={`${fullScreenMode ? 'col-span-1' : 'col-span-3'} bg-white border border-slate-200 rounded-3xl p-5 shadow-lg overflow-hidden flex flex-col h-full`}>
            <div className="flex justify-between items-center mb-4 flex-wrap gap-3">
              <div className="bg-slate-100 p-1.5 rounded-2xl flex gap-2 w-full sm:w-auto">
                <button onClick={() => setTipoCatalogo('BASE')} className={`flex-1 py-2.5 px-4 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${tipoCatalogo === 'BASE' ? 'bg-white text-blue-600 shadow-sm ring-1 ring-slate-200' : 'text-slate-500 hover:bg-slate-200'}`}>Produtos Físicos</button>
                <button onClick={() => setTipoCatalogo('ML')} className={`flex-1 py-2.5 px-4 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${tipoCatalogo === 'ML' ? 'bg-white text-emerald-600 shadow-sm ring-1 ring-slate-200' : 'text-slate-500 hover:bg-slate-200'}`}>Anúncios Vinculados</button>
              </div>
              <div className="flex items-center gap-2 w-full sm:w-auto">
                ...
                {selectedProds.size > 0 && (
                  <button onClick={excluirLoteProdutos} className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-red-50 text-red-600 text-[10px] font-black hover:bg-red-100">
                    <Trash2 className="w-4 h-4" /> Excluir ({selectedProds.size})
                  </button>
                )}
              ...
              <table className="w-full text-left">
                <thead className="bg-slate-50 sticky top-0 z-10 border-b border-slate-200 shadow-sm">
                  <tr className="text-[9px] font-black uppercase tracking-widest text-slate-400">
                    <th className="px-4 py-3 w-10"><button onClick={() => toggleSelectAll(produtosFiltrados, selectedProds, setSelectedProds)} className="text-slate-400 hover:text-slate-600">{selectedProds.size === produtosFiltrados.length && produtosFiltrados.length > 0 ? <CheckSquare className="w-4 h-4" /> : <Square className="w-4 h-4" />}</button></th>
                    <th className="px-4 py-3">Produto</th><th className="px-4 py-3">Peso Fixo</th><th className="px-4 py-3 text-right">Ação</th>
                  </tr>
                </thead>
                      ...
                      <tr key={prod.id} className={`hover:bg-slate-50/80 transition-colors ${sel ? 'bg-blue-50/30' : ''}`}>
                        ...
                        <td className="px-4 py-3.5 text-right">
                          <div className="flex justify-end gap-1.5 relative">
                            <div onMouseEnter={() => setQuickViewItem(prod)} onMouseLeave={() => setQuickViewItem(null)}>
                              <button onClick={() => abrirFullModal(prod)} className="p-2 rounded-xl bg-slate-100 text-slate-600 hover:bg-blue-100 hover:text-blue-600 hover:scale-105 transition-all"><Eye className="w-4 h-4" /></button>
                            </div>
