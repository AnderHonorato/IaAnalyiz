...
import {
  ...
  Eye, Sparkles, Upload, HelpCircle, Unlink, LayoutList, 
  FileText, Maximize2, Minimize2, ArrowLeft, Settings, 
  Wifi, WifiOff, FileSearch, History
} from 'lucide-react';
...
// ── MÁQUINA DE ESCREVER (Linha por linha fluída) ──
function TypewriterLog({ msg, type }) {
  ...
  useEffect(() => {
    ...
    }, 15); // Velocidade de digitação hacker
...
export default function MercadoLivre() {
  ...
  const [logs, setLogs] = useState([{ msg: `[${new Date().toLocaleTimeString()}] IA_Analyiz Módulo Analítico Carregado. Aguardando comandos.`, type: 'info' }]);
  ...
  const [modoLento, setModoLento] = useState(false); // Standby anti-bloqueio
  ...
  useEffect(() => {
    if (!userId) return;
    verificarStatusML(); buscarDivergencias(); buscarDivStats(); buscarProdutos(); buscarNaoVinculados(); buscarInsightIA();
  }, [userId]);
  ...
  const gerarResumoLogistico = async () => {
    ...
    try {
      const dadosStr = `Temos ${divStats.pendente} div. pendentes, ${divStats.reincidente} reincidentes. ${naoVinculados.length} anúncios não vinculados no ML. ${produtos.length} produtos no catálogo base.`;
      const res = await fetch(`${API_BASE_URL}/api/ia/summary`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId, dados: dadosStr })
      });
      const data = await res.json();
      setResumoIA(data.conteudo);
      buscarHistoricoResumos();
    } catch { setResumoIA('Erro ao processar resumo.'); }
    finally { setLoadingResumo(false); }
  };

  const buscarHistoricoResumos = async () => {
    try { setHistoricoResumos(await apiGet(`/api/ia/summary/history?userId=${userId}`)); } catch {}
  };
  ...
  // ── DIVERGÊNCIAS ──
  const buscarDivergencias = async () => { try { setDivergencias(await apiGet(`/api/divergencias?status=${filtroStatus}&plataforma=Mercado Livre&userId=${userId}`) || []); } catch {} };
  ...
  const iniciarBot = () => {
    if (!userId) return;
    setIsBotRunning(true); setProgress(0); setLogs([{ msg: `[${new Date().toLocaleTimeString()}] Inicializando motores. Autenticando com Mercado Livre...`, type: 'info' }]);
  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-4 h-full flex flex-col animate-in fade-in duration-300 relative">
      
      {/* ── HEADER ORGANIZADO E LIMPO ── */}
      {!fullScreenMode && (
        <div className="flex flex-col xl:flex-row justify-between xl:items-center mb-5 shrink-0 gap-4 relative z-10">
          ...
          <div className="flex items-center flex-wrap gap-4">
            ...
            {/* Mensagem da IA rolável e compacta no espaço vazio */}
            ...
            <nav className="flex items-center gap-1.5 bg-white p-1 rounded-xl border border-slate-200 shadow-sm overflow-x-auto">
              {[['bot', 'Scanner ML'], ['pendentes', `Não Vinculados (${naoVinculados.length})`], ['produtos', 'Catálogo Local']].map(([k, l]) => (
                <button key={k} onClick={() => setActiveTab(k)}
                  className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all whitespace-nowrap ${activeTab === k ? 'bg-slate-900 text-[#FFE600] shadow-md' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-800'}`}>
                  {l}
                </button>
              ))}
            </nav>
            <div className="flex gap-1.5">
              <button onClick={() => { buscarHistoricoResumos(); setShowResumoModal(true); }} className="flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 text-indigo-600 rounded-xl hover:bg-indigo-100 transition-colors shadow-sm border border-indigo-100 text-[10px] font-black uppercase" title="Gerar Resumo">
                <FileSearch className="w-3.5 h-3.5" /> Resumir
              </button>
              <button onClick={() => setShowDicas(true)} className="p-2 bg-blue-50 text-blue-600 rounded-xl hover:bg-blue-100 transition-colors shadow-sm" title="Ajuda Detalhada">
                <HelpCircle className="w-4 h-4" />
              </button>
              <button onClick={() => setShowConfig(true)} className="p-2 bg-slate-50 text-slate-600 border border-slate-200 rounded-xl hover:bg-slate-100 transition-colors shadow-sm" title="Configurações">
      ...
      {/* ── MÓDULOS (Conteúdo) ── */}
      {/* Usa absolute z-50 para a Tela Cheia ficar restrita a este container, abaixo do Header principal e Breadcrumb do Layout */}
      <div className={`relative flex-1 min-h-0 transition-all duration-300 ${fullScreenMode ? 'absolute inset-0 z-50 bg-[#f8fafc] p-2 sm:p-4' : ''}`}>
        
        {/* ── ABA SCANNER ── */}
        {activeTab === 'bot' && (
          <div className="grid grid-cols-1 xl:grid-cols-5 gap-5 h-full">
            <section className="xl:col-span-2 bg-white border border-slate-200 rounded-3xl shadow-lg flex flex-col overflow-hidden">
              <div className="p-5 border-b border-slate-100">
                <button onClick={iniciarBot} disabled={isBotRunning || !mlConectado}
                  className={`w-full py-4 rounded-2xl font-black text-[12px] uppercase tracking-widest mb-4 transition-all shadow-sm ${isBotRunning || !mlConectado ? 'bg-slate-100 text-slate-400' : 'bg-blue-600 text-white hover:bg-blue-700 hover:shadow-md'}`}>
                  {isBotRunning ? <span className="flex items-center justify-center gap-2"><Loader2 className="w-4 h-4 animate-spin" /> Processando...</span> : !mlConectado ? '🔒 Conecte o ML' : '🔍 Iniciar Varredura'}
                </button>
                <div className="bg-slate-900 rounded-2xl p-4 border border-slate-800">
                  <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest mb-2">
                    <span className="text-slate-400 flex items-center gap-1.5">Progresso IA {mlConectado ? <Wifi className="w-3 h-3 text-emerald-400"/> : <WifiOff className="w-3 h-3 text-red-400"/>}</span>
            ...
            <section className="xl:col-span-3 bg-white border border-slate-200 rounded-3xl shadow-lg flex flex-col overflow-hidden">
              <div className="p-4 border-b border-slate-100">
                <div className="flex justify-between items-center mb-3 flex-wrap gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    {selectedDivs.size > 0 && (
                      <div className="flex items-center gap-1.5 bg-blue-50 px-2.5 py-1.5 rounded-xl border border-blue-200 shadow-sm">
                        <span className="text-[10px] font-black text-blue-700 px-1">{selectedDivs.size} selecionados</span>
                        <button onClick={() => acaoLoteDivs('corrigido')} className="p-1.5 text-emerald-600 hover:bg-white rounded" title="Corrigir Todos"><Check className="w-3.5 h-3.5" /></button>
                        <button onClick={() => acaoLoteDivs('ignorado')} className="p-1.5 text-slate-500 hover:bg-white rounded" title="Ignorar Lote"><EyeOff className="w-3.5 h-3.5" /></button>
                      </div>
                    )}
                ...
                <div className="flex justify-between items-center gap-3 flex-wrap">
                  <div className="flex gap-1.5 flex-wrap">
                    {['PENDENTE', 'REINCIDENTE', 'CORRIGIDO', 'IGNORADO', 'TODOS'].map(s => {
                      ...
                      return (
                        <button key={s} onClick={() => setFiltroStatus(s)} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[10px] font-black uppercase border transition-all ${filtroStatus === s ? `${cfg.bg} ${cfg.color} ${cfg.border} shadow-sm ring-1 ring-offset-1 ring-${cfg.color.split('-')[1]}-200` : 'bg-white text-slate-500 border-slate-200 hover:bg-slate-50'}`}>
                          {cfg.label} <span className="bg-white/60 px-1.5 rounded text-[11px]">{count}</span>
                        </button>
                      );
                    })}
                  </div>
                  <div className="relative flex-1 min-w-[200px]">
                    ...
                    <input value={buscaDiv} onChange={e => setBuscaDiv(e.target.value)} placeholder="Filtrar por nome ou ID..." className="w-full pl-9 pr-3 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-blue-500 text-[11px]" />
                  </div>
                </div>
              </div>
                        ...
                        return (
                          <tr key={div.id} className={`hover:bg-slate-50/80 transition-colors ${sel ? 'bg-blue-50/30' : ''}`}>
                            ...
                            <td className="py-3 px-4">
                              <span className={`text-[9px] font-black px-1.5 py-0.5 rounded-sm ${cfg.bg} ${cfg.color} flex items-center gap-1 w-fit mb-1 border ${cfg.border}`}><cfg.icon className="w-2.5 h-2.5" />{cfg.label}</span>
      ...
      {/* ── ABA NÃO VINCULADOS ── */}
      {activeTab === 'pendentes' && (
        <section className="bg-white border border-slate-200 rounded-3xl shadow-lg flex flex-col h-full overflow-hidden">
          <div className="p-5 border-b border-slate-100 bg-amber-50/30 flex justify-between items-center flex-wrap gap-4">
            <div>
              <h3 className="text-[14px] font-black text-amber-800 uppercase flex items-center gap-2"><Box className="w-5 h-5" /> Anúncios Desconhecidos ({naoVinculados.length})</h3>
            </div>
            <div className="flex items-center gap-2.5">
              <div className="relative w-56">
                <Search className="absolute left-3 top-2.5 w-4 h-4 text-amber-600/50"/>
                <input value={buscaNaoVinc} onChange={e => setBuscaNaoVinc(e.target.value)} placeholder="Pesquisar..." className="w-full pl-9 pr-3 py-2 bg-white border border-amber-200 rounded-xl outline-none focus:border-amber-400 text-[11px] placeholder:text-amber-600/50 text-amber-900 shadow-sm" />
              </div>
            ...
            {nVincFiltrados.length === 0 ? (
              ...
              <table className="w-full text-left">
                <thead className="sticky top-0 bg-white border-b border-slate-100 shadow-sm z-10">
                  <tr className="text-[10px] font-black uppercase tracking-widest text-slate-400"><th className="px-5 py-3 w-10"><button onClick={() => toggleSelectAll(nVincFiltrados, selectedNaoVinc, setSelectedNaoVinc, 'id')} className="text-slate-400">{selectedNaoVinc.size === nVincFiltrados.length && nVincFiltrados.length > 0 ? <CheckSquare className="w-4 h-4" /> : <Square className="w-4 h-4" />}</button></th><th className="px-5 py-3">Produto ML</th><th className="px-5 py-3 text-right">Ação</th></tr>
                </thead>
                      ...
                      <tr key={prod.id} className={`hover:bg-slate-50/60 transition-colors ${sel ? 'bg-amber-50/30' : ''}`}>
                        ...
                        <td className="px-5 py-4 text-right">
                          <div className="flex items-center justify-end gap-2 relative">
                            <button onClick={() => abrirFullModal(prod)} className="p-2 rounded-xl bg-slate-100 text-slate-600 hover:bg-slate-200"><Eye className="w-4 h-4" /></button>
                            <button onClick={() => setVincularAnuncio(prod)} className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-emerald-500 text-white hover:bg-emerald-600 text-[11px] font-black uppercase shadow-md hover:-translate-y-0.5 transition-all">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-5 h-full">
          <section className="col-span-2 bg-white border border-slate-200 rounded-3xl shadow-lg flex flex-col overflow-hidden">
            <div className="p-5 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
              <h2 className="font-black text-[11px] uppercase flex items-center gap-2 text-slate-800"><Plus className="w-4 h-4 text-blue-600" /> {editandoId ? 'Editar Base' : 'Registro Manual'}</h2>
              <button onClick={() => setShowImportModal(true)} className="flex items-center gap-1 text-[9px] font-black text-emerald-600 bg-emerald-100 px-2.5 py-1.5 rounded-lg uppercase shadow-sm"><Upload className="w-3 h-3"/> Importar CSV</button>
            </div>
            <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
              <form onSubmit={handleSubmitProduto} className="space-y-4 text-[11px] font-semibold text-slate-600">
                <div className="grid grid-cols-2 gap-4">
                  <div><label className="uppercase tracking-widest block mb-1.5 text-[9px]">SKU Interno</label><input required value={formProd.sku} onChange={e => setFormProd({...formProd, sku: e.target.value})} className="w-full bg-white border border-slate-300 rounded-xl p-3 outline-none focus:border-blue-500 text-[12px]" /></div>
                  <div><label className="uppercase tracking-widest block mb-1.5 text-[9px]">Categoria</label><input value={formProd.categoria} onChange={e => setFormProd({...formProd, categoria: e.target.value})} className="w-full bg-white border border-slate-300 rounded-xl p-3 outline-none focus:border-blue-500 text-[12px]" list="cat-list" /><datalist id="cat-list">{categorias.map(c => <option key={c} value={c} />)}</datalist></div>
                </div>
                <div><label className="uppercase tracking-widest block mb-1.5 text-[9px]">Nome (Usado na composição)</label><input required value={formProd.nome} onChange={e => setFormProd({...formProd, nome: e.target.value})} className="w-full bg-white border border-slate-300 rounded-xl p-3 outline-none focus:border-blue-500 text-[12px]" /></div>
                <div className="bg-blue-50/50 border border-blue-100 rounded-2xl p-4">
                  <p className="text-[10px] font-black text-blue-800 uppercase tracking-widest flex items-center gap-1.5 mb-3"><Weight className="w-4 h-4" /> Peso Unitário Físico</p>
                ...
                <button type="submit" disabled={loadingProd} className="w-full py-3.5 bg-slate-900 text-white font-black text-[12px] uppercase tracking-widest rounded-xl hover:bg-blue-600 transition-all shadow-md mt-2">{loadingProd ? 'Processando...' : '📥 Salvar Ficha'}</button>
              </form>
            </div>
          </section>
          ...
          <section className="col-span-3 bg-white border border-slate-200 rounded-3xl p-5 shadow-lg overflow-hidden flex flex-col h-full">
            <div className="flex justify-between items-center mb-4 flex-wrap gap-3">
              <div className="bg-slate-100 p-1.5 rounded-2xl flex gap-2">
                <button onClick={() => setTipoCatalogo('BASE')} className={`py-2.5 px-5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${tipoCatalogo === 'BASE' ? 'bg-white text-blue-600 shadow-sm ring-1 ring-slate-200' : 'text-slate-500 hover:bg-slate-200'}`}>Produtos Base</button>
                <button onClick={() => setTipoCatalogo('ML')} className={`py-2.5 px-5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${tipoCatalogo === 'ML' ? 'bg-white text-emerald-600 shadow-sm ring-1 ring-slate-200' : 'text-slate-500 hover:bg-slate-200'}`}>Anúncios Vinculados</button>
              </div>
              <div className="flex items-center gap-2">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar border border-slate-100 rounded-2xl">
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-slate-50 text-[11px]">
                  {produtosFiltrados.length === 0 && <tr><td colSpan="4" className="py-12 text-center text-slate-400 text-[11px] font-black uppercase tracking-widest">Nenhum produto listado</td></tr>}
                      ...
                      <tr key={prod.id} className={`hover:bg-slate-50/80 transition-colors ${sel ? 'bg-blue-50/30' : ''}`}>
                        ...
                        <td className="px-4 py-3.5">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-[12px] font-bold text-slate-800 truncate max-w-[250px]">{prod.nome}</p>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="px-4 py-3.5 text-right">
                          <div className="flex justify-end gap-1.5 relative">
                            ...
                            {prod.mlItemId && <button onClick={() => desvincularProduto(prod.id)} className="p-2 rounded-xl bg-amber-50 text-amber-600 hover:bg-amber-100" title="Desvincular (Volta p/ Aba Não Vinculados)"><Unlink className="w-4 h-4" /></button>}
      ...
      {fullModalItem && (
        <div className="fixed inset-0 z-[999999] bg-slate-900/80 backdrop-blur-sm flex items-center justify-center p-4 lg:p-10 animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-5xl overflow-hidden flex flex-col max-h-full">
            <div className="p-5 border-b border-slate-100 flex justify-between items-center z-10 shadow-sm">
            ...
            <div className="flex-1 overflow-y-auto bg-slate-50/50 p-6 lg:p-8 custom-scrollbar">
              {loadingFullData ? (
                <div className="flex flex-col items-center justify-center py-32 text-blue-500"><Loader2 className="w-10 h-10 animate-spin mb-4"/><p className="text-[12px] font-black uppercase tracking-widest text-slate-400">Extraindo dados do servidor do Mercado Livre...</p></div>
              ) : fullItemData ? (
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                  <div className="lg:col-span-5 space-y-4">
                    ...
                    {fullItemData.pictures?.length > 1 && (
                      <div className="flex gap-2 overflow-x-auto pb-2 custom-scrollbar">
                        {fullItemData.pictures.slice(1, 5).map((pic, idx) => (
                          <img key={idx} src={pic.url} className="w-20 h-20 object-cover rounded-xl border border-slate-200 shadow-sm" />
                        ))}
                      </div>
                    )}
                    <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
                      ...
                      <a href={fullItemData.permalink} target="_blank" className="mt-5 flex items-center justify-center gap-2 w-full py-3.5 bg-[#FFE600] text-slate-900 rounded-xl text-[12px] font-black uppercase tracking-widest shadow-sm">
                ...
                <div className="text-center py-20 text-slate-400 text-[12px] uppercase tracking-widest font-black">Falha ao obter os dados</div>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-[999999] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="flex-1 overflow-y-auto p-6 bg-slate-50 custom-scrollbar">
              ...
              <div className="mb-8">
                ...
                <div className="relative mb-3">
                  ...
                  <input value={buscaBase} onChange={e => setBuscaBase(e.target.value)} placeholder="Buscar Produto Base..." className="w-full pl-11 pr-4 py-3 bg-white border border-slate-200 rounded-xl outline-none focus:border-blue-500 shadow-sm text-[12px]" />
                </div>
                {buscaBase && (
            ...
            <div className="p-5 border-t border-slate-100 bg-white flex justify-end gap-3 z-10">
              ...
              <button onClick={salvarVinculacao} disabled={composicaoKit.length === 0 && !pesoManual} className="flex items-center gap-2 px-8 py-3 rounded-xl text-[11px] font-black uppercase tracking-widest text-white bg-emerald-500 hover:bg-emerald-600 disabled:bg-slate-200 disabled:text-slate-400 transition-all shadow-md">Confirmar Vinculação</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[999999] bg-slate-900/70 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden animate-in zoom-in-95">
            <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-blue-600 text-white">
              <h3 className="font-black uppercase tracking-widest flex items-center gap-2"><HelpCircle className="w-5 h-5"/> Guia de Operação Logística</h3>
            ...
            <div className="p-8 space-y-6 text-[13px] text-slate-600 leading-relaxed">
              <div className="bg-amber-50 p-5 rounded-2xl border border-amber-100">
                <h4 className="font-black text-amber-800 text-[15px] mb-2 flex items-center gap-2"><AlertTriangle className="w-5 h-5"/> Entendendo as Divergências</h4>
                <ul className="list-disc pl-5 space-y-1.5 text-amber-900/80">
                  <li><b>Pendente:</b> O robô achou um erro. O ML tá cobrando diferente do que você cadastrou.</li>
                  <li><b>Reincidente:</b> Você já arrumou antes, mas o ML foi lá e estragou o peso de novo. Muito cuidado com esses.</li>
                  <li><b>Corrigido:</b> Resolvido! O robô percebeu que o ML arrumou e fechou o alerta sozinho.</li>
                </ul>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100">
                  <h4 className="font-black text-slate-800 text-[14px] mb-2 flex items-center gap-2"><Box className="w-4 h-4 text-emerald-500"/> Não Vinculados</h4>
                  <p className="text-[12px]">São os anúncios "Fantasmas". O robô achou eles na sua conta, mas o seu sistema local não sabe quem eles são nem quanto pesam. Clique em <b>Vincular</b> para resolver.</p>
                </div>
                <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100">
                  <h4 className="font-black text-slate-800 text-[14px] mb-2 flex items-center gap-2"><Package className="w-4 h-4 text-blue-500"/> Produtos Físicos (Base)</h4>
                  <p className="text-[12px]">São as peças unitárias do seu estoque (Ex: 1 Controle de 50g). Eles servem como "blocos de montar" para você criar a composição dos Kits vinculados.</p>
                </div>
              </div>
            </div>
          </div>
      ...
      {showResumoModal && (
        <div className="fixed inset-0 z-[999999] bg-slate-900/70 backdrop-blur-sm flex justify-end animate-in fade-in duration-200">
          <div className="bg-white shadow-2xl w-full max-w-md h-full flex flex-col animate-in slide-in-from-right-8">
            ...
            <div className="flex-1 overflow-y-auto p-6 bg-slate-50 custom-scrollbar">
              {loadingResumo ? (
                <div className="flex flex-col items-center justify-center py-20 text-indigo-500"><Loader2 className="w-8 h-8 animate-spin mb-3"/><p className="text-[11px] font-black uppercase tracking-widest text-indigo-400">A IA está processando o relatório...</p></div>
              ) : (
                ...
                {historicoResumos.length === 0 ? <p className="text-[11px] text-slate-400">Nenhum histórico salvo.</p> : (
                  <div className="space-y-3">
                    {historicoResumos.map(h => (
                      <button key={h.id} onClick={() => setResumoIA(h.conteudo)} className="w-full text-left p-3 bg-white border border-slate-200 rounded-xl hover:border-indigo-300 transition-colors">
                        <p className="text-[10px] font-bold text-indigo-600 mb-1">{new Date(h.createdAt).toLocaleString()}</p>
            ...
            <div className="p-4 border-t border-slate-100 bg-white">
              <button onClick={gerarResumoLogistico} disabled={loadingResumo} className="w-full py-3 bg-indigo-600 text-white rounded-xl text-[11px] font-black uppercase shadow-md hover:bg-indigo-700 disabled:opacity-50">Gerar Novo Resumo Agora</button>
            </div>
          </div>
        </div>
      )}

      {/* 5. Modal Configurações ML */}
      {showConfig && (
        <div className="fixed inset-0 z-[999999] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in">
          <div className="bg-white rounded-3xl w-full max-w-sm shadow-2xl overflow-hidden">
            ...
            <div className="p-6 space-y-6">
              <div>
                <label className="text-[10px] font-black uppercase text-slate-400 block mb-3">Conexão ML</label>
                {!mlConectado ? (
                  ...
                  <div className="flex justify-between items-center bg-emerald-50 border border-emerald-100 p-3 rounded-xl">
                    ...
                    <button onClick={async () => { if(confirm('Desconectar?')) await fetch(`${API_BASE_URL}/api/ml/disconnect?userId=${userId}`, {method:'DELETE'}); setMlConectado(false); }} className="text-[10px] font-black uppercase text-red-500 hover:bg-red-50 px-2 py-1 rounded">Desconectar</button>
                  </div>
                )}
              </div>
              <div className="pt-4 border-t border-slate-100">
                <label className="text-[10px] font-black uppercase text-slate-400 block mb-3">Modo de Operação</label>
                <div className="flex items-center justify-between bg-slate-50 border border-slate-200 p-3 rounded-xl cursor-pointer hover:bg-slate-100 transition-colors" onClick={() => setModoLento(!modoLento)}>
                  <div><p className="text-[11px] font-bold text-slate-800">Modo Standby (Anti-Timeout)</p><p className="text-[9px] text-slate-500 mt-0.5 max-w-[200px]">Ative se o ML estiver bloqueando a leitura por limite de requisições.</p></div>
                  <div className={`w-10 h-5 rounded-full relative transition-colors ${modoLento ? 'bg-emerald-500' : 'bg-slate-300'}`}>
                    <div className={`absolute top-1 bg-white w-3 h-3 rounded-full transition-all ${modoLento ? 'left-6' : 'left-1'}`}></div>
                  </div>
                </div>
              </div>
            </div>
