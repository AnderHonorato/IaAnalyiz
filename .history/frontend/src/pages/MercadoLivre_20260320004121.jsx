...
export default function MercadoLivre() {
  ...
  const excluirDivergencia = async (id) => {
    const ok = await confirm({
      title: 'Excluir Divergência', danger: true,
      message: 'Deseja excluir permanentemente esta divergência?',
  ...
  const acaoLoteDivs = async (tipo) => {
    ...
    if (tipo === 'excluir') {
      const ok = await confirm({
        title: 'Excluir em lote', danger: true,
        message: `Excluir ${selectedDivs.size} divergência(s) permanentemente?`,
        confirmLabel: 'Excluir todas', cancelLabel: 'Cancelar',
      });
      if (!ok) return;
    }
  ...
  const limparCorrigidas = async () => {
    const ok = await confirm({
      title: 'Limpar Corrigidas', type: 'warning',
      message: 'Remover todas as divergências com status "Corrigido"?',
  ...
  const excluirProduto = async (id) => {
    const ok = await confirm({
      title: 'Excluir Produto', danger: true,
      message: 'Este produto será removido permanentemente do banco de dados.',
      confirmLabel: 'Excluir', cancelLabel: 'Cancelar',
    });
    if (!ok) return;
  ...
  const excluirLoteProdutos = async () => {
    ...
    const ok = await confirm({
      title: 'Excluir Produtos', danger: true,
      message: `Excluir ${selectedProds.size} produto(s) permanentemente?`,
      details: ['Esta ação não pode ser desfeita.'],
      confirmLabel: 'Excluir todos', cancelLabel: 'Cancelar',
    });
    if (!ok) return;
  ...
  const importarSelecionados = async () => {
    ...
    try {
      ...
      await buscarProdutos(); await buscarCategorias();
      await alert({
        title: 'Importação concluída', type: 'success',
        message: `${selecionados.length} anúncio(s) importado(s) com sucesso para o catálogo!`,
        confirmLabel: 'OK',
      });
