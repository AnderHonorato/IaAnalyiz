/**
 * frontend/src/pages/MercadoLivre.jsx
 * 
 * Propósito:
 * Painel de auditoria de fretes e divergências de pesos (Radar de Fretes).
 * Detecta e corrige problemas de peso que aumentam custos de envio no Mercado Livre.
 * 
 * Responsabilidades:
 * - Buscar divergências de peso entre anunciado e peso real
 * - Aplicar correções manuais ou via API
 * - Gerenciar histórico de ações (corrigido, pendente, ignorado, reincidente)
 * - Filtrar por status, tipo de aviso e período
 * - Exibir detalhes completos de anúncio e divergências
 * - Permitir upload de ficheiros e exportação
 * 
 * Estados de Divergência:
 *   - PENDENTE: Aguardando correção
 *   - PENDENTE_ENVIO: Na fila de envio para API
 *   - REINCIDENTE: Problema recorrente
 *   - CORRIGIDO: Já foi corrigido
 *   - IGNORADO: Marcado para ignorar
 * 
 * Tipos de Aviso:
 *   - DIMENSOES_INCORRETAS
 *   - PESO_INCORRETO
 *   - ANUNCIO_PAUSADO
 *   - OUTROS
 * 
 * Ferramentas:
 *   - Busca por ID/SKU/Título
 *   - Filtros avançados por status e tipo
 *   - Histórico detalhado de ações
 *   - Exportação CSV/Excel
 *   - Gráficos de impacto de frete
 * 
...
const TOUR_STEPS = [
  {
    ...
    tab:     'bot',
    title:   '🔍 Scanner ML',
    text:    'O coração do sistema. Clique em "Iniciar Varredura Completa" para que a IA audite TODOS os anúncios e detecte pesos divergentes automaticamente.',
    icon:    <FileSearch className="w-4 h-4 text-blue-500"/>,
    action:  'Ir para o Scanner',
    targetTab: 'bot',
  },
  {
    id:      'divergencias',
    ...
    text:    'Lista de anúncios com peso incorreto. Use o botão ✔ para corrigir manualmente ou enviar via API. Itens na "Fila Envio" podem ser corrigidos em lote.',
  ...
  },
  {
    id:      'envio',
    tab:     'bot',
    title:   '🚀 Envio em Massa via API',
    text:    'Marque divergências como "Enviar via API" (botão ➤) — elas vão para a aba "Fila Envio". Depois clique em "Enviar via API" no topo para corrigir tudo de uma vez com 0,6s de pausa entre envios.',
  ...
  },
  {
    id:      'pendentes',
    tab:     'pendentes',
    title:   '📦 Não Vinculados',
    text:    'Anúncios do ML sem peso cadastrado. Clique em "Vincular" para montar o kit com produtos da Base ou informar um peso fixo diretamente.',
    icon:    <Box className="w-4 h-4 text-amber-500"/>,
    action:  'Ir para Não Vinculados',
    targetTab: 'pendentes',
  },
  {
    id:      'catalogo',
    tab:     'produtos',
    title:   '🗂 Catálogo',
    text:    'Gerencie produtos Base (peças físicas) e Anúncios Vinculados (o que é vendido no ML — pode ser um kit). Um anúncio pode conter vários itens da Base.',
    icon:    <Package className="w-4 h-4 text-blue-500"/>,
    action:  'Ir para o Catálogo',
    targetTab: 'produtos',
  },
  {
    id:      'ficha',
    tab:     'ficha',
    title:   '📋 Ficha Técnica',
    text:    'Cadastre EAN, marca, modelo e dimensões nos produtos. A Auditoria de Ficha Técnica compara esses dados com o ML e corrige automaticamente via API.',
    icon:    <GitCompare className="w-4 h-4 text-indigo-500"/>,
    action:  'Ir para Ficha Técnica',
    targetTab: 'ficha',
  },
  {
    id:      'avisos',
    tab:     null,
    title:   '🚨 Avisos do ML',
    text:    'O próprio Mercado Livre pode detectar erros e pausar seus anúncios. O botão vermelho "Avisos ML" no topo mostra quais anúncios foram afetados.',
  ...
  },
  {
    ...
    text:    'Nas Configurações (ícone ⚙ no topo direito) você pode ativar varreduras automáticas de 30min a 24h para manter tudo sempre sincronizado.',
...
// ══════════════════════════════════════════════════════════════════════════
// PAINEL DE AVISOS ML (PATCH 1)
// ══════════════════════════════════════════════════════════════════════════
function PainelAvisosML({ userId, mlConectado, onClose }) {
  const [avisos, setAvisos]           = useState([]);
  const [loading, setLoading]         = useState(true);
  const [sincronizando, setSincronizando] = useState(false);
  const [syncStage, setSyncStage]     = useState('idle'); // idle | buscando | espere | sucesso | erro
  ...
  const sincronizar = async () => {
    if (!mlConectado || sincronizando) return;
    setSincronizando(true);
    setSyncStage('buscando');
    setSyncMsg('Conectando ao Mercado Livre...');

    if (syncTimerRef.current) clearTimeout(syncTimerRef.current);
    syncTimerRef.current = setTimeout(() => {
      setSyncStage('espere');
      setSyncMsg('Espere um pouco, buscando anúncios...');
    }, 4000);

    try {
      const res  = await fetch(`${API_BASE_URL}/api/ml/avisos/sincronizar`, {
      ...
      setSyncStage('sucesso');
      const novos = data.novos || 0;
      setSyncMsg(novos > 0 ? `✅ ${novos} aviso(s) novos encontrados!` : '✅ Tudo sincronizado — nenhum aviso novo.');

      await buscar();
      setTimeout(() => { setSyncStage('idle'); setSyncMsg(''); }, 3000);
    } catch {
      clearTimeout(syncTimerRef.current);
      setSyncStage('erro');
      setSyncMsg('❌ Falha ao sincronizar. Verifique a conexão.');
      setTimeout(() => { setSyncStage('idle'); setSyncMsg(''); }, 3000);
    } finally {
  ...
  const syncBtnStyle = {
    idle:      { bg: 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50', icon: <RefreshCw className="w-3 h-3"/>, label: 'Sincronizar ML' },
    buscando:  { bg: 'bg-blue-50 border border-blue-200 text-blue-700', icon: <Loader2 className="w-3 h-3 animate-spin"/>, label: 'Buscando...' },
    espere:    { bg: 'bg-amber-50 border border-amber-200 text-amber-700', icon: <Loader2 className="w-3 h-3 animate-spin"/>, label: 'Espere...' },
    sucesso:   { bg: 'bg-emerald-50 border border-emerald-200 text-emerald-700', icon: <CheckCircle2 className="w-3 h-3"/>, label: 'Sucesso!' },
    erro:      { bg: 'bg-red-50 border border-red-200 text-red-600', icon: <AlertTriangle className="w-3 h-3"/>, label: 'Erro' },
  };
  const stageStyle = syncBtnStyle[syncStage] || syncBtnStyle.idle;

  return (
    <ModalOverlay onClose={onClose} side="right">
      <div className="bg-white shadow-2xl flex flex-col" style={{ width: '420px', height: '100%' }}>
        <div className="px-4 py-3.5 border-b flex justify-between items-center shrink-0"
        ...
        <div className="px-4 py-2.5 border-b border-slate-100 bg-slate-50 shrink-0 space-y-2">
            ...
            <div className="flex gap-0.5 bg-white border border-slate-200 p-0.5 rounded-lg">
              {[['nao_resolvidos','Ativos'],['resolvidos','Resolvidos']].map(([v,l]) => (
                <button key={v} onClick={() => setFiltro(v)}
                  className={`px-2.5 py-1 rounded text-[9px] font-black uppercase transition-all ${filtro===v?'bg-slate-900 text-white':'text-slate-500 hover:bg-slate-50'}`}>{l}</button>
              ))}
            </div>
            <button onClick={sincronizar} disabled={sincronizando || !mlConectado}
              className={`flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[9px] font-black uppercase disabled:opacity-40 transition-all ${stageStyle.bg}`}>
              {stageStyle.icon}{stageStyle.label}
            </button>
          </div>

          {syncMsg && (
            <div className={`px-3 py-1.5 rounded-lg text-[10px] font-semibold transition-all ${
              syncStage==='sucesso' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' :
              syncStage==='erro'   ? 'bg-red-50 text-red-700 border border-red-200' :
              syncStage==='espere' ? 'bg-amber-50 text-amber-700 border border-amber-200' :
              'bg-blue-50 text-blue-700 border border-blue-200'
            }`} style={{ animation:'fade-in 0.2s ease' }}>
              {syncMsg}
              {(syncStage==='buscando'||syncStage==='espere') && (
                <div className="mt-1.5 h-1 bg-white/50 rounded-full overflow-hidden">
                  <div className="h-full rounded-full bg-current opacity-60" style={{ width:'100%', animation:'progress-indeterminate 1.5s ease-in-out infinite', transformOrigin:'left' }}/>
                </div>
              )}
            </div>
          )}
        ...
        {filtro === 'nao_resolvidos' && (
          <div className="mx-4 mt-3 bg-red-50 border border-red-200 rounded-xl px-3 py-2 shrink-0">
            <p className="text-[9px] text-red-700 font-semibold leading-relaxed">
              <strong>O que são esses avisos?</strong> O ML detecta automaticamente quando o peso ou dimensões diferem do real, podendo pausar o anúncio. Corrija via API para reativar.
            </p>
          </div>
        )}
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {loading ? (
            ...
            <div className="flex flex-col items-center py-10 gap-2 text-center">
              ...
              <p className="text-xs text-slate-400">{filtro==='nao_resolvidos'?'Sincronize para verificar novos avisos.':''}</p>
            </div>
          ) : (
                    ...
                    <div className="p-3">
                      <div className="flex items-start gap-2.5">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <a href={`https://produto.mercadolivre.com.br/MLB-${(aviso.mlItemId||'').replace(/^MLB/i,'')}`} target="_blank" rel="noopener noreferrer"
                            className="text-[8px] font-mono text-blue-600 hover:underline flex items-center gap-0.5">
                      ...
                      {!aviso.resolvido && (
                        <div className="flex justify-end mt-2">
                          <button onClick={() => resolver(aviso.id)}
                            className="flex items-center gap-1 px-2.5 py-1.5 bg-white border border-emerald-300 text-emerald-700 rounded-lg text-[9px] font-black uppercase hover:bg-emerald-50">
...
// ══════════════════════════════════════════════════════════════════════════
// MODAL DICAS COM TOUR GUIADO (PATCH 2)
// ══════════════════════════════════════════════════════════════════════════
function ModalDicas({ onClose, onNavigate }) {
  ...
  const next = () => {
    ...
    else { setTourAtivo(false); setStepIdx(0); onClose(); }
  };

  return (
    <ModalOverlay onClose={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col" style={{ width:'440px', maxWidth:'96vw', maxHeight:'88vh' }}>
        <div className={`px-5 py-4 flex justify-between items-center ${tourAtivo?'bg-blue-600':'bg-white border-b border-slate-100'}`}>
          <div className="flex items-center gap-2">
            {tourAtivo
              ? <><span className="text-xs font-black uppercase tracking-widest text-blue-200">{stepIdx+1}/{TOUR_STEPS.length}</span><span className="font-black text-white text-sm ml-2">{step.title}</span></>
              : <h3 className="font-black text-slate-800 text-sm uppercase flex items-center gap-1.5"><HelpCircle className="w-4 h-4 text-blue-600"/>Como funciona</h3>}
          </div>
          <button onClick={onClose} className={tourAtivo?'text-white/60 hover:text-white':'text-slate-400 hover:text-red-500'}>
            <XCircle className="w-4 h-4"/>
          </button>
        </div>
        ...
        {tourAtivo ? (
          <div className="flex flex-col flex-1 overflow-hidden">
            <div className="h-1 bg-slate-100">
              <div className="h-full bg-blue-500 transition-all duration-400" style={{ width:`${((stepIdx+1)/TOUR_STEPS.length)*100}%` }}/>
            </div>
            <div className="flex-1 overflow-y-auto p-5 space-y-4">
              <div className="bg-blue-50 border border-blue-100 rounded-2xl p-5 flex flex-col items-center text-center gap-3">
                <div className="w-14 h-14 bg-white rounded-2xl flex items-center justify-center shadow-sm border border-blue-100 text-blue-500">
                  {step.icon}
                </div>
                <h4 className="text-base font-black text-slate-800">{step.title}</h4>
                <p className="text-sm text-slate-600 leading-relaxed">{step.text}</p>
                {step.action && (
                  <button onClick={handleAction}
                    className="flex items-center gap-2 px-4 py-2 rounded-xl bg-blue-600 text-white text-xs font-black uppercase hover:bg-blue-700 transition-all mt-1">
              ...
              <div className="flex gap-1.5 flex-wrap justify-center">
                {TOUR_STEPS.map((s, i) => (
                  <button key={s.id} onClick={() => setStepIdx(i)}
                    className={`w-8 h-8 rounded-full text-[9px] font-black border transition-all ${i===stepIdx ? 'bg-blue-600 text-white border-blue-600' : i<stepIdx ? 'bg-emerald-50 text-emerald-600 border-emerald-200' : 'bg-slate-50 text-slate-400 border-slate-200'}`}>
                    {i<stepIdx?'✓':i+1}
                  </button>
                ))}
              </div>
            ...
            <div className="px-5 py-3 border-t border-slate-100 flex justify-between items-center">
              <button onClick={prev} disabled={stepIdx===0} className="px-4 py-2 rounded-xl text-xs font-black uppercase text-slate-400 hover:bg-slate-100 disabled:opacity-30">← Anterior</button>
              <button onClick={() => { setTourAtivo(false); setStepIdx(0); }} className="text-[9px] text-slate-300 hover:text-slate-500">Sair do tour</button>
              <button onClick={next} className="flex items-center gap-2 px-5 py-2 rounded-xl text-xs font-black uppercase text-white bg-blue-600 hover:bg-blue-700">
                {stepIdx===TOUR_STEPS.length-1?'Concluir':'Próximo →'}
              </button>
            </div>
          </div>
        ) : (
          <>
            <div className="flex-1 overflow-y-auto p-4 space-y-2">
              {TOUR_STEPS.map((d) => (
                <div key={d.id} className="bg-slate-50 p-3 rounded-xl border border-slate-100 flex gap-3 items-start">
                  <div className="w-8 h-8 rounded-lg bg-white border border-slate-200 flex items-center justify-center flex-shrink-0 shadow-sm">{d.icon}</div>
                  <div className="flex-1 min-w-0">
                    <h4 className="font-black text-slate-800 text-xs mb-0.5">{d.title}</h4>
                    <p className="text-[11px] text-slate-500 leading-relaxed">{d.text}</p>
                    {d.targetTab && (
                      <button onClick={() => onNavigate(d.targetTab)}
                        className="mt-1.5 flex items-center gap-1 text-[9px] font-black uppercase text-blue-600 hover:text-blue-700">
            ...
            <div className="px-4 py-3 border-t border-slate-100 flex justify-between items-center">
              <button onClick={onClose} className="px-4 py-2 rounded-xl text-xs font-bold text-slate-500 hover:bg-slate-100">Fechar</button>
              <button onClick={() => { setTourAtivo(true); setStepIdx(0); }}
                className="flex items-center gap-2 px-5 py-2 rounded-xl text-xs font-black uppercase text-white bg-blue-600 hover:bg-blue-700">
                <Zap className="w-3.5 h-3.5"/>Iniciar Tour Guiado
              </button>
            </div>
          </>
        )}
...
// ══════════════════════════════════════════════════════════════════════════
// PAINEL CONFIGURAÇÕES (PATCH 3)
// ══════════════════════════════════════════════════════════════════════════
function PainelConfiguracoes({
  userId, mlConectado, mlNickname, agendador, intervalo, setIntervalo,
  savingAgendador, modoLento, setModoLento, salvarAgendador,
  conectarML, desconectarML, formatarProxima, onClose,
}) {
  return (
    <ModalOverlay onClose={onClose} side="right">
      <div className="bg-white shadow-2xl flex flex-col" style={{ width:'340px', height:'100%' }}>
        <div className="px-4 py-3 border-b border-slate-100 flex justify-between items-center shrink-0 bg-slate-900">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-5">
          <section>
            <p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-2 flex items-center gap-1.5">
              <ShoppingBag className="w-3 h-3"/>Conta Mercado Livre
            </p>
            {!mlConectado ? (
              <button onClick={conectarML}
                className="w-full flex items-center justify-center gap-2 bg-[#FFE600] text-slate-900 hover:bg-[#facc15] py-2.5 rounded-xl text-[10px] font-black uppercase shadow-sm transition-all">
                <ShoppingBag className="w-4 h-4"/>Conectar
              </button>
            ) : (
              <div className="bg-slate-50 border border-slate-200 rounded-xl overflow-hidden">
                <div className="flex justify-between items-center p-3">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <p className="text-xs font-black text-slate-800">{mlNickname||'Conta vinculada'}</p>
                      <p className="text-[9px] text-emerald-600 font-bold flex items-center gap-0.5"><CheckCircle2 className="w-2.5 h-2.5"/>Conectado</p>
                    </div>
                  </div>
                  <button onClick={desconectarML}
                    className="flex items-center gap-1 text-[9px] font-black uppercase text-red-500 hover:bg-red-50 px-2 py-1 rounded-lg border border-red-100 transition-all">
          ...
          <section>
            <p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-2 flex items-center gap-1.5">
              <Calendar className="w-3 h-3"/>Varredura Automática
            </p>

            <div className="flex items-center justify-between bg-slate-50 border border-slate-200 p-3 rounded-xl cursor-pointer hover:bg-slate-100 mb-3 transition-all"
              onClick={() => salvarAgendador(!agendador?.ativo, intervalo)}>
              <div>
                <p className="text-[10px] font-black text-slate-800 uppercase flex items-center gap-1.5">
                  <div className={`w-1.5 h-1.5 rounded-full ${agendador?.ativo?'bg-emerald-500 animate-pulse':'bg-slate-300'}`}/>
                  {agendador?.ativo ? 'Varredura Ativa' : 'Ativar Varredura'}
                </p>
                <p className="text-[9px] text-slate-500 mt-0.5 ml-3">
                  {agendador?.ativo
                    ? `Próxima em ${formatarProxima(agendador?.proximaExecucao)}`
                    : 'Clique para ativar'}
                </p>
              </div>
              <div className={`w-10 h-5 rounded-full relative transition-colors duration-200 ${agendador?.ativo?'bg-emerald-500':'bg-slate-300'}`}>
                <div className={`absolute top-0.5 bg-white w-4 h-4 rounded-full shadow-sm transition-all duration-200 ${agendador?.ativo?'left-5':'left-0.5'}`}/>
              </div>
            </div>

            {agendador?.ultimaExecucao && (
              <p className="text-[9px] text-slate-400 mb-2 flex items-center gap-1 pl-1">
                <Clock3 className="w-3 h-3"/>Última varredura: {
                  (() => { const d = new Date(agendador.ultimaExecucao); return isNaN(d.getTime()) ? '—' : d.toLocaleString('pt-BR',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'}); })()
                }
              </p>
            )}

            <p className="text-[9px] font-black uppercase text-slate-400 mb-1.5 ml-0.5">Intervalo entre varreduras</p>
            <div className="grid grid-cols-4 gap-1">
              {INTERVALOS.map(opt => (
                <button key={opt.value} onClick={() => { setIntervalo(opt.value); if(agendador?.ativo) salvarAgendador(true,opt.value); }}
                  className={`py-1.5 rounded-lg text-[9px] font-black uppercase border transition-all ${intervalo===opt.value?'bg-slate-900 text-white border-slate-900':'bg-white text-slate-600 border-slate-200 hover:border-blue-300 hover:text-blue-600'}`}>
                  {opt.label}
                </button>
              ))}
            </div>
            {!agendador?.ativo && (
              <button onClick={() => salvarAgendador(true,intervalo)} disabled={savingAgendador}
                className="w-full py-2 mt-2 bg-emerald-500 text-white rounded-xl text-[9px] font-black uppercase hover:bg-emerald-600 disabled:opacity-50 flex items-center justify-center gap-2">
                {savingAgendador?<><Loader2 className="w-3 h-3 animate-spin"/>Salvando...</>:'▶ Ativar Agora'}
              </button>
            )}
          </section>
          ...
          <section>
            ...
            <div className="flex items-center justify-between bg-slate-50 border border-slate-200 p-3 rounded-xl cursor-pointer hover:bg-slate-100 transition-all"
              onClick={() => setModoLento(!modoLento)}>
              <div>
                <p className="text-[10px] font-black text-slate-800 uppercase">Standby Anti-Timeout</p>
                <p className="text-[9px] text-slate-500 mt-0.5">Pausas entre buscas para evitar bloqueio de API.</p>
              </div>
              <div className={`w-10 h-5 rounded-full relative transition-colors duration-200 ${modoLento?'bg-emerald-500':'bg-slate-300'}`}>
                <div className={`absolute top-0.5 bg-white w-4 h-4 rounded-full shadow-sm transition-all duration-200 ${modoLento?'left-5':'left-0.5'}`}/>
              </div>
            </div>
          </section>
          ...
          <section>
            <p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-2 flex items-center gap-1.5">
              <Info className="w-3 h-3"/>Informações do Sistema
            </p>
            <div className="bg-slate-50 border border-slate-100 rounded-xl p-3 space-y-1.5 text-[9px] text-slate-500 font-mono">
              <div className="flex justify-between"><span>Usuário ID</span><span className="text-slate-700 font-bold">{userId||'—'}</span></div>
              <div className="flex justify-between"><span>Varredura</span><span className={`font-bold ${agendador?.ativo?'text-emerald-600':'text-slate-400'}`}>{agendador?.ativo?`A cada ${INTERVALOS.find(i=>i.value===intervalo)?.label||intervalo+'min'}`:'Inativa'}</span></div>
              <div className="flex justify-between"><span>ML</span><span className={`font-bold ${mlConectado?'text-emerald-600':'text-red-500'}`}>{mlConectado?'Conectado':'Desconectado'}</span></div>
            </div>
          </section>
        </div>

        <div className="px-4 py-3 border-t border-slate-100 shrink-0">
          <button onClick={onClose} className="w-full py-2 bg-slate-100 text-slate-600 rounded-xl text-[10px] font-black uppercase hover:bg-slate-200 transition-all">
            Fechar
          </button>
        </div>
      </div>
...
// ══════════════════════════════════════════════════════════════════════════
// FUNÇÃO: Calcular divergências de ficha técnica (COM BUSCA PROFUNDA NO ML)
// ══════════════════════════════════════════════════════════════════════════
function getMlAttr(item, idMatch) {
  if (!item || !item.attributes) return null;
  const attr = item.attributes.find(a => a.id === idMatch);
  return attr ? attr.value_name : null;
}

function getMlAttrNum(item, idMatch) {
  const val = getMlAttr(item, idMatch);
  if (!val) return 0;
  // Converte "1.5 kg" ou "1,5kg" para 1500 (gramas)
...
function calcularDivergenciasFicha(produtoLocal, itemML) {
  ...
  // 1. PESO
  const pesoLocal = produtoLocal.pesoGramas || 0; 
  const pesoMlStr = getMlAttr(itemML, 'PACKAGE_WEIGHT') || getMlAttr(itemML, 'WEIGHT');
  const pesoMlNum = getMlAttrNum(itemML, 'PACKAGE_WEIGHT') || getMlAttrNum(itemML, 'WEIGHT');
  
  if (pesoLocal > 0 && Math.abs(pesoLocal - pesoMlNum) > 50) {
    divs.push({ 
      campo: 'peso', campoML: 'PACKAGE_WEIGHT', nomeBonito: 'Peso', 
      valorML: pesoMlStr ? pesoMlStr : 'Não preenchido no ML', 
      valorERP: `${pesoLocal}g`, valorCorretoERP: `${pesoLocal} g` 
    });
  }

  // 2. EAN / GTIN
  const eanLocal = (produtoLocal.ean || '').trim(); 
  const eanMl = (getMlAttr(itemML, 'GTIN') || '').trim();
  if (eanLocal && eanLocal !== eanMl) {
    divs.push({ 
      campo: 'ean', campoML: 'GTIN', nomeBonito: 'EAN / GTIN', 
      valorML: eanMl ? eanMl : 'Não preenchido no ML', 
      valorERP: eanLocal, valorCorretoERP: eanLocal 
    });
  }

  // 3. MARCA
  const marcaLocal = (produtoLocal.marca || '').trim().toUpperCase(); 
  const marcaMl = (getMlAttr(itemML, 'BRAND') || '').trim().toUpperCase();
  if (marcaLocal && marcaLocal !== marcaMl) {
    divs.push({ 
      campo: 'marca', campoML: 'BRAND', nomeBonito: 'Marca', 
      valorML: getMlAttr(itemML, 'BRAND') ? getMlAttr(itemML, 'BRAND') : 'Não preenchido no ML', 
      valorERP: produtoLocal.marca, valorCorretoERP: produtoLocal.marca 
    });
  }

  // 4. MODELO
  const modeloLocal = (produtoLocal.modelo || '').trim().toUpperCase(); 
  const modeloMl = (getMlAttr(itemML, 'MODEL') || '').trim().toUpperCase();
  if (modeloLocal && modeloLocal !== modeloMl) {
    divs.push({ 
      campo: 'modelo', campoML: 'MODEL', nomeBonito: 'Modelo', 
      valorML: getMlAttr(itemML, 'MODEL') ? getMlAttr(itemML, 'MODEL') : 'Não preenchido no ML', 
      valorERP: produtoLocal.modelo, valorCorretoERP: produtoLocal.modelo 
    });
  }

  // 5. ALTURA
  const altLocal = parseFloat(produtoLocal.alturaCm) || 0; 
  const altMlStr = getMlAttr(itemML, 'PACKAGE_HEIGHT') || getMlAttr(itemML, 'HEIGHT');
  const altMlNum = getMlAttrNum(itemML, 'PACKAGE_HEIGHT') || getMlAttrNum(itemML, 'HEIGHT');
  if (altLocal > 0 && Math.abs(altLocal - altMlNum) > 0.5) {
    divs.push({ 
      campo: 'alturaCm', campoML: 'PACKAGE_HEIGHT', nomeBonito: 'Altura', 
      valorML: altMlStr ? altMlStr : 'Não preenchido no ML', 
      valorERP: `${altLocal}cm`, valorCorretoERP: `${altLocal} cm` 
    });
  }

  // 6. LARGURA
  const largLocal = parseFloat(produtoLocal.larguraCm) || 0; 
  const largMlStr = getMlAttr(itemML, 'PACKAGE_WIDTH') || getMlAttr(itemML, 'WIDTH');
  const largMlNum = getMlAttrNum(itemML, 'PACKAGE_WIDTH') || getMlAttrNum(itemML, 'WIDTH');
  if (largLocal > 0 && Math.abs(largLocal - largMlNum) > 0.5) {
    divs.push({ 
      campo: 'larguraCm', campoML: 'PACKAGE_WIDTH', nomeBonito: 'Largura', 
      valorML: largMlStr ? largMlStr : 'Não preenchido no ML', 
      valorERP: `${largLocal}cm`, valorCorretoERP: `${largLocal} cm` 
    });
  }

  // 7. COMPRIMENTO
  const compLocal = parseFloat(produtoLocal.comprimentoCm) || 0; 
  const compMlStr = getMlAttr(itemML, 'PACKAGE_LENGTH') || getMlAttr(itemML, 'LENGTH');
  const compMlNum = getMlAttrNum(itemML, 'PACKAGE_LENGTH') || getMlAttrNum(itemML, 'LENGTH');
  if (compLocal > 0 && Math.abs(compLocal - compMlNum) > 0.5) {
    divs.push({ 
      campo: 'comprimentoCm', campoML: 'PACKAGE_LENGTH', nomeBonito: 'Comprimento', 
      valorML: compMlStr ? compMlStr : 'Não preenchido no ML', 
      valorERP: `${compLocal}cm`, valorCorretoERP: `${compLocal} cm` 
    });
  }
...
export default function MercadoLivre() {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-3 py-2 flex flex-col animate-in fade-in duration-300" style={{ minHeight: '100vh' }}>
      ...
      {showDicas && (
        <ModalDicas onClose={() => setShowDicas(false)} activeTab={activeTab} onNavigate={(tab) => { setActiveTab(tab); setShowDicas(false); }} />
      )}
      ...
      {showConfig && (
        <PainelConfiguracoes
          ...
          savingAgendador={savingAgendador}
          modoLento={modoLento}
          setModoLento={setModoLento}
          salvarAgendador={salvarAgendador}
          conectarML={conectarML}
          desconectarML={desconectarML}
          formatarProxima={formatarProxima}
