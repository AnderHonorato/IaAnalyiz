...
import {
  ...
  AlertTriangle, Settings, Activity, Clock
} from 'lucide-react';
...
export default function MLDashboard() {
  ...
  const desconectarML = async () => {
    if (!await confirm({
      ...
      message: 'Isso encerrará a sessão. Todas as funcionalidades de integração serão pausadas.',
  ...
  const ferramentas = [
    {
      ...
      desc: 'Detecta e corrige automaticamente divergências de peso que aumentam seu custo de frete.',
      icon: Scale,
      color: 'text-blue-600', bg: 'bg-blue-50', border: 'border-blue-200', accent: '#3b82f6',
      route: '/ml/auditoria', active: true, requerML: true,
    },
    {
      id: 'precos',
      title: '💰 Precificação',
      desc: 'Altere preços e estoque dos seus anúncios diretamente via API do Mercado Livre.',
      icon: DollarSign,
      color: 'text-emerald-600', bg: 'bg-emerald-50', border: 'border-emerald-200', accent: '#10b981',
    ...
    },
    {
      ...
      icon: MessageCircle,
      color: 'text-amber-600', bg: 'bg-amber-50', border: 'border-amber-200', accent: '#f59e0b',
      route: '#', active: false, requerML: false,
    },
  ];

  const expiresFormatted = fmtDate(mlExpira);

  return (
    <div className="w-full max-w-5xl mx-auto p-4 space-y-4 animate-in fade-in duration-500">

      {/* HERO compacto */}
      <div className="bg-[#FFE600] rounded-2xl px-6 py-4 shadow-sm border border-yellow-400 relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-black text-slate-900">Central Mercado Livre</h2>
              <p className="text-slate-700 text-xs font-semibold">Gerencie anúncios, preços e integrações.</p>
            </div>
          </div>
          {/* Status rápido */}
          <div className={`flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-black ${mlConectado ? 'bg-emerald-600 text-white' : 'bg-white/70 text-slate-600'}`}>
            {loading ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : mlConectado ? <Wifi className="w-3.5 h-3.5" /> : <WifiOff className="w-3.5 h-3.5" />}
            {loading ? 'Verificando...' : mlConectado ? mlNickname || 'Conectado' : 'Desconectado'}
          </div>
        </div>
      </div>
          ...
          {loading ? (
            ...
            <div className="flex items-center justify-between flex-wrap gap-3">
              ...
              <div className="flex items-center gap-2">
                <button onClick={verificarStatusML} className="p-2 rounded-lg bg-slate-50 border border-slate-200 hover:bg-slate-100 text-slate-500" title="Atualizar status">
      ...
      <div>
        <h3 className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3 flex items-center gap-2 pl-1">
          <Activity className="w-3 h-3" /> Ferramentas
        </h3>
            ...
            return (
              ...
              >
                ...
                <div className="flex justify-between items-start mb-3">
                  <div className={`p-3 rounded-xl ${tool.bg} ${tool.color} shadow-sm`}>
                ...
                <h4 className="text-sm font-black text-slate-800 mb-1 leading-tight">{tool.title}</h4>
                <p className="text-xs text-slate-500 leading-relaxed flex-1">{tool.desc}</p>
                {bloqueado && tool.active && (
                  <p className="text-[9px] text-amber-600 font-black uppercase mt-2 flex items-center gap-1">
