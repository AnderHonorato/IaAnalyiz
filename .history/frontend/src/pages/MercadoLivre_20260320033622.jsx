...
import {
  ...
  Wifi, WifiOff, FileSearch, History, Play, Pause, BarChart3, TrendingUp, ShieldCheck
} from 'lucide-react';
...
function TypewriterLog({ msg, type }) {
  ...
  return <div className={`text-[11px] font-mono leading-relaxed ${colorClass}`}><span className="mr-2 text-slate-600 select-none">❯</span>{text}</div>;
}
...
export default function MercadoLivre() {
  ...
  useEffect(() => { 
    if (terminalRef.current) setTimeout(() => { terminalRef.current.scrollTop = terminalRef.current.scrollHeight; }, 100);
  }, [logs]);
  ...
  const desconectarML = async () => {
    if (!await confirm({ title: 'Desconectar', message: 'Desconectar a conta do Mercado Livre?', danger: true })) return;
  ...
  const iniciarBot = () => {
    if (!userId) return;
    setIsBotRunning(true); setProgress(0); setLogs([{ msg: `[${new Date().toLocaleTimeString('pt-BR')}] Processador iniciado.`, type: 'info' }]);
  ...
  // Variáveis para as Métricas Visuais (Dashboards pequenos no topo das tabelas)
  const totalValidados = produtos.filter(p => p.plataforma === 'Mercado Livre').length;
  const taxaSucesso = totalValidados > 0 ? Math.round(((totalValidados - (divStats.pendente + divStats.reincidente)) / totalValidados) * 100) : 100;
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 py-2 h-full flex flex-col relative animate-in fade-in duration-300">
        ...
        <div className="flex flex-col xl:flex-row justify-between xl:items-start mb-4 shrink-0 gap-3 relative z-10">
          <div className="flex items-start gap-3">
            <div className="flex items-center gap-2 mt-1">
              ...
              <h2 className="text-lg font-black tracking-tight text-slate-800 whitespace-nowrap">Auditoria ML</h2>
            </div>
            <div className="hidden md:flex flex-col">
               {aiInsight && (
                 <div className="flex items-start gap-1.5 bg-indigo-50/80 px-3 py-1.5 rounded-lg border border-indigo-100/50 max-w-md max-h-16 overflow-y-auto custom-scrollbar">
                   <Sparkles className="w-3.5 h-3.5 text-indigo-500 animate-pulse mt-0.5 flex-shrink-0" />
                   <span className="text-[11px] font-semibold text-indigo-900/80 leading-snug break-words" dangerouslySetInnerHTML={{ __html: aiInsight.replace(/Tenho novos dados para você:/i, '') }} />
                 </div>
               )}
            </div>
          </div>
          ...
          <div className="flex items-center flex-wrap gap-2 justify-end">
            <nav className="flex items-center gap-1 bg-white p-1 rounded-xl border border-slate-200 shadow-sm overflow-x-auto">
              {[['bot', 'Scanner ML'], ['pendentes', `Não Vinculados (${naoVinculados.length})`], ['produtos', 'Catálogo Local']].map(([k, l]) => (
                <button key={k} onClick={() => setActiveTab(k)} className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all whitespace-nowrap ${activeTab === k ? 'bg-slate-900 text-[#FFE600] shadow-md' : 'text-slate-500 hover:bg-slate-100'}`}>
                  {l}
                </button>
              ))}
            </nav>
            <div className="flex gap-1.5">
              <button onClick={() => { buscarHistoricoResumos(); setShowResumoModal(true); }} className="flex items-center gap-1 px-2 py-1.5 bg-blue-50 text-blue-600 rounded-xl hover:bg-blue-100 shadow-sm border border-blue-100 text-[10px] font-black uppercase" title="Resumo Analítico IA">
                <FileSearch className="w-3.5 h-3.5" /> Analisar
              </button>
              <button onClick={() => setShowDicas(true)} className="p-1.5 bg-slate-50 text-slate-500 rounded-xl hover:bg-slate-200 shadow-sm border border-slate-200" title="Ajuda">
      ...
      {/* ── MÓDULOS DE CONTEÚDO (TELA CHEIA) ── */}
      <div className={`relative flex-1 min-h-0 transition-all duration-300 flex flex-col ${fullScreenMode ? 'fixed inset-0 z-[999999] bg-[#f8fafc] p-3 sm:p-5' : ''}`}>
        
        {/* ── ABA SCANNER ── */}
        {activeTab === 'bot' && (
          <div className="grid grid-cols-1 xl:grid-cols-5 gap-4 h-full">
            <section className="xl:col-span-2 bg-white border border-slate-200 rounded-3xl shadow-sm flex flex-col overflow-hidden h-full">
              <div className="p-4 border-b border-slate-100">
                <button onClick={iniciarBot} disabled={isBotRunning || !mlConectado} className={`w-full py-3.5 rounded-2xl font-black text-[11px] uppercase tracking-widest mb-4 transition-all shadow-sm ${isBotRunning || !mlConectado ? 'bg-slate-100 text-slate-400' : 'bg-blue-600 text-white hover:bg-blue-700'}`}>
                  {isBotRunning ? <span className="flex items-center justify-center gap-2"><Loader2 className="w-4 h-4 animate-spin" /> Escaneando ML...</span> : !mlConectado ? '🔒 Conecte o ML na Engrenagem' : '🔍 Iniciar Varredura de Frete'}
                </button>
                <div className="bg-slate-900 rounded-2xl p-3.5 border border-slate-800">
                  <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest mb-2">
                    <span className="text-slate-400 flex items-center gap-1.5">Rede Neural {mlConectado ? <Wifi className="w-3 h-3 text-emerald-400"/> : <WifiOff className="w-3 h-3 text-red-400"/>}</span>
                    <span className="text-emerald-400">{progress}%</span>
                  </div>
                  <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden"><div className="h-full bg-emerald-500 transition-all duration-300" style={{ width: `${progress}%` }}></div></div>
                </div>
              </div>
              <div ref={terminalRef} className="flex-1 bg-slate-950 m-4 mt-0 rounded-2xl p-5 overflow-y-auto custom-scrollbar flex flex-col gap-1.5">
                {logs.map((log, i) => (
                  <div key={i} className={`text-[11px] font-mono leading-relaxed ${log.type === 'warn' ? 'text-amber-400 font-bold' : log.type === 'success' ? 'text-emerald-400 font-bold' : 'text-slate-300'}`}>
            ...
            <section className="xl:col-span-3 flex flex-col gap-4 h-full">
              ...
              <div className="grid grid-cols-3 gap-4 shrink-0">
                <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm flex flex-col justify-center">
                  <div className="flex items-center gap-2 text-slate-500 mb-1"><ShieldCheck className="w-4 h-4"/> <span className="text-[10px] font-black uppercase tracking-widest">Saúde Operacional</span></div>
                  <p className="text-2xl font-black text-slate-800">{taxaSucesso}% <span className="text-sm font-semibold text-emerald-500 ml-1">Precisão</span></p>
                </div>
                <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm flex flex-col justify-center">
                  <div className="flex items-center gap-2 text-slate-500 mb-1"><Package className="w-4 h-4"/> <span className="text-[10px] font-black uppercase tracking-widest">Catálogo ML</span></div>
                  <p className="text-2xl font-black text-blue-600">{totalValidados} <span className="text-xs font-semibold text-slate-400 uppercase tracking-widest">Vinculados</span></p>
                </div>
                <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm flex flex-col justify-center">
                  <div className="flex items-center gap-2 text-slate-500 mb-1"><AlertTriangle className="w-4 h-4"/> <span className="text-[10px] font-black uppercase tracking-widest">Risco Frete</span></div>
                  <p className="text-2xl font-black text-amber-500">{divStats.pendente + divStats.reincidente} <span className="text-xs font-semibold text-slate-400 uppercase tracking-widest">Alertas</span></p>
                </div>
              </div>
              ...
              <div className="bg-white border border-slate-200 rounded-3xl shadow-sm flex flex-col flex-1 min-h-0 overflow-hidden">
                <div className="p-4 border-b border-slate-100">
                  <div className="flex justify-between items-center mb-3 flex-wrap gap-2">
                    <div className="flex items-center gap-2 text-[12px] font-black uppercase text-slate-700">
                      <BarChart3 className="text-blue-500 w-5 h-5" /> Tabela de Divergências
                    </div>
                    <div className="flex items-center gap-1.5">
                      {selectedDivs.size > 0 && (
                        <div className="flex items-center gap-1 bg-blue-50 px-2.5 py-1.5 rounded-xl border border-blue-200 shadow-sm">
                          <span className="text-[10px] font-black text-blue-700 px-1">{selectedDivs.size} sel.</span>
                      ...
                      <button onClick={() => setFullScreenMode(!fullScreenMode)} className="p-2 rounded-xl bg-slate-50 hover:bg-slate-100 border border-slate-200 shadow-sm">
                        {fullScreenMode ? <Minimize2 className="w-4 h-4 text-slate-600" /> : <Maximize2 className="w-4 h-4 text-slate-600" />}
                      </button>
                    </div>
                  </div>
                  <div className="flex justify-between items-center gap-2 flex-wrap">
                    <div className="flex gap-1.5 flex-wrap">
                      {['PENDENTE', 'REINCIDENTE', 'CORRIGIDO', 'IGNORADO', 'TODOS'].map(s => {
                        ...
                        return (
                          <button key={s} onClick={() => setFiltroStatus(s)} className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl text-[10px] font-black uppercase border transition-all ${filtroStatus === s ? `${cfg.bg} ${cfg.color} ${cfg.border} shadow-sm ring-1 ring-offset-1 ring-${cfg.color.split('-')[1]}-200` : 'bg-white text-slate-500 border-slate-200 hover:bg-slate-50'}`}>
                            {cfg.label} <span className="bg-white/60 px-1.5 rounded text-[10px]">{count}</span>
                          </button>
                        );
                      })}
                    </div>
                    <div className="relative flex-1 min-w-[200px]">
                      ...
                      <input value={buscaDiv} onChange={e => setBuscaDiv(e.target.value)} placeholder="Filtrar tabela..." className="w-full pl-9 pr-3 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-blue-500 text-[11px]" />
                    </div>
                  </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto">
                  {divsFiltradas.length === 0 ? (
                    <div className="py-20 flex flex-col items-center gap-4"><div className="p-5 bg-emerald-100 rounded-full"><CheckCircle2 className="w-8 h-8 text-emerald-600" /></div><p className="text-[12px] font-black text-slate-400 uppercase tracking-[0.2em]">Zero Anomalias</p></div>
                  ) : (
                    <table className="w-full text-left">
                      <thead className="sticky top-0 bg-slate-50 border-b border-slate-200 shadow-sm z-10">
                        <tr className="text-[10px] font-black uppercase tracking-widest text-slate-500">
                          <th className="px-4 py-3 w-10"><button onClick={() => toggleSelectAll(divsFiltradas, selectedDivs, setSelectedDivs, 'id')} className="text-slate-400">{selectedDivs.size === divsFiltradas.length && divsFiltradas.length > 0 ? <CheckSquare className="w-4 h-4" /> : <Square className="w-4 h-4" />}</button></th>
                          <th className="px-4 py-3">Anúncio ML</th><th className="px-4 py-3">Relatório de Divergência</th><th className="px-4 py-3 text-right">Ações</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-100 text-[11px]">
                        {divsFiltradas.map((div) => {
                          ...
                          return (
                            <tr key={div.id} className={`hover:bg-slate-50/80 transition-colors ${sel ? 'bg-blue-50/40' : ''}`}>
                              <td className="px-4 py-3"><button onClick={() => { const n = new Set(selectedDivs); sel ? n.delete(div.id) : n.add(div.id); setSelectedDivs(n); }} className="text-slate-400">{sel ? <CheckSquare className="w-4 h-4 text-blue-600" /> : <Square className="w-4 h-4" />}</button></td>
                              <td className="py-3 px-4">
                                <span className={`text-[9px] font-black px-2 py-0.5 rounded-md ${cfg.bg} ${cfg.color} flex items-center gap-1 w-fit mb-1 border ${cfg.border}`}><cfg.icon className="w-3 h-3" />{cfg.label}</span>
                                <p className="font-bold text-slate-800 text-[12px] truncate max-w-[200px]">{div.titulo || div.mlItemId}</p>
                                <p className="text-slate-400 text-[10px] font-mono mt-0.5">{div.mlItemId}</p>
                              </td>
                              <td className="py-3 px-4"><p className="text-slate-600 text-[11px] font-medium leading-relaxed max-w-[300px]">{div.motivo}</p></td>
                              <td className="py-3 px-4 text-right">
                                <div className="flex items-center justify-end gap-1.5 relative">
                                  <button onClick={() => abrirFullModal(div)} className="p-2 rounded-xl bg-slate-100 text-slate-600 hover:bg-blue-100 hover:text-blue-600 transition-colors"><Eye className="w-4 h-4" /></button>
                                  {div.status !== 'CORRIGIDO' && <button onClick={() => acaoDiv(div.id, 'corrigido')} className="p-2 rounded-xl bg-emerald-50 text-emerald-600 hover:bg-emerald-100"><Check className="w-4 h-4" /></button>}
                                  {div.status !== 'IGNORADO' && <button onClick={() => acaoDiv(div.id, 'ignorado')} className="p-2 rounded-xl bg-slate-100 text-slate-500 hover:bg-slate-200"><EyeOff className="w-4 h-4" /></button>}
                                </div>
                              </td>
                            </tr>
                          );
        ...
        {/* ── ABA NÃO VINCULADOS ── */}
        {activeTab === 'pendentes' && (
          <section className="bg-white border border-slate-200 rounded-3xl shadow-sm flex flex-col h-full overflow-hidden">
            <div className="p-5 border-b border-slate-100 bg-amber-50/30 flex justify-between items-center flex-wrap gap-4">
              <div>
                <h3 className="text-[14px] font-black text-amber-800 uppercase flex items-center gap-2"><Box className="w-5 h-5" /> Anúncios Fantasmas (Não Vinculados)</h3>
                <p className="text-[11px] text-amber-700/70 mt-1 font-medium">Estes itens existem no ML mas o sistema não sabe o peso deles.</p>
              </div>
              <div className="flex items-center gap-2">
                <div className="relative w-56">
                  <Search className="absolute left-3 top-2.5 w-4 h-4 text-amber-600/50"/>
                  <input value={buscaNaoVinc} onChange={e => setBuscaNaoVinc(e.target.value)} placeholder="Pesquisar..." className="w-full pl-9 pr-3 py-2 bg-white/50 border border-amber-200 rounded-xl outline-none focus:border-amber-400 text-[11px] placeholder:text-amber-600/50 text-amber-900 shadow-sm" />
                </div>
                {selectedNaoVinc.size > 0 && (
                  <div className="flex items-center gap-1.5 bg-white border border-amber-200 rounded-xl px-2.5 py-1.5 shadow-sm">
                    <span className="text-[10px] font-black text-amber-700 mr-1">{selectedNaoVinc.size} sel.</span>
                    <button onClick={() => acaoLoteNaoVinculados('ignorado')} className="p-1.5 text-slate-500 hover:bg-slate-100 rounded" title="Ignorar Lote"><EyeOff className="w-4 h-4"/></button>
                    <button onClick={() => acaoLoteNaoVinculados('excluir')} className="p-1.5 text-red-500 hover:bg-red-50 rounded" title="Excluir Lote"><Trash2 className="w-4 h-4"/></button>
                  </div>
                )}
                ...
                <button onClick={() => setFullScreenMode(!fullScreenMode)} className="p-2 rounded-xl bg-white shadow-sm hover:bg-slate-50 border border-slate-200 text-slate-600">
            ...
            <div className="flex-1 overflow-y-auto">
              {nVincFiltrados.length === 0 ? (
                <div className="py-20 flex flex-col items-center justify-center gap-4"><div className="p-4 bg-emerald-50 rounded-full"><CheckCircle2 className="w-10 h-10 text-emerald-500"/></div><p className="text-center text-slate-400 font-black uppercase tracking-[0.2em] text-[12px]">Tudo perfeitamente mapeado!</p></div>
              ) : (
                <table className="w-full text-left">
                  <thead className="sticky top-0 bg-white border-b border-slate-100 shadow-sm z-10">
                    <tr className="text-[10px] font-black uppercase tracking-widest text-slate-400"><th className="px-5 py-4 w-10"><button onClick={() => toggleSelectAll(nVincFiltrados, selectedNaoVinc, setSelectedNaoVinc, 'id')} className="text-slate-400">{selectedNaoVinc.size === nVincFiltrados.length ? <CheckSquare className="w-4 h-4" /> : <Square className="w-4 h-4" />}</button></th><th className="px-5 py-4">Produto ML</th><th className="px-5 py-4 text-right">Ação</th></tr>
                  </thead>
                    ...
                    {nVincFiltrados.map(prod => {
                      ...
                      return (
                        <tr key={prod.id} className={`hover:bg-slate-50/60 transition-colors ${sel ? 'bg-amber-50/30' : ''}`}>
                          <td className="px-5 py-4"><button onClick={() => { const n = new Set(selectedNaoVinc); sel ? n.delete(prod.id) : n.add(prod.id); setSelectedNaoVinc(n); }} className="text-slate-400 hover:text-amber-500">{sel ? <CheckSquare className="w-4 h-4 text-amber-500" /> : <Square className="w-4 h-4" />}</button></td>
                          ...
                          <td className="px-5 py-4 text-right">
                            <div className="flex items-center justify-end gap-2 relative">
                              <button onClick={() => abrirFullModal(prod)} className="p-2 rounded-xl bg-slate-100 text-slate-600 hover:bg-slate-200 hover:scale-105 transition-all shadow-sm"><Eye className="w-4 h-4" /></button>
                              <button onClick={() => { abrirModalVincular(prod); setQuickViewItem(null); }} className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-emerald-500 text-white hover:bg-emerald-600 text-[11px] font-black uppercase shadow-md hover:-translate-y-0.5 transition-all">
                                <Link2 className="w-4 h-4" /> Vincular Kit/Peso
                              </button>
                              <button onClick={() => acaoNaoVinculadoBasica(prod.id, 'ignorado')} className="p-2 rounded-xl bg-slate-50 text-slate-500 hover:bg-slate-100"><EyeOff className="w-4 h-4" /></button>
                            </div>
                          </td>
                        </tr>
                      );
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-5 h-full">
            <section className="col-span-2 bg-white border border-slate-200 rounded-3xl shadow-md flex flex-col overflow-hidden h-full">
              <div className="p-5 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
                <h2 className="font-black text-[12px] uppercase flex items-center gap-2 text-slate-800"><Plus className="w-4 h-4 text-blue-600" /> {editandoId ? 'Editar Base' : 'Registro Manual Base'}</h2>
                <button onClick={() => setShowImportModal(true)} className="flex items-center gap-1.5 text-[10px] font-black text-emerald-600 bg-emerald-100 px-3 py-1.5 rounded-lg uppercase shadow-sm hover:bg-emerald-200"><Upload className="w-3.5 h-3.5"/> Importar CSV</button>
              </div>
              <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
                <form onSubmit={handleSubmitProduto} className="space-y-5 text-[11px] font-semibold text-slate-600">
                  <div className="grid grid-cols-2 gap-4">
                    <div><label className="uppercase tracking-widest block mb-1.5 text-[10px]">SKU Interno</label><input required value={formProd.sku} onChange={e => setFormProd({...formProd, sku: e.target.value})} className="w-full bg-white border border-slate-300 rounded-xl p-3 outline-none focus:border-blue-500 text-[12px]" /></div>
                    <div><label className="uppercase tracking-widest block mb-1.5 text-[10px]">Categoria</label><input value={formProd.categoria} onChange={e => setFormProd({...formProd, categoria: e.target.value})} className="w-full bg-white border border-slate-300 rounded-xl p-3 outline-none focus:border-blue-500 text-[12px]" list="cat-list" /><datalist id="cat-list">{categorias.map(c => <option key={c} value={c} />)}</datalist></div>
                  </div>
                  <div><label className="uppercase tracking-widest block mb-1.5 text-[10px]">Nome (Para compor Kits)</label><input required value={formProd.nome} onChange={e => setFormProd({...formProd, nome: e.target.value})} className="w-full bg-white border border-slate-300 rounded-xl p-3 outline-none focus:border-blue-500 text-[12px]" /></div>
                  <div className="bg-blue-50/50 border border-blue-100 rounded-2xl p-5">
                    <p className="text-[11px] font-black text-blue-800 uppercase tracking-widest flex items-center gap-1.5 mb-3"><Weight className="w-4 h-4" /> Peso Unitário da Peça</p>
                    <div className="flex gap-3">
                      <input required type="number" step="any" min="0" value={formProd.peso} onChange={e => setFormProd({...formProd, peso: e.target.value})} className="flex-1 bg-white border border-blue-200 rounded-xl p-3 outline-none focus:border-blue-500 text-[13px] font-bold" />
                  ...
                  <button type="submit" disabled={loadingProd} className="w-full py-4 bg-slate-900 text-white font-black text-[12px] uppercase tracking-widest rounded-xl hover:bg-blue-600 transition-all shadow-md mt-2">{loadingProd ? 'Processando...' : '📥 Salvar Ficha no Estoque'}</button>
                </form>
              </div>
            </section>

            <section className="col-span-3 bg-white border border-slate-200 rounded-3xl p-5 shadow-md flex flex-col h-full overflow-hidden">
              <div className="flex justify-between items-center mb-5 flex-wrap gap-4">
                <div className="bg-slate-100 p-1.5 rounded-xl flex gap-1">
                  <button onClick={() => setTipoCatalogo('BASE')} className={`px-4 py-2.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${tipoCatalogo === 'BASE' ? 'bg-white text-blue-600 shadow-sm ring-1 ring-slate-200' : 'text-slate-500 hover:bg-slate-200'}`}>Produtos Base</button>
                  <button onClick={() => setTipoCatalogo('ML')} className={`px-4 py-2.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${tipoCatalogo === 'ML' ? 'bg-white text-emerald-600 shadow-sm ring-1 ring-slate-200' : 'text-slate-500 hover:bg-slate-200'}`}>Anúncios ML Vinculados</button>
                </div>
                <div className="flex items-center gap-2">
                  <div className="relative w-56">
                    <Search className="absolute left-3 top-2.5 w-4 h-4 text-slate-400" />
                    <input value={searchProd} onChange={e => setSearchProd(e.target.value)} placeholder="Pesquisar catálogo..." className="w-full pl-9 pr-3 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-blue-500 text-[11px]" />
                  </div>
                  {selectedProds.size > 0 && <button onClick={excluirLoteProdutos} className="p-2 rounded-xl bg-red-50 text-red-600 hover:bg-red-100 shadow-sm"><Trash2 className="w-4 h-4" /></button>}
                  <button onClick={() => setFullScreenMode(!fullScreenMode)} className="p-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-600 shadow-sm">
                    {fullScreenMode ? <Minimize2 className="w-4 h-4" /> : <Maximize2 className="w-4 h-4" />}
                  </button>
                </div>
              </div>
              <div className="flex-1 overflow-y-auto custom-scrollbar border border-slate-100 rounded-2xl">
                <table className="w-full text-left">
                  <thead className="bg-slate-50 sticky top-0 z-10 border-b border-slate-100 shadow-sm">
                    <tr className="text-[10px] font-black uppercase tracking-widest text-slate-400"><th className="px-4 py-3 w-10"><button onClick={() => toggleSelectAll(produtosFiltrados, selectedProds, setSelectedProds)} className="text-slate-400">{selectedProds.size === produtosFiltrados.length && produtosFiltrados.length > 0 ? <CheckSquare className="w-4 h-4" /> : <Square className="w-4 h-4" />}</button></th><th className="px-4 py-3">Produto ou Anúncio</th><th className="px-4 py-3">Peso Fixo Calculado</th><th className="px-4 py-3 text-right">Ação</th></tr>
                  </thead>
                  <tbody className="divide-y divide-slate-50 text-[12px]">
                    {produtosFiltrados.length === 0 && <tr><td colSpan="4" className="py-20 text-center text-slate-400 text-[12px] font-black uppercase tracking-widest">Catálogo Vazio</td></tr>}
                    {produtosFiltrados.map(prod => {
                      ...
                      return (
                        <tr key={prod.id} className={`hover:bg-slate-50/80 transition-colors ${sel ? 'bg-blue-50/30' : ''}`}>
                          <td className="px-4 py-3.5"><button onClick={() => { const n = new Set(selectedProds); sel ? n.delete(prod.id) : n.add(prod.id); setSelectedProds(n); }} className="text-slate-400 hover:text-blue-600">{sel ? <CheckSquare className="w-4 h-4 text-blue-600" /> : <Square className="w-4 h-4" />}</button></td>
                          <td className="px-4 py-3.5">
                            <div className="flex items-center gap-3">
                              {prod.thumbnail ? <img src={prod.thumbnail} className="w-12 h-12 rounded-lg object-cover shadow-sm border border-slate-100" /> : <div className="w-12 h-12 bg-slate-100 rounded-lg flex items-center justify-center"><ImageIcon className="w-5 h-5 text-slate-300"/></div>}
                              <div>
                                <div className="flex flex-wrap items-center gap-1.5 mb-1.5">
                                  ...
                                  {prod.mlItemId && <span className="text-[9px] font-bold text-emerald-700 bg-emerald-100 border border-emerald-200 px-1.5 py-0.5 rounded flex items-center gap-1"><Check className="w-3 h-3"/> {prod.mlItemId}</span>}
                                </div>
                                <p className="text-[12px] font-bold text-slate-800 truncate max-w-[280px]">{prod.nome}</p>
                              </div>
                            </div>
                          </td>
                          <td className="px-4 py-3.5"><span className="text-[11px] text-blue-700 font-black bg-blue-50 border border-blue-100 px-3 py-1 rounded-lg shadow-sm">{prod.pesoGramas}g</span></td>
                          <td className="px-4 py-3.5 text-right">
                            <div className="flex justify-end gap-1.5 relative">
                              <button onClick={() => abrirFullModal(prod)} className="p-2 rounded-xl bg-slate-100 text-slate-600 hover:bg-blue-100"><Eye className="w-4 h-4" /></button>
                              {prod.mlItemId && <button onClick={() => desvincularProduto(prod.id)} className="p-2 rounded-xl bg-amber-50 text-amber-600 hover:bg-amber-100" title="Desvincular (Voltar p/ Aba Não Vinculados)"><Unlink className="w-4 h-4" /></button>}
                              <button onClick={() => excluirProduto(prod.id)} className="p-2 rounded-xl bg-red-50 text-red-500 hover:bg-red-100"><Trash2 className="w-4 h-4" /></button>
                            </div>
                          </td>
                        </tr>
                      );
      ...
      {fullModalItem && (
        <div className="fixed inset-0 z-[999999] bg-slate-900/80 backdrop-blur-sm flex items-center justify-center p-4 lg:p-10 animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl shadow-[0_30px_60px_rgba(0,0,0,0.5)] w-full max-w-5xl flex flex-col max-h-full">
            <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-white rounded-t-3xl z-10 shadow-sm">
              <h3 className="font-black text-slate-800 uppercase tracking-widest flex items-center gap-2.5"><Box className="w-5 h-5 text-blue-600"/> Integração Direta: ML</h3>
            ...
            <div className="flex-1 overflow-y-auto bg-slate-50/50 p-6 lg:p-8 custom-scrollbar rounded-b-3xl">
              {loadingFullData ? (
                <div className="flex flex-col items-center justify-center py-32 text-blue-500"><Loader2 className="w-10 h-10 animate-spin mb-4"/><p className="text-[12px] font-black uppercase tracking-widest text-slate-400">Acessando API do Mercado Livre...</p></div>
              ) : fullItemData ? (
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                  ...
                  <div className="lg:col-span-7 space-y-6">
                    <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                      <h4 className="text-[12px] font-black uppercase text-slate-400 border-b border-slate-100 pb-2 mb-4 flex items-center gap-2"><LayoutList className="w-4 h-4"/> Ficha Técnica</h4>
                    ...
                    <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                      <h4 className="text-[12px] font-black uppercase text-slate-400 border-b border-slate-100 pb-2 mb-4 flex items-center gap-2"><FileText className="w-4 h-4"/> Descrição Cadastrada no ML</h4>
                ...
                <div className="text-center py-20 text-slate-400 text-[12px] uppercase tracking-widest font-black">Falha ao conectar com a API do ML</div>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-[999999] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-5 border-b border-slate-100 bg-white flex justify-between items-center z-10 shadow-sm">
              <h3 className="font-black text-slate-800 uppercase text-[12px] tracking-tight flex items-center gap-2"><Link2 className="w-5 h-5 text-emerald-600"/> Vincular Anúncio ao Catálogo Base</h3>
            ...
            <div className="flex-1 overflow-y-auto p-6 bg-slate-50 custom-scrollbar">
              <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm mb-6 flex gap-4 items-center">
                ...
                <div>
                  ...
                  <p className="font-bold text-[13px] text-slate-800 mt-1 leading-tight">{vincularAnuncio.nome}</p>
                </div>
              </div>
              <div className="mb-8">
                <label className="text-[11px] font-black uppercase tracking-widest text-slate-500 mb-3 block">Opção 1: O que tem dentro da caixa? (Cálculo Automático)</label>
                ...
                <div className="space-y-2">
                  ...
                  {composicaoKit.length > 0 && <div className="text-right p-3 text-[14px] font-black text-slate-800">Peso Calculado da Caixa: <span className="text-emerald-600">{composicaoKit.reduce((a,c)=>a+(c.produto.pesoGramas*c.quantidade),0)}g</span></div>}
                </div>
              </div>
            ...
            <div className="p-5 border-t border-slate-100 bg-white flex justify-end gap-3 z-10 shadow-[0_-4px_10px_rgba(0,0,0,0.02)]">
              <button onClick={() => setVincularAnuncio(null)} className="px-6 py-3 rounded-xl text-[11px] font-bold text-slate-500 hover:bg-slate-100 transition-all">Cancelar</button>
      ...
      {/* MODAL RESUMO IA (Novo com Histórico) */}
      {showResumoModal && (
        <div className="fixed inset-0 z-[999999] bg-slate-900/70 backdrop-blur-sm flex justify-end animate-in fade-in duration-200">
          <div className="bg-white shadow-2xl w-full max-w-md h-full flex flex-col animate-in slide-in-from-right-8">
            <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-indigo-600 text-white">
              <h3 className="font-black uppercase tracking-widest text-[12px] flex items-center gap-2"><Sparkles className="w-4 h-4"/> Resumo Analítico da Operação</h3>
            ...
            <div className="flex-1 overflow-y-auto p-6 bg-slate-50 custom-scrollbar">
              {loadingResumo ? (
                <div className="flex flex-col items-center justify-center py-32 text-indigo-500">
                  ...
                  <p className="text-[11px] font-black uppercase tracking-widest text-indigo-400">A IA está gerando o relatório...</p>
                </div>
              ) : (
                <div className="text-[13px] text-slate-700 leading-relaxed bg-white p-6 rounded-2xl shadow-sm border border-slate-200" dangerouslySetInnerHTML={{ __html: resumoIA || 'Clique abaixo para gerar seu primeiro relatório de saúde da loja.' }} />
              )}
              
              <div className="mt-8 pt-6 border-t border-slate-200">
                <h4 className="text-[10px] font-black uppercase text-slate-400 mb-4 flex items-center gap-1.5"><History className="w-3.5 h-3.5"/> Histórico de Consultas</h4>
                {historicoResumos.length === 0 ? <p className="text-[11px] text-slate-400 text-center py-4">Nenhum histórico salvo no banco.</p> : (
            ...
            <div className="p-5 border-t border-slate-100 bg-white">
              <button onClick={gerarResumoLogistico} disabled={loadingResumo} className="w-full py-3.5 bg-indigo-600 text-white rounded-xl text-[11px] font-black uppercase shadow-md hover:bg-indigo-700 disabled:opacity-50">Gerar Relatório Agora</button>
            </div>
          </div>
        </div>
      )}

      {/* MODAL CONFIGURAÇÕES E DICAS MANTIDOS COMO ESTAVAM... (espaço otimizado) */}
      {showConfig && (
        <div className="fixed inset-0 z-[999999] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in">
          <div className="bg-white rounded-3xl w-full max-w-sm shadow-2xl overflow-hidden">
            ...
            <div className="p-6 space-y-6">
              <div>
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 block mb-3">Conexão API do ML</label>
                {!mlConectado ? (
              ...
              <div className="pt-4 border-t border-slate-100">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 block mb-3">Modo de Varredura do Robô</label>
                <div className="flex items-center justify-between bg-slate-50 border border-slate-200 p-4 rounded-2xl cursor-pointer hover:bg-slate-100 transition-colors" onClick={() => setModoLento(!modoLento)}>
                  <div><p className="text-[11px] font-black text-slate-800 uppercase">Standby (Anti-Timeout)</p><p className="text-[10px] text-slate-500 mt-1 max-w-[200px] leading-snug">O robô fará longas pausas entre as buscas para não ser bloqueado pelo ML.</p></div>
