...
import {
  ...
  Edit3, Save, X, Zap, Info, Tag, TrendingDown, Scale, Ruler, Sliders, ChevronDown, ChevronUp,
  BarChart, Activity, Send
} from 'lucide-react';
...
// ── Chip de variação de peso ──────────────────────────────────────────────────
function VariacaoPesoChip({ pesoMl, pesoLocal }) {
  if (!pesoMl || !pesoLocal || pesoMl === pesoLocal) return null;
  const diff = pesoMl - pesoLocal;
  const pct  = ((Math.abs(diff) / pesoLocal) * 100).toFixed(1);
  const acima = diff > 0;
  return (
    <span className={`inline-flex items-center gap-0.5 text-[9px] font-black px-1.5 py-0.5 rounded-full ${acima ? 'bg-red-50 text-red-600' : 'bg-blue-50 text-blue-600'}`}>
      {acima ? <TrendingUp className="w-2.5 h-2.5" /> : <TrendingDown className="w-2.5 h-2.5" />}
      ML {acima ? '+' : ''}{diff}g ({pct}%)
    </span>
  );
}

// ── Comparação visual de campo (ML vs Local) ──────────────────────────────────
function CampoComparacao({ label, valorMl, valorLocal, unidade = '', editavel = false, onSalvar, salvando = false }) {
  const [editando, setEditando] = useState(false);
  const [valor, setValor]       = useState('');
  const iguais = String(valorMl || '') === String(valorLocal || '');

  const abrir = () => { setValor(String(valorMl || '')); setEditando(true); };
  const salvar = async () => { await onSalvar?.(valor); setEditando(false); };

  return (
    <div className={`p-2.5 rounded-xl border ${iguais ? 'border-slate-100 bg-slate-50/50' : 'border-amber-100 bg-amber-50/40'}`}>
      <div className="flex items-center justify-between mb-1.5">
        <span className="text-[9px] font-black uppercase tracking-widest text-slate-400">{label}</span>
        {!iguais && <span className="text-[8px] bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-full font-black uppercase">Divergente</span>}
      </div>
      <div className="flex items-center gap-2">
        <div className="flex-1">
          <p className="text-[9px] text-slate-400 mb-0.5">No ML</p>
          {editando ? (
            <div className="flex items-center gap-1">
              <input type="text" value={valor} onChange={e => setValor(e.target.value)}
                className="flex-1 border border-blue-300 rounded-lg px-2 py-1 text-xs font-bold outline-none focus:border-blue-500 bg-white"
                onKeyDown={e => { if (e.key === 'Enter') salvar(); if (e.key === 'Escape') setEditando(false); }}
                autoFocus />
              <button onClick={salvar} disabled={salvando} className="p-1 bg-emerald-500 text-white rounded-lg hover:bg-emerald-600 disabled:opacity-50">
                {salvando ? <Loader2 className="w-3 h-3 animate-spin" /> : <Check className="w-3 h-3" />}
              </button>
              <button onClick={() => setEditando(false)} className="p-1 bg-slate-100 text-slate-500 rounded-lg hover:bg-slate-200">
                <X className="w-3 h-3" />
              </button>
            </div>
          ) : (
            <div className="flex items-center gap-1">
              <span className={`text-sm font-black ${!iguais ? 'text-red-600' : 'text-slate-800'}`}>
                {valorMl || '—'}{valorMl ? unidade : ''}
              </span>
              {editavel && (
                <button onClick={abrir} className="p-0.5 text-slate-400 hover:text-blue-600 opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        {!iguais && (
          <>
            ...
            <div className="flex-1">
              <p className="text-[9px] text-slate-400 mb-0.5">Local (correto)</p>
              <span className="text-sm font-black text-emerald-600">{valorLocal || '—'}{valorLocal ? unidade : ''}</span>
            </div>
          </>
        )}
      </div>
...
// ── Painel de Ficha Técnica + Ajuste ─────────────────────────────────────────
function PainelFichaTecnica({ item, userId, onAtualizado }) {
  const [fichaML, setFichaML]         = useState(null);
  const [loading, setLoading]         = useState(false);
  const [salvando, setSalvando]       = useState(false);
  const [logAjuste, setLogAjuste]     = useState([]);
  const [modoAjuste, setModoAjuste]   = useState('automatico'); // 'automatico' | 'manual'
  ...
  const produtoLocal = item;

  useEffect(() => {
    if (aberto && !fichaML && item.mlItemId) carregarFicha();
  }, [aberto]);

  const carregarFicha = async () => {
    setLoading(true);
    try {
      const res  = await fetch(`${API_BASE_URL}/api/ml/item-details/${item.mlItemId}?userId=${userId}`);
      const data = await res.json();
      setFichaML(data);
      // Preenche os campos manuais com os valores do ML
      const getAttr = (id) => data.attributes?.find(a => a.id === id)?.value_name || '';
      const pesoAttr = getAttr('PACKAGE_WEIGHT') || getAttr('SELLER_PACKAGE_WEIGHT');
      setAjusteManual({
        peso:        pesoAttr ? parseInt(pesoAttr.replace(/\D/g, '')) || '' : '',
        altura:      parseInt((getAttr('PACKAGE_HEIGHT') || '').replace(/\D/g, '')) || '',
        largura:     parseInt((getAttr('PACKAGE_WIDTH') || '').replace(/\D/g, '')) || '',
        comprimento: parseInt((getAttr('PACKAGE_LENGTH') || '').replace(/\D/g, '')) || '',
        ean:         getAttr('GTIN') || getAttr('EAN') || '',
        marca:       getAttr('BRAND') || '',
        modelo:      getAttr('MODEL') || '',
      });
    } catch {
      setFichaML(null);
    } finally {
  ...
  const aplicarAjusteAutomatico = async () => {
    if (!produtoLocal || !item.mlItemId) return;
    setSalvando(true);
    setLogAjuste([]);

    const atributos = {};
    if (produtoLocal.pesoGramas) atributos.peso        = produtoLocal.pesoGramas;
    if (produtoLocal.alturaCm)   atributos.altura      = produtoLocal.alturaCm;
    if (produtoLocal.larguraCm)  atributos.largura     = produtoLocal.larguraCm;
    if (produtoLocal.comprimentoCm) atributos.comprimento = produtoLocal.comprimentoCm;

    try {
      const res  = await fetch(`${API_BASE_URL}/api/anuncios/${item.mlItemId}`, {
        method: 'PUT', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId, atributosFicha: atributos }),
      });
      const data = await res.json();

      if (data.success) {
        setLogAjuste([{ tipo: 'ok', msg: `Ficha técnica atualizada com sucesso no Mercado Livre!` }]);
        onAtualizado?.();
        // Recarrega a ficha
        await carregarFicha();
      } else {
        setLogAjuste([{ tipo: 'erro', msg: `Erro: ${data.error || 'Falha na API do ML'}` }]);
      }
    } catch (e) {
      setLogAjuste([{ tipo: 'erro', msg: `Falha de conexão: ${e.message}` }]);
    } finally {
      setSalvando(false);
    }
  };
  ...
  const aplicarAjusteManual = async () => {
    ...
    const atributos = {};
    if (ajusteManual.peso)        atributos.peso        = parseFloat(ajusteManual.peso);
    if (ajusteManual.altura)      atributos.altura      = parseFloat(ajusteManual.altura);
    if (ajusteManual.largura)     atributos.largura     = parseFloat(ajusteManual.largura);
    if (ajusteManual.comprimento) atributos.comprimento = parseFloat(ajusteManual.comprimento);
    if (ajusteManual.ean)         atributos.ean         = ajusteManual.ean;
    if (ajusteManual.marca)       atributos.marca       = ajusteManual.marca;
    if (ajusteManual.modelo)      atributos.modelo      = ajusteManual.modelo;
  ...
  const getAttrMl = (id) => fichaML?.attributes?.find(a => a.id === id)?.value_name || null;
  const pesoMlRaw = getAttrMl('PACKAGE_WEIGHT') || getAttrMl('SELLER_PACKAGE_WEIGHT');
  const pesoMl    = pesoMlRaw ? parseInt(pesoMlRaw.replace(/\D/g, '')) || 0 : 0;
  const pesoLocal = produtoLocal?.pesoGramas || 0;

  if (!item.mlItemId) return null;
    ...
    <div className="border border-slate-200 rounded-xl overflow-hidden mt-2">
      ...
      >
        <span className="text-[10px] font-black uppercase tracking-widest text-slate-600 flex items-center gap-1.5">
          ...
          Ficha Técnica & Ajuste de Divergência
          {pesoMl > 0 && pesoLocal > 0 && pesoMl !== pesoLocal && (
            <span className="text-[8px] bg-red-100 text-red-600 px-1.5 py-0.5 rounded-full font-black ml-1">
              ⚠ {Math.abs(pesoMl - pesoLocal)}g divergência
            </span>
          )}
        </span>
        ...
        <div className="p-3 bg-white space-y-3">
          {loading ? (
            <div className="flex items-center justify-center py-8">
              ...
              <span className="text-xs text-slate-400 ml-2">Carregando ficha técnica do ML...</span>
            </div>
          ) : fichaML ? (
            <>
              {/* Comparação Peso */}
              <div>
                ...
                <div className="group">
                  <CampoComparacao
                    ...
                    valorMl={pesoMl || '—'}
                    valorLocal={pesoLocal || '—'}
                    unidade="g"
                  />
                </div>
                {pesoMl > 0 && pesoLocal > 0 && pesoMl !== pesoLocal && (
                  <div className="mt-1.5 flex items-center gap-2">
                    <VariacaoPesoChip pesoMl={pesoMl} pesoLocal={pesoLocal} />
                    <span className="text-[9px] text-slate-400">
                      Impacto estimado no frete: <strong className="text-slate-600">R$ {((Math.abs(pesoMl - pesoLocal) / 1000) * 12).toFixed(2)}</strong>/envio
                    </span>
                  </div>
                )}
              </div>
              ...
              <div>
                <p className="text-[9px] font-black uppercase tracking-widest text-slate-500 mb-1.5 flex items-center gap-1">
                  <Ruler className="w-3 h-3" />Dimensões
                </p>
                <div className="grid grid-cols-3 gap-1.5">
                  {[
                    ['Altura', 'PACKAGE_HEIGHT', produtoLocal?.alturaCm, 'cm'],
                    ['Largura', 'PACKAGE_WIDTH', produtoLocal?.larguraCm, 'cm'],
                    ['Comprimento', 'PACKAGE_LENGTH', produtoLocal?.comprimentoCm, 'cm'],
                  ].map(([label, attrId, local, un]) => {
                    const mlVal = parseInt((getAttrMl(attrId) || '').replace(/\D/g, '')) || null;
                    return (
                      <div key={label} className={`p-2 rounded-lg border text-center ${mlVal && local && mlVal !== local ? 'border-amber-200 bg-amber-50/40' : 'border-slate-100 bg-slate-50/50'}`}>
                        <p className="text-[8px] text-slate-400 font-black uppercase mb-0.5">{label}</p>
                        <p className={`text-xs font-black ${mlVal && local && mlVal !== local ? 'text-red-600' : 'text-slate-700'}`}>{mlVal ? `${mlVal}${un}` : '—'}</p>
                        {mlVal && local && mlVal !== local && (
                          <p className="text-[8px] text-emerald-600 font-bold">→ {local}{un}</p>
                        )}
                      </div>
                    );
                  })}
              ...
              {/* Outros atributos */}
              {fichaML.attributes?.filter(a => ['GTIN','BRAND','MODEL'].includes(a.id) && a.value_name).length > 0 && (
                <div>
                  <p className="text-[9px] font-black uppercase tracking-widest text-slate-500 mb-1.5 flex items-center gap-1">
                    <Tag className="w-3 h-3" />Identificação
                  </p>
                  <div className="grid grid-cols-3 gap-1.5">
                    {fichaML.attributes.filter(a => ['GTIN','BRAND','MODEL'].includes(a.id) && a.value_name).map(attr => (
                      <div key={attr.id} className="p-2 bg-slate-50 rounded-lg border border-slate-100">
                        <p className="text-[8px] text-slate-400 font-black uppercase mb-0.5">{attr.name}</p>
                        <p className="text-xs font-bold text-slate-700 truncate">{attr.value_name}</p>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="border-t border-slate-100 pt-3">
                <div className="flex items-center justify-between mb-2">
                  <p className="text-[9px] font-black uppercase tracking-widest text-slate-500 flex items-center gap-1">
                    <Sliders className="w-3 h-3" />Modo de Correção
                  </p>
                  <div className="flex gap-0.5 bg-slate-100 p-0.5 rounded-lg">
                    <button onClick={() => setModoAjuste('automatico')}
                      className={`px-2.5 py-1 rounded-md text-[9px] font-black uppercase transition-all ${modoAjuste === 'automatico' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500'}`}>
                      <Zap className="w-2.5 h-2.5 inline mr-0.5" />Auto
                    </button>
                    <button onClick={() => setModoAjuste('manual')}
                      className={`px-2.5 py-1 rounded-md text-[9px] font-black uppercase transition-all ${modoAjuste === 'manual' ? 'bg-white text-purple-600 shadow-sm' : 'text-slate-500'}`}>
                      <Edit3 className="w-2.5 h-2.5 inline mr-0.5" />Manual
                    </button>
                  </div>
                </div>
                  ...
                  <div className="space-y-2">
                    <div className="bg-blue-50/50 border border-blue-100 rounded-xl p-3">
                      <p className="text-[10px] font-bold text-blue-700 mb-1.5 flex items-center gap-1">
                        <Zap className="w-3 h-3" />Correção Automática — usa os dados do catálogo local
                      </p>
                      <div className="grid grid-cols-2 gap-1.5">
                        {[
                          ['Peso', produtoLocal?.pesoGramas, 'g'],
                          ['Altura', produtoLocal?.alturaCm, 'cm'],
                          ['Largura', produtoLocal?.larguraCm, 'cm'],
                          ['Comprimento', produtoLocal?.comprimentoCm, 'cm'],
                        ].filter(([,v]) => v > 0).map(([l, v, u]) => (
                          <div key={l} className="flex items-center justify-between bg-white rounded-lg px-2.5 py-1.5 border border-blue-100">
                            <span className="text-[9px] text-slate-500 font-bold">{l}</span>
                            <span className="text-[10px] font-black text-blue-700">{v}{u}</span>
                          </div>
                        ))}
                      </div>
                      {!produtoLocal?.pesoGramas && (
                        <p className="text-[9px] text-amber-600 mt-1.5 flex items-center gap-1">
                          <AlertTriangle className="w-3 h-3" />Produto sem peso no catálogo — use o modo manual
                        </p>
                      )}
                    </div>
                    <button onClick={aplicarAjusteAutomatico}
                      disabled={salvando || !produtoLocal?.pesoGramas}
                      className="w-full py-2 rounded-xl bg-blue-600 text-white font-black text-[10px] uppercase hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-1.5 transition-all">
                      {salvando ? <><Loader2 className="w-3.5 h-3.5 animate-spin" />Aplicando...</> : <><Zap className="w-3.5 h-3.5" />Aplicar Correção Automática no ML</>}
                    </button>
                  </div>
                ) : (
                  <div className="space-y-2">
                    <div className="grid grid-cols-2 gap-2">
                      {[
                        { key: 'peso', label: 'Peso', unidade: 'g', icon: Scale },
                        { key: 'altura', label: 'Altura', unidade: 'cm', icon: Ruler },
                        { key: 'largura', label: 'Largura', unidade: 'cm', icon: Ruler },
                        { key: 'comprimento', label: 'Compr.', unidade: 'cm', icon: Ruler },
                      ].map(({ key, label, unidade }) => (
                        <div key={key}>
                          <label className="text-[8px] font-black uppercase tracking-widest text-slate-400 block mb-1">{label} ({unidade})</label>
                          <input
                            type="number" step="any" min="0"
                            value={ajusteManual[key]}
                            onChange={e => setAjusteManual(p => ({ ...p, [key]: e.target.value }))}
                            className="w-full border border-slate-200 rounded-lg px-2 py-1.5 text-xs font-bold outline-none focus:border-purple-400 bg-white"
                            placeholder={`Ex: ${key === 'peso' ? '500' : '20'}`}
                          />
                        </div>
                      ))}
                    </div>
                    <div className="grid grid-cols-1 gap-1.5">
                      {[
                        ...
                        <div key={key} className="flex items-center gap-2">
                          <label className="text-[8px] font-black uppercase tracking-widest text-slate-400 w-16 shrink-0">{label}</label>
                          <input
                            type="text"
                            value={ajusteManual[key]}
                            onChange={e => setAjusteManual(p => ({ ...p, [key]: e.target.value }))}
                            className="flex-1 border border-slate-200 rounded-lg px-2 py-1.5 text-xs font-bold outline-none focus:border-purple-400 bg-white"
                            placeholder={`${label}...`}
                          />
                        </div>
                      ))}
                    </div>
                    <button onClick={aplicarAjusteManual}
                      disabled={salvando}
                      className="w-full py-2 rounded-xl bg-purple-600 text-white font-black text-[10px] uppercase hover:bg-purple-700 disabled:opacity-50 flex items-center justify-center gap-1.5 transition-all">
                      {salvando ? <><Loader2 className="w-3.5 h-3.5 animate-spin" />Aplicando...</> : <><Send className="w-3.5 h-3.5" />Enviar Correção Manual para o ML</>}
                    </button>
                  </div>
                )}
                ...
                {/* Log de resultado */}
                {logAjuste.length > 0 && (
                  <div className={`mt-2 p-2.5 rounded-xl border text-[10px] font-bold flex items-center gap-2 ${logAjuste[0].tipo === 'ok' ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : 'bg-red-50 border-red-200 text-red-600'}`}>
                    {logAjuste[0].tipo === 'ok' ? <CheckCircle2 className="w-3.5 h-3.5 shrink-0" /> : <AlertTriangle className="w-3.5 h-3.5 shrink-0" />}
                    {logAjuste[0].msg}
                  </div>
                )}
              </div>
            ...
            <div className="py-6 text-center">
              <p className="text-xs text-slate-400 font-black uppercase">Falha ao carregar ficha do ML</p>
              <button onClick={carregarFicha} className="mt-2 px-3 py-1.5 text-xs font-black text-blue-600 bg-blue-50 rounded-lg hover:bg-blue-100">
                Tentar novamente
              </button>
            </div>
          )}
...
export default function MercadoLivre() {
  ...
  // Estado para controle de edição inline de divergência
  const [divExpandida, setDivExpandida]         = useState(null);
  ...
  // Encontra produto local vinculado a uma divergência
  const getProdutoLocalDaDiv = (div) => {
    return produtos.find(p => (p.mlItemId || '').toUpperCase() === (div.mlItemId || '').toUpperCase()) || null;
  };
    ...
    <div className="w-full max-w-7xl mx-auto px-3 py-2 flex flex-col animate-in fade-in duration-300" style={{ minHeight: '100vh' }}>
                  ...
                  {loadingDiv ? (
                    ...
                    <div className="divide-y divide-slate-100">
                      {divsFiltradas.map((div) => {
                        ...
                        const expand  = divExpandida === div.id;
                        const prodLoc = getProdutoLocalDaDiv(div);

                        return (
                          <div key={div.id} className={`transition-colors ${sel ? 'bg-blue-50/30' : ''} ${expand ? 'bg-slate-50/60' : 'hover:bg-slate-50/80'}`}>
                            {/* Linha principal */}
                            <div className="flex items-start gap-0 px-3 py-2.5">
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-1.5 flex-wrap mb-0.5">
                                  <span className={`text-[8px] font-black px-1.5 py-0.5 rounded ${cfg.bg} ${cfg.color} flex items-center gap-0.5 border ${cfg.border}`}><cfg.icon className="w-2.5 h-2.5" />{cfg.label}</span>
                                ...
                                <div className="flex items-center gap-2 flex-wrap">
                                  ...
                                  {div.pesoMl > 0 && div.pesoLocal > 0 && (
                                    <div className="flex items-center gap-1">
                                      ...
                                      <VariacaoPesoChip pesoMl={div.pesoMl} pesoLocal={div.pesoLocal} />
                                    </div>
                                  )}
                                </div>
                                <p className="text-slate-500 text-[10px] mt-0.5 leading-snug">{div.motivo}</p>
                              </div>
                              {/* Ações */}
                              <div className="flex items-center gap-0.5 ml-2 shrink-0">
                                <div onMouseEnter={() => setQuickViewItem(div)} onMouseLeave={() => setQuickViewItem(null)} className="relative">
                                ...
                                {/* Botão de ajuste direto */}
                                {div.mlItemId && (
                                  <button
                                    onClick={() => setDivExpandida(expand ? null : div.id)}
                                    title="Ajustar ficha técnica"
                                    className={`p-1.5 rounded transition-colors ${expand ? 'bg-blue-600 text-white' : 'bg-blue-50 text-blue-600 hover:bg-blue-100'}`}>
                            ...
                            {/* Painel expansível de Ficha Técnica + Ajuste */}
                            {expand && (
                              <div className="px-3 pb-3">
                                <PainelFichaTecnica
                                  item={{ ...div, ...prodLoc, mlItemId: div.mlItemId, pesoGramas: prodLoc?.pesoGramas || div.pesoLocal, alturaCm: prodLoc?.alturaCm, larguraCm: prodLoc?.larguraCm, comprimentoCm: prodLoc?.comprimentoCm }}
                                  userId={userId}
                                  onAtualizado={() => { buscarDivergencias(); buscarDivStats(); }}
                                />
                              </div>
                            )}
                          </div>
              ...
              <div className="flex-1 overflow-y-auto p-3">
                <form onSubmit={handleSubmitProduto} className="space-y-3 text-xs font-semibold text-slate-600">
                  ...
                  <div className="bg-slate-50/50 border border-slate-100 rounded-xl p-2.5">
                    ...
                    <div className="grid grid-cols-3 gap-1.5">
                      {[['Altura', 'alturaCm'], ['Largura', 'larguraCm'], ['Compr.', 'comprimentoCm']].map(([l, k]) => (
                        <div key={k}>
                          <label className="text-[8px] font-black uppercase text-slate-400 block mb-0.5">{l}</label>
                          <input type="number" step="any" min="0" value={formProd[k]} onChange={e => setFormProd({ ...formProd, [k]: e.target.value })} className="w-full bg-white border border-slate-200 rounded-lg p-1.5 outline-none focus:border-blue-500 text-xs font-bold" placeholder="0" />
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <table className="w-full text-left">
                  <thead className="bg-slate-50 sticky top-0 z-10 border-b border-slate-100">
                    <tr className="text-[9px] font-black uppercase text-slate-400">
                      ...
                      <th className="px-3 py-2.5">Item</th><th className="px-3 py-2.5">Peso / Dim.</th><th className="px-3 py-2.5 text-right">Ação</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-50 text-xs">
                        ...
                        <tr key={prod.id} className={`hover:bg-slate-50/80 transition-colors ${sel ? 'bg-blue-50/30' : ''}`}>
                          ...
                          <td className="px-3 py-2.5">
                            <div className="flex flex-col gap-0.5">
                              <span className="text-[9px] text-blue-700 font-black bg-blue-50 border border-blue-100 px-1.5 py-0.5 rounded w-fit">{prod.pesoGramas}g</span>
                              {(prod.alturaCm > 0 || prod.larguraCm > 0 || prod.comprimentoCm > 0) && (
                                <span className="text-[8px] text-slate-400 font-bold">
                                  {[prod.alturaCm, prod.larguraCm, prod.comprimentoCm].filter(v => v > 0).join('×')}cm
                                </span>
                              )}
                            </div>
                          </td>
                          <td className="px-3 py-2.5 text-right">
                            <div className="flex justify-end gap-0.5 relative">
                              ...
                              {prod.mlItemId && (
                                <button onClick={() => abrirFullModal(prod)} title="Ver ficha técnica" className="p-1.5 rounded bg-slate-50 text-slate-500 hover:bg-indigo-50 hover:text-indigo-600">
                                  <Sliders className="w-3.5 h-3.5" />
                                </button>
                              )}
      ...
      {fullModalItem && (
        <ModalOverlay onClose={() => setFullModalItem(null)}>
          <div className="bg-white rounded-2xl shadow-2xl flex flex-col" style={{ width: '760px', maxWidth: '95vw', maxHeight: '90vh' }}>
              ...
              {loadingFullData ? (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
                  ...
                  <div className="lg:col-span-3 space-y-3">
                    ...
                    {/* Painel de ajuste inline no modal */}
                    {fullModalItem?.mlItemId && (
                      <PainelFichaTecnica
                        item={{
                          mlItemId: fullModalItem.mlItemId,
                          pesoGramas: fullModalItem.pesoGramas || fullModalItem.pesoLocal || 0,
                          alturaCm: fullModalItem.alturaCm || 0,
                          larguraCm: fullModalItem.larguraCm || 0,
                          comprimentoCm: fullModalItem.comprimentoCm || 0,
                        }}
                        userId={userId}
                        onAtualizado={() => { buscarDivergencias(); buscarProdutos(); }}
                      />
                    )}
                  </div>
                </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl" style={{ width: '420px', maxWidth: '95vw' }}>
            ...
            <div className="p-4 space-y-3 text-xs text-slate-600 leading-relaxed">
              {[
                ...
                { icon: <Sliders className="w-3.5 h-3.5 text-blue-500" />, title: 'Ajuste Direto', text: 'Clique no ícone de ajuste (⧑) em qualquer divergência para corrigir a ficha técnica no ML diretamente, no modo automático ou manual.' },
