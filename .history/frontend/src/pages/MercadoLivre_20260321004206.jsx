...
const FORM_INICIAL = {
  ...
  // Campos da ficha técnica completa (novos)
...
// Campos de ficha técnica auditáveis (ML vs catálogo local)
const CAMPOS_FICHA = [
...
// ══════════════════════════════════════════════════════════════════════════
// MODAL DE CONCILIAÇÃO ML vs ERP (NOVA FUNCIONALIDADE)
// Compara campo a campo e envia correção seletiva para API ML
// ══════════════════════════════════════════════════════════════════════════
function ModalConciliacao({ dados, userId, onClose, onSucesso }) {
  ...
  const [selecionados, setSelecionados] = useState(
    divergencias.reduce((acc, d) => ({ ...acc, [d.campo]: true }), {})
  );
  ...
  const toggleCheck = (campo) => {
    if (progresso.ativo || statusCorrecao[campo] === 'ok') return;
    setSelecionados(s => ({ ...s, [campo]: !s[campo] }));
  };
  ...
  const aplicarCorrecao = async () => {
    ...
    for (const d of camposParaEnviar) {
      ...
      try {
        ...
        const res = await fetch(`${API_BASE_URL}/api/anuncios/${mlItemId}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ userId, atributosFicha: { [d.campoML]: d.valorCorretoERP } }),
        });
        const data = await res.json();
        if (data.error || !data.success) throw new Error(data.error || 'Erro na API');
        novosStatus[d.campo] = 'ok';
      } catch (err) {
        novosStatus[d.campo] = `erro: ${err.message.substring(0, 60)}`;
      }

      setStatusCorrecao({ ...novosStatus });

      // Pausa anti-bloqueio entre campos
    ...
    const algumCorrigido = Object.values(novosStatus).some(s => s === 'ok');
    if (algumCorrigido && onSucesso) onSucesso();
  };

  const fechar = () => {
    if (progresso.ativo) return;
    onClose();
  };
    ...
    <ModalOverlay onClose={fechar}>
      <div className="bg-white rounded-2xl shadow-2xl flex flex-col" style={{ width: '720px', maxWidth: '96vw', maxHeight: '90vh' }}>
        ...
        <div className="px-5 py-3.5 border-b border-slate-100 flex justify-between items-center shrink-0 bg-gradient-to-r from-blue-600 to-indigo-600 rounded-t-2xl">
          ...
          <button onClick={fechar} disabled={progresso.ativo} className="p-1 hover:bg-white/20 rounded-full text-white/70 hover:text-white transition-colors disabled:opacity-30">
        ...
        <div className="flex-1 overflow-y-auto p-5">
          ...
          {(itemML.catalogo || itemML.catalog_listing) && (
            <div className="mb-4 flex items-start gap-2.5 bg-amber-50 border border-amber-200 rounded-xl p-3">
              ...
              <div>
                <p className="text-[10px] font-black text-amber-800 uppercase">Produto de Catálogo ML</p>
                <p className="text-[10px] text-amber-700 mt-0.5">Alguns campos podem precisar ser ajustados manualmente no painel do Mercado Livre.</p>
              </div>
            </div>
          )}
          ...
          <div className="flex items-start gap-3 mb-4 bg-slate-50 p-3 rounded-xl border border-slate-200">
            ...
            <div className="min-w-0">
              ...
              <button onClick={() => setExpandirDetalhes(v => !v)} className="flex items-center gap-1 text-[9px] text-slate-400 hover:text-blue-600 mt-1 transition-colors">
                {expandirDetalhes ? <ChevronDown className="w-3 h-3" /> : <ChevronRight className="w-3 h-3" />}
                {expandirDetalhes ? 'Ocultar detalhes' : 'Ver detalhes do produto'}
              </button>
              {expandirDetalhes && produtoLocal && (
                <div className="mt-2 grid grid-cols-3 gap-1">
                  {[
                    ['EAN', produtoLocal.ean || '—'],
                    ['Marca', produtoLocal.marca || '—'],
                    ['Modelo', produtoLocal.modelo || '—'],
                    ['Peso', produtoLocal.pesoGramas ? `${produtoLocal.pesoGramas}g` : '—'],
                    ['Altura', produtoLocal.alturaCm ? `${produtoLocal.alturaCm}cm` : '—'],
                    ['Largura', produtoLocal.larguraCm ? `${produtoLocal.larguraCm}cm` : '—'],
                  ].map(([k, v]) => (
                    <div key={k} className="bg-white border border-slate-100 rounded-lg px-2 py-1">
                      <p className="text-[8px] text-slate-400 font-black uppercase">{k}</p>
          ...
          {(progresso.ativo || progresso.mensagem === 'Finalizado!') && (
            <div className="mb-4 bg-slate-900 rounded-xl p-3.5">
              <div className="flex justify-between items-center mb-2">
                <span className={`text-[10px] font-black uppercase ${progresso.mensagem.includes('Pausa') ? 'text-amber-400' : progresso.mensagem === 'Finalizado!' ? 'text-emerald-400' : 'text-blue-400'}`}>
                  {progresso.mensagem}
                </span>
            ...
            <table className="w-full text-left">
              <thead className="bg-slate-50">
                <tr className="text-[9px] font-black uppercase text-slate-400">
                  ...
                  <th className="px-3 py-2.5">Atributo</th>
                  <th className="px-3 py-2.5 text-red-500">No ML</th>
                  <th className="px-3 py-2.5 text-emerald-600">No Catálogo</th>
                  <th className="px-3 py-2.5 text-right w-32">Status</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {divergencias.map((div, i) => {
                  ...
                  const isErro = st && st.startsWith('erro:');
                  ...
                  return (
                    <tr key={i} className={`transition-colors ${isOk ? 'bg-emerald-50/40' : isErro ? 'bg-red-50/30' : sel ? 'bg-blue-50/30' : 'hover:bg-slate-50'}`}>
                      <td className="px-3 py-3">
                        <button
                          onClick={() => toggleCheck(div.campo)}
                          disabled={progresso.ativo || isOk}
                      ...
                      <td className="px-3 py-3">
                        <div className="flex items-center gap-1.5">
                          <CampoIcon className="w-3.5 h-3.5 text-slate-400 flex-shrink-0" />
                          <span className="text-xs font-bold text-slate-700">{div.nomeBonito}</span>
                        </div>
                      </td>
                      <td className="px-3 py-3">
                        <span className={`text-xs font-semibold ${isOk ? 'text-slate-300 line-through' : 'text-red-600'}`}>
                          {div.valorML || <span className="italic text-slate-400">Faltando</span>}
                        </span>
                      </td>
                      <td className="px-3 py-3">
                      ...
                      <td className="px-3 py-3 text-right">
                        {isEnviando && (
                          <span className="flex items-center justify-end gap-1 text-[9px] text-blue-600 font-black uppercase">
                            <Loader2 className="w-3 h-3 animate-spin" />Enviando
                          </span>
                        )}
                        {isOk && (
                          <span className="flex items-center justify-end gap-1 text-[9px] text-emerald-600 font-black uppercase">
                            <Check className="w-3 h-3" />Corrigido
                          </span>
                        )}
                        {isErro && (
                          <span className="text-[8px] text-red-500 font-bold leading-tight text-right block max-w-[120px] ml-auto">
                            {st.replace('erro: ', '')}
                          </span>
                        )}
                        {!st && (
          ...
          {totalCorrigidos > 0 && (
            <div className="mt-3 flex items-center gap-2 bg-emerald-50 border border-emerald-200 rounded-xl px-3 py-2">
              ...
              <p className="text-xs font-black text-emerald-700">{totalCorrigidos} campo(s) corrigido(s) com sucesso na API do ML.</p>
            </div>
          )}
        </div>
        ...
        <div className="px-5 py-3.5 border-t border-slate-100 flex justify-between items-center shrink-0">
          <button onClick={fechar} disabled={progresso.ativo} className="px-4 py-2 rounded-xl text-xs font-bold text-slate-500 hover:bg-slate-100 transition-all disabled:opacity-40">
            {totalCorrigidos > 0 ? 'Fechar e Recarregar' : 'Cancelar'}
          </button>
          <button
            onClick={aplicarCorrecao}
            disabled={progresso.ativo || totalSelecionados === 0}
            className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-black uppercase text-white bg-blue-600 hover:bg-blue-700 disabled:bg-slate-200 disabled:text-slate-400 transition-all"
          >
            {progresso.ativo
              ? <><Loader2 className="w-3.5 h-3.5 animate-spin" />Processando...</>
              : <><Send className="w-3.5 h-3.5" />Corrigir {totalSelecionados > 0 ? `${totalSelecionados} campo(s)` : ''}</>}
          </button>
        </div>
      </div>
    </ModalOverlay>
...
// ══════════════════════════════════════════════════════════════════════════
// FUNÇÃO: Calcular divergências ficha técnica entre produto local e ML
// ══════════════════════════════════════════════════════════════════════════
function calcularDivergenciasFicha(produtoLocal, itemML) {
  ...
  const pesoMl = itemML.peso || 0;
  if (pesoLocal > 0 && pesoMl > 0 && Math.abs(pesoLocal - pesoMl) > 50) {
    divs.push({
      campo: 'peso', campoML: 'peso', nomeBonito: 'Peso',
      valorML: `${pesoMl}g`, valorERP: `${pesoLocal}g`, valorCorretoERP: pesoLocal,
    });
  }

  // EAN
  const eanLocal = (produtoLocal.ean || '').trim();
  const eanMl = (itemML.ean || '').trim();
  if (eanLocal && eanMl && eanLocal !== eanMl) {
    divs.push({
      campo: 'ean', campoML: 'ean', nomeBonito: 'EAN / GTIN',
      valorML: eanMl, valorERP: eanLocal, valorCorretoERP: eanLocal,
    });
  } else if (eanLocal && !eanMl) {
    divs.push({
      campo: 'ean', campoML: 'ean', nomeBonito: 'EAN / GTIN',
      valorML: null, valorERP: eanLocal, valorCorretoERP: eanLocal,
    });
  }

  // Marca
  const marcaLocal = (produtoLocal.marca || '').trim().toUpperCase();
  const marcaMl = (itemML.marca || '').trim().toUpperCase();
  if (marcaLocal && marcaMl && marcaLocal !== marcaMl) {
    divs.push({
      campo: 'marca', campoML: 'marca', nomeBonito: 'Marca',
      valorML: itemML.marca, valorERP: produtoLocal.marca, valorCorretoERP: produtoLocal.marca,
    });
  } else if (marcaLocal && !marcaMl) {
    divs.push({
      campo: 'marca', campoML: 'marca', nomeBonito: 'Marca',
      valorML: null, valorERP: produtoLocal.marca, valorCorretoERP: produtoLocal.marca,
    });
  }

  // Modelo
  const modeloLocal = (produtoLocal.modelo || '').trim().toUpperCase();
  const modeloMl = (itemML.modelo || '').trim().toUpperCase();
  if (modeloLocal && modeloMl && modeloLocal !== modeloMl) {
    divs.push({
      campo: 'modelo', campoML: 'modelo', nomeBonito: 'Modelo',
      valorML: itemML.modelo, valorERP: produtoLocal.modelo, valorCorretoERP: produtoLocal.modelo,
    });
  } else if (modeloLocal && !modeloMl) {
    divs.push({
      campo: 'modelo', campoML: 'modelo', nomeBonito: 'Modelo',
      valorML: null, valorERP: produtoLocal.modelo, valorCorretoERP: produtoLocal.modelo,
    });
  }

  // Altura
  const altLocal = parseFloat(produtoLocal.alturaCm) || 0;
  const altMlStr = itemML.dimensoes?.altura || '';
  const altMl = parseFloat(altMlStr) || 0;
  if (altLocal > 0 && altMl > 0 && Math.abs(altLocal - altMl) > 0.5) {
    divs.push({
      campo: 'alturaCm', campoML: 'altura', nomeBonito: 'Altura',
      valorML: `${altMl}cm`, valorERP: `${altLocal}cm`, valorCorretoERP: altLocal,
    });
  } else if (altLocal > 0 && !altMl) {
    divs.push({
      campo: 'alturaCm', campoML: 'altura', nomeBonito: 'Altura',
      valorML: null, valorERP: `${altLocal}cm`, valorCorretoERP: altLocal,
    });
  }

  // Largura
  const largLocal = parseFloat(produtoLocal.larguraCm) || 0;
  const largMlStr = itemML.dimensoes?.largura || '';
  const largMl = parseFloat(largMlStr) || 0;
  if (largLocal > 0 && largMl > 0 && Math.abs(largLocal - largMl) > 0.5) {
    divs.push({
      campo: 'larguraCm', campoML: 'largura', nomeBonito: 'Largura',
      valorML: `${largMl}cm`, valorERP: `${largLocal}cm`, valorCorretoERP: largLocal,
    });
  } else if (largLocal > 0 && !largMl) {
    divs.push({
      campo: 'larguraCm', campoML: 'largura', nomeBonito: 'Largura',
      valorML: null, valorERP: `${largLocal}cm`, valorCorretoERP: largLocal,
    });
  }

  // Comprimento
  const compLocal = parseFloat(produtoLocal.comprimentoCm) || 0;
  const compMlStr = itemML.dimensoes?.comprimento || '';
  const compMl = parseFloat(compMlStr) || 0;
  if (compLocal > 0 && compMl > 0 && Math.abs(compLocal - compMl) > 0.5) {
    divs.push({
      campo: 'comprimentoCm', campoML: 'comprimento', nomeBonito: 'Comprimento',
      valorML: `${compMl}cm`, valorERP: `${compLocal}cm`, valorCorretoERP: compLocal,
    });
  } else if (compLocal > 0 && !compMl) {
    divs.push({
      campo: 'comprimentoCm', campoML: 'comprimento', nomeBonito: 'Comprimento',
      valorML: null, valorERP: `${compLocal}cm`, valorCorretoERP: compLocal,
    });
  }
...
export default function MercadoLivre() {
  ...
  const [showFichaTecnica, setShowFichaTecnica] = useState(false); // Toggle seção ficha técnica no form
  ...
  // Estado para Modal de Conciliação ML vs ERP (NOVO)
  const [modalConciliacao, setModalConciliacao] = useState(null);
  // Cache de dados ML para auditoria de ficha técnica (NOVO)
  ...
  const handleSubmitProduto = async (e) => {
    ...
    try {
      ...
      await fetch(url, { method: editandoId ? 'PUT' : 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...formProd,
          pesoGramas: pesoConvertido,
          userId,
          plataforma: formProd.mlItemId ? 'Mercado Livre' : 'BASE',
          // Campos ficha técnica
          ean: formProd.ean || null,
          marca: formProd.marca || null,
          modelo: formProd.modelo || null,
          condicao: formProd.condicao || 'Novo',
        })
      });
  ...
  // ── AUDITORIA DE FICHA TÉCNICA (NOVA FUNÇÃO) ────────────────────────────────
  // Busca dados reais do ML para cada produto vinculado e compara com catálogo local
  const auditarFichaTecnica = async () => {
    if (!mlConectado) {
      alert({ title: 'ML Desconectado', message: 'Conecte o Mercado Livre nas Configurações para auditar a ficha técnica.' });
      return;
    }

    const produtosVinculadosComFicha = produtos.filter(p =>
      p.mlItemId &&
      p.plataforma !== 'ML_PENDENTE' &&
      p.plataforma !== 'ML_IGNORADO' &&
      (p.ean || p.marca || p.modelo || p.alturaCm || p.larguraCm || p.comprimentoCm)
    );

    if (produtosVinculadosComFicha.length === 0) {
      alert({
        title: 'Nenhum produto com ficha técnica',
        message: 'Cadastre EAN, marca, modelo ou dimensões nos produtos vinculados para auditar a ficha técnica.',
      });
      return;
    }

    setAuditandoFicha(true);
    setResultadoAuditoriaFicha(null);
    const resultados = [];
      ...
      try {
        ...
        if (!mlData) {
          ...
          if (Array.isArray(anuncios)) {
            const cache = {};
            anuncios.forEach(a => { cache[a.id] = a; });
            setDadosMLCache(prev => ({ ...prev, ...cache }));
            mlData = cache[prod.mlItemId];
          }
        }
        ...
        if (divs.length > 0) {
          resultados.push({ prod, mlData, divs });
        }
      } catch {}
    }

    setResultadoAuditoriaFicha(resultados);
    setAuditandoFicha(false);

    if (resultados.length === 0) {
      alert({ title: 'Ficha técnica OK!', message: 'Nenhuma divergência encontrada nos campos de ficha técnica dos produtos vinculados.' });
    }
  };

  // Abre modal de conciliação para um produto específico
  const abrirConciliacao = (prod, mlData, divs) => {
    setModalConciliacao({
      itemML: { ...mlData, mlItemId: mlData.id, titulo: mlData.titulo || mlData.title },
      divergencias: divs,
      produtoLocal: prod,
    });
  };
  ...
  const statsItems = [
    ...
    { label: 'Ficha',     value: produtosComFicha, sub: 'com EAN/marca', color: 'text-indigo-600', dot: 'bg-indigo-400' },
  ];
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-3 py-2 flex flex-col animate-in fade-in duration-300" style={{ minHeight: '100vh' }}>
                  ...
                  <div className="bg-slate-50 border border-slate-200 rounded-xl p-2.5">
                    ...
                    <div className="grid grid-cols-3 gap-1.5">
                      {[['alturaCm','Alt'], ['larguraCm','Larg'], ['comprimentoCm','Comp']].map(([field, label]) => (
                        <div key={field}>
                          <label className="text-[8px] text-slate-400 font-black uppercase block mb-0.5">{label}</label>
                          <input type="number" step="any" min="0" value={formProd[field]} onChange={e => setFormProd({ ...formProd, [field]: e.target.value })} className="w-full bg-white border border-slate-200 rounded-lg p-1.5 outline-none focus:border-blue-500 text-xs font-semibold" placeholder="0" />
                        </div>
                      ))}
                    </div>
                  </div>

                  {/* FICHA TÉCNICA COMPLETA (NOVA SEÇÃO) */}
                  <div className="border border-indigo-100 rounded-xl overflow-hidden">
                    <button
                      type="button"
                      onClick={() => setShowFichaTecnica(v => !v)}
                      className="w-full flex items-center justify-between px-3 py-2 bg-indigo-50 text-indigo-700 hover:bg-indigo-100 transition-colors"
                    >
                      <span className="flex items-center gap-1.5 text-[9px] font-black uppercase tracking-widest">
                        <Barcode className="w-3 h-3" />Ficha Técnica Completa
                        {(formProd.ean || formProd.marca || formProd.modelo) && (
                          <span className="bg-indigo-600 text-white text-[7px] px-1.5 py-0.5 rounded-full font-black">Preenchida</span>
                        )}
                      </span>
                    ...
                    {showFichaTecnica && (
                      <div className="p-2.5 space-y-2 bg-white">
                        <p className="text-[8px] text-slate-400 font-semibold leading-tight">Estes campos são usados para auditar a ficha técnica no ML e corrigir automaticamente via API.</p>
                        <div className="grid grid-cols-2 gap-2">
                          <div>
                            <label className="uppercase tracking-widest block mb-1 text-[8px] text-slate-500 font-black">EAN / GTIN</label>
                            <input value={formProd.ean} onChange={e => setFormProd({ ...formProd, ean: e.target.value })} placeholder="Ex: 7891234567890" className="w-full bg-white border border-slate-200 rounded-lg p-2 outline-none focus:border-indigo-400 text-xs font-mono" />
                          </div>
                          <div>
                            <label className="uppercase tracking-widest block mb-1 text-[8px] text-slate-500 font-black">Condição</label>
                            <select value={formProd.condicao} onChange={e => setFormProd({ ...formProd, condicao: e.target.value })} className="w-full bg-white border border-slate-200 rounded-lg p-2 outline-none focus:border-indigo-400 text-xs">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          <div>
                            <label className="uppercase tracking-widest block mb-1 text-[8px] text-slate-500 font-black">Marca</label>
                            <input value={formProd.marca} onChange={e => setFormProd({ ...formProd, marca: e.target.value })} placeholder="Ex: Samsung" className="w-full bg-white border border-slate-200 rounded-lg p-2 outline-none focus:border-indigo-400 text-xs" />
                          </div>
                          <div>
                            <label className="uppercase tracking-widest block mb-1 text-[8px] text-slate-500 font-black">Modelo</label>
                            <input value={formProd.modelo} onChange={e => setFormProd({ ...formProd, modelo: e.target.value })} placeholder="Ex: Galaxy S24" className="w-full bg-white border border-slate-200 rounded-lg p-2 outline-none focus:border-indigo-400 text-xs" />
                          </div>
                        </div>
                        <div className="flex items-center gap-2 bg-amber-50 border border-amber-100 rounded-lg px-2.5 py-1.5">
                          <AlertTriangle className="w-3 h-3 text-amber-500 flex-shrink-0" />
                          <p className="text-[8px] text-amber-700">A auditoria de ficha técnica compara estes campos com os dados do ML e permite correção automática na aba "Ficha Técnica".</p>
                        </div>
                      </div>
                    )}
                  </div>
                ...
                <table className="w-full text-left">
                  <thead className="bg-slate-50 sticky top-0 z-10 border-b border-slate-100">
                    <tr className="text-[9px] font-black uppercase text-slate-400">
                      ...
                      <th className="px-3 py-2.5">Item</th><th className="px-3 py-2.5">Peso</th><th className="px-3 py-2.5">Ficha</th><th className="px-3 py-2.5 text-right">Ação</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-50 text-xs">
                    ...
                    {produtosFiltrados.map(prod => {
                      ...
                      const temFicha = prod.ean || prod.marca || prod.modelo;
                      return (
                          ...
                          <td className="px-3 py-2.5">
                            {temFicha ? (
                              <div className="flex flex-col gap-0.5">
                                {prod.ean && <span className="text-[7px] text-indigo-600 font-mono bg-indigo-50 px-1 py-0.5 rounded truncate max-w-[80px]">{prod.ean}</span>}
                                {prod.marca && <span className="text-[8px] text-slate-600 font-bold truncate max-w-[80px]">{prod.marca}</span>}
                              </div>
                            ) : (
          ...
          <div className="space-y-3">
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm">
              <div className="flex items-center justify-between flex-wrap gap-3">
                <div>
                  <h3 className="text-sm font-black text-slate-800 flex items-center gap-2"><GitCompare className="w-4 h-4 text-blue-600" />Auditoria de Ficha Técnica</h3>
                  <p className="text-[10px] text-slate-500 mt-0.5">Compara EAN, marca, modelo e dimensões do catálogo local com os dados reais do ML. Corrige automaticamente via API.</p>
                </div>
                <div className="flex items-center gap-2">
                  <div className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-500 font-semibold">
                    <ShieldCheck className="w-3.5 h-3.5 text-indigo-500" />
                    {produtosComFicha} produto(s) com ficha preenchida
                  </div>
                  <button
                    onClick={auditarFichaTecnica}
                    disabled={auditandoFicha || !mlConectado}
                    className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-black uppercase transition-all ${auditandoFicha || !mlConectado ? 'bg-slate-100 text-slate-400' : 'bg-indigo-600 text-white hover:bg-indigo-700'}`}
                  >
                    {auditandoFicha
                      ? <><Loader2 className="w-3.5 h-3.5 animate-spin" />Auditando...</>
                      : !mlConectado
                      ? <><WifiOff className="w-3.5 h-3.5" />ML Desconectado</>
                      : <><Zap className="w-3.5 h-3.5" />Auditar Ficha Técnica</>}
                  </button>
                </div>
              </div>

              {/* Info de campos auditáveis */}
              <div className="mt-3 grid grid-cols-2 md:grid-cols-4 gap-2">
                {[
                  { label: 'EAN/GTIN', icon: Barcode, desc: 'Código de barras do produto' },
                  { label: 'Marca', icon: Tag, desc: 'Fabricante/marca do produto' },
                  { label: 'Modelo', icon: Package, desc: 'Modelo/referência exata' },
                  { label: 'Dimensões', icon: Ruler, desc: 'Altura, largura, comprimento' },
                ].map(({ label, icon: Icon, desc }) => (
                  <div key={label} className="flex items-center gap-2 bg-slate-50 border border-slate-100 rounded-lg p-2">
                    <Icon className="w-3.5 h-3.5 text-indigo-500 flex-shrink-0" />
                    <div>
                      <p className="text-[9px] font-black text-slate-700 uppercase">{label}</p>
            ...
            {resultadoAuditoriaFicha === null && !auditandoFicha && (
              <div className="bg-white border border-slate-200 rounded-2xl p-10 shadow-sm flex flex-col items-center gap-3 text-center">
                ...
                <div className="mt-2 bg-amber-50 border border-amber-200 rounded-xl px-4 py-2.5 text-left max-w-sm">
                  <p className="text-[9px] font-black text-amber-800 uppercase mb-1">Como funciona</p>
                  <p className="text-[10px] text-amber-700 leading-relaxed">Cadastre EAN, marca, modelo e dimensões nos produtos do catálogo. A auditoria buscará esses dados no ML e exibirá as diferenças campo a campo para correção automática ou manual.</p>
                </div>
              </div>
            )}

            {auditandoFicha && (
              <div className="bg-white border border-slate-200 rounded-2xl p-10 shadow-sm flex flex-col items-center gap-3">
                <Loader2 className="w-8 h-8 animate-spin text-indigo-400" />
                <p className="text-sm font-black text-slate-600">Buscando dados do ML e comparando com o catálogo...</p>
                ...
                <div className="flex items-center justify-between">
                  <p className="text-xs font-black text-slate-600">
                    {resultadoAuditoriaFicha.length === 0
                      ? '✅ Todos os campos estão conciliados!'
                      : `⚠️ ${resultadoAuditoriaFicha.length} produto(s) com divergências na ficha técnica`}
                  </p>
                  <button onClick={() => setResultadoAuditoriaFicha(null)} className="text-[9px] text-slate-400 hover:text-slate-600 font-black uppercase px-2 py-1 rounded bg-slate-50 border border-slate-200">
                    Limpar
                  </button>
                </div>

                {resultadoAuditoriaFicha.length === 0 ? (
                  <div className="bg-white border border-emerald-200 rounded-2xl p-8 shadow-sm flex flex-col items-center gap-2 text-center">
                    <CheckCircle2 className="w-10 h-10 text-emerald-400" />
                    <p className="text-sm font-black text-emerald-700">Ficha técnica 100% conciliada!</p>
                    <p className="text-xs text-slate-400">Todos os campos auditáveis estão iguais no ML e no catálogo local.</p>
                  </div>
                ) : (
                    ...
                    {resultadoAuditoriaFicha.map(({ prod, mlData, divs }) => (
                      <div key={prod.id} className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
                        ...
                        <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between flex-wrap gap-2">
                          <div className="flex items-center gap-2.5">
                            {(prod.thumbnail || mlData.thumbnail) ? (
                              <img src={prod.thumbnail || mlData.thumbnail} alt="" className="w-10 h-10 rounded-lg object-cover border border-slate-100 flex-shrink-0" />
                            ) : (
                            ...
                            <div>
                              <p className="text-xs font-bold text-slate-800 truncate max-w-xs">{prod.nome}</p>
                              <div className="flex items-center gap-1.5 mt-0.5">
                                <span className="text-[8px] font-bold text-slate-500 bg-slate-100 px-1.5 py-0.5 rounded">SKU: {prod.sku}</span>
                                {prod.mlItemId && (
                                  <a href={`https://produto.mercadolivre.com.br/MLB-${prod.mlItemId.replace(/^MLB/i, '')}`} target="_blank" rel="noopener noreferrer" className="text-[8px] font-mono text-blue-500 hover:underline flex items-center gap-0.5">
                          ...
                          <div className="flex items-center gap-2">
                            <span className="flex items-center gap-1 text-[9px] font-black text-amber-700 bg-amber-50 border border-amber-200 px-2 py-1 rounded-lg">
                              <AlertTriangle className="w-3 h-3" />{divs.length} campo(s) divergente(s)
                            </span>
                            <button
                              onClick={() => abrirConciliacao(prod, mlData, divs)}
                              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-blue-600 text-white text-[10px] font-black uppercase hover:bg-blue-700 transition-all"
                            >
                              <GitCompare className="w-3 h-3" />Ver e Corrigir
                            </button>
                          </div>
                        </div>
                            ...
                            {divs.map((div, i) => {
                              ...
                              return (
                                <div key={i} className="flex items-center gap-1.5 bg-red-50 border border-red-100 rounded-lg px-2 py-1">
                                  ...
                                  <div>
                                    <p className="text-[8px] font-black text-slate-500 uppercase">{div.nomeBonito}</p>
                                    <div className="flex items-center gap-1">
                                      <span className="text-[9px] text-red-600 line-through">{div.valorML || 'Faltando'}</span>
                                      <span className="text-[8px] text-slate-400">→</span>
                                      <span className="text-[9px] text-emerald-700 font-bold">{div.valorERP}</span>
                                    </div>
                                  </div>
                                </div>
                              );
      ...
      {/* Modal de Conciliação ML vs ERP (NOVO) */}
      {modalConciliacao && (
        <ModalConciliacao
          dados={modalConciliacao}
          userId={userId}
          onClose={() => setModalConciliacao(null)}
          onSucesso={() => { buscarProdutos(); setResultadoAuditoriaFicha(null); }}
        />
      )}
          ...
          <div className="bg-white rounded-2xl shadow-2xl" style={{ width: '420px', maxWidth: '95vw' }}>
            ...
            <div className="p-4 space-y-3 text-xs text-slate-600 leading-relaxed">
              {[
                ...
                { icon: <GitCompare className="w-3.5 h-3.5 text-indigo-500" />, title: 'Ficha Técnica', text: 'Cadastre EAN, marca, modelo e dimensões no catálogo para comparar com ML e corrigir automaticamente via API.' },
