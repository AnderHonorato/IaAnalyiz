...
export default function App() {
  ...
  const RobotGif = ({ size = "md" }) => {
    // URL de exemplo de um robô flat gif. Você pode substituir pela URL que desejar.
                  ...
                  <table className="w-full text-left border-collapse">
                    <tbody className="divide-y divide-slate-100 text-xs text-slate-700 italic">
                      {divergencias.length === 0 ? (<tr><td className="py-20 text-center text-slate-300 font-black uppercase tracking-[0.3em]">Scan_Clean</td></tr>) : divergencias.map((div) => (
                        <tr key={div.id} className="hover:bg-blue-50/50 transition-all"><td className="py-4 px-6 font-bold text-blue-600">{div.mlItemId}</td><td className="py-4 px-6">"{div.motivo}"</td></tr>
                      ))}
                    </tbody>
                  </table>
                </div>
      ...
      <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end max-w-[calc(100vw-48px)]">
        ...
        <div className={`chat-anim w-[420px] max-w-full h-[600px] max-h-[85vh] bg-white rounded-3xl shadow-[0_20px_60px_rgba(0,0,0,0.2)] border border-slate-100 flex flex-col mb-4 overflow-hidden origin-bottom-right ${isChatOpen ? 'scale-100 opacity-100' : 'scale-0 opacity-0 pointer-events-none'}`}>
          <div className="bg-[#1e293b] p-6 flex justify-between items-center text-white shrink-0">
            <div className="flex items-center gap-4">
              <RobotGif size="sm" />
              <div><p className="text-sm font-black uppercase tracking-widest leading-none">Aura IA</p><p className="text-[10px] text-emerald-400 font-bold uppercase tracking-widest mt-1">Sistemas Online</p></div>
            </div>
          ...
          <div className="flex-1 bg-slate-50 p-6 overflow-y-auto space-y-5 custom-scrollbar text-xs text-slate-700 leading-relaxed italic">
            <div className="bg-white p-4 rounded-2xl rounded-tl-none border border-slate-200 shadow-sm">
              Olá! Eu sou a **Aura**, sua assistente de integridade logística. Analisei sua conta e detectei inconsistências em anúncios de kits que podem causar prejuízos no frete. Deseja que eu gere o link para correção?
            </div>
          </div>
        ...
        {/* Botão Flutuante (Robô Minimalista com GIF e Tooltip) */}
        <div className="relative group">
            {/* TOOLTIP COM NOME (ESTILO ML) */}
            ...
            {/* Botão de Fechar (Aparece quando o chat está aberto) */}
            <button 
                onClick={() => setIsChatOpen(!isChatOpen)} 
