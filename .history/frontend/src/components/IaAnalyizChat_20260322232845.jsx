// IaAnalyizChat.jsx — v3: raciocínio real em streaming, painel expansível sem fundo azul
...
// ─── Ícone estrela IA (sem dependência externa) ───────────────────────────────
function StarSparkleIcon({ size = 13, color = '#6366f1', animated = false }) {
  return (
    ...
    >
      ...
      />
      <path
        d="M8 2.5L9.2 6.8H13.5L10 9.2L11.2 13.5L8 11.2L4.8 13.5L6 9.2L2.5 6.8H6.8L8 2.5Z"
        fill={color}
      />
    </svg>
  );
...
// ─── ReasoningPanel — painel de raciocínio expansível ────────────────────────
// Design: sem fundo colorido. Linha discreta. Ícone estrela. Texto técnico menor.
function ReasoningPanel({ reasoning, isStreaming }) {
  const [expanded, setExpanded] = useState(false);
  const textRef = useRef(null);

  // Auto-scroll dentro do painel enquanto streaming
  useEffect(() => {
  ...
  // Abre automaticamente quando começa a stream de raciocínio
  useEffect(() => {
  ...
  return (
    <div style={{ marginBottom: '6px' }}>
      {/* Trigger — linha discreta com ícone estrela */}
      <button
      ...
      >
        {/* Ícone estrela — anima enquanto streaming */}
        <StarSparkleIcon
          size={12}
          color={isStreaming ? '#6366f1' : '#94a3b8'}
          animated={isStreaming}
        />

        <span style={{
          fontWeight: 500,
          color: isStreaming ? '#6366f1' : 'inherit',
        ...
        {isStreaming && (
          <span style={{
            ...
            background: '#6366f1',
            marginLeft: '2px',
            animation: 'ia-dot-pulse 1s ease-in-out infinite',
          }} />
        )}
      </button>
...
// ─── ThinkingPanel ao vivo (durante execução de tools) ───────────────────────
// Aparece APÓS o raciocínio, durante a execução das tools
...
function IaMessage({ msg, isTyping, onDone }) {
  ...
  return (
    <div className="ia-msg" style={{ fontSize:'13px', color:'#333', lineHeight:'1.6' }}>
      {/* Painel de raciocínio — sempre acima da resposta, estático após conclusão */}
...
// ═══════════════════════════════════════════════════════════════════════════════
// ESTADO VIVO DO RACIOCÍNIO (durante streaming)
// Renderizado separadamente da lista de mensagens para permitir atualizações
// sem re-renderizar toda a lista
// ═══════════════════════════════════════════════════════════════════════════════
...
function LiveReasoningArea({ reasoning, isReasoningStreaming, toolSteps, isToolsActive }) {
  return (
    <div style={{ marginBottom: '8px' }}>
      ...
      {/* Tools em execução — aparecem após o raciocínio */}
      {(toolSteps.length > 0 || isToolsActive) && (
        <LiveToolPanel steps={toolSteps} isActive={isToolsActive} />
      )}
    </div>
  );
}
...
export default function IaAnalyizChat({ isChatOpen, toggleChat, userRole }) {
  ...
  // Estado do raciocínio ao vivo (streaming)
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      setMessages(prev => [...prev, {
        ...
        sources:     [],
        isProactive: true,
        reasoning:   undefined, // sem painel de raciocínio em notificações proativas
      }]);
  ...
  const mapMessage = m => ({
    ...
    reasoning: undefined, // mensagens históricas não têm raciocínio
  });

  useEffect(() => {
    if (!currentSessionId) {
      setMessages([{ role:'ia', content:'Olá! 😊 Seja bem-vindo à IA Analyiz! Estou aqui para te ajudar com seus anúncios, divergências de frete, pesquisas e muito mais. Como posso te ajudar hoje?', time:getTime(), id:'init', sources:[], reasoning: undefined }]);
      return;
    }
    fetch(`${API_BASE_URL}/api/chat/sessions/${currentSessionId}/messages`)
      .then(r => r.json())
      .then(msgs => {
        setMessages(msgs.length > 0 ? msgs.map(mapMessage) : [{ role:'ia', content:'Olá! 😊 Seja bem-vindo de volta! Como posso te ajudar hoje?', time:getTime(), id:'init', sources:[], reasoning: undefined }]);
      })
      .catch(() => setMessages([{ role:'ia', content:'Olá! 😊 Como posso te ajudar hoje?', time:getTime(), id:'init', sources:[], reasoning: undefined }]));
  }, []); // eslint-disable-line
  ...
  const loadSession = useCallback((sessionId) => {
    ...
    if (!sessionId) {
      setMessages([{ role:'ia', content:'Olá! 😊 Nova conversa! Como posso ajudar?', time:getTime(), id:`init-${Date.now()}`, sources:[], reasoning: undefined }]);
      return;
    }
    fetch(`${API_BASE_URL}/api/chat/sessions/${sessionId}/messages`)
      .then(r => r.json())
      .then(msgs => setMessages(msgs.length > 0 ? msgs.map(mapMessage) : [{ role:'ia', content:'Olá! 😊 Como posso te ajudar?', time:getTime(), id:`init-${Date.now()}`, sources:[], reasoning: undefined }]))
      .catch(() => {});
  }, []);
  ...
  const verificarProativo = useCallback(async () => {
    ...
    } catch (e) { console.warn('[Proactive] Erro:', e.message); }
  }, [userId, userRole]);
  ...
  // ── Envio — fluxo SSE com raciocínio real ─────────────────────────────────
  const handleSend = useCallback(async () => {
    ...
    try {
            ...
            try {
              ...
              if (currentEvent === 'reasoning_start') {
                ...
                setMessages(prev => [...prev, {
                  ...
                  // Raciocínio capturado durante o streaming
      ...
      setMessages(prev => [...prev, {
        role:'ia', content:'⚠️ Erro de conexão. Tente novamente!',
        time:getTime(), id:`err-${Date.now()}`, sources:[], reasoning: undefined,
      }]);
    } finally {
  ...
  return (
    <>
      <style>{`
        ...
        @keyframes ia-dot-pulse   { 0%,100%{opacity:1;transform:scale(1)} 50%{opacity:0.4;transform:scale(0.6)} }
        @keyframes ia-star-pulse  { 0%,100%{opacity:1} 50%{opacity:0.5} }
      ...
      <div style={{ position:'fixed', zIndex:9999, bottom:'1.5rem', right:'1.5rem', width:W, height:H, maxHeight:'calc(100vh - 3rem)', background:'#ffffff', borderRadius:'20px', boxShadow:'0 4px 32px rgba(0,0,0,0.15)', display:'flex', flexDirection:'column', overflow:'hidden', transformOrigin:'bottom right', transform:isChatOpen?'scale(1)':'scale(0)', opacity:isChatOpen?1:0, pointerEvents:isChatOpen?'auto':'none', transition:'transform 0.25s cubic-bezier(0.34,1.56,0.64,1), opacity 0.2s' }}>
        ...
        <div ref={scrollRef} className="ia-scroll" style={{ flex:1, overflowY:'auto', padding:'12px 14px', background:'#ffffff' }}>
          ...
          {/* Área de raciocínio ao vivo + tools — aparece durante o processamento */}
          {isChatLoading && (
            <div style={{ marginBottom: '8px' }}>
              <LiveReasoningArea
