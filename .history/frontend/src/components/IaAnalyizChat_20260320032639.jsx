...
const handleSend = useCallback(async () => {
    ...
    // MANDA A PÁGINA ATUAL NA MENSAGEM OCULTA PARA A IA SABER O CONTEXTO
    const paginaAtual = window.location.pathname;
    const contextualMessage = `[O usuário está atualmente na página do sistema com a URL: ${paginaAtual}]\n\n` + userMessage;

    try {
      const sid = await ensureSession();
      const res = await fetch(`${API_BASE_URL}/api/ia/chat`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ message: contextualMessage, sessionId: sid, userRole, userId, ...(imgToSend ? { imageBase64: imgToSend.base64, imageMimeType: imgToSend.mimeType } : {}) }),
      });
      const data = await res.json();
