// IaAnalyizChat.jsx — corrigido: sem corte, diffusion typing, IA carismática, web search ativo
...
let   _pendingProactivePayload = null;
...
const SPINNER_CONTEXTS = {
  ...
  diverge: ['⚖️ Verificando pesos...','📦 Cruzando fretes...','🚚 Analisando divergências...'],
  produto: ['🏷️ Buscando produtos...','📋 Consultando catálogo...'],
  usuario: ['👤 Consultando usuários...','🔐 Verificando acessos...'],
  imagem:  ['🖼️ Analisando imagem...','🔬 Identificando elementos...'],
...
// ─── Diffusion typing: revela o HTML token por token SEM cortar ───────────────
// A correção-chave: onDone é chamado via ref, não via closure, evitando setState-during-render
function DiffusionText({ html, speed = 6, onDoneRef }) {
  ...
  useEffect(() => {
    // Tokeniza o HTML em unidades seguras (tags completas + chars)
    const tokens = [];
    let i = 0;
    const src = html || '';
    while (i < src.length) {
    ...
    if (!tokens.length) {
      // Chama onDone fora do render cycle
    ...
    timerRef.current = setInterval(() => {
      setVisibleCount(prev => {
        ...
        if (next >= tokens.length) {
          ...
          // Chama onDone fora do setState cycle
          setTimeout(() => onDoneRef?.current?.(), 0);
        }
        return next;
      });
    ...
    <div style={{ position:'relative', display:'inline-flex', flexDirection:'column', alignItems:'flex-end', gap:'6px' }}>
      ...
      {hasUnread && !shortPreview && (
        <div style={{ background:BRAND, color:'white', fontSize:'11px', fontWeight:500, padding:'5px 10px', borderRadius:'10px 10px 10px 2px', boxShadow:'0 2px 8px rgba(30,41,59,0.25)', animation:'ia-fade-in 0.3s ease', pointerEvents:'none' }}>
          Tenho uma nova resposta para você
        </div>
      )}
...
export default function IaAnalyizChat({ isChatOpen, toggleChat, userRole }) {
    ...
    if (isChatOpen) {
      ...
      if (_pendingProactivePayload) {
        ...
        setTimeout(async () => {
          ...
          try {
            const res  = await fetch(`${API_BASE_URL}/api/ia/chat`, { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ message:full, sessionId:sid, userRole, userId }) });
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      if (!pageKey || isChatOpenRef.current) return;
      const now = Date.now();
      ...
      try {
        const resShort = await fetch(`${API_BASE_URL}/api/ia/proactive`, { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ userId, userRole, pageKey }) });
    ...
    return () => { clearTimeout(initial); window.removeEventListener('popstate', onNav); history.pushState = origPush; };
