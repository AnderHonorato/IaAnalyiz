 ...
 * 
 * Propósito:
 * Modal de gerenciamento de perfil do utilizador.
 * Permite editar nome, avatar e exclusão de conta.
 * 
 * Responsabilidades:
 * - Exibir dados do perfil do utilizador
 * - Permitir edição de nome e avatar
 * - Gerenciar fluxo de exclusão de conta (com confirmações)
 * - Salvar alterações no backend
 * - Validar entrada de dados
 * - Upload de imagem (avatar)
 * 
 * Fluxo de Exclusão de Conta (4 etapas):
 * 1. idle: Botão "Excluir Conta" visível
 * 2. aviso: Tela de aviso com confirmação
 * 3. codigo: Aguardando código de confirmação (enviado por email/logs)
 * 4. agendado: Exclusão confirmada, aguardando prazo de 30 dias
 * 
 * Props:
 *   - user: Objeto com dados do utilizador { nome, email, avatar }
 *   - onClose: Callback ao fechar modal
 *   - onSave: Callback ao salvar alterações
 * 
 * Estado:
 *   - editName: Nome em edição
 *   - editAvatar: Avatar em edição (base64)
 *   - saving: Flag de salvamento
 *   - deleteStep: Etapa do fluxo de exclusão
 *   - codigoInput: Código de confirmação inserido
 *   - exclusaoEm: Data agendada para exclusão
 * 
 * Endpoints API:
 *   - PUT /api/auth/profile - Salvar profil
 *   - POST /api/auth/request-delete - Solicitar exclusão
 *   - POST /api/auth/confirm-delete - Confirmar exclusão
 * 
 * Segurança:
 *   - Requer código de confirmação para exclusão
 *   - Período de carência de 30 dias
 *   - Validação de email para confirmação
 * 
