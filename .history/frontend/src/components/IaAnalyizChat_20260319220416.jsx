...
function getSpinnerLabel(message) {
  ...
  if (/usuário|usuario|acesso|desbloqueio|bloqueado|pendente/i.test(l)) return 'Consultando usuários...';
  if (/produto|sku|peso|kit|estoque|ml|preço/i.test(l))                 return 'Buscando produtos...';
  if (/divergên|divergen|anomalia|erro|auditoria/i.test(l))             return 'Verificando divergências...';
  if (/histórico|conversamos|o que falamos/i.test(l))                   return 'Consultando histórico...';
  if (/imagem|foto|arquivo/i.test(l))                                   return 'Analisando imagem...';
  return 'Analisando...';
}
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '2px 0' }}>
      <span style={{
        ...
        border: '2px solid #e2e8f0', borderTop: `2px solid ${BRAND}`,
        animation: 'ia-spin 0.8s linear infinite', flexShrink: 0,
      }} />
      <span style={{ fontSize: '13px', color: '#64748b' }}>{getSpinnerLabel(message)}</span>
    </div>
  );
}

// ─── Typewriter ───────────────────────────────────────────────────────────────
// FIX: não usa flag /s (dotAll) — usa split manual compatível com todos os ambientes
...
// ─── Utilitário: detecta página atual pela URL ───────────────────────────────
...
// ─── Hook: análise proativa de página ────────────────────────────────────────
// Observa a rota atual, chama o backend quando muda, dispara insight se houver
function usePageAnalysis({ userId, userRole, onInsight, enabled }) {
  const lastPageRef    = useRef(null);
  const lastInsightRef = useRef(null); // evita disparar o mesmo insight duas vezes
  ...
  useEffect(() => {
    ...
    const check = async () => {
      ...
      if (!pageKey) return;
      // Não re-analisa a mesma página em menos de 2 minutos
      const cacheKey = `${pageKey}`;
      if (lastPageRef.current === cacheKey) return;
      lastPageRef.current = cacheKey;
      ...
      try {
        ...
        if (data.insight && data.insight !== lastInsightRef.current) {
          ...
          onInsight(data.insight, pageKey);
        }
      } catch (e) {
    ...
    // Observa mudanças de rota (SPA com history API)
    const onPop = () => { lastPageRef.current = null; check(); };
    window.addEventListener('popstate', onPop);

    // Monkey-patch pushState para capturar navegação programática
...
function FloatingButton({ onClick, hasUnread }) {
  ...
  return (
    // Wrapper externo para posicionar o balão de notificação ACIMA do botão
    <div style={{ position: 'relative', display: 'inline-flex', flexDirection: 'column', alignItems: 'flex-end', gap: '6px' }}>

      {/* Balão de notificação — aparece acima do botão quando há mensagem não lida */}
      {hasUnread && (
        <div style={{
          background:    BRAND,
          color:         'white',
      ...
      <button onClick={onClick} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
        style={{ display: 'flex', alignItems: 'center', overflow: 'hidden', borderRadius: '999px', border: 'none', background: BRAND, cursor: 'pointer', boxShadow: '0 4px 16px rgba(30,41,59,0.4)', width: hovered ? '152px' : '52px', height: '52px', transition: 'width 0.25s cubic-bezier(0.34,1.2,0.64,1)', position: 'relative', padding: 0 }}>
        <span style={{ flex: 1, color: 'white', fontSize: '14px', fontWeight: 600, whiteSpace: 'nowrap', paddingLeft: '16px', order: 1, opacity: hovered ? 1 : 0, transition: 'opacity 0.15s' }}>Assistente</span>
        <span style={{ width: '52px', height: '52px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, order: 2 }}>
...
export default function IaAnalyizChat({ isChatOpen, toggleChat, onLog, userRole }) {
  ...
  // Mensagens marcadas como "novo" (insight proativo)
  const [newMsgIds, setNewMsgIds]           = useState(new Set());
  // Ref para o sessionId atual — evita closure stale em handleSend
  const sessionIdRef = useRef(null);

  const { userId } = (() => {
    try { const u = JSON.parse(localStorage.getItem('analyiz_user')); return { userId: u?.id || null }; }
  ...
  // Análise proativa de página
  usePageAnalysis({
    userId,
    userRole,
    enabled: true,
    onInsight: useCallback((insight, pageKey) => {
      // Cria nova sessão para o insight
      const insightId = `proactive-${Date.now()}`;
      const iaId      = `ia-${Date.now()}`;

      // Abre o chat automaticamente
      if (typeof toggleChat === 'function') {
        // Pequeno delay para a animação de abertura ficar visível
        setTimeout(() => toggleChat(true), 400);
      }

      // Adiciona a mensagem proativa ao state
      setMessages([
        { role: 'ia', content: 'Olá! 😊 Como posso te ajudar hoje?', time: getTime(), id: 'init', sources: [] },
        { role: 'ia', content: insight, time: getTime(), id: iaId, sources: [], isProactive: true },
      ]);
      ...
      // Reseta a sessão para que a próxima mensagem crie uma nova
  ...
  // Carrega mensagens ao trocar sessão
  useEffect(() => {
    if (!currentSessionId) {
    ...
    fetch(`${API_BASE_URL}/api/chat/sessions/${currentSessionId}/messages`)
      ...
      .then(msgs => {
        setMessages(msgs.map(m => ({
          ...
          id:      String(m.id),
          sources: [],
          // FIX: imagem ao recarregar — usa imageDesc como indicador, não o base64 truncado
          // O base64 completo NÃO é carregado do banco para o frontend (muito pesado)
          // Mostra apenas um placeholder indicando que havia imagem
  ...
  // Seleciona/cria sessão — retorna o ID garantido
  const ensureSession = useCallback(async () => {
    // FIX: usa ref para pegar o valor atual sem depender do closure
  ...
  const handleSend = useCallback(async () => {
    ...
    const time        = getTime();
    // ID único estável para a mensagem do usuário
    const msgId       = `user-${Date.now()}`;

    // FIX: adiciona a mensagem do usuário ao state ANTES de qualquer await
    // Isso garante que ela não some caso a sessão precise ser criada
    setMessages(prev => [...prev, {
      role:         'user',
      content:      userMessage,
      time,
      id:           msgId,
      imagePreview: imgToSend?.preview || null,
      hadImage:     !!imgToSend,
      sources:      [],
    }]);
    ...
    try {
      // Garante sessão — só após adicionar a mensagem ao state
      ...
      setMessages(prev => [...prev, {
        role: 'ia', content: 'Erro de conexão. Tente novamente. ⚠️',
        time: getTime(), id: `err-${Date.now()}`, sources: [],
      }]);
    } finally {
  ...
  return (
    <>
      <style>{`
        ...
        /* Links gerados pela IA — estilo claro de link clicável */
        .ia-msg a {
          color: #1e40af;
      ...
      <div style={{
        ...
        <div style={{ position: 'relative', padding: '12px 14px', borderBottom: '1px solid #eeeeee', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#ffffff', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '9px' }}>
            <div style={{ width: '30px', height: '30px', borderRadius: '8px', background: BRAND, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          ...
          {showSessions && (
            <SessionsPanel
              ...
              onSelectSession={(id) => { setCurrentSessionId(id); setTypingSet(new Set()); }}
            ...
            <div key={msg.id} style={{ marginBottom: '10px' }}>
              {msg.role === 'ia' ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
                  {/* FIX: imagem em memória (sessão atual) — usa preview do FileReader */}
                  {msg.imagePreview && (
                    <img src={msg.imagePreview} alt="imagem enviada"
                      style={{ maxWidth: '160px', maxHeight: '120px', borderRadius: '10px', marginBottom: '4px', objectFit: 'cover' }} />
                  )}
                  {/* FIX: imagem carregada do banco — mostra badge em vez da imagem corrompida */}
                  {!msg.imagePreview && msg.hadImage && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px', background: '#f0f0f0', borderRadius: '10px', padding: '6px 10px', marginBottom: '4px', fontSize: '12px', color: '#666' }}>
                      <span>📸</span>
                      <span>{msg.imageDesc ? msg.imageDesc.substring(0, 60) + (msg.imageDesc.length > 60 ? '...' : '') : 'Imagem enviada'}</span>
                    </div>
                  )}
                  {msg.content && (
                  ...
                  {msg.time && (
                    <span style={{ fontSize: '11px', color: '#bbb', marginTop: '2px', paddingRight: '2px' }}>{msg.time}</span>
                  )}
                </div>
              )}
            </div>
        ...
        {pendingImage && (
          <div style={{ padding: '6px 12px', background: '#f8fafc', borderTop: '1px solid #eee', display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <span style={{ fontSize: '12px', color: '#64748b', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{pendingImage.name || 'Imagem selecionada'}</span>
            <button onClick={() => setPendingImage(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#94a3b8', flexShrink: 0 }}>
              <X size={14} />
            </button>
          </div>
        )}
        ...
        <div style={{ borderTop: '1px solid #eeeeee', background: '#ffffff', padding: '9px 12px', display: 'flex', alignItems: 'flex-end', gap: '8px', flexShrink: 0 }}>
          ...
          <textarea
            ...
            rows={1}
            placeholder={pendingImage ? 'Adicione uma pergunta sobre a imagem...' : 'Pergunte ao assistente...'}
            className="ia-ta"
