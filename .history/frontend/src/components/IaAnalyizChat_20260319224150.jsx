...
const BRAND        = '#1e293b';
// Chave para persistir a sessão atual no localStorage
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '8px', gap: 0 }}>
      <svg width="56" height="14" viewBox="0 0 56 14" fill="none" style={{ flexShrink: 0 }}>
        <path d="M0 7 Q7 1 14 7 Q21 13 28 7 Q35 1 42 7 Q49 13 56 7"
...
// ─── Botão flutuante com balão de notificação ─────────────────────────────────
...
export default function IaAnalyizChat({ isChatOpen, toggleChat, onLog, userRole }) {
  ...
  // hasUnread: usa sessionStorage — limpa automaticamente ao fechar a aba/recarregar
  // Não deve persistir entre sessões de página (recarregar = usuário viu a página)
  const [hasUnread, setHasUnread] = useState(false);
  const [showSessions, setShowSessions]     = useState(false);
  const [newMsgIds, setNewMsgIds]           = useState(new Set());
  const [pendingImage, setPendingImage]     = useState(null);

  // Lê userId do localStorage de forma reativa — detecta troca de usuário
  const readUserId = () => {
    try {
  ...
  const [currentSessionId, setCurrentSessionId] = useState(() => {
    try {
      ...
      // Só restaura se o userId atual bater com o dono da sessão salva
      // (verificação simples: chave salva por userId)
  ...
  // Detecta troca de usuário via storage event (outro tab) ou polling leve
  useEffect(() => {
    const checkUser = () => {
      const newId = readUserId();
      if (newId !== userId) {
        console.log(`[IA Chat] Usuário trocou: ${userId} → ${newId} — resetando conversa`);
        setUserId(newId);
        // Limpa sessão e mensagens do usuário anterior
    ...
    // Escuta evento storage (troca em outra aba)
    window.addEventListener('storage', checkUser);
    // Polling leve a cada 2s para detectar troca na mesma aba
    const iv = setInterval(checkUser, 2000);
    return () => { window.removeEventListener('storage', checkUser); clearInterval(iv); };
  }, [userId]);

  // FIX: mensagens iniciam com conteúdo vazio — carregadas pelo useEffect abaixo
  const [messages, setMessages] = useState([]);
  const [typingSet, setTypingSet] = useState(new Set());

  // FIX: flag para saber se já carregou a sessão inicial — evita reset durante handleSend
  ...
  // Persiste sessionId + dono no localStorage
  useEffect(() => {
  ...
  // hasUnread não persiste (resetado a cada reload — comportamento correto)

  // Badge — não reage a messages no mount inicial
  ...
  // FIX: carrega sessão ao montar — sem sobrescrever mensagens em voo
  useEffect(() => {
    if (!currentSessionId) {
    ...
    fetch(`${API_BASE_URL}/api/chat/sessions/${currentSessionId}/messages`)
      .then(r => r.json())
      .then(msgs => {
        // Só aplica se não houve mensagens novas no meio tempo
        sessionLoadedRef.current = true;
  ...
  }, []); // ← executa APENAS no mount — não reage a mudanças de currentSessionId durante a sessão

  // Troca explícita de sessão (pelo painel de sessões)
  ...
  // Cria/restaura sessão sem resetar mensagens
  const ensureSession = useCallback(async () => {
    ...
    try {
      ...
      // FIX: atualiza apenas o ID, NÃO chama loadSession (que resetaria messages)
  ...
  // ── Análise proativa contínua ──────────────────────────────────────────────
  useEffect(() => {
    if (!userId) return;

    // Cache local para não repetir o mesmo insight
    const seenInsights = new Set();
    let lastPageKey    = null;

    const poll = async () => {
      const pageKey = getCurrentPageKey();
      console.log(`[IA Proativa] ▶ Poll disparado | página: ${pageKey || 'null'} | chatAberto: ${isChatOpenRef.current} | userId: ${userId}`);

      // Só dispara se chat estiver FECHADO
      if (isChatOpenRef.current) {
        console.log(`[IA Proativa] ⏸ Chat aberto — análise pausada`);
        return;
      }

      if (!pageKey) {
        console.log(`[IA Proativa] ⏭ Página sem análise: ${window.location.pathname}`);
        return;
      }

      console.log(`[IA Proativa] 🔍 Chamando backend para página: ${pageKey}...`);
      ...
      try {
        ...
        if (!data.insight) {
          console.log(`[IA Proativa] Nenhum dado novo para ${pageKey}`);
          return;
        }

        // Evita repetir o mesmo insight
        const insightKey = data.insight.substring(0, 80);
        if (seenInsights.has(insightKey)) {
          console.log(`[IA Proativa] Insight já exibido — ignorando`);
          return;
        }
        seenInsights.add(insightKey);

        console.log(`[IA Proativa] ✅ Novo insight encontrado para ${pageKey}:`, data.insight.substring(0, 60));

        // Adiciona mensagem proativa na conversa atual (sem criar nova sessão)
        const iaId = `proactive-${Date.now()}`;
        setMessages(prev => [
          ...prev,
          { role: 'ia', content: data.insight, time: getTime(), id: iaId, sources: [], isProactive: true },
        ]);
        ...
        if (!isChatOpenRef.current && typeof toggleChat === 'function') {
          setTimeout(() => toggleChat(), 500);
        }

      } catch (e) {
        console.log(`[IA Proativa] ❌ Erro:`, e.message);
      }
    };

    // Primeira análise após 3s (deixa a página carregar)
  ...
  const handleSend = useCallback(async () => {
    ...
    // 1. Adiciona mensagem do usuário imediatamente — ANTES de qualquer await
    setMessages(prev => [...prev, {
      role: 'user', content: userMessage, time, id: msgId,
      imagePreview: imgToSend?.preview || null, hadImage: !!imgToSend, sources: [],
    }]);
    setChatInput('');
    ...
    try {
      // 2. Garante sessão — NÃO reseta messages (ensureSession só atualiza o ID)
      ...
      // 3. Adiciona resposta da IA — append, nunca reset
      setMessages(prev => [...prev, {
        role: 'ia', content: data.reply, time: getTime(), id: iaId, sources: data.sources || [],
      }]);
      setTypingSet(prev => new Set(prev).add(iaId));
      ...
      <div style={{
            ...
            <div key={msg.id} style={{ marginBottom: '10px' }}>
              {msg.role === 'ia' ? (
                <div className="ia-msg" style={{ fontSize: '13.5px', color: '#333', lineHeight: '1.55' }}>
                  {/* Badge "novo" apenas para mensagens proativas ainda não lidas */}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
                  {msg.imagePreview && (
                    <img src={msg.imagePreview} alt="imagem enviada" style={{ maxWidth: '160px', maxHeight: '120px', borderRadius: '10px', marginBottom: '4px', objectFit: 'cover' }} />
                  )}
                  {!msg.imagePreview && msg.hadImage && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px', background: '#f0f0f0', borderRadius: '10px', padding: '6px 10px', marginBottom: '4px', fontSize: '12px', color: '#666' }}>
                      <span>📸</span>
                      <span>{msg.imageDesc ? msg.imageDesc.substring(0, 60) + '...' : 'Imagem enviada'}</span>
                    </div>
                  )}
                  {msg.content && (
        ...
        {pendingImage && (
          <div style={{ padding: '6px 12px', background: '#f8fafc', borderTop: '1px solid #eee', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <img src={pendingImage.preview} alt="preview" style={{ width: '40px', height: '40px', objectFil: 'cover', borderRadius: '6px' }} />
