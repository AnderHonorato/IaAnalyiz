...
export default function MlConfigPanel({ onStatusChange }) {
  ...
  const conectarML = async () => {
    setConnecting(true);
    try {
      const res  = await fetch(`${API_BASE_URL}/api/ml/auth-url`);
      const data = await res.json();
  ...
  const desconectar = async () => {
    ...
    await fetch(`${API_BASE_URL}/api/ml/disconnect`, { method: 'DELETE' });
    carregarStatus();
  };
  ...
  const salvarAgendador = async (novoAtivo, novoIntervalo) => {
    ...
    try {
      const res  = await fetch(`${API_BASE_URL}/api/agendador`, {
        ...
        body:    JSON.stringify({ ativo: novoAtivo, intervalo: novoIntervalo })
      });
      const data = await res.json();
      setAgendador(data);
