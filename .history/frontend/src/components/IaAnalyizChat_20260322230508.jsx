...
// ─── Painel de Raciocínio (estilo Gemini/Claude) ────────────────────────────
function ThinkingPanel({ thinking, isActive }) {
  const [expanded, setExpanded] = useState(false);

  // Dados do thinking
  const steps     = thinking?.steps     || [];
  const reasoning = thinking?.reasoning || null;
  const duration  = thinking?.durationMs;
  const nTools    = steps.length;

  // Durante carregamento: sempre mostra o painel animado
  // Após concluir: colapsado por padrão, expansível
  ...
  const summaryLabel = isActive
    ? null
    : nTools > 0
      ? `Executou ${nTools} ação${nTools > 1 ? 'ões' : ''}`
      : 'Respondeu diretamente';

  const canExpand = !isActive && (reasoning || steps.length > 0);
  ...
  return (
    <div style={{
      ...
      >
        {/* Ícone / spinner */}
        {isActive ? (
          <span style={{ display:'inline-block', width:'14px', height:'14px', borderRadius:'50%', border:'2px solid #cbd5e1', borderTop:'2px solid #6366f1', animation:'ia-spin 0.7s linear infinite', flexShrink:0 }}/>
        ) : (
          <span style={{ fontSize:'13px', flexShrink:0 }}>🧠</span>
        )}

        {/* Label */}
        <span style={{ color: isActive ? '#6366f1' : '#475569', fontWeight:600, flex:1 }}>
          {isActive ? (
            <span style={{ animation:'ia-pulse 1.4s ease-in-out infinite' }}>
              {steps.length > 0 ? steps[steps.length - 1] : 'Pensando...'}
            </span>
          ) : (
            <>
              <span style={{ color:'#475569' }}>{summaryLabel}</span>
              {durationStr && (
                <span style={{ color:'#94a3b8', fontWeight:400, marginLeft:'6px' }}>· {durationStr}</span>
              )}
            </>
          )}
        </span>
      ...
      {/* Corpo expandido */}
      {expanded && canExpand && (
        <div style={{ borderTop:'1px solid #e2e8f0', padding:'8px 10px', display:'flex', flexDirection:'column', gap:'10px' }}>

          {/* Raciocínio */}
          {reasoning && (
            <div>
              <p style={{ fontSize:'10px', fontWeight:700, textTransform:'uppercase', letterSpacing:'0.06em', color:'#94a3b8', marginBottom:'4px' }}>Raciocínio</p>
              <p style={{ color:'#475569', lineHeight:'1.55', fontSize:'12px' }}>{reasoning}</p>
            </div>
          )}
          ...
          {steps.length > 0 && (
            <div>
              <p style={{ fontSize:'10px', fontWeight:700, textTransform:'uppercase', letterSpacing:'0.06em', color:'#94a3b8', marginBottom:'4px' }}>
                Ações executadas
              </p>
              <div style={{ display:'flex', flexDirection:'column', gap:'4px' }}>
                {steps.map((s, i) => (
                  <div key={i} style={{ display:'flex', alignItems:'flex-start', gap:'7px' }}>
                    <span style={{ color:'#10b981', flexShrink:0, marginTop:'1px', fontSize:'11px' }}>✓</span>
                    <span style={{ color:'#475569', fontSize:'12px', lineHeight:'1.45' }}>{s}</span>
                  </div>
                ))}
              </div>
            </div>
...
function IaMessage({ msg, isTyping, onDone }) {
  ...
  // Mostra painel de raciocínio se: a msg terminou E tem thinking data
  const hasThinking = !!msg.thinking;

  return (
    <div className="ia-msg" style={{ fontSize:'13px', color:'#333', lineHeight:'1.6' }}>
      {/* Painel de raciocínio — aparece antes da resposta, sempre que disponível */}
      {hasThinking && (
        <ThinkingPanel thinking={msg.thinking} isActive={false} />
      )}
      {isTyping ? (
          ...
          if (line.startsWith('event: ')) {
            ...
            try {
              ...
              if (currentEvent === 'step') {
                ...
                // Monta thinking data: reasoning do backend + steps acumulados + duration
                const thinkingData = {
  ...
  return (
    <>
      <style>{`
        ...
        @keyframes ia-pulse    { 0%,100%{opacity:1} 50%{opacity:0.55} }
        ...
        <div ref={scrollRef} className="ia-scroll" style={{ flex:1, overflowY:'auto', padding:'12px 14px', background:'#ffffff' }}>
          ...
          {isChatLoading && (
            <div style={{ marginBottom:'10px' }}>
              {/* Painel de raciocínio AO VIVO enquanto processa */}
              <ThinkingPanel
                thinking={{ steps: toolSteps, reasoning: null, durationMs: null }}
                isActive={true}
              />
            </div>
          )}
