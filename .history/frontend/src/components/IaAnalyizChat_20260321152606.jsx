// IaAnalyizChat.jsx — corrigido: sem loop proativo, sem falar sozinha, insight só com dados reais
...
// Guarda apenas os pageKeys onde o insight já foi exibido (sem re-disparar)
...
function SpinnerThinking({ message, startTime }) {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:'4px', padding:'2px 0' }}>
      ...
      {isLate && <span style={{ fontSize:'11px', color:'#94a3b8', marginLeft:'24px', fontStyle:'italic' }}>Está demorando um pouco mais... ⏳</span>}
    </div>
  );
}
      ...
      {open && (
        <div style={{ marginTop:'6px', display:'flex', flexDirection:'column', gap:'4px', animation:'ia-fade-in 0.2s ease' }}>
          {links.map((l, i) => {
            ...
            return (
              <a key={i} href={l.href} target={isInternal ? '_self' : '_blank'} rel="noopener noreferrer"
                ...
                {!isInternal && <ExternalLink size={10} style={{ opacity:0.4 }}/>}
              </a>
            );
          })}
        </div>
  ...
  return (
    <div style={{ position:'relative', display:'inline-flex', flexDirection:'column', alignItems:'flex-end', gap:'6px' }}>
      {hasUnread && shortPreview && (
        <div style={{ background:BRAND, color:'white', fontSize:'12px', fontWeight:500, padding:'8px 12px', borderRadius:'12px 12px 12px 4px', maxWidth:'240px', lineHeight:'1.4', boxShadow:'0 2px 12px rgba(30,41,59,0.3)', animation:'ia-fade-in 0.3s ease', cursor:'pointer', wordBreak:'break-word' }} onClick={onClick}>
          {shortPreview}
        </div>
      )}
      {hasUnread && !shortPreview && (
        <div style={{ background:BRAND, color:'white', fontSize:'11px', fontWeight:500, padding:'5px 10px', borderRadius:'10px 10px 10px 2px', boxShadow:'0 2px 8px rgba(30,41,59,0.25)', animation:'ia-fade-in 0.3s ease', pointerEvents:'none' }}>
          Tenho uma nova análise para você
        </div>
      )}
...
// ─── Valida se um insight tem conteúdo real útil ──────────────────────────────
// Descarta emojis soltos, textos muito curtos, ou strings sem informação
function insightTemConteudoReal(insight) {
  if (!insight) return false;
  // Remove emojis, espaços e pontuação para checar se sobra texto
  const semEmoji = insight.replace(/[\u{1F300}-\u{1FFFF}\u{2600}-\u{26FF}\u{2700}-\u{27BF}✅⚠️🚨💡📦⏱️👥]/gu, '').trim();
  // Precisa ter ao menos 15 caracteres de conteúdo real
  if (semEmoji.length < 15) return false;
  // Não pode ser só um emoji ou símbolo
  if (/^[\s\W]+$/.test(semEmoji)) return false;
  return true;
}
...
export default function IaAnalyizChat({ isChatOpen, toggleChat, userRole }) {
  ...
  // Guarda o insight completo para exibir quando o chat abrir (sem re-chamar API)
  const pendingProactiveInsight = useRef(null);
  ...
  // Quando o chat abre: exibe o insight proativo pendente como mensagem da IA (não como input do usuário)
  useEffect(() => {
    if (!isChatOpen) return;
    setHasUnread(false);
    setShortPreview('');

    const payload = pendingProactiveInsight.current;
    if (!payload) return;
    pendingProactiveInsight.current = null;

    // Exibe o insight completo diretamente como mensagem da IA — sem enviar para a API
    // Isso evita o loop de "falando sozinha"
    setTimeout(() => {
      const iaId = `ia-proactive-reveal-${Date.now()}`;
      setMessages(prev => [...prev, {
        role: 'ia',
        content: payload.fullInsight,
        time: getTime(),
        id: iaId,
        sources: [],
        isProactive: true,
  ...
  const mapMessage = m => ({
    ...
    time: (() => { try { const d = new Date(m.createdAt); return isNaN(d.getTime()) ? getTime() : d.toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'}); } catch { return getTime(); } })(),
    id:String(m.id), sources:[], hadImage:!!m.imageBase64,
  });

  useEffect(() => {
    if (!currentSessionId) {
      setMessages([{ role:'ia', content:'Olá! 😊 Seja bem-vindo à IA Analyiz! Estou aqui para te ajudar com tudo sobre seus anúncios, divergências de frete, catálogo e muito mais. Como posso te ajudar hoje?', time:getTime(), id:'init', sources:[] }]);
      return;
    }
  ...
  const loadSession = useCallback((sessionId) => {
    ...
    if (!sessionId) {
      setMessages([{ role:'ia', content:'Olá! 😊 Nova conversa iniciada! Como posso ajudar?', time:getTime(), id:`init-${Date.now()}`, sources:[] }]);
      return;
    }
  ...
  // ── Proativa — dispara APENAS quando há dados reais relevantes ────────────
  useEffect(() => {
    if (!userId) return;

    const poll = async () => {
      const pageKey = getCurrentPageKey();
      // Não dispara se chat está aberto ou já enviou para esta página
      ...
      // Respeita cooldown de 10 minutos entre disparos
      ...
      try {
        ...
        // ── GUARDA 1: insight deve ter conteúdo real ──
        if (!data.insight || !insightTemConteudoReal(data.insight)) return;

        // ── GUARDA 2: o backend deve confirmar que há dados relevantes ──
        // O campo `hasRelevantData` é enviado pelo backend (ver iaRoutes.js corrigido)
        if (data.hasRelevantData === false) return;

        _sentInsightPages.add(insightKey);
        lastProactiveTime.current = now;

        const insightTexto = data.insight;
        const insightCompleto = data.fullInsight || insightTexto;

        // Exibe no balão flutuante (curto)
        const preview = insightTexto.replace(/<[^>]+>/g, '').substring(0, 90);
        setShortPreview(preview);
        setHasUnread(true);

        // Salva o insight completo para exibir quando o chat abrir
        // NÃO adiciona como mensagem ainda — evita aparecer na fila e ser interpretado como input
        pendingProactiveInsight.current = { fullInsight: insightCompleto };
    ...
    const onNav = () => {
      // Nova página: reseta cooldown para permitir novo insight contextual
      lastProactiveTime.current = 0;
      ...
      <div style={{ position:'fixed', zIndex:9999, bottom:'1.5rem', right:'1.5rem', width:W, height:H, maxHeight:'calc(100vh - 3rem)', background:'#ffffff', borderRadius:'20px', boxShadow:'0 4px 32px rgba(0,0,0,0.15)', display:'flex', flexDirection:'column', overflow:'hidden', transformOrigin:'bottom right', transform:isChatOpen?'scale(1)':'scale(0)', opacity:isChatOpen?1:0, pointerEvents:isChatOpen?'auto':'none', transition:'transform 0.25s cubic-bezier(0.34,1.56,0.64,1), opacity 0.2s' }}>
        ...
        <div style={{ borderTop:'1px solid #eeeeee', background:'#ffffff', padding:'9px 12px', display:'flex', alignItems:'flex-end', gap:'8px', flexShrink:0 }}>
          ...
          <textarea ref={textareaRef} value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={handleKeyDown} onPaste={handlePaste} disabled={isChatLoading} rows={1}
            placeholder={pendingImage?'Pergunta sobre a imagem... 🖼️':'Pergunte ao assistente...'} style={{ flex:1, border:'none', outline:'none', fontSize:'13px', color:'#333', background:'transparent', lineHeight:'1.45', minHeight:'22px', maxHeight:'100px', fontFamily:'inherit', padding:0, opacity:isChatLoading?0.5:1, resize:'none', overflowY:'auto' }}/>
