// IaAnalyizChat.jsx
// Melhorias:
// - Proativo: resumo curto no balão → mensagem completa em fila → ao abrir envia tudo
// - Links/MLB em negrito + card expansível de links abaixo da mensagem
// - Spinner dinâmico contextual com "está demorando..." após 8s
// - Follow-up natural (não menciona que o usuário pediu)
...
// ─── Controle de insights fora do componente (sobrevive remounts) ─────────────
const _sentInsightPages = new Set();
let   _pendingProactivePayload = null; // { short, full } — aguarda o chat abrir
...
function getCurrentPageKey() {
  ...
  if (path.includes('divergen') || path.includes('auditoria')) return 'divergencias';
  if (path.includes('produto'))   return 'produtos';
  if (path.includes('usuario'))   return 'usuarios';
  if (path.includes('preco'))     return 'precos';
  if (path.includes('ml'))        return 'divergencias';
  return null;
}

// ─── Spinner dinâmico contextual ─────────────────────────────────────────────
const SPINNER_CONTEXTS = {
  web:      ['Buscando na web...','Analisando fontes...','Lendo resultados...','Compilando dados...'],
  diverge:  ['Verificando divergências...','Cruzando pesos...','Analisando frete...'],
  produto:  ['Buscando produtos...','Consultando catálogo...','Verificando SKUs...'],
  usuario:  ['Consultando usuários...','Verificando acessos...'],
  imagem:   ['Analisando imagem...','Identificando elementos...','Processando visual...'],
  default:  ['Analisando...','Processando...','Pensando...','Consultando dados...'],
};

function getSpinnerMessages(message) {
  if (!message) return SPINNER_CONTEXTS.default;
  const m = message.toLowerCase();
  if (/busca|web|google|pesquis|mercado livre|taxa|tarifa/i.test(m)) return SPINNER_CONTEXTS.web;
  if (/divergên|peso|frete|anúncio|auditoria/i.test(m)) return SPINNER_CONTEXTS.diverge;
  if (/produto|sku|kit|catálogo/i.test(m)) return SPINNER_CONTEXTS.produto;
  if (/usuário|acesso|bloqueio/i.test(m)) return SPINNER_CONTEXTS.usuario;
  if (/imagem|foto|image/i.test(m)) return SPINNER_CONTEXTS.imagem;
  return SPINNER_CONTEXTS.default;
}

// ─── Spinner com rotação de mensagens + aviso de demora ──────────────────────
function SpinnerThinking({ message, startTime }) {
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', gap:'8px', padding:'2px 0', flexDirection:'column', alignItems:'flex-start' }}>
      <div style={{ display:'flex', alignItems:'center', gap:'8px' }}>
        ...
        <span style={{ fontSize:'13px', color:'#64748b', transition:'all 0.3s' }}>{msgs[idx]}</span>
      </div>
      {isLate && <span style={{ fontSize:'11px', color:'#94a3b8', marginLeft:'24px', fontStyle:'italic' }}>Está demorando mais que o esperado...</span>}
    </div>
  );
}
...
function TypewriterText({ html, speed = 8, onDone }) {
...
// ─── Extrai links do HTML para card expansível ────────────────────────────────
function extractLinks(html) {
  ...
  return [...new Map(links.map(l => [l.href, l])).values()]; // dedup
}

// ─── Card de links expansível ─────────────────────────────────────────────────
function LinkCard({ links }) {
  ...
  return (
    <div style={{ marginTop:'10px', borderTop:'1px solid #f0f0f0', paddingTop:'8px' }}>
      <button
        onClick={() => setOpen(v => !v)}
        style={{ display:'flex', alignItems:'center', gap:'6px', background:'none', border:'none', cursor:'pointer', padding:'4px 8px', borderRadius:'8px', background:'#f8fafc', border:'1px solid #e2e8f0', fontSize:'11px', color:'#64748b', fontWeight:600 }}
      >
        <Globe size={12}/>
        {links.length} link{links.length > 1 ? 's' : ''} referenciado{links.length > 1 ? 's' : ''}
        {open ? <ChevronUp size={12}/> : <ChevronDown size={12}/>}
      </button>

      {open && (
        <div style={{ marginTop:'6px', display:'flex', flexDirection:'column', gap:'4px', animation:'ia-fade-in 0.2s ease' }}>
          {links.map((l, i) => {
            const isInternal = l.href.startsWith('/');
            const isMLB = /MLB\d+/i.test(l.label) || /mercadolivre|mercadolibre/i.test(l.href);
            return (
              <a key={i} href={l.href} target={isInternal ? '_self' : '_blank'}
                rel={isInternal ? undefined : 'noopener noreferrer'}
                style={{
                  display:'flex', alignItems:'center', gap:'8px', fontSize:'12px',
                  color: isMLB ? '#f59e0b' : BRAND, textDecoration:'none',
                  background: isMLB ? '#fffbeb' : '#f8fafc', borderRadius:'8px',
                  padding:'6px 10px', border:`1px solid ${isMLB ? '#fde68a' : '#e2e8f0'}`,
                  fontWeight: isMLB ? 700 : 400, transition:'all 0.15s',
                }}
                onMouseEnter={e => e.currentTarget.style.background = isMLB ? '#fef3c7' : '#f0f4ff'}
                onMouseLeave={e => e.currentTarget.style.background = isMLB ? '#fffbeb' : '#f8fafc'}
              >
                {isMLB ? '🛒' : isInternal ? '🔗' : <ExternalLink size={11}/>}
                <span style={{ flex:1, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{l.label}</span>
                {!isInternal && <ExternalLink size={10} style={{ opacity:0.4, flexShrink:0 }}/>}
              </a>
            );
          })}
        </div>
...
// ─── Renderizador de HTML com bold em MLB + links ─────────────────────────────
function enrichHTML(html) {
  if (!html) return '';
  return html
    // MLB IDs → negrito + link
    .replace(/\b(MLB\d+)\b(?![^<]*>)/g,
      '<a href="https://produto.mercadolivre.com.br/MLB-$1" target="_blank" style="color:#f59e0b;font-weight:700;text-decoration:underline">$1</a>')
    // Bold markdown residual
    .replace(/\*\*(.+?)\*\*/g, '<b>$1</b>')
    .replace(/\*([^*\n<]+)\*/g, '<b>$1</b>')
    // Headers
    .replace(/^#{1,6}\s+/gm, '')
...
// ─── Mensagem IA completa (com link card) ────────────────────────────────────
function IaMessage({ msg, isTyping, onDone, newBadge }) {
  ...
  return (
    <div className="ia-msg" style={{ fontSize:'13px', color:'#333', lineHeight:'1.55' }}>
      {newBadge && <NewBadge/>}
      {isTyping ? (
        <TypewriterText html={enriched} speed={8} onDone={onDone}/>
      ) : (
    ...
    <div style={{ position:'relative', display:'inline-flex', flexDirection:'column', alignItems:'flex-end', gap:'6px' }}>
      {hasUnread && shortPreview && (
        <div style={{
          ...
          padding:'8px 12px', borderRadius:'12px 12px 12px 4px', maxWidth:'240px',
          lineHeight:'1.4', boxShadow:'0 2px 12px rgba(30,41,59,0.3)',
          animation:'ia-fade-in 0.3s ease', cursor:'pointer', wordBreak:'break-word',
        }} onClick={onClick}>
          {shortPreview}
        </div>
      )}
...
export default function IaAnalyizChat({ isChatOpen, toggleChat, onLog, userRole }) {
  ...
  const [shortPreview, setShortPreview]     = useState(''); // texto curto para o balão
  ...
  try {
    const { confirm } = require('../components/Modal').useModal
      ? require('../components/Modal').useModal()
      : { confirm:null };
  ...
  // ── Ao abrir o chat: entrega a mensagem completa em fila ──────────────────
  useEffect(() => {
    if (isChatOpen) {
      setHasUnread(false);
      setShortPreview('');

      if (_pendingProactivePayload) {
        const { full, sessionId } = _pendingProactivePayload;
        _pendingProactivePayload = null;

        // Entrega a mensagem completa naturalmente (sem mencionar "você pediu")
        setTimeout(async () => {
          if (!full) return;
          ...
          try {
            ...
            const iaId = `ia-proactive-full-${Date.now()}`;
          ...
          } catch {} finally { setIsChatLoading(false); setPendingMessage(''); setSpinnerStart(null); }
        }, 600);
      }
    }
  }, [isChatOpen]);
  ...
  // ── Proativa: resumo curto no balão → completo em fila ───────────────────
  useEffect(() => {
    if (!userId) return;
    ...
    const poll = async () => {
      ...
      if (_sentInsightPages.has(insightKey)) return;
      if (now - lastProactiveTime.current < 10 * 60 * 1000) return;

      try {
        // 1. Busca insight curto para o balão
        const resShort = await fetch(`${API_BASE_URL}/api/ia/proactive`, {
          method:'POST', headers:{'Content-Type':'application/json'},
          body: JSON.stringify({ userId, userRole, pageKey }),
        });
        const dataShort = await resShort.json();
        if (!dataShort.insight) return;

        _sentInsightPages.add(insightKey);
        lastProactiveTime.current = now;

        // Exibe resumo curto no balão e na conversa
        const iaId = `proactive-${Date.now()}`;
        setMessages(prev => [...prev, { role:'ia', content:dataShort.insight, time:getTime(), id:iaId, sources:[], isProactive:true }]);
        setNewMsgIds(prev => new Set(prev).add(iaId));
        setTypingSet(prev => new Set(prev).add(iaId));
        setShortPreview(dataShort.insight.replace(/<[^>]+>/g,'').substring(0,120)+'...');
        setHasUnread(true);

        // 2. Prepara a mensagem completa para entregar quando o chat abrir
        const sid = await ensureSession();
        const fullPrompt = `Aprofunde esta análise com detalhes práticos, impacto financeiro estimado e passos de correção: "${dataShort.insight.replace(/<[^>]+>/g,'').substring(0,200)}". Finalize com uma pergunta relevante ao usuário.`;
        _pendingProactivePayload = { full: fullPrompt, sessionId: sid };
  ...
  const handleSend = useCallback(async () => {
    ...
    setPendingMessage(isImageOnly ? 'imagem' : userMessage);
    setSpinnerStart(Date.now());
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/ia/chat`, {
        ...
        body: JSON.stringify({
          ...
          sessionId: sid, userRole, userId, imageOnly: isImageOnly,
    ...
    } finally { setIsChatLoading(false); setPendingMessage(''); setSpinnerStart(null); }
      ...
      <div style={{ position:'fixed', zIndex:9999, bottom:'1.5rem', right:'1.5rem', width:W, height:H, maxHeight:'calc(100vh - 3rem)', background:'#ffffff', borderRadius:'20px', boxShadow:'0 4px 32px rgba(0,0,0,0.15)', display:'flex', flexDirection:'column', overflow:'hidden', transformOrigin:'bottom right', transform:isChatOpen?'scale(1)':'scale(0)', opacity:isChatOpen?1:0, pointerEvents:isChatOpen?'auto':'none', transition:'transform 0.25s cubic-bezier(0.34,1.56,0.64,1), opacity 0.2s' }}>
            ...
            <div key={msg.id} style={{ marginBottom:'12px' }}>
              {msg.role === 'ia' ? (
                <IaMessage
                  ...
                  newBadge={msg.isProactive && newMsgIds.has(msg.id)}
        ...
        <div style={{ borderTop:'1px solid #eeeeee', background:'#ffffff', padding:'9px 12px', display:'flex', alignItems:'flex-end', gap:'8px', flexShrink:0 }}>
          ...
          <textarea ref={textareaRef} value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={handleKeyDown} onPaste={handlePaste} disabled={isChatLoading} rows={1}
            placeholder={pendingImage?'Pergunta sobre a imagem...':'Pergunte ao assistente...'} style={{ flex:1, border:'none', outline:'none', fontSize:'13px', color:'#333', background:'transparent', lineHeight:'1.45', minHeight:'22px', maxHeight:'100px', fontFamily:'inherit', padding:0, opacity:isChatLoading?0.5:1 }}/>
