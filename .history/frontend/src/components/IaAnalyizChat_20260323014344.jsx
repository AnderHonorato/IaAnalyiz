// IaAnalyizChat.jsx — v8: Raciocínio Claude/Gemini Style (Clean), Interpretador de Markdown e Painel Lateral de Documentos
...
function enrichHTML(html) {
  if(!html)return'';
  return html
    .replace(/\b(MLB\d+)\b(?![^<]*>)/g,'<a href="[https://produto.mercadolivre.com.br/MLB-$1](https://produto.mercadolivre.com.br/MLB-$1)" target="_blank" style="color:#f59e0b;font-weight:600;text-decoration:underline">$1</a>')
...
// ─── FileAttachmentGrid e PendingFilesGrid (Inalterados) ─────────────────────
...
// ─── Painel de Configuração e Sessões ─────────────────────────────────────────
...
export default function IaAnalyizChat({ isChatOpen, toggleChat, userRole, pageBaseUrl }) {
  ...
  // Painel lateral para códigos/documentos
  ...
  useEffect(()=>{
    if(!isChatOpen) {
      setSidePanelContent(null); // Fecha o painel lateral ao fechar o chat
      setHasUnread(false);
      setShortPreview('');
    }
          ...
          if(line.startsWith('event: ')){ev=line.slice(7).trim();}
            ...
            try{
              ...
              if(ev==='reasoning_start'){
                ...
                // Se a mensagem contiver blocos de código, auto-abre o painel com o primeiro bloco
  ...
  const handleCopyCode = () => {
    if (sidePanelContent?.code) {
      navigator.clipboard.writeText(sidePanelContent.code);
  ...
  const handleDownloadCode = () => {
    if (sidePanelContent?.code) {
      ...
      a.href = url;
      a.download = `documento.${sidePanelContent.lang || 'txt'}`;
  ...
  return(
    <>
      ...
      {/* ── Painel Lateral de Documentos/Códigos ────────────────────────────── */}
      <div 
        style={{
          position:'fixed',zIndex:9998,bottom:'1.5rem',
          right: isChatOpen && sidePanelContent ? `calc(1.5rem + ${W} + 1rem)` : '1.5rem',
          width: W, height: H, maxHeight:'calc(100vh - 3rem)',
          background: th.bg, borderRadius:'20px',
          boxShadow: '0 4px 32px rgba(0,0,0,0.2)',
          display:'flex', flexDirection:'column', overflow:'hidden',
          transformOrigin:'bottom right',
          transform: (isChatOpen && sidePanelContent) ? 'scale(1)' : 'scale(0)',
          opacity: (isChatOpen && sidePanelContent) ? 1 : 0, 
          pointerEvents: (isChatOpen && sidePanelContent) ? 'auto' : 'none',
          transition:'transform 0.25s cubic-bezier(0.34,1.56,0.64,1),opacity 0.2s, right 0.25s cubic-bezier(0.34,1.56,0.64,1)',
          border: `1px solid ${th.border}`,
        }}
      >
        {/* Header do Painel Lateral */}
        <div style={{
          padding:'12px 16px', borderBottom:`1px solid ${th.headerBorder}`, display:'flex', 
          alignItems:'center', justifyContent:'space-between', background:th.headerBg, flexShrink:0
        }}>
          <div style={{display:'flex', alignItems:'center', gap:'8px', color: th.text}}>
            <FileText size={18} style={{color: th.brand}}/>
            <span style={{fontWeight:600, fontSize:'14px', fontFamily:'inherit'}}>
              Visualizador {sidePanelContent?.lang ? `(${sidePanelContent.lang.toUpperCase()})` : ''}
            </span>
          </div>
          <div style={{display:'flex', alignItems:'center', gap:'4px'}}>
            <button onClick={handleCopyCode} title="Copiar"
              style={{background:'none', border:'none', cursor:'pointer', padding:'6px', color:th.textFaint, display:'flex', borderRadius:'8px', transition:'all 0.1s'}}
              onMouseEnter={e => e.currentTarget.style.color = th.brand}
              onMouseLeave={e => e.currentTarget.style.color = th.textFaint}>
              <Copy size={16}/>
            </button>
            <button onClick={handleDownloadCode} title="Baixar Arquivo"
              style={{background:'none', border:'none', cursor:'pointer', padding:'6px', color:th.textFaint, display:'flex', borderRadius:'8px', transition:'all 0.1s'}}
              onMouseEnter={e => e.currentTarget.style.color = th.brand}
              onMouseLeave={e => e.currentTarget.style.color = th.textFaint}>
              <Download size={16}/>
            </button>
            <button onClick={() => setSidePanelContent(null)} title="Fechar"
              style={{background:'none', border:'none', cursor:'pointer', padding:'6px', color:th.textFaint, display:'flex', borderRadius:'8px', transition:'all 0.1s'}}
        ...
        <div className="ia-sidepanel-scroll" style={{flex:1, overflowY:'auto', padding:'16px', background: th.surface}}>
          <pre style={{
            ...
            fontFamily:"'Google Sans Mono', monospace", fontSize:`${fontSize}px`, 
            color: th.text, lineHeight:'1.5'
          }}>
      ...
      <div className="ia-chat-root" ref={dropRef}
        ...
        <div style={{textAlign:'center',padding:'3px 16px',fontSize:'10px',color:th.textFaint,borderBottom:`1px solid ${th.headerBorder}`,background:th.headerBg,flexShrink:0,fontFamily:'inherit'}}>
          Essa conversa usa IA para te responder e pode cometer erros.
        </div>
