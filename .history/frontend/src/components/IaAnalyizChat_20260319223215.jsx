...
export default function IaAnalyizChat({ isChatOpen, toggleChat, onLog, userRole }) {
  ...
  // userId from localStorage - stable, read once on mount
  const [userId] = useState(() => {
    try {
      const u = JSON.parse(localStorage.getItem('analyiz_user'));
      const id = u?.id || null;
      if (!id) console.warn('[IA Chat] userId não encontrado no localStorage');
      else console.log('[IA Chat] userId:', id, '| role:', u?.role);
      return id;
    } catch (e) {
      console.warn('[IA Chat] Erro ao ler userId:', e.message);
      return null;
    }
  });

  // FIX: restaura a sessão mais recente do localStorage ao montar
  ...
  useEffect(() => {
    if (currentSessionId) localStorage.setItem(SESSION_STORAGE_KEY, String(currentSessionId));
