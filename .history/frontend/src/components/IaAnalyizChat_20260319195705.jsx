...
export default function IaAnalyizChat({ isChatOpen, toggleChat, onLog, userRole }) {
  ...
  // Persistência de Memória no LocalStorage
  const [chatMessages, setChatMessages] = useState(() => {
    const saved = localStorage.getItem('ia_analyiz_memory');
    if (saved) return JSON.parse(saved);
    
    // Se o usuário estiver bloqueado, a mensagem inicial avisa sobre a restrição
    if (userRole === 'BLOQUEADO') {
      return [{ role: 'ia', content: '🔒 **Acesso Restrito.** Você está bloqueado no sistema. Solicite o desbloqueio no topo da tela para utilizar minhas funções neurais.' }];
    }
    
    return [{ role: 'ia', content: 'Olá! Eu sou a **IA Analyiz**, sua inteligência multi-plataforma. O que analisaremos hoje?' }];
  });
  
  const chatScrollRef = useRef(null);

  // Salva no banco de memória local sempre que uma mensagem é adicionada
  useEffect(() => {
  ...
  const handleSendChat = async () => {
    if (!chatInput.trim() || isChatLoading || userRole === 'BLOQUEADO') return;
    ...
    if (onLog) {
      ...
      onLog(`[IA_Analyiz] Sincronizando contexto com o Kernel e Database...`, 'info');
    }
    ...
    try {
      ...
      if (onLog) onLog(`[IA_Analyiz] ✅ Resposta recebida e processada.`, 'success');
      
    } catch (error) {
      setChatMessages(prev => [...prev, { role: 'ia', content: '⚠️ Erro ao se conectar com o servidor da IA Analyiz.' }]);
      if (onLog) onLog(`[IA_Analyiz] ❌ Falha de conexão.`, 'warn');
    } finally {
        ...
        <div className="p-3 border-t border-slate-200 bg-white shrink-0">
          ...
          <div className="flex gap-2">
            <input 
              ...
              disabled={isChatLoading || userRole === 'BLOQUEADO'}
              placeholder={userRole === 'BLOQUEADO' ? "Acesso bloqueado..." : isChatLoading ? "Acessando o banco..." : "Comandar IA Analyiz..."} 
              className={`flex-1 bg-slate-100 rounded-xl px-4 py-2.5 text-xs outline-none focus:ring-2 focus:ring-blue-500/20 transition-all font-medium ${(isChatLoading || userRole === 'BLOQUEADO') ? 'opacity-50 cursor-not-allowed' : ''}`} 
            />
            <button onClick={handleSendChat} disabled={isChatLoading || userRole === 'BLOQUEADO'} className={`bg-blue-600 text-white p-2.5 rounded-xl hover:bg-blue-700 active:scale-95 transition-all flex items-center justify-center shadow-md ${(isChatLoading || userRole === 'BLOQUEADO') ? 'opacity-50 cursor-not-allowed' : ''}`}>
