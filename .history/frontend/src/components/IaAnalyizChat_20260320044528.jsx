...
function SpinnerThinking({ message }) {
  ...
  const label =
    ...
    /mercado\s*livre|taxa|frete|categoria/i.test(l)? 'Pesquisando na web...'       :
    'Analisando...';
  return (
...
// ─── Tokenizer para typewriter Mercury-style ──────────────────────────────────
...
// ─── Typewriter estilo Mercury Diffusion ─────────────────────────────────────
// Gera texto com efeito de "difusão" — cada token aparece com leve fade-in
// Os tokens ficam levemente transparentes e vão ganhando opacidade progressivamente
function TypewriterText({ html, speed = 10, onDone }) {
  ...
  useEffect(() => {
    ...
    // Batch reveal: revela 2-3 tokens por tick para efeito fluido
  ...
  // Monta o HTML revelado + não revelado (invisible)
  ...
  return (
    <SafeHTMLMessage
      html={
        visibleHtml +
        (hiddenHtml
          ? `<span style="opacity:0;user-select:none;pointer-events:none">${hiddenHtml}</span>`
          : '') +
        cursorHtml
      }
    />
...
// ─── Renderizador seguro de HTML com links clicáveis ─────────────────────────
function SafeHTMLMessage({ html }) {
  ...
  useEffect(() => {
    ...
    // Links externos
    const extAnchors = ref.current.querySelectorAll('a[href^="http"]');
...
// ─── Modal de confirmação inline (usa o ModalContext se disponível) ───────────
// Importa useModal dinamicamente para não quebrar se não estiver no provider
let _useModal = null;
try {
  // Tenta importar — em runtime será resolvido pelo bundler
  _useModal = require('../components/Modal').useModal;
} catch (_) {}
...
export default function IaAnalyizChat({ isChatOpen, toggleChat, onLog, userRole }) {
  ...
  // Tenta usar o ModalContext para confirmação estilizada
  ...
  const confirmDelete = useCallback(async () => {
    if (modalConfirm) {
      return await modalConfirm({ title: 'Excluir conversa', message: 'Esta conversa será removida permanentemente.', confirmLabel: 'Excluir', danger: true });
    }
  ...
  const readUserName = () => {
    try { const u = JSON.parse(localStorage.getItem('analyiz_user')); return u?.nome?.split(' ')[0] || null; }
    catch { return null; }
  };
  ...
  // Flag para controlar se já exibiu a mensagem de boas-vindas nesta sessão de chat
  ...
  const seenInsightsRef   = useRef(new Set());
  const lastInsightTimeRef = useRef(0);
  const mountedRef        = useRef(false);
  // Controla o último hasUnread para não repetir notificação sem nova mensagem IA real
  ...
  useEffect(() => {
    ...
    // Só marca como não lido se for mensagem IA NOVA (id diferente da última registrada)
  ...
  const mapMessage = (m) => ({
    role:    m.role,
    // Ao restaurar do banco, mantém o conteúdo HTML sem prefixos de URL
    content: m.content,
    time:    new Date(m.createdAt).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' }),
    id:      String(m.id),
    sources: [],
    hadImage: !!m.imageBase64,
    imagePreview: m.imageBase64 ? `data:${m.imageMimeType || 'image/jpeg'};base64,${m.imageBase64}` : null,
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      const now = Date.now();
      if (now - lastInsightTimeRef.current < 5 * 60 * 1000) return;
      ...
      try {
        ...
        if (seenInsightsRef.current.has(insightKey)) return;
        seenInsightsRef.current.add(insightKey);
        ...
        // Só marca hasUnread se o chat estiver fechado
    ...
    const onNav = () => setTimeout(poll, 800);
    window.addEventListener('popstate', onNav);
    const origPush = history.pushState.bind(history);
    history.pushState = (...args) => { origPush(...args); setTimeout(poll, 800); };
  ...
  // ── Cole imagem do clipboard (Ctrl+V) ─────────────────────────────────────
      ...
      if (item.type.startsWith('image/')) {
        ...
        reader.onload = (ev) => {
          ...
          setPendingImage({
            ...
            preview:  dataUrl,
            name:     `imagem_colada_${new Date().toLocaleTimeString('pt-BR').replace(/:/g, '-')}.png`,
          });
        };
        reader.readAsDataURL(file);
        break;
  ...
  const handleSend = useCallback(async () => {
    ...
    // Se só imagem sem texto, envia mensagem especial para a IA perguntar o que deseja
    ...
    // pageUrl vai separado — NÃO aparece na mensagem visível ao usuário
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/ia/chat`, {
        ...
        body:    JSON.stringify({
          // Se for só imagem, manda instrução interna para a IA — não fica visível ao usuário
      ...
      setMessages(prev => [...prev, {
        role: 'ia', content: data.reply, time: getTime(),
        id: iaId, sources: data.sources || []
      }]);
      setTypingSet(prev => new Set(prev).add(iaId));
  ...
  return (
    <>
      <style>{`
        ...
        .ia-msg a:hover { color:#1d4ed8 !important;text-decoration-color:#1d4ed8; }
      `}</style>
      ...
      <div style={{
        ...
        <div ref={scrollRef} className="ia-scroll" style={{ flex:1, overflowY:'auto', padding:'12px 14px', background:'#ffffff' }}>
          ...
          {isChatLoading && pendingMessage && (
            <div style={{ marginBottom:'10px' }}><SpinnerThinking message={pendingMessage} /></div>
          )}
        </div>
        ...
        <div style={{ borderTop:'1px solid #eeeeee', background:'#ffffff', padding:'9px 12px', display:'flex', alignItems:'flex-end', gap:'8px', flexShrink:0 }}>
          <button
            ...
            title="Enviar imagem (ou Cole com Ctrl+V)"
          ...
          <textarea
            ...
            disabled={isChatLoading}
            rows={1}
            placeholder={pendingImage ? 'Pergunta sobre a imagem... (ou cole outra)' : 'Pergunte ao assistente... (Ctrl+V para colar imagem)'}
            className="ia-ta"
            style={{ flex:1, border:'none', outline:'none', fontSize:'13px', color:'#333', background:'transparent', lineHeight:'1.45', minHeight:'22px', maxHeight:'100px', fontFamily:'inherit', padding:0, opacity: isChatLoading ? 0.5 : 1 }}
          />
          <button
            onClick={handleSend}
            ...
            style={{
              ...
              background: (chatInput.trim() || pendingImage) && !isChatLoading ? BRAND : '#e8e8e8',
              cursor: (chatInput.trim() || pendingImage) && !isChatLoading ? 'pointer' : 'default',
