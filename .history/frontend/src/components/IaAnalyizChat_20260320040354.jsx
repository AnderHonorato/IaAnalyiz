...
import { Send, Minus, Maximize2, Minimize2, MessageSquare, Trash2, ChevronDown, ChevronUp, Paperclip, X, Plus, ExternalLink } from 'lucide-react';
...
function SpinnerThinking({ message }) {
  ...
  const label =
    ...
    /preço|mercado|tendência|vender/i.test(l)      ? 'Pesquisando na web...'       :
    'Analisando...';
  return (
...
// Renderiza HTML com links clicáveis de forma segura
function SafeHTMLMessage({ html }) {
  // Garante que links no HTML são clicáveis
  const processed = (html || '')
    .replace(/<a\s+href="(\/[^"]+)"/g, '<a href="$1" style="color:#1e40af;text-decoration:underline;font-weight:500;cursor:pointer;"')
    .replace(/<a\s+href="(https?[^"]+)"/g, '<a href="$1" target="_blank" rel="noopener noreferrer" style="color:#1e40af;text-decoration:underline;font-weight:500;cursor:pointer;"');
...
export default function IaAnalyizChat({ isChatOpen, toggleChat, onLog, userRole }) {
  ...
  // Conjunto persistente de insights já vistos (para não repetir o badge)
  ...
  // Carrega mensagens da sessão ao montar
  useEffect(() => {
    if (!currentSessionId) {
      ...
      .then(msgs => {
        ...
        setMessages(msgs.length > 0
          ? msgs.map(m => ({
              ...
              // CORREÇÃO: mantém a imagem visível ao recarregar
              imagePreview: m.imageBase64 ? `data:image/jpeg;base64,${m.imageBase64}` : null,
  ...
  // ── Análise proativa (CORRIGIDA: sem auto-abrir, com deduplicação robusta) ───
  useEffect(() => {
    if (!userId) return;

    const poll = async () => {
      const pageKey = getCurrentPageKey();
      // Não dispara se o chat já está aberto ou não há página relevante
      if (isChatOpenRef.current || !pageKey) return;

      // Limita frequência: mínimo 5 min entre proativas
      const now = Date.now();
      if (now - lastInsightTime.current < 5 * 60 * 1000) return;
      ...
      try {
        ...
        // Deduplicação: ignora insights muito similares
        const insightKey = data.insight.replace(/<[^>]+>/g, '').substring(0, 60).toLowerCase().trim();
        ...
        // CORREÇÃO: Não abre o chat automaticamente — apenas mostra o badge
        setHasUnread(true);
      } catch {}
    };

    const initial  = setTimeout(poll, 5000);
    const interval = setInterval(poll, 5 * 60 * 1000); // Polling a cada 5 minutos

    const onNav = () => {
      // Reseta o timer de proativas ao navegar, mas mantém deduplicação
      lastInsightTime.current = 0;
      setTimeout(poll, 1500);
    };
    window.addEventListener('popstate', onNav);
    const origPush = history.pushState.bind(history);
    history.pushState = (...args) => { origPush(...args); setTimeout(poll, 1500); };
  ...
  // ── NOVA: Colar imagem do clipboard (Ctrl+V) ───────────────────────────────
      ...
      if (item.type.startsWith('image/')) {
        ...
        reader.onload = (ev) => {
          ...
          setPendingImage({
            ...
            preview: dataUrl,
            name: `captura-${new Date().toLocaleTimeString('pt-BR').replace(/:/g, '-')}.png`
          });
        };
        reader.readAsDataURL(file);
        break;
  ...
  const handleSend = useCallback(async () => {
    ...
    // Contexto de página — enviado APENAS para a API, não exibido ao usuário
    const pageUrl     = window.location.pathname;

    // Adiciona ao chat com a mensagem LIMPA (sem prefixo de URL)
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/ia/chat`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          message: userMessage,     // Mensagem limpa para o BD
          pageUrl,                  // Contexto de página separado
          sessionId: sid,
          userRole,
          userId,
      ...
      setMessages(prev => [...prev, {
        role: 'ia',
        content: 'Erro de conexão. Tente novamente. ⚠️',
        time: getTime(),
        id: `err-${Date.now()}`,
        sources: []
      }]);
      ...
      <div style={{
            ...
            <div key={msg.id} style={{ marginBottom:'10px' }}>
              {msg.role === 'ia' ? (
                ...
                <div style={{ display:'flex', flexDirection:'column', alignItems:'flex-end' }}>
                  {/* CORREÇÃO: Mostra imagem mesmo ao recarregar da sessão */}
                  {msg.imagePreview && (
                  ...
                  {!msg.imagePreview && msg.hadImage && (
                    <div style={{ display:'flex', alignItems:'center', gap:'6px', background:'#f8fafc', border:'1px solid #e2e8f0', borderRadius:'10px', padding:'6px 10px', marginBottom:'4px', fontSize:'12px', color:'#64748b' }}>
                      <span>📸</span>
                      <span>{msg.imageDesc ? msg.imageDesc.substring(0, 50) + '...' : 'Imagem enviada'}</span>
                    </div>
                  )}
                  {msg.content && (
        ...
        {pendingImage && (
          <div style={{ padding:'6px 12px', background:'#f8fafc', borderTop:'1px solid #eee', display:'flex', alignItems:'center', gap:'8px' }}>
            <img src={pendingImage.preview} alt="preview" style={{ width:'40px', height:'40px', objectFit:'cover', borderRadius:'6px', border:'1px solid #e2e8f0' }} />
            ...
            <button onClick={() => setPendingImage(null)} style={{ background:'none', border:'none', cursor:'pointer', color:'#94a3b8', padding:'2px' }}><X size={14} /></button>
          </div>
        )}
        ...
        <div style={{ borderTop:'1px solid #eeeeee', background:'#ffffff', padding:'9px 12px', display:'flex', alignItems:'flex-end', gap:'8px', flexShrink:0 }}>
          <button onClick={() => fileInputRef.current?.click()} disabled={isChatLoading} title="Enviar imagem (ou cole com Ctrl+V)"
          ...
          <textarea
            ...
            rows={1}
            placeholder={pendingImage ? 'Pergunta sobre a imagem...' : 'Pergunte ou cole uma imagem (Ctrl+V)...'}
            className="ia-ta"
