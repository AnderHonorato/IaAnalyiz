...
function TypewriterText({ html, speed = 12, onDone }) {
  ...
  return <SafeHTMLMessage html={displayed + (!done ? '<span style="opacity:0.5;animation:ia-blink 0.8s step-end infinite">▍</span>' : '')} />;
}
...
function SafeHTMLMessage({ html }) {
  ...
  useEffect(() => {
    ...
    // Intercepta links internos para usar window.location
...
export default function IaAnalyizChat({ isChatOpen, toggleChat, onLog, userRole }) {
  ...
  // Para deduplicação de insights proativos
  ...
  const mapMessage = (m) => ({
    ...
    // Restaura preview da imagem a partir do base64 armazenado
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      // Rate limit: no mínimo 5 minutos entre insights proativos
      const now = Date.now();
      ...
      try {
        ...
        // Deduplicação: não repete o mesmo insight
        ...
        // Apenas marca como não lido — NÃO abre o chat automaticamente
        setHasUnread(true);
      } catch {}
    };
  ...
  const handleSend = useCallback(async () => {
    ...
    // pageUrl vai separado — NÃO aparece na mensagem visível
            ...
            <div key={msg.id} style={{ marginBottom:'10px' }}>
              {msg.role === 'ia' ? (
                ...
                <div style={{ display:'flex', flexDirection:'column', alignItems:'flex-end' }}>
                  {/* Preview de imagem (sessão atual OU restaurada do banco) */}
                  {msg.imagePreview && (
                    <img
