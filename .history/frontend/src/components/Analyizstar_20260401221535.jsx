/**
 * frontend/src/components/Analyizstar.jsx
 * 
 * ╪═════════════════════════════════════════════════════════════════════════════════
 * Propósito: Componente de estrela animada Analyiz (IA mãe) com sistema de órbita
 * de cometas para indicar atividade de processamento e análise em tempo real.
 * 
 * Responsabilidades:
 * - Renderizar estrela de 4 pontas estilo Gemini com gradiente dinâmico (azul→roxo→rosa)
 * - Implementar diferentes modos de animação: idle (respiração) e active (pulso rápido)
 * - Gerenciar 4 cometas rotatórios ao redor da estrela central com velocidades variadas
 * - Apoiar temas claro/escuro com cores ajustadas para contraste ótimo
 * - Aplicar CSS de animação injetado dinamicamente para garantir efeitos visuais
 * 
 * Estado:
 * - size: Tamanho do componente em pixels (padrão 32px)
 * - active: Boolean que ativa modo pulso rápido vs. respiração lenta
 * - dark: Ativa gradiente vibrante (true) ou contraste escuro em fundo claro (false)
 * - Cometas: Array de 4 objetos com distância orbital, direção (cw/ccw), cores e delays
 * 
 * Animações Implementadas:
 * - as-breathe: Respiração lenta (3.2s) para estado idle
 * - as-pulse: Pulso rápido (1.1s) para estado active
 * - as-orbit-cw/ccw: Rotação de órbita contínua (3.0s e 3.8s idle, 1.1s e 1.4s fast)
 * - as-comet-glow: Brilho lento dos cometas (2.4s idle, 0.9s fast)
 * 
 * Dependências:
 * - React (hooks)
 * - SVG nativo (gradientes e paths)
 * - CSS3 Animations (keyframes injetados)
 * 
...
// AnalyizStar.jsx — estrela animada para indicar atividade de análise ou processamento
