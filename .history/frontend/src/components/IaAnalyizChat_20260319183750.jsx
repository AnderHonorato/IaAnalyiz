...
export default function IaAnalyizChat({ isChatOpen, toggleChat, onLog }) {
  ...
  const [isExpanded, setIsExpanded] = useState(false); // Estado para controlar o tamanho do chat
  ...
  const handleSendChat = async () => {
    ...
    // Dispara log para o Terminal do App.jsx
    if (onLog) {
      onLog(`[IA_Analyiz] Requisitando análise: "${userMessage.substring(0, 30)}..."`, 'info');
      onLog(`[IA_Analyiz] Sincronizando contexto com o Kernel do Gemini 2.5...`, 'info');
    }
  ...
  // Dinâmica de classes para o tamanho do chat (Normal vs Expandido)
  const chatSizeClasses = isExpanded 
    ? "w-[600px] h-[750px] max-w-[85vw] max-h-[85vh]" // Expandido para esquerda e para cima
    : "w-[320px] h-[550px] max-h-[80vh]"; // Normal (Fino e Comprido)
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end max-w-[calc(100vw-32px)]">
        ...
        <div className="bg-[#1e293b] p-4 flex justify-between items-center text-white shrink-0 shadow-sm z-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[9px] text-emerald-400 font-bold uppercase tracking-widest mt-1">Status: Conectada</p>
            </div>
          </div>
          <div className="flex items-center gap-1">
              ...
              {/* Note: O botão X foi removido daqui. O chat é fechado clicando no ícone do robô flutuante. */}
          </div>
        </div>
        ...
        <div className="p-3 border-t border-slate-200 bg-white shrink-0">
          <div className="flex gap-2">
            <input 
              ...
              placeholder="Comandar IA Analyiz..." 
              className="flex-1 bg-slate-100 rounded-xl px-4 py-3 text-xs outline-none focus:ring-2 focus:ring-blue-500/20 transition-all font-medium" 
            />
      ...
      {/* Botão Flutuante que controla o abrir/fechar */}
      <div className="relative group">
          ...
          <button 
              onClick={toggleChat} 
              className={`transition-all duration-300 hover:scale-110 active:scale-90 relative`}
          >
              <RobotGif />
              {!isChatOpen && (
