...
// ─── Detecta intenção da mensagem para montar os passos visuais ───────────────
function buildProcessingSteps(message) {
  ...
  if (isGreeting) {
    steps.push({ text: 'Pronto para te atender.', delay: 900 });
    return steps;
  }
...
function ProcessingSteps({ message }) {
  ...
  useEffect(() => {
    ...
    const timers = steps.map((step, i) => setTimeout(() => setVisibleStep(i), step.delay));
    ...
    <div className="flex gap-2.5 items-start">
      <div className="h-7 w-7 bg-blue-600 rounded-lg flex items-center justify-center shrink-0 shadow-md mt-0.5">
        <Sparkles
          ...
          style={{ animation: 'ia-spin 2s linear infinite' }}
        />
      </div>
      <div className="flex flex-col gap-1 pt-1">
        <p className="text-[11px] text-slate-400 italic transition-all duration-500">
          {currentText}
        </p>
...
export default function IaAnalyizChat({ isChatOpen, toggleChat, onLog, userRole }) {
  ...
  // Lê userId do localStorage para passar ao backend
  const { userId, userName } = (() => {
    try {
      const u = JSON.parse(localStorage.getItem('analyiz_user'));
      return { userId: u?.id || null, userName: u?.nome || null };
    ...
    try {
      const res  = await fetch(`${API_BASE_URL}/api/ia/chat`, {
        ...
        body:    JSON.stringify({
          ...
          userRole,
          userId,    // passa o ID do usuário logado para o backend excluí-lo das contagens
        }),
      });
      ...
      <div className="fixed bottom-6 right-6 z-[9999] flex flex-col items-end pointer-events-none">
        ...
        <div className={`pointer-events-auto ${chatSizeClasses} bg-white rounded-[2rem] shadow-[0_20px_60px_rgba(0,0,0,0.3)] border border-slate-200 flex flex-col mb-4 overflow-hidden origin-bottom-right transition-all duration-500 ${isChatOpen ? 'scale-100 opacity-100' : 'scale-0 opacity-0'}`}>
          ...
          <div className="border-t border-slate-200 bg-white shrink-0">
            {/* Aviso discreto acima do input */}
            <div className="flex items-center justify-center gap-1.5 pt-2 pb-1 px-4">
              <Bot className="w-3 h-3 text-slate-300" />
              <p className="text-[10px] text-slate-300 select-none">
                Essa conversa usa IA para te responder
              </p>
            </div>
