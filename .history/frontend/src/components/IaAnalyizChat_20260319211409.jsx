  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '7px', padding: '2px 0' }}>
      <span style={{
        ...
        opacity: dotOn ? 1 : 0, transition: 'opacity 0.15s',
      }} />
  ...
  return (
    <button
      ...
      style={{
        ...
        // Expande para a esquerda incluindo o texto
        width:          hovered ? '150px' : '52px',
        height:         '52px',
    ...
    >
      {/* Ícone — sempre visível, fica à direita */}
      <span style={{
        width:          '52px',
        height:         '52px',
        display:        'flex',
      ...
      {/* Label "Assistente" — aparece à esquerda do ícone */}
      <span style={{
        order:       1,
        flex:        1,
        color:       'white',
...
export default function IaAnalyizChat({ isChatOpen, toggleChat, onLog, userRole }) {
  ...
  const handleSend = useCallback(async () => {
    ...
    // Isso garante que o backend recebe o histórico correto sem a pergunta atual duplicada
    ...
    try {
      const res  = await fetch(`${API_BASE_URL}/api/ia/chat`, {
        ...
        body:    JSON.stringify({
          message:  userMessage,
          history:  historySnapshot, // histórico sem a mensagem atual
          userRole,
          userId,
        }),
      });
  ...
  return (
    <>
      ...
      <div style={{
        ...
        background:      '#ffffff',
        borderRadius:    '20px',          // mais arredondado — igual ML
      ...
      {/* ══ BOTÃO FLUTUANTE — expande com "Assistente" ao hover ══════════════ */}
      <div style={{
        position:      'fixed',
        zIndex:        9998,
        bottom:        '1.5rem',
