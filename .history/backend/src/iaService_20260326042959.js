...
async function loopAprendizadoAutonomo() {
  ...
  // Agenda o próximo ciclo (se não tiver sido cancelado)
  if (isAutonomousLearningActive) {
    autonomousLearningInterval = setTimeout(loopAprendizadoAutonomo, 15000); // 15 segundos entre ciclos
  }
}
...
// Emite eventos específicos de agente (Com tag action para garantir captura no frontend)
...
export async function buildAnswerStream(message, history = [], context = {}, onStep, attempt = 1) {
  ...
  if (isOwner && message) {
    ...
    // Controles do Aprendizado Autônomo
    if (ml.match(/iniciar aprendizado/)) {
      isAutonomousLearningActive = true;
      loopAprendizadoAutonomo(); // Inicia o worker solto em background
  ...
  try {
    ...
    // ── 1. Raciocínio dinâmico PRIMEIRO (Para o usuário ler enquanto o agente trabalha) ──
    ...
    if (needsWebSearch(message) && !isImageOnly) {
      // Agora o frontend já está pronto para renderizar o agente
    ...
    while (response.functionCalls?.length > 0 && callCount < 5) {
      ...
      // Opcional: mostrar visual de agente para consulta a banco
      emitAgentStart(onStep, 'banco', `Consultando ferramenta: ${call.name}...`);
