 ...
 * Adaptado para arquitetura Multi-tenant.
 */
...
export async function runBot(userId) {
  ...
  // Removida a verificação do process.env.ML_ACCESS_TOKEN, pois agora o token vem do banco.
  ...
  try {
    ...
    // ── 2. Carregar produtos cadastrados localmente DO USUÁRIO ────────────────
    ...
    if (mapaLocal.size === 0) {
      log('⚠️ Nenhum produto com ID ML cadastrado no seu banco. Cadastre produtos primeiro.', 'warn');
      broadcast({ type: 'done' });
      return;
    }
    ...
    for (let i = 0; i < idsParaAudioria.length; i += BATCH) {
      ...
      for (const entry of detalhes) {
        ...
        if (isDivergente(pesoMl, pesoLocal)) {
          ...
          // Verifica se divergência já existe para O USUÁRIO para não duplicar
