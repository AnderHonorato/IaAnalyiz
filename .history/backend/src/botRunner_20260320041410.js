...
function checkDivergencia(pesoMl, pesoLocal) {
  ...
  if (pesoMl === 0 && pesoLocal > 0)
    return { isDiv: true, texto: `Anúncio ML sem peso declarado (0g) | Cadastro local: ${pesoLocal}g` };
  ...
  try {
    ...
    for (let i = 0; i < allIds.length; i += BATCH) {
      ...
      for (const entry of detalhes) {
        ...
        const pesoLocal = local.eKit && local.itensDoKit?.length > 0
        ...
        if (check.isDiv) {
          ...
          if (!existing) {
            ...
            log(`🚨 Nova Divergência: "${titulo.substring(0, 45)}" → ${diff}g de diferença!`, 'warn');
          } else {
