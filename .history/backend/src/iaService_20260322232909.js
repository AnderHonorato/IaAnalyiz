...
export const UI_FEATURES = {
  tema: {
    ...
    onde:      'Barra superior (header) do site — ícone de paleta na barra do topo',
...
function buildTools(userRole) {
  ...
  const tools = [
    ...
    {
      name: 'listarAvisosML',
      description: 'Lista avisos ativos do Mercado Livre sobre anúncios com problema.',
      parameters: {
        type: 'OBJECT',
        properties: {
  ...
  if (isPrivileged) {
    tools.push(
      ...
      },
      {
        ...
        description: 'Retorna métricas globais de todos os usuários da plataforma. Apenas ADMIN/OWNER.',
        parameters: { type: 'OBJECT', properties: {} },
      }
    );
  }
...
function buildSystemInstruction(ctx) {
  ...
  const paginasDoRole = caps.paginasAcesso
    ...
    .join('\n');
  const restricoes = caps.restricoes.length ? caps.restricoes.map(r => `  ${r}`).join('\n') : '  (sem restrições específicas)';
...
• Alterar TEMA/COR do site → ícone de paleta na barra do topo (canto superior direito). São 20 temas.
...
• SE a intenção é CLARA → EXECUTE A TOOL IMEDIATAMENTE sem perguntar.
• SE a intenção é AMBÍGUA → PERGUNTE primeiro e peça confirmação.
• SE o usuário responde "sim", "pode", "faz", "ok" → EXECUTE IMEDIATAMENTE.
• NUNCA execute ações irreversíveis (bloquear usuário) sem confirmar primeiro.
...
// ═══════════════════════════════════════════════════════════════════════════════
// REASONING INSTRUCTION — Modelo separado para raciocínio detalhado
// Gera o raciocínio passo a passo ANTES da resposta principal
// ═══════════════════════════════════════════════════════════════════════════════

function buildReasoningInstruction(ctx) {
  const { userRole = 'USUARIO', usuarioAtual = null, dataBlock = null } = ctx;
  const caps = ROLE_CAPABILITIES[userRole] || ROLE_CAPABILITIES['USUARIO'];
...
Sua tarefa: produzir um raciocínio detalhado e honesto sobre o que precisa ser feito para responder à mensagem do usuário.
Escreva em português, de forma técnica e direta, SEM emojis, SEM markdown, apenas texto simples.
O raciocínio deve ter entre 80 e 200 palavras e cobrir:
1. O que o usuário está pedindo (interpretação da intenção)
2. Se é necessário consultar o banco de dados e quais tabelas/ferramentas
3. Se há ação a executar ou apenas resposta informativa
4. Quaisquer ambiguidades ou pontos de atenção
5. Como será estruturada a resposta

Não inclua saudações. Escreva como um engenheiro pensando em voz alta antes de agir.`;
}

// ═══════════════════════════════════════════════════════════════════════════════
// GERA RACIOCÍNIO REAL via Gemini (streaming token a token)
// Chamado ANTES do pipeline principal — emite evento SSE 'reasoning_chunk'
// ═══════════════════════════════════════════════════════════════════════════════
...
export async function generateReasoningStream(message, context, onChunk) {
  ...
  try {
    const ai = getClient();
    const systemInstruction = buildReasoningInstruction(context);
    ...
    return fullReasoning.trim();
  } catch (err) {
    console.warn('[Reasoning]', err.message);
    // Fallback: raciocínio sintético baseado na mensagem
    return buildFallbackReasoning(message, context);
  }
}

function buildFallbackReasoning(message, context) {
  const { userRole = 'USUARIO', totalDivergencias = 0, totalProdutos = 0 } = context;
  const parts = [];

  parts.push(`Analisando a solicitação do usuário com cargo ${userRole}.`);

  if (/divergen|peso|frete|corrig/i.test(message || '')) {
    parts.push(`A mensagem envolve divergências de frete ou peso. O usuário tem ${totalDivergencias} divergências ativas. Será necessário consultar a ferramenta listarDivergenciasAtivas para obter os IDs e detalhes.`);
    if (/corrig|envia|fila/i.test(message || '')) {
      parts.push('Identifico uma intenção de ação: envio para fila de correção. Verificarei se é uma divergência específica ou lote completo.');
    }
  } else if (/produto|sku|catálogo/i.test(message || '')) {
    parts.push(`A solicitação é sobre produtos. Catálogo atual tem ${totalProdutos} itens. Usarei a ferramenta listarProdutos com filtros adequados.`);
  } else if (/resumo|status|como está/i.test(message || '')) {
    parts.push('O usuário quer um panorama geral do sistema. Chamarei resumoGeral para consolidar métricas de produtos, divergências, agendador e conexão ML.');
  } else if (/agendador|varredura|automático/i.test(message || '')) {
    parts.push('Solicitação relacionada ao agendador de varredura automática. Verificarei o estado atual antes de executar qualquer alteração.');
  } else {
    parts.push('A mensagem não requer consulta ao banco de dados. Resposta informativa baseada no contexto disponível.');
  }

  parts.push('Estruturarei a resposta de forma concisa, confirmando ações executadas quando aplicável.');
  return parts.join(' ');
}
...
export const TOOL_STEP_MESSAGES = {
  ...
  desativarAgendador:         'Acessando configurações... Desativando agendador...',
...
// Fluxo:
//   1. Emite evento 'reasoning_start' imediatamente
//   2. Gera raciocínio real via Gemini streaming → emite 'reasoning_chunk' token a token
//   3. Emite 'reasoning_end'
//   4. Executa tools com eventos 'step'
//   5. Emite 'done' com a resposta final
// ═══════════════════════════════════════════════════════════════════════════════
...
export async function buildAnswerStream(message, history = [], context = {}, onStep, attempt = 1) {
  ...
  try {
    ...
    // ── FASE 1: Raciocínio real em streaming ─────────────────────────────────
    // Emite imediatamente o sinal de início — o frontend mostra "Mostrar raciocínio" de imediato
    ...
    try {
      fullReasoning = await generateReasoningStream(
        ...
        (chunk) => {
          // Cada token do raciocínio é enviado em tempo real
          if (onStep) onStep({ type: 'reasoning_chunk', text: chunk });
        }
      );
    } catch (reasoningErr) {
      console.warn('[Reasoning stream]', reasoningErr.message);
      fullReasoning = buildFallbackReasoning(message, context);
      if (onStep) onStep({ type: 'reasoning_chunk', text: fullReasoning });
    }

    if (onStep) onStep({ type: 'reasoning_end', fullText: fullReasoning });

    // ── FASE 2: Pipeline principal com tools ─────────────────────────────────
    const config = {
      systemInstruction: buildSystemInstruction({ ...context, isFirstMessage }),
      temperature: 0.25,
      maxOutputTokens: 1000,
    ...
    if (response.candidates?.[0]?.finishReason === 'SAFETY')
      return { reply: 'Mensagem bloqueada por questões de segurança.', sources: [], reasoning: fullReasoning, toolsExecutadas: [] };
    ...
    return { reply, sources, reasoning: fullReasoning, toolsExecutadas, toolStepTexts };
