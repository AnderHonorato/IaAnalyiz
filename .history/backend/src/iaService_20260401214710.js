// ═══════════════════════════════════════════════════════════════════════════════
// IA SERVICE — Motor Principal de Inteligência Artificial
// ═══════════════════════════════════════════════════════════════════════════════
// Arquivo central que coordena toda a lógica de IA:
// - Integração com motor de raciocínio (chatEngine)
// - Recuperação de conhecimento local (RAG)
// - Aprendizado autônomo contínuo
// - Validação de respostas
// - Geração de respostas personalizadas para usuários
// 
...
import { GoogleGenAI } from '@google/genai';    // Cliente API Google (LLM externo)
import { PrismaClient } from '@prisma/client';  // ORM para banco de dados
import crypto from 'crypto';                    // Hashing e criptografia
import fs from 'fs';                            // File system (ler arquivos)
import path from 'path';                        // Manipulação de caminhos de arquivo
import { fileURLToPath } from 'url';            // Obter __dirname em módulos ESM

// Importação de módulos internos da IA
import { queryKnowledge, saveKnowledge } from './ia-engine/knowledge.js';  // Base de conhecimento local
import { runOrchestrator } from './ia/brain/agents/agentOrchestrator.js';   // Orquestrador de agentes

// ─────────────────────────────────────────────────────────────────────────────
// INICIALIZAÇÃO E CONFIGURAÇÃO
// ─────────────────────────────────────────────────────────────────────────────

const prisma    = new PrismaClient();                              // Cliente Prisma para banco de dados
const __dirname = path.dirname(fileURLToPath(import.meta.url));   // Diretório atual (src/)

// Carregamento da skill especializada (SKILL.md)
// Este arquivo contém instruções de negócio para a IA (quando usar cada skill)
let SKILL_CONTENT = '';
try {
  SKILL_CONTENT = fs.readFileSync(path.join(__dirname, 'skills', 'SKILL.md'), 'utf-8');
  console.log('[iaService.js] SKILL.md loaded successfully');
} catch {
...
// Modelos de IA disponíveis (hoje usando Gemini como base, mas abstraído)
const GEMINI_MODEL          = 'gemini-2.5-flash';        // Modelo principal (mais rápido)
const GEMINI_MODEL_FALLBACK = 'gemini-2.5-flash';        // Fallback se principal falhar

// Limites e parâmetros de processamento
const MAX_PAIRS             = 20;      // Máximo de pares (Q&A) a manter em memory
const MAX_FILES             = 10;      // Máximo de documentos por sessão
const REASONING_CHUNK_SIZE  = 5;       // Chunks de raciocínio para streaming
const REASONING_CHUNK_DELAY = 2;       // Delay em ms entre chunks

// ─────────────────────────────────────────────────────────────────────────────
// PROMPT DE RACIOCÍNIO EM CADEIA (CHAIN OF THOUGHT)
// ─────────────────────────────────────────────────────────────────────────────
// Instruções internas da IA para que ela pense passo a passo
// NUNCA é exibido ao usuário — apenas usado internamente para melhorar qualidade
...
// ─────────────────────────────────────────────────────────────────────────────
// ESTADO GLOBAL DO APRENDIZADO AUTÔNOMO
// ─────────────────────────────────────────────────────────────────────────────

let isAutonomousLearningActive = false;        // Flag: loop de aprendizado está ativo?
let autonomousLearningInterval = null;         // ID do setInterval para limpeza posterior
...
async function generateLocalStreamingResponse(message, onStep) {
  ...
  return { reply:`<b>[Analyiz — Modo Autônomo]</b><br><br>${content}<br><br><i>Confiança local: ${confidence}%</i>`, sources:[], reasoning, toolsExecutadas:[], sugestoes:[] };
}
...
async function processarLivroEAprender(fileContexts, onStep) {
  ...
  return { reply:`<b>🧠 Aprendizagem concluída!</b><br><br>• Fragmentos absorvidos: <b>${total}</b><br>• Disponível no Modo Autônomo.`, sources:[], reasoning, toolsExecutadas:[], sugestoes:[] };
}
...
export async function buildAnswerStream(message, history = [], context = {}, onStep, attempt = 1) {
  ...
  if (isOwner && message) {
    ...
    if (ml.match(/iniciar aprendizado/)) {
      ...
      return { reply:'<b>🧠 Aprendizado Autônomo Iniciado.</b>', sources:[], reasoning:'', toolsExecutadas:[], sugestoes:[] };
    }
    if (ml.match(/parar aprendizado/)) {
      isAutonomousLearningActive = false;
      clearTimeout(autonomousLearningInterval);
      return { reply:'<b>🛑 Aprendizado Autônomo Interrompido.</b>', sources:[], reasoning:'', toolsExecutadas:[], sugestoes:[] };
    }
    if (ml.match(/desativar(\s+o|\s+a)?\s+gemini/)) {
      await prisma.usuario.update({ where:{ id:parseInt(userId) }, data:{ geminiAtivo:false } });
      return { reply:'<b>Gemini desativado.</b><br>Operando no Modo Autônomo Local.', sources:[], reasoning:'', toolsExecutadas:[], sugestoes:[] };
    }
    if (ml.match(/ativar(\s+o|\s+a)?\s+gemini/)) {
      await prisma.usuario.update({ where:{ id:parseInt(userId) }, data:{ geminiAtivo:true } });
      return { reply:'<b>Gemini reativado.</b><br>Capacidade total restaurada.', sources:[], reasoning:'', toolsExecutadas:[], sugestoes:[] };
    }
  }
  ...
  try {
    ...
    // Aqui acionamos o orquestrador que agora GERA organicamente o raciocínio E as sugestões
    ...
    return { 
        ...
        suggestions: runResult.sugestoes // Retorna as sugestões criadas
    };

  } catch (error) {
    if (error.message === 'AbortError') return { reply:'⚠️ Geração interrompida pelo usuário.', sources:[], reasoning:null, toolsExecutadas:[], suggestions:[] };
    ...
    return { reply:'⚠️ Erro no processamento. Tente novamente.', sources:[], reasoning:null, toolsExecutadas:[], suggestions:[] };
  }
}
