  ...
  try {
    ...
    if (pageKey === 'divergencias' || pageKey === 'dashboard') {
      const divs = await prisma.divergencia.findMany({
        where: { resolvido: false }, orderBy: { createdAt: 'desc' }, take: 5
      });
      if (divs.length > 0) {
        contextLines.push(`Divergências abertas: ${divs.length}`);
        divs.forEach(d => contextLines.push(`  • ${d.mlItemId}: ${d.motivo}`));
      } else {
        contextLines.push('Nenhuma divergência aberta.');
      }
    }
