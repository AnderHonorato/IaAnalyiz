...
import pkg from '@google/genai'; // Importação do pacote completo para compatibilidade
const { GoogleGenAI } = pkg;   // Extração manual da classe para evitar TypeError
...
// ==========================================
// IA ANALYIZ (CORREÇÃO DE MODELO E CONTEXTO)
// ==========================================
...
app.post('/api/ia/chat', async (req, res) => {
  ...
  try {
    // 1. Resposta para usuários bloqueados
    ...
    const systemInstruction = `
      Você é a "IA Analyiz", inteligência artificial corporativa de alto nível.
      Você trabalha para o Criador "Ander".
      Dados atuais: ${produtos.length} produtos e ${divergencias.length} divergências ativas.
      Lista de Problemas:
      ${listaDiv}
      Regras: Seja profissional e direta. Use Markdown.
    `;

    // 3. Instanciação segura da IA
    ...
    const result = await chat.sendMessage(`INSTRUÇÃO DO SISTEMA: ${systemInstruction}\n\nPERGUNTA DO USUÁRIO: ${message}`);
