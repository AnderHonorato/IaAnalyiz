...
import iaProativaRoutes, { limparNotificacoesAntigas } from './routes/iaProativaRoutes.js'; // ← NOVO (deve vir ANTES de iaRoutes)
...
// ── Routers ───────────────────────────────────────────────────────────────────
// ORDEM IMPORTA:
//   iaProativaRoutes ANTES de iaRoutes — evita que /api/ia/proactive
//   seja capturada por uma rota genérica do iaRoutes
...
app.use(iaProativaRoutes);                  // /api/ia/proactive, /api/ia/proactive/seen, /api/ia/proactive/exibida  ← ANTES do iaRoutes
app.use(iaRoutes);                          // /api/ia/*, /api/chat/*
app.use('/api/ml/precos', mlPrecosRoutes);  // /api/ml/precos/*
app.use(sessaoRoutes);                      // /api/sessao/*
app.use(mlResearchRoutes);                  // /api/ml/research/:mlbId
app.use(mlAnunciosRoutes);                  // /api/ml/meus-anuncios
...
const server = app.listen(PORT, () => {
  ...
  console.log(`   Routers: auth | ml | divergencias | ia-proativa | ia | ml/precos | sessao | ml/research | ml/anuncios`);
  console.log('');
...
// Limpeza de sessões de usuário antigas — a cada 1h
setInterval(() => {
  limparSessoesAntigas().catch(() => {});
}, 60 * 60 * 1000);

// Limpeza de notificações proativas antigas (> 7 dias) — a cada 24h
setInterval(() => {
