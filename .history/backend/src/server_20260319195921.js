...
app.post('/api/auth/register', async (req, res) => {
  ...
  } catch (error) { res.status(500).json({ error: 'Erro ao registrar usuário.' }); }
});
...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    res.json({ 
      token, 
      user: { id: user.id, nome: user.nome, email: user.email, avatar: user.avatar, role: user.role, solicitouDesbloqueio: user.solicitouDesbloqueio } 
    });
...
app.put('/api/usuarios/:id/role', async (req, res) => {
  try {
    const { id } = req.params;
    const { role } = req.body;
    const updatedUser = await prisma.usuario.update({ where: { id: parseInt(id) }, data: { role, solicitouDesbloqueio: false } });
...
app.get('/api/divergencias', async (req, res) => {
  try {
    ...
    res.json(div);
  } catch (error) { res.status(500).json({ error: 'Erro ao buscar divergências.' }); }
});

// ==========================================
// IA ANALYIZ BLINDADA (MODELO ESTÁVEL)
// ==========================================
...
app.post('/api/ia/chat', async (req, res) => {
  ...
  try {
    if (userRole === 'BLOQUEADO') {
      ...
      return res.json({ reply: '🔒 **Acesso Restrito.** Atualmente seu perfil está sob análise do Criador. Por favor, utilize o botão **"Solicitar Acesso"** no menu superior para que eu possa liberar minhas funções de auditoria logística para você.' });
    }
    ...
    const listaDiv = divergencias.length > 0 
      ? divergencias.map(d => `- [${d.plataforma}] ID: ${d.mlItemId} | Motivo: "${d.motivo}"`).join('\n')
    ...
    const systemInstruction = `
      ...
      DADOS: ${produtos.length} produtos registrados e ${divergencias.length} divergências ativas.
      LISTA ATUAL: ${listaDiv}
      Regras: Seja profissional, analítica e direta. Use Markdown.
    `;

    const genAI = new GoogleGenAI(process.env.GEMINI_API_KEY);
    const model = genAI.getGenerativeModel({ model: "gemini-1.5-flash" }); // MODELO ESTÁVEL
    ...
    res.status(500).json({ reply: '⚠️ Erro Crítico: Kernel Neural instável. Tente novamente.' });
  }
});
...
app.post('/api/auth/forgot-password', async (req, res) => { res.json({ message: 'Código gerado nos logs.' }); });
app.post('/api/auth/reset-password', async (req, res) => { res.json({ message: 'Senha redefinida!' }); });

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => { console.log(`🚀 IA Analyiz API rodando na porta ${PORT}`); });