...
export async function refreshMlTokenIfNeeded(userId) {
  try {
    ...
    console.log(`[jobs.js] ML token refreshed for user ${userId}`);
    return true;
  } catch (e) { console.error(`[jobs.js] Failed to refresh token for user ${userId}:`, e.response?.data || e.message); return false; }
}
...
export async function iniciarAgendadorUsuario(userId) {
  ...
  try {
    ...
    console.log(`[jobs.js] Scheduler user ${userId} next run in ${Math.round(delay / 60000)} min`);
  ...
  } catch (e) { console.error('[jobs.js] Error in scheduler:', e.message); }
}
...
export async function iniciarTodosAgendadores() {
  try {
    ...
    console.log(`[jobs.js] Restored ${agendadores.length} scheduler(s)`);
  } catch (_) {}
}
...
export async function verificarExclusoesAgendadas() {
  try {
    ...
    for (const user of contas) {
      console.log(`[jobs.js] Deleting account ${user.email} (id: ${user.id})`);
      try {
        ...
        console.log(`[jobs.js] Account ${user.email} deleted successfully`);
      } catch (err) { console.error(`[jobs.js] Failed to delete account ${user.email}:`, err.message); }
    }
  } catch (e) { console.error('[jobs.js] Error in account deletion job:', e.message); }
}