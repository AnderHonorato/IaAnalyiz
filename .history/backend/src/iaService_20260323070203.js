// backend/src/iaService.js — Chain of Thought completo + reasoning em blocos estilo Gemini
...
// Streaming do reasoning: chars por pacote e delay entre eles
const REASONING_CHUNK_SIZE  = 5;   // chars por emit
const REASONING_CHUNK_DELAY = 16;  // ms → ~312 chars/s, visível e fluido

// ═══════════════════════════════════════════════════════════════════════════════
// CHAIN OF THOUGHT — System Prompt Principal
// Instrui o Gemini a pensar em voz alta antes de responder,
// exatamente como o Gemini 2.5 Flash faz nos prints
// ═══════════════════════════════════════════════════════════════════════════════
...
1. DETECÇÃO DE CONTEXTO E EMOÇÃO
   ...
   (raiva, urgência, confusão, alegria, tristeza, frustração, neutro)
   Se detectar emoção forte, NOMEIE e ajuste o tom da resposta.

2. CLASSIFICAÇÃO DO TIPO DE ENTRADA
   - CÓDIGO: análise de sintaxe → lógica → segurança
   - IMAGEM: detecção visual → contexto → insights
   - DOCUMENTO/DADOS: extração → correlação → síntese
   - TEXTO/PERGUNTA: intenção → contexto do sistema → resposta
   - SENTIMENTO FORTE: reconhecimento → empatia → solução
...
// ═══════════════════════════════════════════════════════════════════════════════
// GERADOR DE BLOCOS DE REASONING ESTILO GEMINI
// Gera o texto de raciocínio que será exibido no ThinkingPanel
// Formato: **Título**\n\nParágrafo de análise\n\n**Próximo Título**\n\nParágrafo...
// ═══════════════════════════════════════════════════════════════════════════════

// Detecta emoção da mensagem
...
function classifyInput(message, fileContexts) {
  if (fileContexts.length > 0) {
    ...
    if (hasImg)   return { type: 'IMAGEM',     label: 'Analisando Entrada Visual' };
    if (hasPdf)   return { type: 'DOCUMENTO',  label: 'Analisando Documento PDF' };
    if (hasExcel) return { type: 'DADOS',       label: 'Analisando Dados Estruturados' };
    if (hasAudio) return { type: 'ÁUDIO',       label: 'Processando Entrada de Áudio' };
    return { type: 'ARQUIVO', label: `Processando ${types.join(', ')}` };
  }

  const m = (message || '').toLowerCase();
  if (/```|function|const |let |var |import |class |def |<div|<button|\btsx\b|\bjsx\b|\bcomponent\b/i.test(message))
    return { type: 'CÓDIGO', label: 'Analisando Código-Fonte' };
  if (/divergen|peso|frete|auditoria|varredura|anúncio/i.test(m))
    return { type: 'LOGÍSTICA', label: 'Analisando Solicitação Logística' };
  if (/produto|sku|catálogo|estoque|kit/i.test(m))
    return { type: 'CATÁLOGO', label: 'Analisando Solicitação de Catálogo' };
  if (/usuário|acesso|aprovar|bloquear|permissão/i.test(m))
    return { type: 'USUÁRIO', label: 'Analisando Gestão de Usuários' };
  if (/preço|precific|valor|custo|faturamento/i.test(m))
    return { type: 'PREÇO', label: 'Analisando Dados de Preço' };
  if (/resumo|relatório|métricas|dashboard|visão geral/i.test(m))
    return { type: 'RESUMO', label: 'Gerando Visão Geral do Sistema' };

  return { type: 'TEXTO', label: 'Definindo a Intenção do Usuário' };
}

// Gera o texto de raciocínio completo em formato Gemini
// Retorna string com blocos **Título**\n\nParágrafo
function buildChainOfThoughtText(message, fileContexts, context, hasFiles) {
  const blocks = [];
  const role   = context.userRole || 'USUARIO';
  const user   = context.usuarioAtual?.nome || 'o usuário';
  const msg    = message || '';

  // ── Bloco 1: Emoção (se detectada) ──────────────────────────────────────
  ...
  // ── Bloco 2: Classificação da entrada ────────────────────────────────────
  const classification = classifyInput(msg, fileContexts);
  let classText = '';

  switch (classification.type) {
    case 'CÓDIGO':
      classText = `Identifiquei que a entrada contém código-fonte. Meu processo será: Etapa A (Sintaxe) — verificar erros de escrita e compatibilidade; Etapa B (Lógica) — mapear o fluxo de dados e identificar gargalos; Etapa C (Segurança) — verificar vulnerabilidades como vazamento de dados ou tratamento incorreto de erros. Manterei exatamente os nomes de variáveis e funções fornecidos pelo usuário.`;
      break;
    case 'IMAGEM':
      classText = `A entrada contém uma ou mais imagens. Vou executar: Etapa A (Detecção) — identificar todos os elementos visuais, textos e contexto; Etapa B (Interpretação) — analisar a relação entre os elementos e o que eles representam no contexto do sistema. A saída será uma descrição analítica com insights acionáveis.`;
      break;
    case 'DOCUMENTO':
    case 'DADOS':
      classText = `Há um documento ou planilha de dados para processar. Protocolo: Etapa A (Extração) — identificar métricas-chave e informações principais; Etapa B (Correlação) — cruzar os dados com o contexto do projeto de e-commerce e logística do usuário; Etapa C (Síntese) — produzir um resumo executivo com as descobertas mais relevantes.`;
      break;
    case 'ÁUDIO':
      classText = `A entrada é um arquivo de áudio. Vou transcrevê-lo fielmente e depois aplicar a análise de sentimento e intenção ao conteúdo transcrito para fornecer uma resposta contextualizada.`;
      break;
    case 'LOGÍSTICA':
      classText = `Identifico uma solicitação relacionada a divergências de peso, frete ou auditoria de anúncios. Vou consultar o estado atual das divergências no banco de dados, verificar os anúncios afetados e determinar o plano de ação mais eficiente — seja correção automática via API, ignorar ou corrigir manualmente.`;
      break;
    case 'CATÁLOGO':
      classText = `A intenção envolve o catálogo de produtos — SKUs, estoque, kits ou vínculos com o Mercado Livre. Preciso acessar os dados do catálogo para fornecer informações precisas sobre o estado atual dos produtos cadastrados.`;
      break;
    case 'USUÁRIO':
      if (role === 'OWNER' || role === 'ADMIN') {
        classText = `Solicitação de gerenciamento de usuários. O usuário possui permissão de nível ${role}, então posso executar ações administrativas como aprovação, bloqueio e alteração de roles. Verificarei a lista de pendentes se necessário.`;
      } else {
      ...
      break;
    case 'RESUMO':
      classText = `Pedido de visão geral ou dashboard. Compilarei as principais métricas do sistema: produtos cadastrados, divergências ativas por status, avisos do Mercado Livre, estado do agendador e status da conexão OAuth. Apresentarei de forma estruturada e com indicadores de saúde do sistema.`;
      break;
    default:
      classText = `Processando a mensagem: "${msg.substring(0, 100)}${msg.length > 100 ? '…' : ''}". Vou determinar se é necessário consultar dados do banco de dados, executar alguma ferramenta do sistema ou responder com base no conhecimento disponível.`;
  }

  blocks.push({ title: classification.label, text: classText });

  // ── Bloco 3: Verificação de dados e ferramentas ───────────────────────────
  let toolText = '';
  ...
  if (hasFiles && fileContexts.length > 0) {
    ...
    blocks.push({
      ...
      text:  `Os arquivos ${nomes} foram lidos com sucesso — ${totalChars.toLocaleString()} caracteres disponíveis para análise. O conteúdo extraído será o contexto principal da minha resposta, sobrepondo qualquer suposição que eu pudesse ter sem esses dados.`,
    });
  }

  // ── Bloco 5: Estratégia de resposta ──────────────────────────────────────
  let strategyText = '';
  const hasCode = /```/.test(msg) || classification.type === 'CÓDIGO';

  if (hasCode) {
    strategyText = `Vou entregar o código corrigido e otimizado de forma completa, sem elipses ou cortes. Manterei exatamente os nomes originais das funções, variáveis e classes. Após o código, incluirei uma explicação técnica clara do que foi alterado e por quê.`;
  } else if (emotion?.type === 'raiva' || emotion?.type === 'urgência') {
    strategyText = `Dado o tom da mensagem, vou ser direto e objetivo. Primeiro reconhecerei a situação, depois apresentarei a solução de forma concisa e acionável. Evitarei explicações excessivas que possam parecer patronizantes no contexto atual.`;
  } else {
  ...
  // ── Serializa em formato Gemini: **Título**\n\nParágrafo\n\n ────────────────
  return blocks.map(b => `**${b.title}**\n\n${b.text}`).join('\n\n');
}

// ═══════════════════════════════════════════════════════════════════════════════
// EMISSÃO DO REASONING EM CHUNKS (streaming letra por letra)
// ═══════════════════════════════════════════════════════════════════════════════
...
async function emitReasoningStreamed(onStep, text) {
  ...
  // Emite em pequenos chunks para criar o efeito de digitação progressiva
...
// ═══════════════════════════════════════════════════════════════════════════════
// PAGE CATALOG / ROLES / TOOLS (sem alteração)
// ═══════════════════════════════════════════════════════════════════════════════
...
async function extractCodeBlocksAndSave(sessionId, replyText, userMessage) {
  ...
  while ((match = regex.exec(replyText)) !== null) {
    ...
    const base = LANG_NAMES[lang] || 'codigo';

    // Gera nome único baseado na linguagem
...
async function extractExcel(base64, mimeType, fileName) {
  try {
    ...
    if (mimeType === 'text/csv') {
      ...
      const r    = await ai.models.generateContent({ model:GEMINI_MODEL, config:{temperature:0}, contents:[{ role:'user', parts:[{ text:`Analise este CSV: ${fileName}\n\n${text}` }] }] });
      return r.text?.trim() || text;
    }
    const r = await ai.models.generateContent({ model:GEMINI_MODEL, config:{temperature:0}, contents:[{ role:'user', parts:[{ text:`Extraia as tabelas: ${fileName}` },{ inlineData:{ mimeType, data:base64 } }] }] });
    return r.text?.trim() || null;
  } catch { return null; }
}
...
// ─── System Instruction — SKILL.md + CoT + contexto da sessão ─────────────────
function buildSystemInstruction(ctx) {
  const {
    totalProdutos=0, totalDivergencias=0, userRole='USUARIO',
    usuarioAtual=null, dataBlock=null, fileContexts=[], pageBaseUrl=null,
  } = ctx;
...
${fileBlock}=== FIM DO CONTEXTO ===`;
  ...
  // Ordem: SKILL → Chain of Thought → Contexto da sessão
  const parts = [];
...
async function generateLocalStreamingResponse(message, onStep) {
  ...
  const reasoning = buildChainOfThoughtText(message, [], { userRole:'USUARIO', usuarioAtual:{nome:'Usuário'} }, false)
    + `\n\n**Modo Autônomo Ativo**\n\nAPI Gemini desativada. Acessando banco vetorial local (PostgreSQL RAG). Encontrei ${best.length} resultado(s) com ${confidence}% de confiança.`;
  ...
  return {
    ...
    sources:[], reasoning, toolsExecutadas:[],
  };
}

// ─── Aprendizado de livros ─────────────────────────────────────────────────────
async function processarLivroEAprender(fileContexts, onStep) {
  const reasoning = `**Processando Comando de Aprendizagem**\n\nDetectei o comando de ingestão profunda. Iniciarei a fragmentação do conteúdo dos ${fileContexts.length} arquivo(s) em blocos de 2000 caracteres cada um, que serão indexados individualmente no banco de dados vetorial PostgreSQL para uso futuro no Modo Autônomo.`;
  await emitReasoningStreamed(onStep, reasoning);
...
export async function buildAnswerStream(message, history = [], context = {}, onStep, attempt = 1) {
  ...
  try {
    ...
    // Imagem principal já analisada
    if (context.imageContext) fileContexts.push({ label:'imagem', name:'imagem enviada', group:'image', context:context.imageContext });
