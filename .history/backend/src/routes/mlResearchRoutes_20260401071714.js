...
// Agente de pesquisa com logs SSE no terminal e Fallback Antibot em Cascata (Puppeteer)

import express from 'express';
import axios   from 'axios';
import { PrismaClient } from '@prisma/client';
import puppeteer from 'puppeteer'; // NOVO: Importação do navegador headless
...
// ─── Função Auxiliar de Scraping Avançado (Puppeteer) ────────────────────────
async function fetchWithPuppeteer(url, debug) {
  ...
  try {
    ...
    // Comportamento humano: tempo de espera aleatório antes de avançar
...
async function buscarVendedoresCatalogo(catalogId, api, debug) {
  ...
  for (const url of urls) {
    try {
      ...
      // Detecção de Bloqueio WAF / Captcha do ML
...
async function buscarViaScraping(url, mlbId, debug) {
  ...
  try {
    ...
    html = res.data || '';
    if (html.includes('captcha') || html.includes('Verifica que eres un ser humano') || html.length < 10000) {
        debug.push(`⚠️ WAF/Captcha detectado. Alternando para navegador real...`);
        html = await fetchWithPuppeteer(targetUrl, debug);
    }
  } catch(e) {
     debug.push(`⚠️ Falha no Axios HTTP (${e.message?.substring(0,40)}). Alternando para navegador real...`);
     html = await fetchWithPuppeteer(targetUrl, debug);
  }
