...
async function geminiQuick(prompt, maxTokens = 300) {
...
async function routeDataNeeded(userMessage, userId, userRole) {
  ...
  // Monta o prompt em array para evitar problemas de escape
  const promptParts = [
    'Você é um roteador de dados. Responda SOMENTE com JSON puro, sem texto extra, sem markdown, sem backticks.',
    '',
    ...
    '',
    'REGRAS:',
    '- Se a mensagem precisar de dados do sistema, responda: {"relevante":true,"tabelas":["chave1"],"raciocinio":"motivo"}',
    '- Se não precisar, responda: {"relevante":false,"tabelas":[],"raciocinio":"pergunta geral"}',
    '- Máximo 3 tabelas. Use exatamente as chaves listadas acima.',
    '- Nenhum texto fora do objeto JSON.',
  ];
  const prompt = promptParts.join('\n');

  try {
    const raw = await geminiQuick(prompt, 300);

    // 1. Remove blocos de markdown (```json ... ``` ou ``` ... ```)
    let cleaned = raw.replace(/```(?:json)?[\s\S]*?```/g, m =>
      m.replace(/```(?:json)?/g, '').replace(/```/g, '')
    ).trim();

    // 2. Extrai apenas o primeiro objeto JSON encontrado no texto
    const jsonMatch = cleaned.match(/\{[\s\S]*\}/);
    if (!jsonMatch) throw new Error('Nenhum objeto JSON encontrado na resposta');

    let jsonStr = jsonMatch[0];

    // 3. Remove trailing commas antes de } ou ] (JSON inválido comum do Gemini)
    jsonStr = jsonStr.replace(/,\s*([\}\]])/g, '$1');

    // 4. Remove comentários estilo // ... (às vezes o modelo adiciona)
    jsonStr = jsonStr.replace(/\/\/[^\n]*/g, '');

    const parsed = JSON.parse(jsonStr);

    // 5. Normaliza a estrutura garantindo os campos esperados
    return {
      relevante:  Boolean(parsed.relevante),
      tabelas:    Array.isArray(parsed.tabelas) ? parsed.tabelas : [],
