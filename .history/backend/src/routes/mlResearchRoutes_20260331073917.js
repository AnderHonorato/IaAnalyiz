...
// Ativa o plugin Stealth para mascarar o navegador e evitar o Captcha do ML
...
function extrairJSON(html, target) {
  ...
  if (start === -1) start = startArr;
  else if (startArr !== -1 && startArr < start) start = startArr;
...
function extrairVendedoresDoHTML(html, targetUrl, paginaAtual, debug) {
  ...
  if (jsonStr) {
      try {
          ...
          debug.push(`  🧠 JSON extraído com sucesso!`);
      } catch (e) {}
  }
              ...
              if (arr[0].components) {
                  ...
                  arr.forEach((item, index) => {
                      ...
                      if (preco > 0 || nome !== '—') vendedores.push({
                          ...
                          link, desconto, thumbnail: thumb, titulo: null, freteGratis, envio,
...
// ============================================================================
// FUNÇÃO ORIGINAL RESTAURADA: buscarVendedoresCatalogo
// ============================================================================
async function buscarVendedoresCatalogo(catalogId, urlBase, api, debug) {
  debug.push(`🏷️  Catálogo ID: ${catalogId}`);
  
  // 1. Scraping Direto (O único que traz os concorrentes de forma confiável no novo layout)
  const cleanId = catalogId.replace(/^MLB/i, '');
  const url = `https://www.mercadolivre.com.br/p/MLB${cleanId}/s`;
  
  try {
      debug.push(`🌐 Scraping de opções de compra: ${url}`);
      
      let res = await scraperAgent.get(url);
      let html = res.data || '';
      debug.push(`   → HTML recebido via Axios: ${(html.length/1024).toFixed(0)}KB`);
      
      // NOTA: Passando TODOS os 4 argumentos agora para não quebrar a função!
      let vendedores = extrairVendedoresDoHTML(html, url, 1, debug);
      
      if (vendedores.length === 0 || html.includes('Captcha') || html.includes('human')) {
          debug.push(`   ⚠️ Axios falhou ou pegou Captcha. Tentando Puppeteer Stealth...`);
          let browser;
          try {
              ...
              debug.push(`   ✅ HTML obtido via Puppeteer Stealth: ${(html.length/1024).toFixed(0)}KB`);
              vendedores = extrairVendedoresDoHTML(html, url, 1, debug);
          } catch (err) {
      ...
      if (vendedores.length > 0) {
          debug.push(`✅ ${vendedores.length} vendedor(es) extraído(s) via scraping do catálogo`);
      ...
      debug.push(`❌ Scraping Catálogo falhou: ${e.message?.substring(0,60)}`);
  }

  // 2. Busca API por ID (Fallback da versão v5 original)
  try {
...
async function buscarItem(mlbId, url, api, debug) {
  ...
  if (ehCatalogo) {
    ...
    debug.push(`📦 Produto de CATÁLOGO detectado. Buscando opções...`);
    concorrentes = await buscarVendedoresCatalogo(catalogId, url, api, debug);
  } else {
    debug.push(`🏪 Anúncio normal. Buscando concorrentes por título...`);
    try {
...
async function buscarViaScraping(urlOriginal, mlbId, debug) {
  ...
  while (page <= 40 && errorsConsecutivos < MAX_ERRORS_CONSECUTIVOS) {
    ...
    if (!html || html.includes('Verifique se você é humano') || html.includes('Captcha')) {
      ...
      try {
        ...
        const isCaptcha = await pageObj.evaluate(() => document.title.includes('Captcha') || document.body.innerHTML.includes('Verifique se você é humano'));
        ...
        if (!isCaptcha) {
            ...
            debug.push(`  ❌ Puppeteer também caiu no Captcha.`);
            html = null; 
        }
        await browser.close();
      } catch (e) {
    ...
    if (html) {
      ...
      // IMPORTANTE: Chamando a função extração com a URL da página e a Pagina Atual!
  ...
  if (todosVendedores.length === 0) throw new Error('Nenhum vendedor encontrado por nenhuma das estratégias de scraping.');
...
router.get('/api/ml/research/:mlbId', async (req, res) => {
  ...
  // Limpeza de dupla codificação (Resolve o erro do link sujo gerando 404)
  if (urlOriginal) {
  ...
  if (urlOriginal) {
      try {
          const dados = await buscarViaScraping(urlOriginal, mlbId, debug);
