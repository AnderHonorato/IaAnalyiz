// backend/src/routes/mlResearchRoutes.js — v8
// Correção: Matemática de Paginação Virtual (10 itens por página) preservando requisições em lote de 50 para alta performance.
...
// ─── Lógica Corrigida: Paginação Virtual (Limit=50 para speed, Math=10 para UI) ─────────────
async function buscarVendedoresCatalogo(catalogId, api, debug) {
  ...
  try {
    let offset = 0;
    const limit = 50; // Voltamos para 50 para o Bot voar
    let hasMore = true;
    ...
    while (hasMore && offset < 1000) {
      ...
      for (let i = 0; i < results.length; i++) {
        ...
        // Matemática Perfeita de Paginação
  ...
  // TENTATIVA 2: Busca por catalog_product_id (Paginação Virtual)
  try {
    let offset = 0;
    const limit = 50;
    let hasMore = true;
  ...
  for (const url of urls) {
    try {
      ...
      if (vendedores.length) {
        ...
        // Aqui o array "paginas" é calculado a partir dos vendedores no HTML
...
async function buscarItem(mlbId, url, api, debug) {
  ...
  if (ehCatalogo) {
    ...
    // Para buscas simples (Não-Catálogo), faremos o mesmo: Baixa 50 de vez, mas calcula página a cada 10
