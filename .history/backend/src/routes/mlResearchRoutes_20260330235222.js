...
// ATUALIZADO: Fuga de Bloqueios (Cloudflare) priorizando a API paginada.
...
// ============================================================================
// BUSCADOR ROBUSTO API (VARRER TODAS AS PÁGINAS SEM TOMAR BLOCK CLOUDFLARE)
// ============================================================================
...
// ============================================================================
// MOTOR DE SCRAPING HTML DA BUYBOX (CATÁLOGO) E BUSCA ITERATIVA WEB
// ============================================================================
...
function extrairVendedoresDoHTML(html, targetUrl, debug) {
  ...
  // Fallbacks de segurança em SEO Tags
...
async function buscarViaScraping(url, mlbId, debug) {
  ...
  while(continuarScrap && paginaScrap <= 40) { 
    // Usando quantity=1&page=X como você solicitou na sua requisição
    ...
    try {
      ...
      if (paginaScrap === 1) {
        ...
        if (!tituloBase) tituloBase = `Catálogo ID ${mlbId}`;
      }
      
      const vendedoresPagina = extrairVendedoresDoHTML(html, urlPage, debug);
      
      if (vendedoresPagina.length > 0) {
        // TRAVA ANTI-LOOP: Se o ML jogar a gente de volta para a primeira página por falta de anúncios
       ...
       debug.push(`⚠️ Parando Scraping na pág ${paginaScrap}: Erro HTTP (Ex: 404 da página vazia)`);
       continuarScrap = false;
    }
  }
  
  if (todosVendedoresScrap.length === 0) throw new Error('Falha completa ao tentar extrair dados por Scraping iterativo');
...
async function buscarViaProducts(mlbId, api, debug) {
  ...
  let p;
  try {
    const res = await axios.get(`https://api.mercadolibre.com/products/${mlbId}`, { timeout: 10000 });
    p = res.data;
  } catch (e) {
    const res = await api.get(`/products/${mlbId}`, { timeout: 10000 });
    p = res.data;
  }
...
router.get('/api/ml/research/:mlbId', async (req, res) => {
  ...
  if (urlOriginal) {
    try {
      ...
      await salvarHistorico(userId, mlbId, urlOriginal, dados, null);
      return res.json({ ...dados, debug });
    } catch (eC) { debug.push(`❌ Scraping Paginado falhou: ${eC.message?.substring(0,60)}`); }
  }
