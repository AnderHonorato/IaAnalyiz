...
function formatarEnvio(shippingInfo, hasFulfillment) {
  // Full = quando o ML armazena e envia
  if (hasFulfillment 
    || shippingInfo?.logistic_type === 'fulfillment'
    || shippingInfo?.tags?.includes('fulfillment')
    || shippingInfo?.tags?.includes('self_service_in')  // também indica Full
  ) return 'Full';
  
  if (shippingInfo?.free_shipping 
    || shippingInfo?.shipping_conditions === 'free_gap'
    || shippingInfo?.shipping_conditions === 'free_ratio'
  ) return 'Mercado Envios (Grátis)';  // ou apenas 'Mercado Envios' + flag gratis
