// backend/src/routes/iaRoutes.js — v6.2: Correção do caminho do iaBrain
...
} from '../ia/brain/iaBrain.js'; // Caminho corrigido para a pasta correta
  ...
  try{
    res.json(await prisma.chatMessage.findMany({
      ...
      select: {
        ...
        role:        true,
        content:     true,
        reasoning:   true,     // ← ÚNICA LINHA ADICIONADA
        imageBase64: true,
        imageDesc:   true,
        createdAt:   true,
      },
...
// ─── NOVO: Gerenciamento de Documentos da Sessão (Visualizador de Códigos) ────
...
router.post('/api/chat/sessions/:id/documents', async (req, res) => {
  try {
    ...
    const doc = await prisma.chatDocument.create({
      data: { sessionId, filename, tipo, language, content, versao }
    });
    res.json(doc);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
  ...
  try {
    ...
    await prisma.chatMessage.create({
      data: {
        ...
        role:      'ia',
        content:   reply,
        reasoning: reasoning || null,   // ← ÚNICA LINHA ADICIONADA
      },
    });
    ...
    // ADICIONAMOS O SESSION ID AQUI PARA SALVAR OS DOCUMENTOS CORRETAMENTE!
