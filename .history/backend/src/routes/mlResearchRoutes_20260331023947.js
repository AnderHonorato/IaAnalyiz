...
import puppeteer from 'puppeteer'; // ← ADICIONADO: Importação do Puppeteer
...
/**
 * Formata envio EXATAMENTE como no catálogo:
 * Full | Mercado Envios (Grátis) | Mercado Envios | Grátis Parcelado
 */
...
/**
 * Extrai texto de vendas visível na página do ML.
 * O ML mostra texto como "+5 mil vendas", "+50 mil", "+1000 vendas" etc.
 */
function extrairVendasTexto(rawText) {
  if (!rawText) return null;
  const s = String(rawText).trim();
  // Já está formatado (+5mil vendas, +50mil vendas etc)
  if (s.startsWith('+') && s.includes('vendas')) return s;
  // Texto bruto do ML: "5 mil vendas" → "+5mil vendas"
...
// ============================================================================
// EXTRATOR DO HTML COM SUPORTE COMPLETO A VENDAS E ENVIO
// ============================================================================

function extrairVendedoresDoHTML(html, targetUrl, paginaAtual, debug) {
  const vendedores = [];

  // ── Estratégia 1: __PRELOADED_STATE__ (padrão principal do ML) ──────────
  const jsonPatterns = [
    ...
    try {
      ...
      for (const arr of paths) {
        ...
        arr.forEach((o, index) => {
          ...
          // ── Extrai vendas do texto visível (como no catálogo) ──────────
          let vendasStr = null;
          // Tenta fontes diretas primeiro
          ...
          // ── Envio formatado igual ao catálogo ──────────────────────────
  ...
  // ── Estratégia 2: Extração via JSON-LD (schema.org) ─────────────────────
  ...
  // ── Estratégia 3: Regex direto no HTML (fallback agressivo) ─────────────
  if (vendedores.length === 0) {
    // Tenta capturar blocos de preço + vendedor do HTML renderizado
...
/**
 * Busca com MÚLTIPLAS estratégias anti-bloqueio rotativas.
 * Usa a URL exata com paginação igual ao catálogo do usuário.
 */
async function buscarViaScraping(urlOriginal, mlbId, debug) {
  // Normaliza a URL base do produto (sem query string)
  let baseUrl = urlOriginal.split('?')[0];
  if (!baseUrl.startsWith('http')) {
    baseUrl = `https://www.mercadolivre.com.br/p/${mlbId}/s`;
  }
  // Garante terminação /s (padrão catálogo)
  ...
  while (page <= 40 && errorsConsecutivos < MAX_ERRORS_CONSECUTIVOS) {
    // ── URL de paginação EXATA igual ao catálogo ──────────────────────────
    const urlPage = `${baseUrl}?page=${page}`;
    debug.push(`📡 Pág ${page}: ${urlPage}`);

    // Tenta com múltiplas estratégias anti-bloqueio
    let html = null;
    let estrategiaUsada = null;

    // ESTRATÉGIA A: Scraper humanizado com UA rotativo
    ...
    // ESTRATÉGIA B: URL com ?quantity=1 (formato legado ML)
    if (!html) {
      try {
    ...
    // ESTRATÉGIA C: Mobile UA (ML serve HTML diferente para mobile, menos protegido)
    if (!html) {
      try {
    ...
    // ESTRATÉGIA D: API pública do ML (sem autenticação, JSON)
    if (!html) {
      try {
    ...
    // ESTRATÉGIA E: URL alternativa sem /s + cookie simulado
    if (!html) {
      try {
    ...
    // ── Processa o HTML se obtido ─────────────────────────────────────────
    if (html) {
      errorsConsecutivos = 0;
      debug.push(`  ✅ HTML obtido via: ${estrategiaUsada} (${html.length} chars)`);

      // Extrai dados básicos do produto na pág 1
      if (page === 1) {
      ...
      // Todas as estratégias falharam nesta página
      errorsConsecutivos++;
  ...
  return {
    ...
    // Todas as páginas coletadas (para filtro no frontend)
    paginasColetadas: [...new Set(todosVendedores.map(v => v.pagina))].sort((a,b) => a-b),
  };
}

// ============================================================================
// ADICIONADO: ESTRATÉGIA F (PUPPETEER FALLBACK)
// ============================================================================
async function buscarViaPuppeteer(urlOriginal, mlbId, debug) {
  ...
  try {
    ...
    if (!productData.titulo) {
        throw new Error('Anúncio não carregou corretamente no Puppeteer.');
    }
...
router.get('/api/ml/research/:mlbId', async (req, res) => {
  ...
  if (urlOriginal) {
    ...
    try {
      ...
      // ← ADICIONADO AQUI: Fallback com Puppeteer antes de devolver o 404
      try {
