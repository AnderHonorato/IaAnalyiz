...
// Atualizado: Links de perfil corretos (loja/ ou pagina/), link de anuncio sempre usa catalogId pai,
// vendas formatadas corretamente em todos os fluxos, estrategias em cascata robustas.
...
function formatarVendasApi(completed) {
  ...
  return String(completed) + ' vendas';
}
 ...
 *
 * @param {string} itemId    - ID individual do item (ex: MLB4069811147)
 * @param {string} catalogId - ID do catalogo pai (ex: MLB16061588) - tem prioridade
 */
...
/**
 * Link do PERFIL do vendedor — construido a partir do nickname da API.
 *
 * Regras do ML:
 *   - Com selo azul (loja oficial): https://www.mercadolivre.com.br/loja/{slug}
 *     Ex: "Tudo Forte" -> /loja/tudo-forte
 *   - Sem selo (vendedor comum):    https://www.mercadolivre.com.br/pagina/{slug}
 *     Ex: "GarciaJundiaId" -> /pagina/garciajundiaid
 *
 * A API retorna URLs como "https://perfil.mercadolivre.com.br/NICK" — IGNORAR.
 * Sempre reconstruimos a URL a partir do nickname + tags do usuario.
 *
 * @param {string}   nickname - ex: "Tudo Forte", "DJSECURITY"
 * @param {string[]} userTags - s.tags da API (ex: ["brand", "certified_user"])
 */
function buildPerfilVendedor(nickname, userTags) {
  if (!nickname) return '#';
  ...
  // Determina se e loja oficial pelo campo tags da API
  const isLojaOficial = tags.some(t => ['brand', 'official_store', 'meli_choice'].includes(t));
  const tipo = isLojaOficial ? 'loja' : 'pagina';

  return 'https://www.mercadolivre.com.br/' + tipo + '/' + slug;
}
...
// Extrai: nickname, tags (loja/pagina), vendas, mercadoLider, permalink correto
// =============================================================================
...
async function getSellerFullCached(sellerId, api) {
  ...
  try {
    ...
    // Vendas: transactions.completed e o campo oficial que o ML usa para "+10mil vendas"
    const completed  = s.seller_reputation && s.seller_reputation.transactions
      ? (s.seller_reputation.transactions.completed || 0) : 0;
    const vendasNum  = completed;
    const vendasStr  = formatarVendasApi(completed);

    // Mercado Lider: power_seller_status
    ...
    // Perfil: reconstroi usando nickname + tags — NUNCA usa o permalink da API
    // (a API retorna "perfil.mercadolivre.com.br/NICK" que e invalido)
    const nome     = s.nickname || s.first_name || String(sellerId);
    const permalink = buildPerfilVendedor(nome, s.tags || []);
...
function montarConcorrente(opts) {
  ...
  return {
    ...
    // Link sempre aponta para o catalogo pai, nunca para o item individual
...
// catalogId e passado para buildItemLink usar o ID correto
// =============================================================================
    ...
    try {
      ...
      if (options.length > 0) {
        options.forEach((o, i) => {
          ...
          else if (badge.toLowerCase().includes('lider') || badge.toLowerCase().includes('lider')) mercadoLider = 'MercadoLider';

          // Perfil: reconstroi a partir do nome, nunca usa "perfil.mercadolivre"
          let permalink = (o.sellerInfo && o.sellerInfo.permalink) ? o.sellerInfo.permalink : '';
          if (!permalink || permalink.includes('perfil.mercadolivre')) {
            permalink = buildPerfilVendedor(nome, []);
          }
  ...
  if (blocks.length > 1) {
    for (let i = 1; i < blocks.length; i++) {
      ...
      if (sellerMatch) {
        ...
        // Corrige URLs "perfil.mercadolivre.com.br" — reconstroi
        linkLoja = rawLink.includes('perfil.mercadolivre') ? buildPerfilVendedor(nome, []) : rawLink;
      }
      ...
      if (preco > 0 || nome !== '—') {
        vendedores.push({
          ...
          perfilLoja: linkLoja || buildPerfilVendedor(nome, []),
    ...
    try {
      ...
      while (hasMore && offset < 150) {
        ...
        for (let i = 0; i < results.length; i++) {
          ...
          // Anuncio classico: nao tem catalogId pai — link aponta para o item individual
