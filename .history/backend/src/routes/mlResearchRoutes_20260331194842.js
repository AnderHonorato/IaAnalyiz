...
// Atualizado: Extrator Recursivo Universal de JSON e Simulação Humana Avançada
...
function lerCaptura(mlbId, pagina, debug) {
    try {
        ...
        if (fs.existsSync(filepath)) {
            ...
            // Rejeita cache sujo (Captcha ou Login)
...
// ============================================================================
// POOL DE USER-AGENTS REAIS E RESOLUÇÕES (Simulação Múltiplos Browsers)
// ============================================================================
const UA_POOL = [
...
async function simularHumano(page, debug) {
    try {
        ...
        // Movimento errático do mouse em curvas
  ...
  if (Array.isArray(obj)) {
    if (obj.length > 0) {
      ...
      if (first && typeof first === 'object') {
          // Detecta NOVO layout (Components Array)
          if (Array.isArray(first.components)) {
            results.push({ type: 'components', data: obj });
          }
          // Detecta layout TRADICIONAL (Flat Array com dados diretos na raiz)
...
function extrairVendedoresDoHTML(html, targetUrl, paginaAtual, debug) {
  ...
  // 1. Extração Recursiva - Procura por todos os JSONs embutidos na página
  const allJsons = [
  ...
  // Add JSON.parse fallback que eles usam
  ...
  // Se não achar nada nas vars nativas, tenta o SEO JSON-LD
  ...
  // Processa as arrays encontradas (Para no primeiro array que tiver dados válidos)
  for (const arrObj of foundSellerArrays) {
      ...
      if (vendedores.length > 0) break; // Para na primeira array de sellers válida encontrada
  }
...
// ============================================================================
async function buscarVendedoresCatalogo(catalogId, urlBase, api, debug) {
  ...
  while (page <= 40 && errorsConsecutivos < MAX_ERRORS_CONSECUTIVOS) {
      ...
      if (!html || html.includes('auth-login-frontend') || html.includes('suspicious') || html.includes('Verifique se você é humano')) {
          debug.push(`   ⚠️ Bloqueio detectado. Acionando Navegação Real (Puppeteer)...`);
          let browser;
          try {
  ...
  // 4. FALLBACK FINAL: API SEARCH (A versão v5 Original)
  try {
      debug.push(`📡 API: GET /sites/MLB/search?catalog_product_id=${catalogId}`);
      ...
      if (results.length > 0) {
          ...
          debug.push(`✅ ${vendedores.length} vendedor(es) resgatado(s) via API fallback`);
          return vendedores;
      }
  } catch (e) {}
...
async function buscarItem(mlbId, url, api, debug) {
  ...
  if (ehCatalogo) {
    ...
    concorrentes = await buscarVendedoresCatalogo(catalogId, url, api, debug);
  } else {
...
async function buscarViaProducts(mlbId, url, api, debug) {
  debug.push(`🔍 Tentando /products/${mlbId}...`);
  const res = await api.get(`/products/${mlbId}`, { timeout: 12000 });
  const p   = res.data;
  
  const concorrentes = await buscarVendedoresCatalogo(mlbId, url, api, debug);
...
async function buscarViaScrapingGeral(url, mlbId, debug) {
  ...
  let html = '';
  try {
    debug.push(`🌐 Scraping Geral Direto: ${targetUrl}`);
  ...
  if (!html || html.includes('auth-login-frontend') || html.includes('suspicious')) {
    debug.push(`🤖 Link direto bloqueado. Iniciando Navegação Real...`);
    let browser;
    try {
      const randomViewport = VIEWPORTS[Math.floor(Math.random() * VIEWPORTS.length)];
      browser = await puppeteer.launch({
...
router.get('/api/ml/research/:mlbId', async (req, res) => {
  ...
  try { 
      const session = await getMlApi(userId); 
      api = session.api;
  ...
  try {
      const dados = await buscarViaProducts(mlbId, urlOriginal, api, debug);
      await salvarHistorico(userId, mlbId, urlOriginal || mlbId, dados, null);
      return res.json({ ...dados, debug });
  } catch (eB) {
  ...
  if (urlOriginal) {
      try {
          const dados = await buscarViaScrapingGeral(urlOriginal, mlbId, debug);
