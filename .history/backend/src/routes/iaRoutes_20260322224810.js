...
// ─── Helper: monta contexto base para o buildAnswerStream ─────────────────────
...
// Envia eventos de progresso das tools em tempo real antes da resposta final.
// Eventos:
//   event: step  → { msg, tool } — uma tool está sendo executada
//   event: done  → { reply, sources, sessionId } — resposta final
...
router.post('/api/ia/chat/stream', async (req, res) => {
  ...
  try {
    ...
    let imageDesc = null;
    if (imageBase64) {
      send('step', { msg: '🖼️ Analisando a imagem enviada...' });
    ...
    const { reply, sources } = await buildAnswerStream(
      msgEfetiva,
      historyRaw,
      context,
      (stepEvent) => {
        // Repassa evento de progresso da tool para o frontend
...
// Mantido por compatibilidade. Usa processarMensagemComAprendizado (sem stream).
// ═══════════════════════════════════════════════════════════════════════════════
