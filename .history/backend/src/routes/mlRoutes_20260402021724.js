...
router.get('/api/ml/auth-url', (req, res) => {
  const state = req.query.userId 
    ? Buffer.from(String(req.query.userId)).toString('base64url')
    : '';
  
  res.json({ 
    url: `https://auth.mercadolivre.com.br/authorization?response_type=code&client_id=${process.env.ML_APP_ID}&redirect_uri=${encodeURIComponent(process.env.ML_REDIRECT_URI)}&state=${encodeURIComponent(state)}` 
  });
});
router.get('/api/ml/callback', async (req, res) => {
  const { code, state } = req.query;
  try {
    const userId = parseInt(Buffer.from(state, 'base64url').toString('utf-8'));
...
// ═════════════════════════════════════════════════════════════════════════════════
// ⏱️ AGENDADOR — Executar bot automaticamente em intervalos
// ═════════════════════════════════════════════════════════════════════════════════

// GET /api/agendador
// ─────────────────────────────────────────────────────────────────────────────
// Retorna configuração de agendamento do usuário: se está ativo, intervalo em
// minutos, quando foi última execução, quando é próxima execução.
//
// Query params:
//   @userId (required) - ID do usuário
//
// Response:
//   {
//     ativo: true,              // Bot agendado está ativo?
//     intervalo: 360,           // A cada quantos minutos executar (mínimo 30)
//     ultimaExecucao: "2024-03-15T10:30:00Z",
//     proximaExecucao: "2024-03-15T16:30:00Z"
//   }
//
// Padrão se não existir: { ativo: false, intervalo: 360, ultimaExecucao: null, proximaExecucao: null }
router.get('/api/agendador', async (req, res) => {
  try { 
    res.json(await prisma.agendadorConfig.findUnique({ where: { usuarioId: parseInt(req.query.userId) } }) || { ativo: false, intervalo: 360, ultimaExecucao: null, proximaExecucao: null }); 
  } catch { 
...
// PUT /api/agendador
// ─────────────────────────────────────────────────────────────────────────────
// Atualiza configuração de agendamento. Ativa/desativa bot automático e define
// intervalo entre execuções.
//
// Validações:
// • Intervalo mínimo = 30 minutos (proteção contra spam/sobrecarga)
// • Se ativo=false, proximaExecucao é setado para null
// • Se ativo=true, proximaExecucao = now + intervalo
// • Após atualizar, tenta iniciar agendador (TODO: implementar)
//
// Body:
//   @userId (required) - ID do usuário
//   @ativo (boolean) - Ativar ou desativar agendamento
//   @intervalo (number) - Minutos entre execuções (será validado min=30)
//
// Response:
//   Objeto agendadorConfig atualizado
//
// Exemplo body:
//   { userId: 1, ativo: true, intervalo: 120 }
router.put('/api/agendador', async (req, res) => {
  try {
    const { userId, ativo, intervalo } = req.body;
    const uid = parseInt(userId); 
    const intMin = Math.max(30, parseInt(intervalo) || 360);
    ...
    const config = await prisma.agendadorConfig.upsert({ 
      ...
      update: { 
        ativo, 
        intervalo: intMin, 
        proximaExecucao: ativo ? new Date(Date.now() + intMin * 60000) : null 
      }, 
      create: { 
        usuarioId: uid, 
        ativo, 
    ...
    // TODO: chamar iniciarAgendadorUsuario(uid) para ativar job
...
// ═════════════════════════════════════════════════════════════════════════════════
// 📦 ANÚNCIOS ML — Sincronização, listagem e atualização
// ═════════════════════════════════════════════════════════════════════════════════

// GET /api/anuncios
// ─────────────────────────────────────────────────────────────────────────────
// Busca e sincroniza anúncios do vendedor com Mercado Livre. Retorna lista
// tratada com ficha técnica completa (peso, dimensões, etc extraídos de
// attributes e shipping.dimensions).
//
// Fluxo:
// 1. Valida token ML e status de expiração
// 2. Chama /users/{sellerId}/items/search para listar IDs (max 50)
// 3. Calls /items?ids={...} (multiget) para buscar todos de uma vez
// 4. Processa cada item: extrai attributes, trata dimensões de shipping
// 5. Retorna array normalizado com campos: id, titulo, sku, preco, estoque,
//    ean, brand, modelo, weight (em gramas), dimensões, tipo (Premium/Clássico/Grátis)
//
// Estratégia de dimensões:
// • Primeiro tenta attributes: PACKAGE_WEIGHT, PACKAGE_HEIGHT, etc
// • Se não tiver, tenta shipping.dimensions (formato "HxLxCxP,W")
// • Normaliza unidades para: gramas (weight) e cm (dimensões)
//
// Query params:
//   @userId (required) - ID do usuário autenticado
//
// Response:
//   Array de anúncios tratados: [
//     {
//       id: "MLB123456789",
//       titulo: "Produto XYZ",
//       preco: 99.90,
//       estoque: 5,
//       peso: 1500,  (em gramas)
//       dimensoes: { altura: "10", largura: "5", comprimento: "20" },
//       tipoAnuncio: "Premium",
//       ...
//     }
//   ]
...
router.get('/api/anuncios', async (req, res) => {
  try {
    ...
    const anunciosTratados = itemsData.map(item => {
      ...
      // ─ Tentar extrair dimensões e peso dos attributes
      ...
      // ─ Fallback: tentar extrair de shipping.dimensions se attributes não tiver
...
// PUT /api/anuncios/:id
// ─────────────────────────────────────────────────────────────────────────────
// Atualiza anúncio: preço, estoque, e atributos de ficha técnica (EAN, marca,
// modelo, peso, dimensões). Valida token, busca anúncio atual para detectar
// variações, e dispara PUT para o ML com dados tratados.
//
// Lógica importante:
// • Se item tem VARIAÇÕES: atualiza todas as variações com mesmo preço/estoque
// • Se item simples: atualiza diretamente price e available_quantity
// • Atributos (GTIN, BRAND, MODEL, etc) sempre são setados no item principal
//
// Estratégia de erro:
// • Captura erros da API ML
// • Extrai "cause" array do response (detalhe de cada bloqueio)
// • Join múltiplos causes com " | " para exibir todos ao usuário
//
// Params:
//   @id - mlItemId do anúncio
//
// Body (pode ser body ou query, prefere body):
//   @userId - ID de autenticação
//   @preco (optional) - Novo preço (número)
//   @estoque (optional) - Nova quantidade (número)
//   @atributosFicha (optional) - Object com campos:
//     { ean, marca, modelo, peso, altura, largura, comprimento }
//
// Response (success):
...
//   500: Erro ao atualizar no ML (detalhe em error)
//
// Exemplo body:
//   {
//     userId: 1,
...
router.put('/api/anuncios/:id', async (req, res) => {
  try {
    ...
    // ─ Buscar item atual para verificar se tem variações
    const itemRes = await mlApi.get(`/items/${id}`);
    const itemData = itemRes.data;

    const updateData = {};
    
    // ─ Processar atributos da ficha técnica se fornecidos
    ...
    // ─ Processar preço e estoque (respeita variações se existirem)
    if (itemData.variations && itemData.variations.length > 0) {
      // ─ Atualizar em todas as variações
      ...
      // ─ Item simples, atualizar diretamente
...
// POST /api/ml/logout
// ─────────────────────────────────────────────────────────────────────────────
// Remove token ML do usuário (desconectar). Similar ao DELETE /disconnect mas
// como POST. Após isto, usuário precisa reconectar para usar endpoints de ML.
//
// Body ou query:
//   @userId - ID do usuário desconectando
//
// Response:
