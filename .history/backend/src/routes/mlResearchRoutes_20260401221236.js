 ...
 * 
 * Propósito:
 * Pesquisa e análise competitiva profunda de anúncios Mercado Livre.
 * Extrai dados de qualquer anúncio do ML e fornece inteligência de mercado.
 * 
 * Responsabilidades:
 * - POST /api/ml/research - Buscar anúncio por ID/URL
 * - GET /api/ml/research/:itemId - Detalhe completo do anúncio
 * - GET /api/ml/research/seller/:sellerId - Análise do vendedor
 * - GET /api/ml/competitors - Encontrar concorrentes por categoria
 * - POST /api/ml/price-history - Histórico de preços (mercado)
 * 
 * Extracção de Dados:
 * - Vendas: utiliza transactions.total (não .completed)\n *   - Formatação: '0 vendas', '+50 vendas', '+500 vendas', '+1000 vendas', etc
 *   - Mapeia intervalos segundo ML display logic
 * 
 * - Perfil de Vendedor:
 *   1. eshop.permalink (prioridade 1) - URL pronta da API
 *   2. GET /users/{id}/brands (se user_type=brand) - slug real
 *   3. Fallback: /pagina/slug-nickname (vendedores comuns)
 *   - NUNCA construir /loja/ a partir do nickname
 * 
 * - Link de Anúncio:
 *   - Sempre: http://mercadolivre.com.br/p/MLB{catalogId}/s
 *   - Aponta para página de catálogo pai, não para anúncio específico
 * 
 * Web Scraping:
 * - Puppeteer para dados não disponíveis em API
 * - Avaliações, comentários, imagens adicionais
 * - Rate limiting: 300-1000ms entre requisições
 * 
 * IA Integrada:
 ...
 * @version 2.0.0
 * @requires express, axios, puppeteer, @prisma/client
 */
