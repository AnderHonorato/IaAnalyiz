...
// ----------------------------------------------------------------------------
// EXTRATOR CIRÚRGICO DE JSON (COMPILADOR DE CHAVES {})
// ----------------------------------------------------------------------------
function extractJSONObj(html, keyword) {
  let idx = html.indexOf(keyword);
  if (idx === -1) return null;
  
  let start = html.indexOf('{', idx);
  let startArray = html.indexOf('[', idx);
  
  if (start === -1 && startArray === -1) return null;
  if (start === -1) start = startArray;
  else if (startArray !== -1 && startArray < start) start = startArray;
  ...
  for (let i = start; i < html.length; i++) {
    ...
    if (!inString) {
      if (char === '{' || char === '[') openBraces++;
        ...
        openBraces--;
        if (openBraces === 0) return html.substring(start, i + 1);
      }
    }
  }
  return null;
...
// ============================================================================
// NOVO EXTRATOR NATIVO (COMPATÍVEL COM NORDIC_RENDERING_CTX MINIFICADO)
// ============================================================================
function extrairVendedoresDoHTML(html, targetUrl, paginaAtual, debug) {
  const vendedores = [];
  let state = null;

  // Busca cirúrgica do JSON no HTML
  let jsonStr = extractJSONObj(html, '_n.ctx.r') || 
                extractJSONObj(html, '__NORDIC_RENDERING_CTX__') || 
                extractJSONObj(html, '__PRELOADED_STATE__');

  // Fallback para JSON.parse() embedado
  if (!jsonStr) {
    const matchParse = html.match(/__PRELOADED_STATE__\s*=\s*JSON\.parse\(['"](.*?)['"]\);/);
    if (matchParse) {
      jsonStr = matchParse[1].replace(/\\"/g, '"').replace(/\\\\/g, '\\').replace(/\\'/g, "'");
    }
  }
  ...
  if (jsonStr) {
    try {
      ...
      debug.push(`  🧠 Estado JSON extraído e decodificado com sucesso!`);
    } catch (e) {
      debug.push(`  ⚠️ Falha ao fazer parse do JSON isolado: ${e.message}`);
    }
  }
  ...
  if (state) {
    ...
    // TIPO 1: Padrão components array (Novo Catálogo do Mercado Livre)
    ...
    for (const arr of componentsPaths) {
      if (Array.isArray(arr) && arr.length > 0 && arr[0].components) {
        debug.push(`  🎯 Extraindo ${arr.length} opções (Novo Formato do Catálogo)`);
    ...
    // TIPO 2: Padrão Flat object (Listas tradicionais do ML)
    if (!itemsExtracted) {
      const flatPaths = [
  ...
  // Schema JSON-LD fallback de segurança (SEO)
  ...
  // Regex agressivo de última esperança
  if (vendedores.length === 0) {
...
async function buscarViaScraping(urlOriginal, mlbId, debug) {
  ...
  while (page <= 40 && errorsConsecutivos < MAX_ERRORS_CONSECUTIVOS) {
    ...
    if (!html) {
      try {
        ...
        const scraperMobile = axios.create({
          timeout: 18000,
          headers: {
            'User-Agent': mobileUA, 'Accept': 'text/html,application/xhtml+xml,*/*;q=0.9',
            'Accept-Language': 'pt-BR,pt;q=0.9', 'Accept-Encoding': 'gzip, deflate',
            'Referer': 'https://www.mercadolivre.com.br/', 'sec-ch-ua-mobile': '?1',
          },
          decompress: true,
        });
        ...
        if (res.data && res.data.results) {
          ...
          items.forEach((item, idx) => {
            todosVendedores.push({
              ...
              link: item.permalink || '',
              desconto: item.original_price && item.original_price > item.price ? `${Math.round((1 - item.price/item.original_price)*100)}% OFF` : '0% OFF',
    ...
    // ESTRATÉGIA F: Puppeteer Stealth Integrado
    ...
    if (html) {
      ...
      if (page === 1) {
        let jsonLdStr = extractJSONObj(html, 'type="application/ld+json"');
        if (jsonLdStr) {
          try {
