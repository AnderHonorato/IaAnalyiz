...
// Correção: Paginação exata de Catálogo (12 itens por página) para simular fielmente a UI do Mercado Livre e preencher o Excel.
...
// ─── CORREÇÃO AQUI: Paginação de 12 em 12 itens para simular a web ─────────────
async function buscarVendedoresCatalogo(catalogId, api, debug) {
  ...
  try {
    let offset = 0;
    const limit = 12; // <- O Segredo: Reduzido de 50 para 12 para contar as páginas perfeitamente
    let hasMore = true;

    while (hasMore && offset < 600) { // Máximo de 50 páginas de 12 itens
      const currPage = Math.floor(offset / limit) + 1;
      debug.push(`📡 API (Pág ${currPage}): GET /products/${catalogId}/items?limit=${limit}&offset=${offset}`);
      ...
      debug.push(`   → ${results.length} vendedor(es) retornado(s) na Página ${currPage}`);
      ...
      if (results.length < limit) {
        ...
        offset += limit;
        await sleep(500); // Pausa anti-rate-limit
      }
    }

    if (allVendedores.length > 0) {
      allVendedores.sort((a, b) => a.preco - b.preco);
      debug.push(`✅ ${allVendedores.length} vendedor(es) catalogado(s) em ${paginasColetadas.length} página(s)`);
  ...
  // TENTATIVA 2: Busca por catalog_product_id também padronizada para 12
  try {
    let offset = 0;
    const limit = 12;
    let hasMore = true;
    ...
    while (hasMore && offset < 600) {
      const currPage = Math.floor(offset / limit) + 1;
      debug.push(`📡 API (Pág ${currPage}): GET /sites/MLB/search?catalog_product_id=${catalogId}&limit=${limit}&offset=${offset}`);
      ...
      debug.push(`   → ${results.length} resultado(s) na Página ${currPage}`);
  ...
  // TENTATIVA 3: Scraping HTTP Fallback (Preservada)
...
async function buscarItem(mlbId, url, api, debug) {
  ...
  if (ehCatalogo) {
    ...
    debug.push(`📦 Produto de CATÁLOGO detectado. Buscando todas as opções com paginação...`);
    ...
    // Para buscas simples (Não-Catálogo) mantemos limit 50, pois a página de busca principal do ML é de 50
    debug.push(`🏪 Anúncio normal. Buscando concorrentes com paginação...`);
    try {
      ...
      while (hasMore && offset < 150) {
        ...
        for (let i = 0; i < results.length; i++) {
          ...
          concorrentes.push({
            ...
            freteGratis: r.shipping?.free_shipping === true, envio: r.shipping?.free_shipping ? 'Mercado Envios (Grátis)' : 'Mercado Envios',
            estoque: r.available_quantity, vendidos: r.sold_quantity, tipoAnuncio: mapTipo(r.listing_type_id),
            pagina: currPage, posicao: offset + i + 1
          });
        }
...
async function buscarViaScraping(url, mlbId, debug) {
  ...
  return {
    ...
    ehCatalogo: true, fonte: 'scraping', paginasColetadas: [1], analisadoEm: new Date().toISOString(), pagina: 1, posicao: 1
  };
}

// ── Histórico de pesquisas, IA e CRUD mantidos exatamente como estavam ─────────
