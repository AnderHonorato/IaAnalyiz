...
function lerCaptura(mlbId, pagina, debug) {
    try {
        ...
        if (fs.existsSync(filepath)) {
            ...
            // Não lê do cache se for a página de bloqueio/login
...
// ----------------------------------------------------------------------------
async function simularHumano(page) {
    try {
...
function extrairVendedoresDoHTML(html, targetUrl, paginaAtual, debug) {
  ...
  if (html.includes('auth-login-frontend') || html.includes('suspicious-traffic') || html.includes('Verifique se você é humano')) {
      debug.push(`  ❌ O HTML é a página de Login Forçado ou Captcha do ML.`);
      return vendedores; 
  }
  ...
  for (const m of ldMatches) {
      try {
          ...
          for (const o of offers) {
              ...
              if (preco > 0 || nome !== '—') vendedores.push({
                  nome, preco, link: o.url || targetUrl, desconto: '0% OFF', thumbnail: null, titulo: ld.name || null, freteGratis: false, frete: '—',
...
async function buscarVendedoresCatalogo(catalogId, urlBase, api, debug) {
  ...
  // GARANTE QUE A URL BASE NÃO PERCA OS ATRIBUTOS DE VARIAÇÃO SE EXISTIREM
  let url = urlBase;
  ...
  // Limpa o parâmetro 'page' e 'quantity' se já existirem na URL original para não duplicar
  ...
  while (page <= 40 && errorsConsecutivos < MAX_ERRORS_CONSECUTIVOS) {
      ...
      // 3. TENTA PUPPETEER SIMULANDO HUMANO (AQUECIMENTO + NAVEGAÇÃO)
      if (!html || html.includes('auth-login-frontend') || html.includes('suspicious') || html.includes('Verifique se você é humano')) {
          ...
          try {
              ...
              // TÉCNICA DE WARM-UP (Visita a Home do ML antes pra pegar cookies limpos e evitar 403)
      ...
      if (html && !html.includes('account-verification') && !html.includes('suspicious-traffic')) {
          ...
          // PROCESSA A PÁGINA USANDO A URL REAL DA PÁGINA E O NÚMERO
