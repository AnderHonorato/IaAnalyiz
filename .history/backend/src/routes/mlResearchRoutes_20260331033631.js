...
// ============================================================================
// ESTRATÉGIA F (PUPPETEER STEALTH) COM PROTEÇÃO ANTI-BOT
// ============================================================================
async function buscarViaPuppeteer(urlOriginal, mlbId, debug) {
  debug.push(`📍 Iniciando Puppeteer Stealth para: ${mlbId}`);
  let browser = null;
  try {
    browser = await puppeteer.launch({
        ...
        args: [
            ...
            '--disable-blink-features=AutomationControlled' // Impede que o ML saiba que é robô
        ]
    });
    ...
    debug.push(`⏳ Navegando para a página...`);
    await page.goto(urlOriginal, { waitUntil: 'networkidle2', timeout: 45000 });

    // Pequena pausa para garantir renderização do framework Javascript do ML
    await new Promise(r => setTimeout(r, 2000));

    // Proteção extra: Detectar se caiu no captcha de novo
    const isCaptcha = await page.evaluate(() => {
        return document.body.innerHTML.includes('Verifique se você é humano') ||
               document.body.innerHTML.includes('não é um robô') ||
               document.title.includes('Verificação') ||
               document.title.includes('Captcha');
    });

    if (isCaptcha) {
        throw new Error('Página bloqueada pelo Captcha (Mesmo usando Stealth).');
    }
    ...
    if (!productData.titulo) {
        throw new Error('Anúncio carregou, mas não encontrou o título no HTML.');
    }

    debug.push(`✅ Puppeteer Stealth extraiu: ${productData.titulo}`);
...
router.get('/api/ml/research/:mlbId', async (req, res) => {
  ...
  if (urlOriginal) {
    ...
    try {
      ...
      // Fallback com Puppeteer Stealth
      try {
        ...
        debug.push(`❌ Puppeteer Stealth também falhou: ${ePup.message}`);
      }
