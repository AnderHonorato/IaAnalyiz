...
// Configuração para salvamento local de capturas
...
function salvarCaptura(mlbId, pagina, conteudo, debug) {
    try {
        ...
        debug.push(`  💾 Captura salva em cache local (${filepath})`);
    } catch (e) {
...
function lerCaptura(mlbId, pagina, debug) {
    try {
        ...
        if (fs.existsSync(filepath)) {
            debug.push(`  📂 Lendo captura local de segurança...`);
...
// ============================================================================
// MOTOR DE PAGINAÇÃO — /p/MLB.../s?page=N
// ============================================================================
async function buscarVendedoresCatalogo(catalogId, urlBase, api, debug) {
  ...
  try {
      ...
      // Se falhou rede/captcha, usa captura local
      if (!html || html.includes('Captcha') || html.includes('human')) {
          const cache = lerCaptura(catalogId, 1, debug);
          if (cache) html = cache;
      }

      if (!html || html.includes('Captcha') || html.includes('human')) {
          debug.push(`   ⚠️ Sem cache e com erro. Tentando Puppeteer Stealth...`);
          let browser;
          try {
      ...
      if (html) {
          salvarCaptura(catalogId, 1, html, debug);
