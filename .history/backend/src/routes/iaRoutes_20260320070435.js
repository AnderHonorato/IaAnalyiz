import express from 'express';
import { prisma } from '../prisma.js';
import { buildAnswer, analyzeImage } from '../iaService.js';

const router = express.Router();

// ═══════════════════════════════════════════════════════════════════════════
// PIPELINE INTELIGENTE DE 2 FASES
//
// Fase 1 — ROUTER (gemini-2.5-flash-lite, temperature=0, sem thinking):
//   Recebe a pergunta + catálogo de tabelas (só metadados).
//   Devolve JSON com quais tabelas buscar. Se falhar o parse, usa keywords.
//
// Fase 2 — ANSWER (gemini-2.5-flash, histórico completo):
//   Recebe a pergunta + dados reais do banco + histórico da conversa.
//   Gera resposta final precisa e profissional.
//
// Regras de ouro:
//   - A IA NUNCA menciona em qual página o usuário está, a menos que seja perguntado.
//   - A IA NUNCA inventa dados — só usa o que veio do banco.
//   - A IA NUNCA adiciona frases de abertura ou encerramento genéricas.
// ═══════════════════════════════════════════════════════════════════════════

// ── Catálogo de tabelas disponíveis (só metadados, nunca dados reais) ─────────
const DATA_CATALOG = {
  produtos: {
    label:   'Catálogo de Produtos',
    desc:    'Produtos cadastrados com SKU, nome, peso (pesoGramas), preço, se é kit, ID do anúncio ML vinculado, categoria.',
    colunas: ['id','sku','nome','preco','pesoGramas','eKit','mlItemId','plataforma','categoria','status'],
  },
  divergencias: {
    label:   'Divergências de Peso/Frete',
    desc:    'Anúncios ML cujo peso declarado diverge do cadastro local. Cada registro tem o motivo da divergência, peso no ML (pesoMl), peso real (pesoLocal) e status (PENDENTE, REINCIDENTE, CORRIGIDO, IGNORADO).',
    colunas: ['id','mlItemId','titulo','motivo','pesoMl','pesoLocal','status','plataforma','createdAt'],
  },
  divergencias_stats: {
    label:   'Estatísticas de Divergências',
    desc:    'Totais agrupados: quantas pendentes, reincidentes, corrigidas, ignoradas e total geral.',
    colunas: ['pendente','reincidente','corrigido','ignorado','total'],
  },
  usuarios: {
    label:      'Usuários do Sistema',
    desc:       'Usuários cadastrados com nome, email, cargo (role) e status de bloqueio/acesso.',
    colunas:    ['id','nome','email','role','solicitouDesbloqueio','verificado','createdAt'],
    privileged: true,
  },
  precificacao_historico: {
    label:   'Histórico de Preços',
    desc:    'Registro de cada alteração de preço em anúncios ML: qual item, preço aplicado, quantidade e quando.',
    colunas: ['mlItemId','titulo','preco','quantidade','categoriaId','criadoEm'],
  },
  categorias_ml: {
    label:   'Categorias ML',
    desc:    'Categorias dos anúncios do Mercado Livre vinculados à conta.',
    colunas: ['categoriaId','nome'],
  },
  agendador: {
    label:   'Agendador de Varredura Automática',
    desc:    'Configuração da varredura automática: se está ativa, intervalo em minutos, última e próxima execução.',
    colunas: ['ativo','intervalo','ultimaExecucao','proximaExecucao'],
  },
  ml_token: {
    label:   'Status da Conexão ML',
    desc:    'Status da conexão OAuth com o Mercado Livre: nickname da conta, validade do token.',
    colunas: ['nickname','expiresAt','mlUserId'],
  },
};

// ── Fallback por keywords quando o parse do router falha ──────────────────────
function keywordRoute(msg) {
  const m = msg.toLowerCase();
  const tabelas = [];
  if (/divergen|peso|frete|anúncio|anuncio|auditoria|reincidente|pendente|corrigido/i.test(m)) {
    tabelas.push('divergencias', 'divergencias_stats');
  }
  if (/produto|sku|catálogo|catalogo|kit|peso/i.test(m)) {
    tabelas.push('produtos');
  }
  if (/usuário|usuario|acesso|bloqueio|desbloqueio|pendente de acesso/i.test(m)) {
    tabelas.push('usuarios');
  }
  if (/preço|preco|precific|histórico de preço/i.test(m)) {
    tabelas.push('precificacao_historico');
  }
  if (/agendador|varredura|automático|automatico|próxima execução/i.test(m)) {
    tabelas.push('agendador');
  }
  if (/categoria/i.test(m)) {
    tabelas.push('categorias_ml');
  }
  if (/conectado|conexão|token ml|conta ml|nickname/i.test(m)) {
    tabelas.push('ml_token');
  }
  return { relevante: tabelas.length > 0, tabelas: [...new Set(tabelas)].slice(0, 4), raciocinio: 'keyword fallback' };
}

// ── Gemini helper para o router (lite, sem thinking, resposta curta) ──────────
async function geminiRouter(catalogoStr, userMessage) {
  const { GoogleGenAI } = await import('@google/genai');
  const ai = new GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY });

  // Usamos gemini-2.5-flash-lite para o router: mais rápido, sem thinking, barato
  // Se não existir, cai para flash normal com thinking desabilitado
  const modelId = 'gemini-2.5-flash-lite-preview-06-17';

  const systemInstruction = [
    'Você é um classificador de intenção. Sua única função é analisar uma mensagem e decidir quais tabelas de dados são necessárias para respondê-la.',
    'Responda EXCLUSIVAMENTE com um objeto JSON. Nenhum texto antes ou depois. Nenhum markdown. Nenhum backtick.',
    'Formato obrigatório: {"relevante":true,"tabelas":["nome_tabela"],"raciocinio":"uma frase"}',
    'Se nenhuma tabela for necessária: {"relevante":false,"tabelas":[],"raciocinio":"uma frase"}',
  ].join('\n');

  const userPrompt = [
    'TABELAS DISPONÍVEIS:',
    catalogoStr,
    '',
    'MENSAGEM DO USUÁRIO: ' + userMessage,
    '',
    'Responda com JSON puro.',
  ].join('\n');

  try {
    const response = await ai.models.generateContent({
      model: modelId,
      config: {
        temperature:     0.0,
        maxOutputTokens: 200,
        // Desabilita thinking para o router — queremos resposta determinística e rápida
        thinkingConfig: { thinkingBudget: 0 },
        systemInstruction,
      },
      contents: [{ role: 'user', parts: [{ text: userPrompt }] }],
    });
    return (response.text || '').trim();
  } catch {
    // Se o modelo lite não existir, usa flash normal sem thinking
    const response = await ai.models.generateContent({
      model: 'gemini-2.5-flash',
      config: {
        temperature:     0.0,
        maxOutputTokens: 200,
        thinkingConfig:  { thinkingBudget: 0 },
        systemInstruction,
      },
      contents: [{ role: 'user', parts: [{ text: userPrompt }] }],
    });
    return (response.text || '').trim();
  }
}

// ── Parse robusto do JSON retornado pelo router ───────────────────────────────
function parseRouterJson(raw) {
  if (!raw) throw new Error('Resposta vazia');

  // Extrai tudo que estiver entre o primeiro { e o último }
  const firstBrace = raw.indexOf('{');
  const lastBrace  = raw.lastIndexOf('}');
  if (firstBrace === -1 || lastBrace === -1) throw new Error('Nenhum { } encontrado');

  let jsonStr = raw.slice(firstBrace, lastBrace + 1);

  // Limpa problemas comuns do Gemini
  jsonStr = jsonStr
    .replace(/,\s*([\}\]])/g, '$1')  // trailing commas
    .replace(/\/\/[^\n]*/g, '')       // comentários //
    .replace(/\/\*[\s\S]*?\*\//g, '') // comentários /* */
    .replace(/(['"])?([a-zA-Z_][a-zA-Z0-9_]*)(['"])?:/g, '"$2":') // unquoted keys
    .replace(/:\s*'([^']*)'/g, ':"$1"'); // single quotes → double quotes

  const parsed = JSON.parse(jsonStr);

  return {
    relevante:  Boolean(parsed.relevante),
    tabelas:    Array.isArray(parsed.tabelas) ? parsed.tabelas.slice(0, 4) : [],
    raciocinio: String(parsed.raciocinio || ''),
  };
}

// ── Fase 1: Router ────────────────────────────────────────────────────────────
async function routeDataNeeded(userMessage, userRole) {
  const isPrivileged = userRole === 'OWNER' || userRole === 'ADMIN';

  const catalogoStr = Object.entries(DATA_CATALOG)
    .filter(([, v]) => !v.privileged || isPrivileged)
    .map(([key, v]) => `- ${key}: ${v.label}. ${v.desc}`)
    .join('\n');

  try {
    const raw    = await geminiRouter(catalogoStr, userMessage);
    const result = parseRouterJson(raw);
    console.log(`[Router] relevante=${result.relevante} | tabelas=[${result.tabelas.join(',')}] | ${result.raciocinio}`);
    return result;
  } catch (parseErr) {
    console.warn('[Router] Parse falhou, usando keyword fallback:', parseErr.message);
    const fallback = keywordRoute(userMessage);
    console.log(`[Router/KB] relevante=${fallback.relevante} | tabelas=[${fallback.tabelas.join(',')}]`);
    return fallback;
  }
}

// ── Fase 2: Fetch seletivo no banco ───────────────────────────────────────────
async function fetchSelectedData(tabelas, userId, userRole) {
  const uid  = parseInt(userId);
  const data = {};

  await Promise.all(tabelas.map(async (tabela) => {
    try {
      switch (tabela) {
        case 'produtos':
          data.produtos = await prisma.produto.findMany({
            where:   { usuarioId: uid },
            orderBy: { id: 'desc' },
          });
          break;

        case 'divergencias':
          data.divergencias = await prisma.divergencia.findMany({
            where:   { usuarioId: uid },
            orderBy: { createdAt: 'desc' },
          });
          break;

        case 'divergencias_stats': {
          const [pendente, reincidente, corrigido, ignorado] = await Promise.all([
            prisma.divergencia.count({ where: { usuarioId: uid, status: 'PENDENTE' } }),
            prisma.divergencia.count({ where: { usuarioId: uid, status: 'REINCIDENTE' } }),
            prisma.divergencia.count({ where: { usuarioId: uid, status: 'CORRIGIDO' } }),
            prisma.divergencia.count({ where: { usuarioId: uid, status: 'IGNORADO' } }),
          ]);
          data.divergencias_stats = { pendente, reincidente, corrigido, ignorado, total: pendente+reincidente+corrigido+ignorado };
          break;
        }

        case 'usuarios':
          if (userRole === 'OWNER' || userRole === 'ADMIN') {
            data.usuarios = await prisma.usuario.findMany({
              orderBy: { createdAt: 'desc' },
              select:  { id: true, nome: true, email: true, role: true, solicitouDesbloqueio: true, verificado: true, createdAt: true },
            });
          }
          break;

        case 'precificacao_historico':
          data.precificacao_historico = await prisma.precificacaoHistorico.findMany({
            where:   { usuarioId: uid },
            orderBy: { criadoEm: 'desc' },
            take:    50,
          }).catch(() => []);
          break;

        case 'categorias_ml':
          data.categorias_ml = await prisma.mlCategoria.findMany({
            where: { usuarioId: uid }, orderBy: { nome: 'asc' },
          }).catch(() => []);
          break;

        case 'agendador':
          data.agendador = await prisma.agendadorConfig.findUnique({
            where: { usuarioId: uid },
          });
          break;

        case 'ml_token':
          data.ml_token = await prisma.mlToken.findUnique({
            where:  { usuarioId: uid },
            select: { nickname: true, expiresAt: true, mlUserId: true },
          });
          break;

        default:
          console.warn('[Fetch] Tabela desconhecida:', tabela);
      }
    } catch (e) {
      console.warn(`[Fetch] Erro em "${tabela}":`, e.message);
    }
  }));

  return data;
}

// ── Formata dados reais para o system prompt ──────────────────────────────────
function formatDataBlock(data) {
  if (!data || Object.keys(data).length === 0) return null;

  const lines = ['=== DADOS DO BANCO DE DADOS (use estes para responder) ===', ''];

  for (const [key, value] of Object.entries(data)) {
    const schema = DATA_CATALOG[key];
    lines.push(`--- ${schema?.label || key} ---`);

    if (Array.isArray(value)) {
      lines.push(`(${value.length} registros)`);
      value.forEach((row, i) => lines.push(`[${i+1}] ${JSON.stringify(row)}`));
    } else if (value && typeof value === 'object') {
      lines.push(JSON.stringify(value, null, 2));
    } else {
      lines.push(String(value ?? 'sem dados'));
    }
    lines.push('');
  }

  lines.push('=== FIM DOS DADOS ===');
  return lines.join('\n');
}

// ═══════════════════════════════════════════════════════════════════════════
// SESSÕES DE CHAT
// ═══════════════════════════════════════════════════════════════════════════

router.get('/api/chat/sessions/:userId', async (req, res) => {
  try {
    res.json(await prisma.chatSession.findMany({
      where:   { usuarioId: parseInt(req.params.userId) },
      orderBy: { updatedAt: 'desc' },
      select:  { id: true, titulo: true, createdAt: true, updatedAt: true },
    }));
  } catch { res.status(500).json({ error: 'Erro.' }); }
});

router.post('/api/chat/sessions', async (req, res) => {
  try {
    res.status(201).json(await prisma.chatSession.create({
      data: { usuarioId: parseInt(req.body.userId), titulo: req.body.titulo || 'Nova conversa' },
    }));
  } catch { res.status(500).json({ error: 'Erro.' }); }
});

router.delete('/api/chat/sessions/:id', async (req, res) => {
  try { await prisma.chatSession.delete({ where: { id: parseInt(req.params.id) } }); res.json({ ok: true }); }
  catch { res.status(500).json({ error: 'Erro' }); }
});

router.get('/api/chat/sessions/:id/messages', async (req, res) => {
  try {
    res.json(await prisma.chatMessage.findMany({
      where:   { sessionId: parseInt(req.params.id) },
      orderBy: { createdAt: 'asc' },
      select:  { id: true, role: true, content: true, imageDesc: true, createdAt: true },
    }));
  } catch { res.status(500).json({ error: 'Erro.' }); }
});

// ═══════════════════════════════════════════════════════════════════════════
// CHAT PRINCIPAL — pipeline 2 fases
// ═══════════════════════════════════════════════════════════════════════════

router.post('/api/ia/chat', async (req, res) => {
  const { message, sessionId, userRole, userId, imageBase64, imageMimeType, imageOnly } = req.body;
  // NOTA: pageUrl removido intencionalmente — a IA não deve mencionar a página atual
  // a menos que o usuário pergunte explicitamente.

  try {
    if (userRole === 'BLOQUEADO') return res.json({ reply: 'Seu perfil está bloqueado. 🔒', sources: [] });
    if (!process.env.GEMINI_API_KEY) return res.status(500).json({ reply: 'Chave API ausente.', sources: [] });

    // ── Garante sessão ──────────────────────────────────────────────────
    let session = sessionId
      ? await prisma.chatSession.findUnique({ where: { id: parseInt(sessionId) } })
      : null;
    if (!session) {
      session = await prisma.chatSession.create({
        data: { usuarioId: parseInt(userId), titulo: 'Nova conversa' },
      });
    }

    // ── Analisa imagem se enviada ───────────────────────────────────────
    const imageDesc = imageBase64
      ? await analyzeImage(imageBase64, imageMimeType || 'image/jpeg', imageOnly ? '' : message)
      : null;

    // ── Salva mensagem do usuário ───────────────────────────────────────
    await prisma.chatMessage.create({
      data: { sessionId: session.id, role: 'user', content: message || '', imageBase64: imageBase64 || null, imageDesc: imageDesc || null },
    });

    // ── Atualiza título da sessão ───────────────────────────────────────
    if (message && message.length > 3) {
      const count = await prisma.chatMessage.count({ where: { sessionId: session.id } });
      if (count <= 2) {
        await prisma.chatSession.update({
          where: { id: session.id },
          data:  { titulo: message.substring(0, 50) + (message.length > 50 ? '...' : '') },
        });
      }
    }

    // ── Busca histórico da conversa (sem imageBase64 para não explodir tokens) ──
    const dbMsgs = await prisma.chatMessage.findMany({
      where:   { sessionId: session.id },
      orderBy: { createdAt: 'asc' },
      take:    40,
      select:  { role: true, content: true, imageDesc: true },
    });
    const history = dbMsgs.map(m => ({
      role:    m.role,
      content: m.content || (m.imageDesc ? '[imagem]' : ''),
    }));

    // ── Mensagem efetiva para o router ──────────────────────────────────
    const msgEfetiva = imageOnly
      ? (imageDesc ? `Usuário enviou imagem. Conteúdo: ${imageDesc}` : 'Usuário enviou imagem')
      : (message || '[imagem]');

    // ── FASE 1: Router ──────────────────────────────────────────────────
    let dataBlock = null;
    if (!imageOnly && message && message.trim().length > 2) {
      const roteamento = await routeDataNeeded(message, userRole);

      if (roteamento.relevante && roteamento.tabelas.length > 0) {
        const dados = await fetchSelectedData(roteamento.tabelas, userId, userRole);
        dataBlock = formatDataBlock(dados);
      }
    }

    // ── Dados de contexto mínimo (nunca vazia para o system prompt) ─────
    const uid = parseInt(userId);
    const [totalProdutos, totalDivergencias, usuarioAtual, pendentes] = await Promise.all([
      prisma.produto.count({ where: { usuarioId: uid } }),
      prisma.divergencia.count({ where: { usuarioId: uid, status: { in: ['PENDENTE','REINCIDENTE'] } } }),
      prisma.usuario.findUnique({ where: { id: uid }, select: { id: true, nome: true, role: true } }),
      (userRole === 'OWNER' || userRole === 'ADMIN')
        ? prisma.usuario.findMany({ where: { solicitouDesbloqueio: true, role: 'BLOQUEADO' }, select: { id: true, nome: true, email: true, role: true, solicitouDesbloqueio: true } })
        : Promise.resolve([]),
    ]);

    // ── FASE 2: Resposta final ──────────────────────────────────────────
    const { reply, sources } = await buildAnswer(msgEfetiva, history, {
      totalProdutos,
      totalDivergencias,
      userRole,
      usuarioAtual,
      pendentes,
      imageContext: imageDesc,
      imageOnly:    !!imageOnly,
      dataBlock,                // dados reais do banco, ou null
    });

    // ── Salva resposta da IA ────────────────────────────────────────────
    await prisma.chatMessage.create({
      data: { sessionId: session.id, role: 'ia', content: reply },
    });

    res.json({ reply, sessionId: session.id, sources });

  } catch (error) {
    console.error('[IA Chat]', error);
    res.status(500).json({ reply: 'Erro interno. ⚠️', sources: [] });
  }
});

// ═══════════════════════════════════════════════════════════════════════════
// INSIGHTS PROATIVOS
// ═══════════════════════════════════════════════════════════════════════════

async function gerarInsight(contextLines) {
  const { GoogleGenAI } = await import('@google/genai');
  const ai = new GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY });
  const response = await ai.models.generateContent({
    model:    'gemini-2.5-flash',
    config:   { temperature: 0.3, maxOutputTokens: 120, thinkingConfig: { thinkingBudget: 0 } },
    contents: [{
      role: 'user',
      parts: [{ text: 'DADOS:\n' + contextLines.join('\n') + '\n\nGere 1 alerta curto para o gestor de e-commerce. Foque no risco operacional ou financeiro. Sem markdown. Comece com 1 emoji.' }],
    }],
  });
  return response.text?.trim() || null;
}

// POST (padrão novo)
router.post('/api/ia/proactive', async (req, res) => {
  const { userId, userRole, pageKey } = req.body;
  try {
    if (!userId || userRole === 'BLOQUEADO' || !process.env.GEMINI_API_KEY)
      return res.json({ insight: null });

    const uid          = parseInt(userId);
    const isPrivileged = userRole === 'OWNER' || userRole === 'ADMIN';
    const lines        = [];

    const divs = await prisma.divergencia.findMany({
      where: { usuarioId: uid, status: { in: ['PENDENTE','REINCIDENTE'] } },
      orderBy: { createdAt: 'desc' }, take: 10,
    });
    if (divs.length > 0) {
      lines.push(`${divs.length} divergências ativas de peso/frete.`);
      divs.slice(0, 4).forEach(d => lines.push(`- ${d.mlItemId}: ${d.motivo} (${d.status})`));
    } else {
      lines.push('Logística sem divergências ativas no momento.');
    }

    if (isPrivileged) {
      const pendentes = await prisma.usuario.count({ where: { solicitouDesbloqueio: true, role: 'BLOQUEADO' } });
      if (pendentes > 0) lines.push(`${pendentes} usuário(s) aguardando aprovação de acesso.`);
    }

    if (lines.length === 0) return res.json({ insight: null });

    const insight = await gerarInsight(lines);
    res.json({ insight });
  } catch (e) {
    console.error('[Proactive]', e);
    res.json({ insight: null });
  }
});

// GET (compatibilidade com MercadoLivre.jsx que usa GET)
router.get('/api/ia/proactive', async (req, res) => {
  const { userId, userRole } = req.query;
  try {
    if (!userId || userRole === 'BLOQUEADO' || !process.env.GEMINI_API_KEY)
      return res.json({ insight: null });

    const uid  = parseInt(userId);
    const divs = await prisma.divergencia.findMany({
      where: { usuarioId: uid, status: { in: ['PENDENTE','REINCIDENTE'] } },
      orderBy: { createdAt: 'desc' }, take: 10,
    });

    if (divs.length === 0) return res.json({ insight: null });

    const lines = [`${divs.length} divergências ativas.`];
    divs.slice(0, 3).forEach(d => lines.push(`- ${d.mlItemId}: ${d.motivo}`));

    const insight = await gerarInsight(lines);
    res.json({ insight });
  } catch { res.json({ insight: null }); }
});

// ═══════════════════════════════════════════════════════════════════════════
// RESUMO EXECUTIVO
// ═══════════════════════════════════════════════════════════════════════════

router.post('/api/ia/summary', async (req, res) => {
  const { userId, dados } = req.body;
  try {
    const { GoogleGenAI } = await import('@google/genai');
    const ai = new GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY });
    const prompt = `Você é um Consultor Logístico de e-commerce. Analise os dados e produza um relatório executivo.

DADOS: ${typeof dados === 'string' ? dados : JSON.stringify(dados)}

Produza em HTML limpo (tags: <b>, <ul>, <li>, <br>, <i>), em 3 seções:
<b>Diagnóstico Geral</b>
<b>Pontos de Atenção</b>
<b>Recomendações</b>

Comece DIRETO no conteúdo. Sem introdução, sem meta-comentários.`;

    const response = await ai.models.generateContent({
      model:    'gemini-2.5-flash',
      config:   { temperature: 0.3, maxOutputTokens: 900 },
      contents: [{ role: 'user', parts: [{ text: prompt }] }],
    });

    const conteudo = (response.text || '').trim()
      .replace(/^(conforme solicitado|aqui está|claro[,!]|segue|com base nos dados)[^\n<]*[\n<]/im, '')
      .trim();

    await prisma.resumoIA.create({ data: { usuarioId: parseInt(userId), conteudo } });
    res.json({ conteudo });
  } catch (e) {
    console.error('[Summary]', e);
    res.status(500).json({ error: 'Erro ao gerar resumo.' });
  }
});

router.get('/api/ia/summary/history', async (req, res) => {
  try {
    res.json(await prisma.resumoIA.findMany({
      where: { usuarioId: parseInt(req.query.userId) },
      orderBy: { createdAt: 'desc' },
      take: 10,
    }));
  } catch { res.status(500).json({ error: 'Erro.' }); }
});

export default router;