...
function extrairVendedoresDoHTML(html, targetUrl, paginaAtual, debug) {
  ...
  // Estratégia A: Novo padrão ML com JSON.parse
  ...
  // Estratégia B: Objeto literal tradicional
  if (!state) {
  ...
  // Estratégia C: initialState genérico
  if (!state) {
    const matchInit = html.match(/"initialState"\s*:\s*(\{[\s\S]+?\})\s*,\s*"components"/);
    if (matchInit) {
      try { state = JSON.parse(matchInit[1]); debug.push(`  🧠 Estado ML (initialState) decodificado.`);} catch(e) {}
    }
  }

  // Se encontrou o JSON, vamos navegar por ele
  if (state) {
    const paths = [
      state?.initialState?.components?.buyBox?.state?.buyingOptions,
      state?.components?.buyBox?.state?.buyingOptions,
      state?.appProps?.pageProps?.initialState?.components?.buyBox?.state?.buyingOptions,
      ...
      state?.components?.buyBox?.state?.sellerOptions,
    ...
    if (!html || html.includes('Verifique se você é humano') || html.includes('Captcha')) {
      ...
      try {
        ...
        if (!isCaptcha) {
            ...
            html = null; // Zera o html para contar como erro
        }
        await browser.close();
      } catch (e) {
