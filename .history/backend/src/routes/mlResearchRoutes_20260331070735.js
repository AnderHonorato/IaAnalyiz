...
// Ativa o plugin Stealth para mascarar o navegador e evitar o Captcha
...
function extrairVendedoresDoHTML(html, debug) {
  ...
  // 1. Tentativa de extração do JSON Limpo
  ...
  if (!jsonStr) {
      ...
      if (matchParse) jsonStr = matchParse[1].replace(/\\"/g, '"').replace(/\\\\/g, '\\').replace(/\\'/g, "'");
  }
    ...
    try {
      ...
      for (const arr of paths) {
        if (Array.isArray(arr) && arr.length > 0) {
          if (arr[0].components) { 
            // Novo Padrão (Components)
            arr.forEach((item) => {
              let nome = item.seller_name || '—', preco = item.price || 0, original = null, desconto = null;
              let freteGratis = false, full = false, envio = 'Mercado Envios', rep = null, thumb = null;
              
              item.components.forEach(c => {
                if ((c.id === 'price' || c.type === 'price') && c.price) {
                  preco = c.price.value || c.price.amount || preco;
                  original = c.price.original_value || original;
                ...
                if (c.id === 'shipping_summary' || c.type === 'generic_summary') {
              ...
              if (preco > 0 || nome !== '—') vendedores.push({ nome, preco: parseFloat(preco), precoOriginal: original, desconto, link: '', thumbnail: thumb, freteGratis, frete: envio, mercadoLider: rep });
            });
          } else { 
            // Padrão Flat (antigo)
            arr.forEach(o => {
              const nome  = o.seller?.nickname || o.sellerInfo?.nickname || o.seller?.name || o.sellerNickname || o.seller_name || '—';
              ...
              if (parseFloat(preco) > 0 || nome !== '—') {
                vendedores.push({ nome, preco: parseFloat(preco) || 0, link, desconto: descPct, thumbnail: o.thumbnail || o.item?.thumbnail || null, titulo: o.title || o.item?.title || null, freteGratis: o.shipping?.free_shipping === true || o.freeShipping === true, frete: o.shipping?.free_shipping ? 'Grátis' : 'Pago' });
              }
            });
          }
          if (vendedores.length) {
            debug.push(`  🧠 JSON NATIVO: ${vendedores.length} vendedores.`);
            return vendedores;
          }
        }
      }
    } catch(e) { debug.push(`  ⚠️ Erro Parse: ${e.message}`); }
  }

  // 2. Tentar JSON-LD (O SEO Data do ML)
  ...
  for (const m of ldMatches) {
    try {
      ...
      if (vendedores.length) {
        debug.push(`  📦 JSON-LD Extraído: ${vendedores.length} vendedores.`);
        return vendedores;
      }
    } catch {}
  }

  // 3. Fallback Agressivo da versão v5 (O Salvadador da Pátria)
...
async function buscarVendedoresCatalogo(catalogId, api, debug) {
  ...
  for (const url of urls) {
    ...
    try {
      ...
      html = res.data || '';
      debug.push(`   → HTML Axios: ${(html.length/1024).toFixed(0)}KB`);
    } catch (e) {
      debug.push(`⚠️ Axios falhou: ${e.message?.substring(0,60)}`);
    }

    // Proteção PUPPETEER
    if (!html || html.includes('Captcha') || html.includes('Verifique se você é humano')) {
      debug.push(`🤖 Acionando Puppeteer Stealth para burlar Captcha...`);
      let browser;
      try {
        ...
        debug.push(`   ✅ HTML Puppeteer: ${(html.length/1024).toFixed(0)}KB`);
        await browser.close();
      } catch(e) {
        if (browser) await browser.close();
        debug.push(`❌ Puppeteer falhou: ${e.message?.substring(0,60)}`);
      }
    }
    ...
    if (html) {
      ...
      if (vendedores.length) {
        debug.push(`✅ ${vendedores.length} vendedor(es) extraído(s) do HTML!`);
        return vendedores;
      }
    }
  }
...
async function buscarItem(mlbId, url, api, debug) {
  ...
  if (ehCatalogo) {
    ...
    // O pulo do gato original: cruzar dados na API
    ...
    debug.push(`🏪 Anúncio normal. Buscando concorrentes por API...`);
    try {
...
router.get('/api/ml/research/:mlbId', async (req, res) => {
  ...
  for (const id of todosIds) {
    ...
    // 2. TENTA API DE PRODUTOS/CATALOGO
    try {
  ...
  // 3. SE AS APIS FALHAREM, VAI PRO SCRAPING DIRETO (Axios + Puppeteer)
  if (urlOriginal) {
    try {
      ...
      return res.json({ ...dados, debug });
    } catch (eC) { debug.push(`❌ Scraping Geral: ${eC.message?.substring(0,60)}`); }
  }
