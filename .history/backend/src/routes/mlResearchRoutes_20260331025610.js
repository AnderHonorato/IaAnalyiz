...
async function buscarViaPuppeteer(urlOriginal, mlbId, debug) {
  ...
  try {
    browser = await puppeteer.launch({
        headless: true, // CORREÇÃO: Alterado de "new" para true
        args: [
            '--no-sandbox', 
            '--disable-setuid-sandbox',
            '--disable-gpu', // CORREÇÃO: Importante para evitar travamentos
            '--no-zygote' // CORREÇÃO: Importante para evitar travamentos
        ]
    });
    ...
    if (browser) {
        try {
            ...
            // Ignorar erro ao fechar o browser
        }
    }
    throw error;
  }
