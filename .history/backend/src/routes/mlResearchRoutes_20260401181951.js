...
async function getSellerFullCached(sellerId, api) {
  ...
  try {
    ...
    const s  = sv.data;
    const permalink = s.permalink || `https://perfil.mercadolivre.com.br/${s.nickname}`;
    const res = {
    ...
    while (hasMore && offset < 1000) {
      ...
      for (let i = 0; i < results.length; i++) {
        ...
        allVendedores.push({
          ...
          link: `http://www.mercadolivre.com.br/p/${id}/s`, // Força padronização
    ...
    while (hasMore && offset < 1000) {
      ...
      for (let i = 0; i < results.length; i++) {
        ...
        allVendedores.push({
          ...
          link: `http://www.mercadolivre.com.br/p/${r.id}/s`, thumbnail: r.thumbnail, titulo: r.title, frete: r.shipping?.free_shipping ? 'Grátis' : 'Pago',
...
function extrairVendedoresDoHTML(html, debug) {
    ...
    try {
      ...
      if (options.length > 0) {
        options.forEach((o, i) => {
          ...
          let permalink = `https://perfil.mercadolivre.com.br/${encodeURIComponent(nome)}`;
          if (o.sellerInfo?.subtitle) vendasStr = o.sellerInfo.subtitle.replace(/MercadoLíder.*?\|\s*/i, '').trim();
          ...
          const realLink = theId ? `http://www.mercadolivre.com.br/p/${theId}/s` : (o.permalink || '');
          ...
          if (parseFloat(preco) > 0 || nome !== '—') {
            vendedores.push({
              ...
              link: realLink, desconto: descPct, thumbnail: o.thumbnail || o.item?.thumbnail || null,
  ...
  // Fallback para Regex direta no HTML (caso a UI mude e esconda o JSON)
  ...
  if (blocks.length > 1) {
     for (let i = 1; i < blocks.length; i++) {
        ...
        if (inputItem) linkAnuncio = `http://www.mercadolivre.com.br/p/${inputItem[1]}/s`;

        if (preco > 0 || nome !== '—') {
            vendedores.push({
               nome, vendasStr, vendasNum: 0, perfilLoja: linkLoja || `https://perfil.mercadolivre.com.br/${encodeURIComponent(nome)}`,
...
async function buscarItem(mlbId, url, api, debug) {
  ...
  if (ehCatalogo) {
    ...
    debug.push(`📦 [Produto de Catálogo] Mapeando todas as variações de preço do mercado...`);
      ...
      while (hasMore && offset < 150) {
        ...
        for (let i = 0; i < results.length; i++) {
          ...
          concorrentes.push({
            ...
            preco: r.price || 0, precoOriginal: r.original_price || r.price, desconto: '0% OFF', link: `http://www.mercadolivre.com.br/p/${r.id}/s`,
  ...
  // Extrair Variações Mestre (Cores, Voltagem)
  const masterVariations = (it.attributes || []).filter(a => ['COLOR', 'VOLTAGE', 'FREQUENCY', 'POWER', 'SIZE', 'GEAR_SIZE'].includes(a.id)).map(a => ({ nome: a.name, valor: a.value_name }));
  ...
  return {
    ...
    thumbnail: it.thumbnail, link: `http://www.mercadolivre.com.br/p/${mlbId}/s`, freteGratis: it.shipping?.free_shipping === true,
    ...
    variacoes: masterVariations, pictures: (it.pictures || []).map(p => p.secure_url || p.url),
...
async function buscarViaProducts(mlbId, api, debug) {
  ...
  const masterVariations = (p.attributes || []).filter(a => ['COLOR', 'VOLTAGE', 'FREQUENCY'].includes(a.id)).map(a => ({ nome: a.name, valor: a.value_name }));
  ...
  return {
    ...
    link: `http://www.mercadolivre.com.br/p/${mlbId}/s`, freteGratis: concorrentes[0]?.freteGratis ?? false, frete: concorrentes[0]?.frete || '—', envio: concorrentes[0]?.envio || 'Mercado Envios',
    ...
    variacoes: masterVariations, pictures: (p.pictures || []).map(pic => pic.secure_url || pic.url),
...
async function buscarViaScraping(url, mlbId, debug) {
  ...
  return {
    ...
    condicao: 'Novo', tipoAnuncio: 'Catálogo', thumbnail, link: targetUrl, freteGratis: false, frete: '—', envio: 'Mercado Envios', avaliacoes: null, atributos: [], variacoes: [], pictures: [thumbnail].filter(Boolean),
