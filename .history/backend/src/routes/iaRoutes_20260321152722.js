...
router.post('/api/ia/proactive', async (req, res) => {
  ...
  try {
    ...
    // ── Coleta métricas reais do banco ────────────────────────────────────
    ...
    // ── Guarda: só prossegue se tiver algo relevante de verdade ──────────
    const temDivergencias = divs.length > 0;
    const temAvisos       = avisos > 0;
    const temPendentes    = pendentesAprovacao > 0;
 
    if (!temDivergencias && !temAvisos && !temPendentes) {
      // Tudo ok — silêncio total, não incomoda o usuário
      return res.json({ insight: null, hasRelevantData: false });
    }
 
    // ── Monta linhas de contexto apenas com dados reais ───────────────────
    const linhas = [];
    ...
    if (temDivergencias) {
      ...
      if (reincidentes > 0) linhas.push(`${reincidentes} reincidente${reincidentes > 1 ? 's' : ''}`);
      if (pendenteEnvio > 0) linhas.push(`${pendenteEnvio} na fila de correção via API`);
      if (maxErro > 0) linhas.push(`maior erro: ${maxErro}g`);
    }
 
    if (temAvisos) {
      linhas.push(`${avisos} aviso${avisos > 1 ? 's' : ''} ativo${avisos > 1 ? 's' : ''} do Mercado Livre`);
    }
 
    if (temPendentes) {
      linhas.push(`${pendentesAprovacao} usuário${pendentesAprovacao > 1 ? 's' : ''} aguardando aprovação de acesso`);
    }
 
    // ── Gera insight curto para o balão flutuante ─────────────────────────
    const insight = await gerarInsightCurto(linhas);
 
    // Valida de novo que o insight gerado tem conteúdo real
    // (às vezes o Gemini retorna algo inútil)
    if (!insight || insight.replace(/[\u{1F300}-\u{1FFFF}\u{2600}-\u{26FF}✅⚠️🚨]/gu, '').trim().length < 10) {
      return res.json({ insight: null, hasRelevantData: false });
    }
 
    // ── Gera insight completo para exibir no chat quando abrir ───────────
    // Isso evita que o frontend precise fazer uma segunda chamada
    const fullInsight = await gerarFullInsight(linhas, divs, avisos, uid);
 
    return res.json({
      insight,           // curto — vai no balão flutuante
      fullInsight,       // completo — vai como mensagem da IA no chat
...
// ─── Gera o insight completo formatado para o chat ────────────────────────────
async function gerarFullInsight(linhas, divs, avisos, uid) {
  ...
  try {
    ...
    // Detalhes extras para enriquecer a análise
    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        ...
        systemInstruction: [
          ...
          'NÃO pergunte contexto. NÃO faça perguntas. Apenas apresente os dados.',
          'Finalize com UMA pergunta direta e relevante ao usuário.',
        ].join(' '),
      },
      contents: [{ role: 'user', parts: [{ text: `Dados do sistema: ${contexto}` }] }],
    });
