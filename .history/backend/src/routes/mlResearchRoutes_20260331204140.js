...
// Baseado na v5 - Modificado para buscar TODOS os concorrentes via Paginação na API Oficial
// Evita tela de Login/Captcha usando o Token de Autenticação apenas nas rotas oficiais.
...
// ── O SEGREDO: PAGINAÇÃO VIA API OFICIAL (Burla a tela de login perfeitamente) ──
async function buscarVendedoresCatalogo(catalogId, api, debug) {
  debug.push(`🏷️  Catálogo ID: ${catalogId}`);
  let todosVendedores = [];
  
  // ESTRATÉGIA 1: API /products/X/items (Paginada)
  try {
    let offset = 0;
    let temMais = true;
    debug.push(`📡 Buscando opções de compra via API Oficial do Catálogo...`);
    
    while (temMais && offset <= 250) { // Max 5 páginas para não estourar
        ...
        if (results.length > 0) {
          debug.push(`   → Pág ${Math.floor(offset/50)+1}: ${results.length} vendedores resgatados.`);
          for (const r of results) {
            ...
            todosVendedores.push({
              ...
              pagina: Math.floor(offset/50) + 1, posicao: todosVendedores.length + 1
            });
          }
          offset += 50;
    ...
    if (todosVendedores.length > 0) {
      ...
      debug.push(`✅ ${todosVendedores.length} opções mapeadas via API Oficial!`);
      return todosVendedores;
    }
  } catch (e) {
    debug.push(`❌ /products/items falhou: ${e.response?.status || e.message}`);
  }

  // ESTRATÉGIA 2: API Search (Paginada)
  try {
    let offset = 0;
    let temMais = true;
    debug.push(`📡 Buscando via API Search (catalog_product_id)...`);
    ...
    while (temMais && offset <= 250) {
        ...
        if (results.length > 0) {
          debug.push(`   → Pág ${Math.floor(offset/50)+1}: ${results.length} resultados.`);
          for (const r of results) {
    ...
    debug.push(`❌ Search catalog_product_id falhou: ${e.response?.status || e.message}`);
  }
        ...
        for (const o of arr) {
          ...
          if (parseFloat(preco) > 0 || nome !== '—') {
            vendedores.push({ nome, preco: parseFloat(preco) || 0, link, desconto: descPct,
              ...
              vendas: o.sold_quantity ? formatarVendas(o.sold_quantity) : '+100 vendas' });
          }
        }
...
async function buscarItem(mlbId, url, api, debug) {
  ...
  if (ehCatalogo) {
    ...
    debug.push(`📦 Produto de CATÁLOGO detectado. Puxando opções via API...`);
      ...
      try {
        ...
        for (const r of results) {
          ...
          concorrentes.push({
            ...
            estoque: r.available_quantity, vendidos: r.sold_quantity, vendas: formatarVendas(r.sold_quantity), tipoAnuncio: mapTipo(r.listing_type_id),
          });
        }
        concorrentes.sort((a, b) => a.preco - b.preco);
      } catch (e) { debug.push(`❌ Busca por título falhou: ${e.message}`); }
    }
  } else {
    ...
    try {
      ...
      concorrentes.sort((a, b) => a.preco - b.preco);
    } catch (e) { debug.push(`❌ Busca concorrentes falhou: ${e.message}`); }
  }
...
async function buscarViaScraping(url, mlbId, debug) {
  ...
  if (!html || html.includes('auth-login-frontend') || html.includes('suspicious')) {
      debug.push(`🤖 Acionando Navegador Real para bypass...`);
      let browser;
      try {
...
// ── ROTA PRINCIPAL: AGORA USANDO RESPONSE JSON NORMAL (FIX DO ERRO NO FRONTEND) ──
