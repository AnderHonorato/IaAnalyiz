...
// Adicionado: Paginação Profunda em Loop (offset), Cache Otimizado de Vendas e Mapeamento de Colunas Excel
...
// NOVO: Paginação profunda com mapeamento exato das colunas da planilha
async function buscarVendedoresCatalogo(catalogId, api, debug) {
  ...
  try {
    ...
    while (hasMore && offset < 1000) { // Trava de segurança: máximo 20 páginas
      ...
      debug.push(`   → ${results.length} item(s) retornado(s) na Página ${currPage}`);
      ...
      if (results.length < limit) {
        ...
        offset += limit;
        await sleep(600); // Pausa anti-rate-limit
      }
    }
