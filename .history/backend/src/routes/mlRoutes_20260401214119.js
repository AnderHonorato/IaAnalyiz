...
router.get('/api/bot/stream', (req, res) => {
  ...
  addSseClient(res); 
  // Passa o modoLento se for 'true'
  runBot(parseInt(req.query.userId), req.query.modoLento === 'true').catch(err => res.write(`data: ${JSON.stringify({ type: 'error', msg: 'Erro: ' + err.message })}\n\n`));
});
