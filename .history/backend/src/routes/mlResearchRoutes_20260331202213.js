...
// Atualizado: Remoção do Token de Autenticação no Scraper Público para evitar 403 (Login Forçado)
// Atualizado: Rota modificada para SSE (Server-Sent Events) para logs em Tempo Real no frontend
...
// ----------------------------------------------------------------------------
// SISTEMA DE CAPTURA LOCAL (CACHE DE HTML) E LOGGER SSE
// ----------------------------------------------------------------------------
...
function salvarCaptura(mlbId, pagina, conteudo, debug) {
    try {
        ...
        debug.push(`  💾 HTML salvo com sucesso em: ${filepath}`);
    } catch (e) {}
}
...
const createLogger = (res) => {
    ...
    return {
        push: (msg) => {
            logs.push(msg);
            // Formata o log para envio imediato via stream
...
async function getMlApi(userId) {
  ...
  return {
      ...
      publicApi: axios.create({
        baseURL: 'https://api.mercadolibre.com',
        timeout: 15000, // Sem token para evitar 403 em buscas públicas!
      })
  };
}
...
async function simularHumano(page, debug) {
    try {
        debug.push(`   🕺 Simulando comportamento humano...`);
        const width = page.viewport().width;
        const height = page.viewport().height;
        await page.mouse.move(100 + Math.random() * (width - 200), 100 + Math.random() * (height - 200), { steps: 5 + Math.random() * 10 });
...
// SCRAPER LIMPO (SEM BEARER TOKEN PARA NÃO DAR 403)
...
// ============================================================================
// EXTRATOR NATIVO CIRÚRGICO RECURSIVO (INQUEBRÁVEL)
// ============================================================================

function extrairTodosJSONs(html, target) {
  const jsons = [];
  let idx = 0;
  while ((idx = html.indexOf(target, idx)) !== -1) {
    ...
    if (start === -1) { idx += target.length; continue; }

    const openChar = html[start];
    const closeChar = openChar === '{' ? '}' : ']';
    let open = 0, inStr = false, escape = false, found = false;
    ...
    for (let i = start; i < html.length; i++) {
        ...
        if (!inStr) {
            if (char === openChar) open++;
                ...
                open--;
                if (open === 0) {
                    jsons.push(html.substring(start, i + 1));
                    idx = i + 1;
                    found = true;
                    break;
                }
    ...
    if (!found) idx += target.length;
  }
  return jsons;
}

function findSellerArrays(obj, results = []) {
  if (!obj || typeof obj !== 'object') return results;
  if (Array.isArray(obj)) {
    if (obj.length > 0) {
      const first = obj[0];
      if (first && typeof first === 'object') {
          if (Array.isArray(first.components)) {
            results.push({ type: 'components', data: obj });
          }
          else if ((first.price !== undefined || first.salePrice !== undefined || first.currency_id) && 
                   (first.seller_name || first.seller || first.item_id || first.seller_id)) {
            results.push({ type: 'flat', data: obj });
          }
      }
    }
    obj.forEach(child => findSellerArrays(child, results));
  } else {
    for (const key in obj) {
      findSellerArrays(obj[key], results);
    }
  }
  return results;
}
...
function extrairVendedoresDoHTML(html, targetUrl, paginaAtual, debug) {
  ...
  const addVendedor = (v) => {
    if (!v.preco || isNaN(v.preco)) return;
    const key = `${v.nome}-${v.preco}`;
    if (!mapVendedores.has(key)) {
        mapVendedores.set(key, v);
        vendedores.push(v);
    }
  };

  if (html.includes('auth-login-frontend') || html.includes('suspicious-traffic') || html.includes('Verifique se você é humano')) {
      debug.push(`  ❌ ML Bloqueou a requisição (Tela de Login ou Captcha).`);
      return vendedores; 
  }

  const allJsons = [
      ...extrairTodosJSONs(html, '_n.ctx.r'),
      ...extrairTodosJSONs(html, '__NORDIC_RENDERING_CTX__'),
      ...extrairTodosJSONs(html, '__PRELOADED_STATE__'),
      ...extrairTodosJSONs(html, 'melidata("add","event_data",')
  ];

  const matchParse = html.match(/__PRELOADED_STATE__\s*=\s*JSON\.parse\(['"](.*?)['"]\);/);
  if (matchParse) {
      allJsons.push(matchParse[1].replace(/\\"/g, '"').replace(/\\\\/g, '\\').replace(/\\'/g, "'"));
  }

  let foundSellerArrays = [];

  for (const jsonStr of allJsons) {
      try {
          const state = JSON.parse(jsonStr);
          findSellerArrays(state, foundSellerArrays);
      } catch (e) {}
  }

  if (foundSellerArrays.length === 0) {
      ...
      for (const m of ldMatches) {
          try {
              ...
              if (offers.length > 0) {
                  foundSellerArrays.push({ type: 'ld', data: offers });
              }
          } catch (e) {}
      }
  }
  ...
  if (foundSellerArrays.length > 0) {
      debug.push(`  🧠 Array de Vendedores Mapeado (Estrutura Encontrada)`);
  }

  for (const arrObj of foundSellerArrays) {
      const { type, data } = arrObj;

      if (type === 'components') {
          data.forEach((item, index) => {
              let nome = item.seller_name || '—', preco = 0, original = null, desconto = '0% OFF';
              ...
              let mlbId = item.id || item.item_id || null;
              let link = targetUrl;
              ...
              if (preco > 0 || nome !== '—') addVendedor({
                  mlbId, nome, preco: parseFloat(preco), precoOriginal: original ? parseFloat(original) : null,
                  link: mlbId ? buildItemLink(mlbId) : link, 
                  desconto, thumbnail: thumb, titulo: 'Anúncio Catálogo', freteGratis, envio,
          ...
          data.forEach((o, index) => {
              ...
              let full = o.shipping?.logistic_type === 'fulfillment' || o.shipping?.tags?.includes('fulfillment') || o.has_full_filment === true || o.fulfillment === true || o.shipping_conditions === 'fulfillment';
              ...
              let tipo = mapTipo(o.listing_type_id || o.item_condition);
              ...
              const mlbId = o.item_id || o.id || null;

              addVendedor({
                  mlbId, nome, preco: parseFloat(preco), precoOriginal: precoOriginal ? parseFloat(precoOriginal) : null,
                  link: o.permalink || o.item?.permalink || o.url || (mlbId ? buildItemLink(mlbId) : targetUrl), 
                  desconto, thumbnail: o.thumbnail || o.item?.thumbnail || null,
                  titulo: o.title || o.item?.title || 'Anúncio Catálogo', freteGratis, envio, tipoAnuncio: tipo, vendas: vendasStr, parcelamento: parcelas,
                  pagina: paginaAtual, posicao: index + 1, mercadoLider: o.seller_info?.power_seller_status?.title || null
              });
          });
      }
      else if (type === 'ld') {
          data.forEach((offer, idx) => {
              const nome = offer.seller?.name || offer.seller?.nickname || '—';
              const preco = parseFloat(offer.price || 0);
              if (preco > 0 || nome !== '—') addVendedor({
                  mlbId: null, nome, preco, link: offer.url || targetUrl, desconto: '0% OFF', thumbnail: null, titulo: offer.name || 'Anúncio', freteGratis: false, envio: 'Mercado Envios',
                  tipoAnuncio: 'Clássico', vendas: '+100 vendas', parcelamento: '—', pagina: paginaAtual, posicao: idx + 1, mercadoLider: null
              });
          });
      }
...
// ============================================================================
async function buscarVendedoresCatalogo(catalogId, urlBase, api, publicApi, debug) {
  ...
  while (page <= 40 && errorsConsecutivos < MAX_ERRORS_CONSECUTIVOS) {
      ...
      if (cache) {
          ...
          try {
              ...
              // AXIOS LIMPO (Sem Bearer para não ativar 403)
      ...
      // SE O AXIOS FOR BARRADO PELO DATADOME (CAPTCHA OU LOGIN FORÇADO)
      if (!html || html.includes('auth-login-frontend') || html.includes('suspicious') || html.includes('Verifique se você é humano')) {
          debug.push(`   ⚠️ Bloqueio detectado. Acionando Navegação Real (Puppeteer Anônimo)...`);
          let browser;
          try {
              ...
              await pageObj.evaluateOnNewDocument(() => {
              ...
              debug.push(`   🚀 Navegando para a página do catálogo...`);
      ...
      if (html && !html.includes('auth-login-frontend') && !html.includes('suspicious-traffic')) {
          ...
          errorsConsecutivos++;
          debug.push(`❌ Erro pág ${page} (${errorsConsecutivos}/${MAX_ERRORS_CONSECUTIVOS}) - Bloqueio/Login detectado`);
  ...
  // 4. FALLBACK FINAL: API PÚBLICA DE SEARCH (Se usar authApi dá 403)
  try {
      debug.push(`📡 Buscando dados via API Interna (Search PÚBLICA)...`);
      ...
      // Usa a API limpa (sem token) para busca
      const searchRes = await publicApi.get('/sites/MLB/search', {
      ...
      if (results.length > 0) {
          ...
          for (const r of results.slice(0, 20)) {
              // getSellerName pode usar a authApi normal pois não é busca
          ...
          debug.push(`✅ ${vendedores.length} opções encontradas via API Pública.`);
          return vendedores;
      }
  } catch (e) {
...
async function buscarItem(mlbId, url, api, publicApi, debug) {
  ...
  if (ehCatalogo) {
    ...
    concorrentes = await buscarVendedoresCatalogo(catalogId, url, api, publicApi, debug);
  } else {
...
async function buscarViaProducts(mlbId, url, api, publicApi, debug) {
  debug.push(`🔍 Tentando /products/${mlbId}...`);
  const res = await api.get(`/products/${mlbId}`, { timeout: 12000 });
  const p   = res.data;
  
  const concorrentes = await buscarVendedoresCatalogo(mlbId, url, api, publicApi, debug);
...
async function buscarViaScrapingGeral(url, mlbId, debug) {
...
// ── ROTA PRINCIPAL MODIFICADA PARA SSE (STREAMING) ──────────────────────────────────
router.get('/api/ml/research/:mlbId', async (req, res) => {
  ...
  // Configura a requisição para enviar dados em Streaming (SSE)
  ...
  const api = mlInstance.authApi;
  const publicApi = mlInstance.publicApi;

  let dadosFinais = null;

  // TENTATIVA 1: API OFICIAL DO ML
  try {
      dadosFinais = await buscarItem(mlbId, urlOriginal, api, publicApi, logger);
  } catch (eA) {
      logger.push(`⚠️  /items/${mlbId}: ${eA.response?.status || eA.message?.substring(0,60)}`);
  }

  // TENTATIVA 2: API DE CATÁLOGOS
  if (!dadosFinais || !dadosFinais.titulo) {
      try {
          dadosFinais = await buscarViaProducts(mlbId, urlOriginal, api, publicApi, logger);
      } catch (eB) {
          logger.push(`⚠️  /products/${mlbId}: ${eB.response?.status || eB.message?.substring(0,60)}`);
      }
  }
  ...
  if ((!dadosFinais || !dadosFinais.titulo) && urlOriginal) {
      try {
          dadosFinais = await buscarViaScrapingGeral(urlOriginal, mlbId, logger);
      } catch (eC) {
          logger.push(`❌ Scraping: ${eC.message?.substring(0,60)}`);
      }
  }

  // Resposta Final do Stream
  if (dadosFinais && dadosFinais.titulo) {
      dadosFinais.debug = logger.getAll();
      await salvarHistorico(userId, mlbId, urlOriginal || mlbId, dadosFinais, null);
      res.write(`event: done\ndata: ${JSON.stringify(dadosFinais)}\n\n`);
  } else {
      logger.push(`❌ Todas as estratégias falharam para ${mlbId}`);
      const msg = logger.getAll().filter(l => l.startsWith('❌') || l.startsWith('⚠️')).join(' | ');
      await salvarHistorico(userId, mlbId, urlOriginal || mlbId, null, msg);
      res.write(`event: error\ndata: ${JSON.stringify({ error: 'Anúncio não encontrado' })}\n\n`);
  }
