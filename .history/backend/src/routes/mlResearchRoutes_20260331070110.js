...
// ============================================================================
// NOVO EXTRATOR NATIVO CIRÚRGICO (À PROVA DE MINIFICAÇÃO DO ML)
// ============================================================================
...
function extrairVendedoresDoHTML(html, debug) {
  ...
  if (!jsonStr) {
      // Fallback para buscar o dataLayer ou outro objeto JSON embutido na página
      const dlMatch = html.match(/dataLayer\s*=\s*\[(\{[\s\S]+?\})\]/);
      if (dlMatch) jsonStr = dlMatch[1];
  }
  ...
  if (jsonStr) {
      try {
          ...
          debug.push(`  🧠 JSON extraído (${jsonStr.length} chars)`);
      } catch (e) {}
  }
              ...
              if (arr[0].components) {
                  arr.forEach((item, index) => {
                      ...
                      if (preco > 0 || nome !== '—') vendedores.push({
                          ...
                          link: '', desconto, thumbnail: thumb, titulo: 'Catálogo', freteGratis, envio,
                          tipoAnuncio: 'Clássico', vendas: '+100 vendas', parcelamento: parcelas, mercadoLider: rep
                      });
                  });
              } else {
                  arr.forEach((o, index) => {
                      ...
                      vendedores.push({
                          ...
                          link: o.permalink || o.item?.permalink || o.url || '', desconto, thumbnail: o.thumbnail || o.item?.thumbnail || null,
                          titulo: o.title || o.item?.title || 'Catálogo', freteGratis, envio, tipoAnuncio: tipo, vendas: '+100 vendas', parcelamento: parcelas
                      });
                  });
              }
  ...
  for (const m of ldMatches) {
      try {
          ...
          for (const o of offers) {
              ...
              if (preco > 0 || nome !== '—') vendedores.push({
                  nome, preco, link: o.url || '', desconto: null, thumbnail: null, titulo: ld.name || null, freteGratis: false, frete: '—'
              });
          }
          if (vendedores.length > 0) return vendedores;
      } catch (e) {}
...
async function buscarVendedoresCatalogo(catalogId, api, debug) {
  ...
  // 1. Tentar fazer scraping da página principal do catálogo com as opções de compra
  ...
  try {
      ...
      // Tentativa 1: Axios puro
      let res = await scraperAgent.get(url);
      let html = res.data || '';
      debug.push(`   → HTML recebido via Axios: ${(html.length/1024).toFixed(0)}KB`);
      
      let vendedores = extrairVendedoresDoHTML(html, debug);
      
      // Tentativa 2: Se o Axios falhar, usa o Puppeteer (Stealth)
      if (vendedores.length === 0 || html.includes('Captcha') || html.includes('human')) {
          ...
          try {
              ...
              vendedores = extrairVendedoresDoHTML(html, debug);
          } catch (err) {
      ...
      if (vendedores.length > 0) {
          ...
          // Adicionar o mlbId e link correto a todos os vendedores
  ...
  // 2. Se o scraping falhar, tenta a busca da API por categoria/titulo (Fallback final)
  try {
...
async function buscarViaScrapingGeral(url, mlbId, debug) {
  ...
  debug.push(`🌐 Scraping Geral (Último Recurso): ${targetUrl}`);
  ...
  if (!titulo) throw new Error('Não foi possível extrair dados da página via Scraping Geral.');
...
router.get('/api/ml/research/:mlbId', async (req, res) => {
  ...
  // TENTATIVA 3: SCRAPING GERAL (Se for url externa ou API travada)
  if (urlOriginal) {
      try {
          const dados = await buscarViaScrapingGeral(decodeURIComponent(urlOriginal), mlbId, debug);
