...
function lerCaptura(mlbId, pagina, debug) {
    try {
        ...
        if (fs.existsSync(filepath)) {
            ...
            // Não lê do cache se for a página de Captcha
            if (!html.includes('suspicious-traffic') && !html.includes('account-verification')) {
                debug.push(`  📂 Lendo captura local (Bypass de Rede)...`);
                return html;
            }
        }
    } catch (e) {}
...
// ============================================================================
// POOL DE USER-AGENTS REAIS E RESOLUÇÕES (rotação anti-bloqueio)
// ============================================================================
const UA_POOL = [
...
async function simularHumano(page) {
    try {
        ...
        // Clique aleatório (em área inofensiva)
...
function extrairVendedoresDoHTML(html, targetUrl, paginaAtual, debug) {
  ...
  // Se for página de erro/captcha, aborta direto
  if (html.includes('suspicious-traffic') || html.includes('account-verification') || html.includes('Verifique se você é humano')) {
      debug.push(`  ❌ O HTML analisado é a página de Captcha (Bloqueio DataDome)`);
      return vendedores;
  }
...
async function buscarVendedoresCatalogo(catalogId, urlBase, api, debug) {
  ...
  while (page <= 40 && errorsConsecutivos < MAX_ERRORS_CONSECUTIVOS) {
      ...
      if (!html || html.includes('suspicious-traffic') || html.includes('account-verification')) {
          ...
          try {
              ...
              // TÉCNICA DE WARM-UP (Visita a Home do ML antes pra pegar cookies limpos)
              ...
              debug.push(`   🚀 Navegando para o catálogo alvo...`);
      ...
      if (html && !html.includes('account-verification')) {
          ...
          if (vendedoresPagina.length > 0) {
              ...
              page++;
              await humanDelay(1500, 3000); // Delay robusto entre páginas
          } else {
          ...
          errorsConsecutivos++;
          debug.push(`❌ Erro pág ${page} (${errorsConsecutivos}/${MAX_ERRORS_CONSECUTIVOS}) - Captcha detectado`);
  ...
  // 4. FALLBACK FINAL: API SEARCH (Segurança)
  try {
...
async function buscarViaScrapingGeral(url, mlbId, debug) {
  ...
  if (!html || html.includes('account-verification') || html.includes('suspicious')) {
  ...
  if (html && !html.includes('account-verification')) salvarCaptura(mlbId, 1, html, debug);
...
router.get('/api/ml/research/:mlbId', async (req, res) => {
  ...
  // Limpeza de dupla codificação e espaços (Resolve o erro do link sujo)
  if (urlOriginal) {
      try { 
