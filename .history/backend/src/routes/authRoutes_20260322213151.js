...
// POST /api/sessao/entrar — registra entrada do usuário
router.post('/api/sessao/entrar', async (req, res) => {
  const { userId } = req.body;
  if (!userId) return res.status(400).json({ error: 'userId obrigatório' });
  try {
    // Marca sessões anteriores como inativas (caso tenha ficado aberta)
...
// POST /api/sessao/sair — registra saída do usuário
...
// GET /api/sessao/stats — retorna contagens para o header
router.get('/api/sessao/stats', async (req, res) => {
  try {
    // Considera "ativo" quem entrou nos últimos 30 min sem sair
    ...
    const [ativos, total] = await Promise.all([
      ...
      prisma.sessaoUsuario.count(), // total histórico de sessões únicas por usuário
    ]);
...
// Limpa sessões ativas antigas (>2h sem heartbeat) — chame num cron ou junto com o agendador
