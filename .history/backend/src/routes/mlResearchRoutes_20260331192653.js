...
// Atualizado: Sistema de Captura Local, Simulação Humana Avançada e Múltiplos Extratores
...
// ============================================================================
// EXTRATOR NATIVO (MESCLA O CÓDIGO PLANO COM O NOVO CÓDIGO DE COMPONENTES)
// ============================================================================
...
function extrairVendedoresDoHTML(html, targetUrl, paginaAtual, debug) {
  ...
  if (jsonStr) {
      try {
          ...
          debug.push(`  🧠 JSON de Estado Global Extraído!`);
      } catch (e) {}
  }
  ...
  if (state) {
      ...
      for (let arr of paths) {
          if (Array.isArray(arr) && arr.length > 0) {
              ...
              // MODELO 1: COMPONENTS ARRAY (ML Atualizado)
              if (arr[0].components) {
                  debug.push(`  🎯 Formato Detectado: Components Array (${arr.length} itens)`);
              ...
              // MODELO 2: FLAT ARRAY (Modelo Antigo e algumas páginas internas, ex: pagina2Ml.txt)
              else {
                  debug.push(`  🎯 Formato Detectado: Flat Array (${arr.length} itens)`);
                  arr.forEach((o, index) => {
...
async function buscarVendedoresCatalogo(catalogId, urlBase, api, debug) {
  ...
  while (page <= 40 && errorsConsecutivos < MAX_ERRORS_CONSECUTIVOS) {
      ...
      // 3. TENTA PUPPETEER SIMULANDO HUMANO SE TIVER CAPTCHA OU NADA DE HTML
      if (!html || html.includes('Captcha') || html.includes('Verifique se você é humano')) {
          debug.push(`   ⚠️ Sem cache e com bloqueio. Acionando Navegação Real (Puppeteer)...`);
          let browser;
          try {
              // Rotação de viewport para simular telas diferentes
              const viewports = [
                {width: 1366, height: 768}, {width: 1920, height: 1080}, {width: 1440, height: 900}, {width: 1536, height: 864}
              ];
              ...
              // Disfarce extra de Webdriver (Garante o Bypass do DataDome)
              ...
              debug.push(`   ⏳ Acessando com Navegação Real...`);
              await pageObj.goto(urlPage, { waitUntil: 'domcontentloaded', timeout: 45000 });
              
              // SIMULAÇÃO HUMANA
              try {
                debug.push(`   🕺 Simulando comportamento humano...`);
                // Move o mouse randomicamente
                await pageObj.mouse.move(100 + Math.random()*200, 100 + Math.random()*200);
                await humanDelay(200, 500);
                await pageObj.mouse.down();
                await pageObj.mouse.up();
                
                // Scroll down
                await pageObj.evaluate(() => { window.scrollBy(0, window.innerHeight * 0.7); });
                await humanDelay(500, 1200);
                
                // Scroll up
                await pageObj.evaluate(() => { window.scrollBy(0, -(window.innerHeight * 0.3)); });
              } catch(e) {}
      ...
      if (html) {
          ...
          if (vendedoresPagina.length > 0) {
              ...
              vendedoresPagina.forEach(v => {
                  v.mlbId = catalogId;
                  v.link = buildItemLink(catalogId, v.link);
              });

              todosVendedores.push(...vendedoresPagina);
              debug.push(`  📊 Pág ${page}: +${vendedoresPagina.length} vendedores extraídos (Total: ${todosVendedores.length})`);
              errorsConsecutivos = 0;
              page++;
              await humanDelay(1000, 2500); // Mais delay para evitar bloqueios
          } else {
  ...
  // 4. FALLBACK FINAL: API SEARCH (Versão v5 mantida intacta)
  try {
...
async function buscarViaProducts(mlbId, url, api, debug) {
  debug.push(`🔍 Tentando rota de catálogo /products/${mlbId}...`);
...
async function buscarViaScrapingGeral(url, mlbId, debug) {
  ...
  if (!html || html.includes('Captcha') || html.includes('human')) {
    const cache = lerCaptura(mlbId, 1, debug);
    if (cache) html = cache;
  }

  if (!html || html.includes('Captcha') || html.includes('human')) {
    debug.push(`🤖 Acionando Navegação Real (Puppeteer) no link direto...`);
    let browser;
    try {
      const viewports = [{width: 1366, height: 768}, {width: 1920, height: 1080}, {width: 1440, height: 900}];
      const randomViewport = viewports[Math.floor(Math.random() * viewports.length)];
...
router.get('/api/ml/research/:mlbId', async (req, res) => {
  ...
  // Limpeza robusta contra duplo encode (%253A -> :) e remove espaços em branco da URL
  if (urlOriginal) {
      try { 
