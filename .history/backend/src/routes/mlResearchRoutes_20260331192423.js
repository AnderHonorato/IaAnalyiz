...
// Atualizado: Lógica de extração de componentes NORDIC e fallback de Puppeteer
...
function salvarCaptura(mlbId, pagina, conteudo, debug) {
    try {
        ...
        debug.push(`  💾 HTML Capturado e salvo localmente em ${filepath}`);
    } catch (e) {
...
function lerCaptura(mlbId, pagina, debug) {
    try {
        ...
        if (fs.existsSync(filepath)) {
            debug.push(`  📂 Lendo captura local (Bypass de Bloqueio de Rede)...`);
...
function buildItemLink(id, permalink) {
  ...
  return `https://produto.mercadolivre.com.br/MLB-${num}-_JM`; // Link corrigido
}
...
// ============================================================================
// EXTRATOR NATIVO CIRÚRGICO - MESCLA O CÓDIGO v5 COM O NOVO HTML DO ML
// ============================================================================

// Compilador de JSON - Ignora minificações sujas lendo as chaves {}
...
function extrairVendedoresDoHTML(html, targetUrl, paginaAtual, debug) {
  ...
  if (jsonStr) {
      try {
          ...
          debug.push(`  🧠 JSON Global decodificado com sucesso!`);
      } catch (e) {}
  }
  ...
  if (state) {
      ...
      for (let arr of paths) {
          if (Array.isArray(arr) && arr.length > 0) {
              ...
              if (arr[0].components) {
                  debug.push(`  🎯 Extraindo via Components Array (Novo Catálogo) - ${arr.length} itens`);
                  ...
                  debug.push(`  🎯 Extraindo via Flat Array (Catálogo Tradicional) - ${arr.length} itens`);
                  arr.forEach((o, index) => {
                      ...
                      let freteGratis = o.shipping?.free_shipping === true || o.freeShipping === true || o.shipping_conditions === 'free_gap';
                      ...
                      vendedores.push({
                          ...
                          titulo: o.title || o.item?.title || null, freteGratis, envio, tipoAnuncio: tipo, vendas: vendasStr, parcelamento: parcelas,
                          pagina: paginaAtual, posicao: index + 1
                      });
                  });
              }
  ...
  // MELIDATA EVENT (Alternativa caso o state geral quebre)
  const melidataJson = extrairJSON(html, 'melidata("add","event_data",');
  if (melidataJson) {
      try {
          const meli = JSON.parse(melidataJson);
          if (meli.items && Array.isArray(meli.items)) {
              debug.push(`  🧠 JSON Melidata isolado extraído com sucesso (${meli.items.length} itens)`);
              meli.items.forEach((o, index) => {
                  const nome = o.seller_name || '—';
                  const preco = o.price || 0;
                  const original = o.original_price || null;
                  const desconto = (original && original > preco) ? `${Math.round((1 - preco/original)*100)}% OFF` : '0% OFF';
                  const isFull = o.has_full_filment === true;
                  const freteGratis = o.shipping_conditions === 'free_gap' || o.shipping_conditions === 'free_ratio';
                  const envio = isFull ? 'Full' : (freteGratis ? 'Mercado Envios (Grátis)' : 'Mercado Envios');
                  const parcelas = o.installment_info ? `${o.installment_info.replace('f','')}x` : '—';
                  
                  vendedores.push({
                      nome, preco: parseFloat(preco), precoOriginal: original ? parseFloat(original) : null,
                      link: targetUrl, desconto, thumbnail: null, titulo: 'Catálogo', freteGratis, envio,
                      tipoAnuncio: 'Clássico', vendas: '+100 vendas', parcelamento: parcelas,
                      pagina: paginaAtual, posicao: index + 1
                  });
              });
              if (vendedores.length > 0) return vendedores;
  ...
  for (const m of ldMatches) {
      try {
          ...
          for (const o of offers) {
              ...
              if (preco > 0 || nome !== '—') vendedores.push({
                  nome, preco, link: o.url || targetUrl, desconto: null, thumbnail: null, titulo: ld.name || null, freteGratis: false, frete: '—',
...
async function buscarVendedoresCatalogo(catalogId, urlBase, api, debug) {
  ...
  while (page <= 40 && errorsConsecutivos < MAX_ERRORS_CONSECUTIVOS) {
      ...
      // 1. TENTA LER O CACHE SE A REDE FOR BLOQUEADA
      ...
      // 3. TENTA PUPPETEER SE TIVER CAPTCHA OU NADA DE HTML
      if (!html || html.includes('Captcha') || html.includes('Verifique se você é humano')) {
          debug.push(`   ⚠️ Sem cache e com bloqueio. Acionando Puppeteer Stealth...`);
          let browser;
          try {
              browser = await puppeteer.launch({
                  headless: true,
                  args: ['--no-sandbox', '--disable-setuid-sandbox', '--disable-gpu', '--no-zygote', '--disable-dev-shm-usage', '--disable-blink-features=AutomationControlled']
              });
              const pageObj = await browser.newPage();
              
              // Disfarce extra de Webdriver
              ...
              debug.push(`   ✅ HTML obtido via Puppeteer: ${(html.length/1024).toFixed(0)}KB`);
          } catch (err) {
      ...
      if (html) {
          ...
          errorsConsecutivos++;
          debug.push(`❌ Erro pág ${page} (${errorsConsecutivos}/${MAX_ERRORS_CONSECUTIVOS})`);
  ...
  // 4. FALLBACK FINAL: API SEARCH (O que você tinha no v5)
  // Isso não vai mais funcionar, mas deixei de garantia, conforme seu pedido.
  try {
...
async function buscarViaScrapingGeral(url, mlbId, debug) {
  ...
  let html = '';
  try {
    debug.push(`🌐 Scraping Axios direto: ${targetUrl}`);
  ...
  if (!html || html.includes('Captcha') || html.includes('Verifique se você é humano')) {
    debug.push(`🤖 Acionando Puppeteer Stealth para burlar Captcha no link direto...`);
    let browser;
    try {
      browser = await puppeteer.launch({
        headless: true,
  ...
  if (html) salvarCaptura(mlbId, 1, html, debug);
...
router.get('/api/ml/research/:mlbId', async (req, res) => {
  ...
  // Limpeza de dupla codificação de segurança
  if (urlOriginal) {
      try { urlOriginal = decodeURIComponent(decodeURIComponent(urlOriginal)); } 
      catch (e) { urlOriginal = decodeURIComponent(urlOriginal); }
  }
  ...
  // TENTATIVA 1: API OFICIAL DO ML (Padrão para itens comuns)
  try {
  ...
  // TENTATIVA 2: API DE CATÁLOGOS (Para itens MLB genéricos)
  try {
  ...
  // TENTATIVA 3: SCRAPING GERAL (Paginação e HTML Direto)
  if (urlOriginal) {
      try {
