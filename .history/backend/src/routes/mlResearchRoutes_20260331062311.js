...
function extrairVendedoresDoHTML(html, targetUrl, paginaAtual, debug) {
  ...
  // ── Estratégia 0: Novo NORDIC_RENDERING_CTX (Padrão Atual do Catálogo) ──────────
  const nordicMatch = html.match(/<script[^>]*id="__NORDIC_RENDERING_CTX__"[^>]*>_n\.ctx\.r\s*=\s*(\{[\s\S]+?\});?<\/script>/i) || 
                      html.match(/window\.__NORDIC_RENDERING_CTX__\s*=\s*(\{[\s\S]+?\});?<\/script>/i);
  
  if (nordicMatch) {
    try {
      const nordicState = JSON.parse(nordicMatch[1]);
      const resultsItems = nordicState?.appProps?.pageProps?.initialState?.results?.items;
      
      if (Array.isArray(resultsItems) && resultsItems.length > 0) {
        debug.push(`  🧠 NORDIC_CTX: Estrutura 'results.items' encontrada (${resultsItems.length} itens)`);
        ...
        resultsItems.forEach((item, index) => {
          ...
          if (Array.isArray(item.components)) {
            item.components.forEach(comp => {
              ...
              if (comp.id === 'seller') {
                nome = comp.seller?.name || comp.title_value || '—';
                if (comp.subtitles) {
              ...
              if (comp.id === 'shipping_summary') {
                ...
                if (promiseText.includes('grátis')) freteGratis = true;
      ...
      // Se não caiu no results.items, guarda o state geral para tentar a Estratégia 1
      state = nordicState;
      debug.push(`  🧠 NORDIC_CTX decodificado, buscando rotas genéricas...`);
    } catch(e) {
      debug.push(`  ⚠️ Erro no NORDIC_CTX: ${e.message}`);
    }
  }

  // ── Estratégia 1: __PRELOADED_STATE__ ou fallback do Nordic ──────────
  if (!state) {
    const matchParse = html.match(/window\.__PRELOADED_STATE__\s*=\s*JSON\.parse\(['"](.*?)['"]\);/);
    if (matchParse) {
      try {
        const raw = matchParse[1].replace(/\\"/g, '"').replace(/\\\\/g, '\\').replace(/\\'/g, "'");
        state = JSON.parse(raw);
        debug.push(`  🧠 Estado ML (JSON.parse) decodificado.`);
      } catch(e) {}
    }
  }

  if (!state) {
    const matchObj = html.match(/window\.__PRELOADED_STATE__\s*=\s*(\{[\s\S]+?\});\s*(?:<\/script>|window)/);
    if (matchObj) {
      try { state = JSON.parse(matchObj[1]); debug.push(`  🧠 Estado ML (Objeto literal) decodificado.`); } catch(e) {}
    }
  }

  // Se encontrou o JSON por qualquer caminho, vamos navegar por ele
  if (state) {
    const paths = [
      ...
      state?.initialState?.components?.buyBox?.state?.sellerOptions,
...
async function buscarViaScraping(urlOriginal, mlbId, debug) {
  ...
  while (page <= 40 && errorsConsecutivos < MAX_ERRORS_CONSECUTIVOS) {
    ...
    // ESTRATÉGIA F: Puppeteer Stealth Integrado na Paginação
