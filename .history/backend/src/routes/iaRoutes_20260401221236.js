 ...
 * 
 * Propósito:
 * Rotas principais do sistema de IA com suporte a chat, streaming SSE,
 * processamento de imagens e aprendizagem contínua da IA Analyiz.
 * 
 * Responsabilidades:
 * - POST /api/ia/chat - Receber mensagem do utilizador
 * - GET /api/ia/chat/stream - Streaming SSE de resposta da IA
 * - POST /api/ia/image - Processar imagem com visão da IA
 * - GET /api/ia/brain - Dados do cérebro (conhecimento, métricas)
 * - GET /api/ia/knowledge - Query base de conhecimento
 * - POST /api/ia/knowledge - Salvar novo conhecimento
 * - GET /api/ia/feedback - Feedback dos utilizadores
 * - POST /api/ia/feedback - Registrar feedback
 * - GET /api/ia/sessions - Histórico de chats
 * 
 * Sistema de Roteamento de Dados:
 * - Catálogo de 8 tabelas principais (produtos, divergências, utilizadores, etc)
 * - Dois modos de routing: keyword-based e Gemini intelligence routing
 * - Selecção automática de contexto relevante para cada mensagem
 * - Fallback intelligente entre modelos Gemini
 * 
 * Aprendizagem Autónoma:
 * - Processamento contínuo de mensagens com aprendizagem
 * - Armazenamento de conhecimento em Prisma
 * - Análise de sistemas com dados da BD
 * - Validação de respostas com scoring
 * 
 * APIs Utilizadas:
 * - Gemini 2.5 Flash Lite (lightweight, fast)
 * - Gemini 2.5 Flash (fallback completo)
 * - Prisma ORM para persistência
 * 
 * @author Anderson Honorato
 * @version 6.3.0
 * @requires express, prisma, Google Gemini API
 */

import express from 'express';
import { prisma } from '../prisma.js';
import { buildAnswerStream, analyzeImage, buildResumoPrompt } from '../iaService.js';
import {
...
async function geminiRouter(catalogoStr, userMessage) {
  ...
  try {
    const r = await ai.models.generateContent({
      ...
      config: { temperature:0, maxOutputTokens:200, thinkingConfig:{ thinkingBudget:0 }, systemInstruction:sys },
    ...
    const r = await ai.models.generateContent({
      ...
      config: { temperature:0, maxOutputTokens:200, systemInstruction:sys },
      contents: [{ role:'user', parts:[{ text:prompt }] }],
    });
    return (r.text || '').trim();
  }
...
function parseRouterJson(raw) {
  ...
  return {
    relevante:  Boolean(p.relevante),
    tabelas:    Array.isArray(p.tabelas) ? p.tabelas.slice(0, 5) : [],
    raciocinio: String(p.raciocinio || ''),
  };
}

async function routeDataNeeded(userMessage, userRole) {
  const isPriv = userRole === 'OWNER' || userRole === 'ADMIN';
  const cat    = Object.entries(DATA_CATALOG)
    .filter(([, v]) => !v.privileged || isPriv)
    .map(([k, v]) => `- ${k}: ${v.label}. ${v.desc}`)
    .join('\n');
  try {
    const raw = await geminiRouter(cat, userMessage);
    ...
    try {
      switch (t) {
        ...
        case 'precificacao_historico':
          data.precificacao_historico = await prisma.precificacaoHistorico
            .findMany({ where:{ usuarioId:uid }, orderBy:{ criadoEm:'desc' }, take:50 })
            .catch(() => []);
          break;
        case 'agendador':
        ...
        case 'ia_brain':
          data.ia_brain = await buscarTodoConhecimento(25);
          break;
      }
    } catch (e) {
      console.warn(`[Fetch] "${t}":`, e.message);
    }
  }));
  return data;
}
...
// ── Histórico — retorna reasoning para reexibir o ThinkingPanel ───────────────
  ...
  try {
    res.json(await prisma.chatMessage.findMany({
      ...
      select: {
        ...
        role:        true,
        content:     true,
        reasoning:   true,   // ← campo adicionado ao schema
        imageBase64: true,
        imageDesc:   true,
        createdAt:   true,
      },
...
// ── Documentos (visualizador de código) ──────────────────────────────────────
router.get('/api/chat/sessions/:id/documents', async (req, res) => {
  try {
    res.json(await prisma.chatDocument.findMany({
      where:   { sessionId: parseInt(req.params.id) },
      orderBy: { createdAt:'asc' },
    }));
...
router.post('/api/chat/sessions/:id/documents', async (req, res) => {
  try {
    const { filename, tipo, language, content } = req.body;
    ...
    res.json(await prisma.chatDocument.create({ data:{ sessionId, filename, tipo, language, content, versao } }));
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
router.post('/api/ia/chat/stream', async (req, res) => {
  ...
  try {
    if (userRole === 'BLOQUEADO') {
      send('done', { reply:'🔒 Seu perfil está bloqueado.', sources:[] });
      return res.end();
    }
    ...
    // IMPORTANTE: só a mensagem do usuário vai aqui — reply ainda não existe
    ...
    const attachDescArr    = attachmentMeta.length > 0
      ? attachmentMeta.map(a => {
          ...
          return {
            mimeType: a.mimeType, name: a.name, group: a.group, sizeBytes: a.sizeBytes,
            ...(needsPreview && a.preview && a.preview.length < MAX_PREVIEW_SIZE ? { preview: a.preview } : {}),
          };
        })
      : null;
    ...
    // ── 3. Atualiza título da sessão (usa as primeiras mensagens) ─────────────
    ...
    const historyRaw = dbMsgs.map(m => ({
      role:    m.role,
      content: m.content || (m.imageDesc ? '[arquivo]' : ''),
    }));

    // ── 5. Roteamento de dados do banco ───────────────────────────────────────
    let dataBlock = null;
    ...
    // ── 9. buildAnswerStream — reply e reasoning só existem APÓS esta linha ───
    ...
    // ── 10. Persiste resposta da IA COM reasoning ─────────────────────────────
    // Esta linha fica DEPOIS do buildAnswerStream — reply e reasoning já existem
    await prisma.chatMessage.create({
      data: {
        sessionId: session.id,
        role:      'ia',       // ← mensagem da IA
        content:   reply,
        reasoning: reasoning || null,   // ← salva o Chain of Thought no banco
      },
    });

    // ── 11. Aprendizado em background (não bloqueia o SSE) ───────────────────
...
router.post('/api/ia/chat', async (req, res) => {
  ...
  try {
    ...
    if (message && message.length > 3) {
      const c = await prisma.chatMessage.count({ where:{ sessionId: session.id } });
      if (c <= 2) {
        await prisma.chatSession.update({
          where: { id: session.id },
          data:  { titulo: message.substring(0, 50) + (message.length > 50 ? '…' : '') },
    ...
    let dataBlock = null;
    if (!imageOnly && message?.trim().length > 2) {
      const r = await routeDataNeeded(message, userRole);
      if (r.relevante && r.tabelas.length > 0) {
        const d = await fetchSelectedData(r.tabelas, userId, userRole);
        dataBlock = formatDataBlock(d);
      }
    }
    ...
    res.status(500).json({ reply:'⚠️ Erro interno.', sources:[] });
  }
});
...
router.post('/api/ia/summary', async (req, res) => {
  ...
  try {
    ...
    const response = await ai.models.generateContent({
      ...
      contents: [{ role:'user', parts:[{ text: buildResumoPrompt(dadosStr) }] }],
    });

    const conteudo = (response.text || '').trim()
      .replace(/^(conforme solicitado|aqui está|claro[,!]|segue)[^\n<]*[\n<]/im, '')
      .replace(/\*\*(.+?)\*\*/g, '<b>$1</b>')
      .replace(/\*([^*\n<]+)\*/g, '<b>$1</b>')
      .replace(/^#{1,6}\s+/gm, '')
      .replace(/^-\s+/gm, '• ')
...
router.get('/api/ia/brain/estudos', async (req, res) => {
  try {
    const l = Math.min(parseInt(req.query.limite || '50'), 100);
    res.json(await buscarEstudosRecentes(l));
  } catch { res.status(500).json({ error:'Erro.' }); }
});

router.get('/api/ia/brain/conhecimentos', async (req, res) => {
  try {
    res.json(await prisma.iaConhecimento.findMany({
...
router.get('/api/ia/brain/aprendizados', async (req, res) => {
  try {
    res.json(await prisma.iaAprendizado.findMany({
      orderBy: { createdAt:'desc' },
      take:    50,
...
router.post('/api/ia/brain/analisar', async (req, res) => {
  try { res.json(await analisarSistemaAutonomamente(req.body.userId || null)); }
  catch { res.status(500).json({ error:'Erro.' }); }
});
