...
// ============================================================================
// NOVO EXTRATOR NATIVO (COMPATÍVEL COM NORDIC_RENDERING_CTX E COMPONENTS)
// ============================================================================
function extrairVendedoresDoHTML(html, targetUrl, paginaAtual, debug) {
  const vendedores = [];
  let state = null;

  // 1. Regex à prova de falhas para isolar o JSON no _n.ctx.r
  const nordicMatch1 = html.match(/_n\.ctx\.r\s*=\s*(\{[\s\S]+?\});\s*_n\.ctx\.r/);
  if (nordicMatch1) {
    try { state = JSON.parse(nordicMatch1[1]); debug.push(`  🧠 NORDIC_CTX (_n.ctx.r) decodificado.`); } catch(e) { debug.push(`  ⚠️ Erro parse Nordic 1: ${e.message}`); }
  }
  ...
  if (!state) {
    ...
    if (preloadedMatch1) {
      try { state = JSON.parse(preloadedMatch1[1]); debug.push(`  🧠 PRELOADED_STATE decodificado.`); } catch(e) {}
    }
  }
  ...
  if (!state) {
    ...
    if (preloadedMatch2) {
      try { 
        ...
        debug.push(`  🧠 PRELOADED_STATE (JSON.parse) decodificado.`);
      } catch(e) {}
    }
  }
  ...
  if (state) {
    ...
    for (const arr of paths) {
      if (Array.isArray(arr) && arr.length > 0) {
        ...
        // TIPO 1: Novo Formato de Catálogo (Usa Array de Components por Item)
        if (arr[0].components) {
          ...
          arr.forEach((item, index) => {
            ...
            if (Array.isArray(item.components)) {
              item.components.forEach(comp => {
                ...
                if (comp.id === 'seller' || comp.type === 'seller') {
                  nome = comp.seller?.name || comp.title_value || comp.seller_info?.title || comp.seller_name || '—';
                  if (comp.subtitles) {
            ...
            if (nome === '—' && item.seller_name) nome = item.seller_name;
            ...
            if (preco > 0 || nome !== '—') {
              vendedores.push({
                ...
                link, desconto, thumbnail: null, titulo: 'Anúncio Catálogo', freteGratis, envio, tipoAnuncio: 'Clássico',
