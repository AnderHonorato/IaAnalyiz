...
function buildPerfilVendedor(nickname, userType, eshop) {
  ...
  // user_type "brand" = loja oficial (/loja/)
  // eshop presente e não null = Mercado Shops mas ainda /pagina/
  // demais = /pagina/
  const tipo = userType === 'brand' ? 'loja' : 'pagina';
...
async function getSellerFullCached(sellerId, api) {
  ...
  try {
    ...
    const permalink = buildPerfilVendedor(s.nickname, s.user_type, s.eshop);
