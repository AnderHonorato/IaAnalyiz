...
// ═════════════════════════════════════════════════════════════════════════════════
// 🚀 ENVIO EM MASSA — Atualizar peso/dimensões no Mercado Livre
// ═════════════════════════════════════════════════════════════════════════════════

// POST /api/divergencias/envio-massa
// ─────────────────────────────────────────────────────────────────────────────
// Processa TODAS divergências PENDENTE_ENVIO (ou lista específica), atualiza
// cada uma via API ML (PUT /items/{id}), marca como CORRIGIDO, e registra histórico.
//
// Estratégia:
// 1. Busca token OAuth do usuário e inicializa cliente Axios com auth header
// 2. Itera sobre divergências PENDENTE_ENVIO da fila
// 3. Para cada item:
//    a. Busca produto local (com kit se houver) e calcula peso correto
//    b. Busca item atual no ML para pegar quantidade disponível
//    c. Constrói array de attributes (peso, dimensões, EAN, marca, modelo)
//    d. Envia PUT /items/{itemId} para atualizar atributos
//    e. Marca como CORRIGIDO + registra histórico + resolve avisos ML
// 4. Aguarda 600ms entre requisições para respeitar rate limit do ML
// 5. Retorna relatório: total processado, sucessos, erros com mensagens
//
// Body:
//   @userId (required) - ID do usuário autenticado
//   @ids (optional) - Array de IDs de divergência. Se vazio, processa TODAS
//
// Response:
//   {
//     ok: true,
//     total: 42,           // Quantidade processada
//     sucesso: 38,         // Quantas atualizações funcionaram
//     erros: 4,            // Quantas falharam
//     resultados: [        // Array com detalhe de cada operação
//       { id: 1, mlItemId: "MLB123", status: "ok", pesoEnviado: 1500 },
//       { id: 2, mlItemId: "MLB456", status: "erro", msg: "Token expirado" },
//       ...
//     ]
//   }
//
// Erros esperados:
//   • Status 500 + mensagem se token não encontrado ou expirado
//   • Peso inválido → registrado como "erro" em resultados, mas não interrompe loop
//   • API ML indisponível → erro capturado e reportado
//
// Nota: Cada requisição ao ML é consolidada em 1 PUT (não múltiplos). Weight
// sempre em gramas, dimensões em cm, conforme padrão Mercado Livre.
router.post('/api/divergencias/envio-massa', async (req, res) => {
  ...
  try {
    ...
    for (const div of divergencias) {
      ...
      try {
        // ─ Buscar produto local com kit (se houver) e calcular peso correto
        ...
        // ─ Buscar item atual no ML para pegar quantidade
        const itemRes = await mlApi.get(`/items/${mlItemId}`);
        const qtd    = itemRes.data.available_quantity ?? 1;

        // ─ Construir attributes array com peso + dimensões + metadados
        ...
        // ─ Enviar PUT para atualizar attributes no ML
        await mlApi.put(`/items/${mlItemId}`, { attributes, available_quantity: qtd });

        // ─ Marcar como CORRIGIDO + registrar histórico
...
// ═════════════════════════════════════════════════════════════════════════════════
// ⚠️  AVISOS ML — Sincronização de health issues do Mercado Livre
// ═════════════════════════════════════════════════════════════════════════════════

// GET /api/ml/avisos
// ─────────────────────────────────────────────────────────────────────────────
// Lista todos os avisos de saúde do anúncio (health issues) registrados no DB.
// Filtra por usuário e opcionalmente por status de resolução.
//
// Query params:
//   @userId (required) - ID do usuário autenticado
//   @resolvido (optional) - "true" ou "false" para filtrar apenas resolvidos/pendentes
//
// Response:
//   Array de objetos AvisoML com: tipoAviso, severidade, mensagem, etc
...
// POST /api/ml/avisos/sincronizar
// ─────────────────────────────────────────────────────────────────────────────
// Sincroniza avisos de saúde do Mercado Livre com o banco local. Chama:
// 1. /users/me para obter ID do vendedor
// 2. /users/{sellerId}/items/search (status: active + paused) para lista de IDs
// 3. /items?ids=... (batch) para cada lote de 20 items, buscando health
// 4. Processa cada health.issue: filtra por tipo (peso/dimensão), cria/atualiza AvisoML
//
// Estratégia de tipagem:
// • Se mensagem contém "peso" ou "weight" → tipo='PESO_INCORRETO'
// • Se contém "dimens", "package", "logist", "frete", "shipping" → tipo='DIMENSOES_INCORRETAS'
// • Ignora other issues (preço, descrição, etc)
//
// Severidade:
// • Se "critical", "severe", "bloqueado", "desativado" na mensagem → ALTO
// • Senão → MEDIO
//
// Body:
//   @userId (required) - ID do usuário autenticado
...
//   {
//     ok: true,
//     novos: 3,     // Quantos novos avisos foram criados/atualizados
//     total: 15     // Total de avisos ativos (não resolvidos) agora
//   }
//
// Rate throttling: 400ms entre batches de 20 items para não sobrecarregar API
router.post('/api/ml/avisos/sincronizar', async (req, res) => {
  ...
  try {
    ...
    // ─ Obter ID do vendedor e listar TODOS items (active + paused)
    ...
    // ─ Processar em lotes de 20 items
    for (let i = 0; i < todosIds.length; i += 20) {
      ...
      try {
        ...
        for (const entry of (detRes.data || [])) {
          ...
          const itemId = item.id, 
                ...
                statusML = item.status || 'unknown';
          const healthIssues = item.health?.issues || [];
          
          // ─ Processa cada health issue e cria aviso se relevante
          for (const issue of healthIssues) {
            const codigo = issue.code || 'UNKNOWN', 
                  msg = issue.cause?.join('. ') || issue.message || JSON.stringify(issue);
            
            // Filtrar apenas issues de peso/dimensão/frete (ignorar preço, descrição, etc)
            ...
            // Upsert: cria novo ou atualiza existente (chave composta)
          ...
          // ─ Se item estiver ativo e sem health issues → marca avisos como resolvidos
      ...
      // ─ Rate throttling: 400ms entre batches
...
// PUT /api/ml/avisos/:id/resolver
// ─────────────────────────────────────────────────────────────────────────────
// Marca um aviso específico como resolvido + registra timestamp. Usado após
// usuário ter corrigido a questão no Mercado Livre.
//
// Response:
//   Objeto AvisoML atualizado com resolvido=true, resolvidoEm=now
...
// GET /api/ml/avisos/count
// ─────────────────────────────────────────────────────────────────────────────
// Retorna contagem rápida de avisos NÃO resolvidos de um usuário. Usado em
// badges/counters na UI para mostrar quantos problemas pendentes existem.
//
// Query params:
...
// ═════════════════════════════════════════════════════════════════════════════════
// 📦 PRODUTOS — CRUD com suporte a kits (produtos compostos)
// ═════════════════════════════════════════════════════════════════════════════════
//
// Um PRODUTO pode ser:
// • Item simples: peso e dimensões diretos
// • Kit (eKit=true): composição de múltiplos produtos (cada um com quantidade)
//   Peso do kit = sum(produto.peso * quantidade) para todos os itens
//
// Campos principais:
//   - nome, sku, mlItemId, preco, pesoGramas, alturaCm, larguraCm, comprimentoCm
//   - eKit (boolean), categoria, status (active/inactive), thumbnail
//   - ean, marca, modelo, condicao (Novo/Usado), descricao (novo campo)

// GET /api/produtos
// ─────────────────────────────────────────────────────────────────────────────
// Lista produtos do usuário com múltiplos filtros. Se for kit, inclui array
// `itensDoKit` com relação para cada componente e suas quantidades.
//
// Query params:
//   @userId (required) - ID do usuário autenticado
//   @categoria (optional) - Filtrar por categoria exata
//   @status (optional) - Filtrar por status (active, inactive, etc)
//   @plataforma (optional) - Filtrar por plataforma de origem
//   @search (optional) - Busca full-text em nome, SKU ou mlItemId (case-insensitive)
//
// Response:
//   Array de objetos Produto, incluindo `itensDoKit` se for kit
...
// ─────────────────────────────────────────────────────────────────────────────
// Retorna lista de categorias ÚNICAS do usuário: útil para popular dropdowns.
// Query faz SELECT DISTINCT categoria e filtra null.
//
// Query params:
//   @userId (required) - ID do usuário autenticado
//
// Response:
//   Array de strings: ["Eletrônicos", "Livros", "Moda", ...]
...
// ─────────────────────────────────────────────────────────────────────────────
// HELPER: Parse dados de produto do request body e normalizar tipos
// ─────────────────────────────────────────────────────────────────────────────
// Extrai campos de produto do body, converte tipos para os esperados:
// • string: nome, sku, mlItemId, plataforma, categoria, status, thumbnail, ean, marca, modelo, condicao, descricao
// • number: preco, pesoGramas, alturaCm, larguraCm, comprimentoCm
// • boolean: eKit
// Retorna objeto limpo pronto para .create() ou .update()
function parseProdutoBody(body) {
  ...
  return {
    ...
    descricao:     descricao  || null, // ← campo novo de descrição estendida
  };
}

// POST /api/produtos
// ─────────────────────────────────────────────────────────────────────────────
// Cria um novo produto para o usuário. Usa helper parseProdutoBody para
// normalizar e validar tipos.
//
// Body:
//   @userId (required)
//   + campos de produto (ver parseProdutoBody)
//
// Response:
//   201 Created + objeto Produto criado
...
// PUT /api/produtos/:id
// ─────────────────────────────────────────────────────────────────────────────
// Atualiza um produto existente. Recebe campo ID da URL, dados do body.
//
// Params:
//   @id - ID do produto
//
// Body:
//   + campos de produto a atualizar (ver parseProdutoBody)
//
// Response:
...
// ─────────────────────────────────────────────────────────────────────────────
// Remove um produto e sua composição (KitItems deletados por cascade automático).
//
// Params:
//   @id - ID do produto
//
...
// ─────────────────────────────────────────────────────────────────────────────
// Importa lote de produtos (típico após sincronização com Mercado Livre).
// Para cada produto: verifica se já existe (by mlItemId), atualiza ou cria novo.
//
// Body:
//   @userId (required)
//   @produtos (array de produtos com: sku, nome, mlItemId, preco, pesoGramas,
//              categoria, status, thumbnail)
//
// Response:
//   { ok: true, criados: 10, atualizados: 5 }
...
// ─────────────────────────────────────────────────────────────────────────────
// Converte um produto simples em KIT: associa múltiplos produtos base como
// componentes e calcula peso total automaticamente.
//
// Estratégia:
// 1. Limpa KitItems antigos (DELETE WHERE kitId={id})
// 2. Se composicao vazia → usa peso manual, marca eKit=false
// 3. Se composicao fornecida → calcula sum(peso * qtd), marca eKit=true
// 4. Atualiza produto com novo pesoGramas e eKit
//
// Body:
//   @composicao (array de { produtoId, quantidade }) - itens do kit
//   @pesoManual (number, opcional) - peso se não for kit ou sem itens
//
// Response:
//   Objeto Produto atualizado com pesoGramas calculado
//
// Exemplo:
//   Request: { composicao: [{produtoId: 5, quantidade: 2}, {produtoId: 7, quantidade: 1}] }
//   Result: pesoGramas = (produto5.peso * 2) + (produto7.peso * 1)
router.put('/api/produtos/:id/vincular', async (req, res) => {
  try {
    ...
    if (composicao && composicao.length > 0) {
      eKit = true;
      // ─ Processa cada item da composição
...
// ─────────────────────────────────────────────────────────────────────────────
// Desfaz composição de kit: remove todos KitItems e marca como plataforma
// ML_PENDENTE (precisa de sincronização), eKit=false, pesoGramas=0.
//
// Params:
//   @id - ID do kit a desvincular
//
// Response:
//   Objeto Produto atualizado (sem composição)
