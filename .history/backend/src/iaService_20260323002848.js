// backend/src/iaService.js — v5: Fix result bug, todos tipos de arquivo, navegação de página
...
// ═══════════════════════════════════════════════════════════════════════════════
// PROCESSAMENTO DE ARQUIVOS — corrigido: sem bug de 'result' antes de init
// ═══════════════════════════════════════════════════════════════════════════════
...
/**
 * Processa UM arquivo — CORRIGIDO: sem referência a result antes de inicialização
 * Retorna { context, label, name, stepsUsed }
 */
export async function processFileToContext(file, userMessage, onStep, baseStepKey = 0) {
  const { base64, mimeType, name, group } = file;
  let context = null;
  let fileTypeLabel = '';
  let stepKeyLocal = baseStepKey; // Contador LOCAL para este arquivo

  // Helper: emite step + retorna key; o caller chama emitDone depois
...
// ═══════════════════════════════════════════════════════════════════════════════
// FUNCTION CALLING (TOOLS) — inclui lerPagina
// ═══════════════════════════════════════════════════════════════════════════════
...
async function executeTool(name, args, userId, userRole, pageBaseUrl) {
  ...
  try {
    switch (name) {
      ...
      // ── NOVA FERRAMENTA: lerPagina ─────────────────────────────────────────
      case 'lerPagina': {
        let url = args.caminho || '/';
        // Detecta se é caminho relativo ou URL completa
        if (!url.startsWith('http')) {
          // Usa o base URL do contexto (local ou produção)
...
Exemplos de uso:
• "O que tem na página de auditoria?" → lerPagina({ caminho: "/ml/auditoria" })
• "Me explica o que está em /ml/precos" → lerPagina({ caminho: "/ml/precos" })
• "Leia a home pra mim" → lerPagina({ caminho: "/" })
A ferramenta acessa o HTML real da página e extrai o texto.

=== FUNCIONALIDADES DE INTERFACE (sem ferramenta) ===
• Tema → ícone paleta na barra do topo. Zoom → botões +/- no header.
• Perfil → avatar no canto superior direito. Conectar ML → /ml → "Conectar ML".
...
6. Para ações impossíveis no cargo atual: explique claramente a limitação.`;
}
...
function buildReasoningInstruction(ctx) {
  ...
  return `Você é o módulo de raciocínio interno da IA Analyiz — o que escrever será exibido ao usuário em tempo real.
Usuário: ${nomeUsuario} | Cargo: ${userRole} — ${caps.desc}
${hasFiles ? `ARQUIVOS RECEBIDOS:\n${fileDetails}` : ''}
Ferramentas disponíveis: ${toolsAvail || 'nenhuma'}
Ferramentas INDISPONÍVEIS para este cargo: ${toolsUnavail}

ESCREVA em português corrido, SEM emojis, SEM markdown, SEM listas numeradas. Parágrafos fluidos, 150-220 palavras.
O usuário vê esse texto em tempo real — seja claro, específico e humano.

Aborde em parágrafos naturais:
- Quem é o usuário ${nomeUsuario} e o que pediu/enviou — mencione cada arquivo PELO NOME exato e tipo
- Como cada arquivo será processado: imagem=análise visual; PDF=extração de texto; Excel/CSV=estrutura e dados; áudio=transcrição; texto=leitura direta
- Quais ações/ferramentas serão executadas — OU se NÃO É POSSÍVEL: "O usuário ${nomeUsuario} pediu para realizar [ação] mas verifiquei que não tenho acesso a essa funcionalidade para o cargo ${userRole}"
- Pontos de atenção (qualidade do arquivo, ambiguidade, permissões)
- Como a resposta final será estruturada

Escreva em fluxo contínuo, sem títulos. Texto aparece letra por letra.`;
}
...
function buildFallbackReasoning(message, context) {
  ...
  if (hasFiles && fileNames.length > 0) {
    const TIPO_LABELS = { image:'imagem', pdf:'PDF', excel:'planilha', txt:'texto', audio:'áudio' };
    const desc = fileNames.map((n, i) => `"${n}" (${TIPO_LABELS[fileTypes[i]] || fileTypes[i] || 'arquivo'})`).join(', ');
    parts.push(`O usuário ${nome} enviou ${fileNames.length} arquivo(s): ${desc}.`);
    parts.push(`Cada arquivo será processado sequencialmente. PDFs terão texto extraído, imagens serão analisadas visualmente, áudios transcritos, planilhas terão estrutura identificada e arquivos de texto lidos diretamente.`);
  }

  if (/divergen|peso|frete|corrig/i.test(message||'')) {
    parts.push(`A mensagem envolve divergências de peso/frete. ${totalDivergencias} divergências ativas. Consultarei listarDivergenciasAtivas.`);
  } else if (/produto|sku|catálogo/i.test(message||'')) {
    parts.push(`Pergunta sobre catálogo de produtos. ${totalProdutos} itens cadastrados.`);
  } else if (/página|pagina|url|site|acessa/i.test(message||'')) {
    parts.push(`O usuário quer informações sobre uma página do sistema. Usarei lerPagina para acessar e ler o conteúdo da URL solicitada.`);
  } else if (/aprovar|bloquear|usuário.*acesso/i.test(message||'')) {
    if (userRole === 'USUARIO') {
      parts.push(`O usuário ${nome} pediu para gerenciar acessos, mas verifiquei que não tenho acesso a essa funcionalidade para o cargo USUARIO. Apenas ADMIN e OWNER podem aprovar ou bloquear usuários.`);
    } else {
      parts.push(`Ação de gerenciamento de usuários. Listarei pendentes e executarei a ação.`);
    }
  } else if (!hasFiles) {
    parts.push(`Resposta informativa com base no contexto disponível.`);
  }

  parts.push(`A resposta será estruturada de forma concisa, confirmando ações executadas quando aplicável.`);
  return parts.join(' ');
}
...
// ═══════════════════════════════════════════════════════════════════════════════
// buildAnswerStream — CORRIGIDO: sem bug de 'result' antes de init
// ═══════════════════════════════════════════════════════════════════════════════
...
export async function buildAnswerStream(message, history = [], context = {}, onStep, attempt = 1) {
  ...
  try {
    ...
    // ── FASE 2: Processa cada arquivo — CORRIGIDO sem bug 'result' ─────────────
    const fileContexts = [];
    let globalStepKey  = 0;
    ...
    if (incomingFiles.length > 0) {
      ...
      for (let i = 0; i < incomingFiles.length; i++) {
        ...
        // Captura o globalStepKey atual para este arquivo
        const fileBaseKey = globalStepKey;

        // Wrapper que traduz os step keys locais para globais
        const fileOnStep = (evt) => {
          if (evt.type === 'step') {
            if (onStep) onStep({ type:'step', msg:evt.msg, tool:evt.tool });
          } else if (evt.type === 'step_done') {
            // O stepIndex local precisa ser mapeado para global
        ...
        // CORRIGIDO: processFileToContext agora usa baseStepKey = globalStepKey
        // e retorna stepsUsed; não há mais referência a result antes de init
    ...
    while (response.functionCalls && response.functionCalls.length > 0 && callCount < 5) {
      ...
      // Step especial para lerPagina: mostra qual URL está acessando
