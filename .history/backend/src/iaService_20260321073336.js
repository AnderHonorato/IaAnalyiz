...
const SYSTEM_TOOLS = [{
  functionDeclarations: [
    ...
    },
    {
      ...
      description: 'Verifica se a varredura automática do usuário está ativa e de quanto em quanto tempo ela roda.',
      parameters: {
        type: 'OBJECT',
        properties: {}
      }
    },
    {
      ...
      description: 'Liga a varredura automática do Mercado Livre e define o intervalo de tempo.',
      parameters: {
        type: 'OBJECT',
        properties: {
          intervaloMinutos: { type: 'INTEGER', description: 'Tempo em minutos (ex: 30, 60, 120, 240, 360)' }
        },
  ...
  try {
    switch (name) {
      ...
      case 'consultarAgendador': {
        ...
        if (!ag) return { mensagem: "Nenhum agendador configurado para este usuário ainda." };
        return { statusAgendador: ag.ativo ? "Ligado" : "Desligado", intervaloMinutos: ag.intervalo, ultimaExecucao: ag.ultimaExecucao };
      }
      ...
      case 'ativarAgendador': {
        ...
        return { sucesso: true, mensagem: `✅ Perfeito! Agendador ativado para rodar a cada ${min} minutos.` };
      }
...
Você AGORA TEM MÃOS no sistema! Você pode chamar funções locais para resolver problemas ou consultar o agendador.
Se o utilizador pedir para "corrigir a divergência" ou "mandar pra fila":
...
Se o utilizador pedir para ligar a varredura/agendador:
1. Chame "ativarAgendador" com o tempo solicitado.

NÃO mande o utilizador clicar em botões se VOCÊ mesma pode chamar a função para ele!
...
export async function buildAnswer(message, history = [], context = {}, attempt = 1) {
  ...
  try {
    ...
    // FIX: A Google não permite misturar Google Search com Function Calling.
    if (useSearch) {
      config.tools = [{ googleSearch: {} }];
      console.log('[IA] 🌐 Google Search ativado (Funções internas desativadas nesta resposta)');
    } else {
      config.tools = [...SYSTEM_TOOLS]; // Só usa as ferramentas se não for pesquisar na net
    }
    ...
    while (response.functionCalls && response.functionCalls.length > 0 && callCount < 3) {
      ...
      // 4. Chama a IA de novo para ela processar o resultado e responder ao utilizador
