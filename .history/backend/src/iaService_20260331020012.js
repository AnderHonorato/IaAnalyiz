...
// ═══════════════════════════════════════════════════════════════════════════════
// CHAIN OF THOUGHT E CORES — System Prompt Principal
// ═══════════════════════════════════════════════════════════════════════════════
...
function buildDynamicReasoning(message, fileContexts, context, hasFiles) {
  ...
  if (hasFiles && fileContexts.length > 0) {
    ...
    parts.push(`**Mensagem recebida de ${nome}:** "${preview}${msg.length > 120 ? '…' : ''}"\nClassifico como uma ${complexidade} (${numPalavras} palavras). Preciso identificar a intenção real antes de decidir como responder.`);
  }
  ...
  if (/site|website|landing page|html.*css|criar.*página/i.test(msg)) {
    ...
    parts.push(`**Intenção identificada: desenvolvimento web.**\nO usuário quer criar um "${assunto}". Antes de gerar código, preciso planejar: estrutura HTML semântica, responsividade CSS, paleta de cores adequada ao tema e hierarquia visual. Vou entregar o código completo e funcional, sem truncamentos, com nome de arquivo baseado no projeto.`);
  } else if (/```|function|const |import |class |def |<div|<button|\bjsx\b|\btsx\b/i.test(msg)) {
    const tipo = /bug|erro|fix|broken|quebr/i.test(msg) ? 'correção de bug' : /refactor|melhor|otimiz|clean/i.test(msg) ? 'refatoração de código' : 'análise e revisão de código';
    parts.push(`**Intenção identificada: ${tipo}.**\nHá código-fonte na mensagem. Preciso analisar: sintaxe, lógica, edge cases e possíveis falhas silenciosas. Vou manter os nomes originais de variáveis e funções, e entregar o código corrigido sem elipses.`);
  } else if (needsWebSearch(msg)) { 
    const tema = msg.replace(/pesquisar?|buscar?|ver|qual|quanto|como/gi, '').trim().substring(0, 80);
    parts.push(`**Intenção identificada: pesquisa de dados externos.**\nA pergunta sobre "${tema}" requer informações atualizadas da internet. Meu conhecimento interno pode estar desatualizado para este tema, por isso estou acionando o Agente de Pesquisa Web para buscar dados em tempo real.`);
  } else if (/divergen|peso|frete|auditoria|varredura|reincidente/i.test(msg)) {
    const n = context.totalDivergencias || 0;
    const urgencia = n > 10 ? 'situação crítica — muitas divergências acumuladas' : n > 3  ? 'atenção necessária' : n === 0 ? 'sistema saudável no momento' : `${n} divergência(s) ativa(s)`;
    parts.push(`**Intenção identificada: logística Mercado Livre.**\nEstado atual do sistema: ${urgencia}. Vou consultar o banco de dados para trazer os detalhes reais — IDs, motivos, status e histórico. Não vou presumir dados; usarei apenas o que estiver registrado.`);
  } else if (/produto|sku|catálogo|estoque|kit/i.test(msg)) {
    const p = context.totalProdutos || 0;
    parts.push(`**Intenção identificada: gestão de catálogo.**\n${nome} tem ${p} produto(s) cadastrado(s). Vou usar as ferramentas do sistema para buscar dados precisos — SKU, peso, vínculos ML, status. Se houver produtos sem peso ou sem vínculo, vou destacar isso na resposta.`);
  } else if (/usuário|acesso|aprovar|bloquear|permissão/i.test(msg)) {
    if (role === 'OWNER' || role === 'ADMIN') {
      const pend = (context.pendentes || []).length;
      parts.push(`**Intenção identificada: gestão de usuários.**\n${nome} opera no nível ${role}, com permissão total de gerenciamento. ${pend > 0 ? `Há ${pend} usuário(s) aguardando aprovação — vou listar com nome e e-mail.` : 'Nenhum usuário pendente no momento.'} Vou verificar o estado atual antes de responder.`);
    } else {
      parts.push(`**Intenção identificada: gestão de usuários — acesso insuficiente.**\nO nível de acesso atual (${role}) não permite esta operação. Vou explicar a limitação de forma clara e indicar quem pode realizar essa ação.`);
    }
  } else if (/preço|precific|valor|custo|faturamento|margem/i.test(msg)) {
    parts.push(`**Intenção identificada: precificação.**\nVou buscar o histórico de preços no banco para embasar a resposta com dados reais. Se necessário, calcularei margens considerando frete, impostos e comissão ML.`);
  } else if (/resumo|relatório|métricas|dashboard|visão geral|panorama/i.test(msg)) {
    const p = context.totalProdutos || 0;
    const d = context.totalDivergencias || 0;
    parts.push(`**Intenção identificada: visão geral do sistema.**\nEstado atual: ${p} produto(s) no catálogo, ${d} divergência(s) ativa(s). Vou compilar as métricas disponíveis e apresentar em formato claro, destacando pontos de atenção.`);
  } else if (hasFiles) {
    const tipoArq = fileContexts[0]?.group || 'arquivo';
    const chars   = fileContexts[0]?.context?.length || 0;
    parts.push(`**Intenção identificada: análise de ${tipoArq}.**\nExtraí ${chars.toLocaleString('pt-BR')} caracteres do arquivo. Vou estruturar a resposta com base no conteúdo real extraído, usando a pergunta do usuário para guiar o que priorizar.`);
  } else {
    const numPalavras = msg.trim().split(/\s+/).length;
    const tipo = numPalavras <= 5 ? 'pergunta direta' : numPalavras <= 20 ? 'consulta conversacional' : 'mensagem elaborada';
    parts.push(`**Intenção identificada: ${tipo}.**\nNão há arquivos nem dados específicos do sistema envolvidos. Vou verificar se preciso de contexto adicional ou se consigo responder diretamente com base na conversa e no conhecimento disponível.`);
  }

  if (needsWebSearch(msg)) {
    parts.push(`**Estratégia:** Agente de Pesquisa Web ativado.\nVou aguardar os resultados da varredura antes de formular a resposta final. Depois, o Agente Validador vai cruzar os dados para garantir precisão.`);
  } else if (context.dataBlock && context.dataBlock.length > 200) {
    parts.push(`**Estratégia:** Usando contexto carregado do sistema (${context.dataBlock.length.toLocaleString('pt-BR')} chars).\nOs dados já estão disponíveis — vou basear cada afirmação nas informações reais, sem presumir valores.`);
  } else if (/divergen|produto|usuário|preço|agendador/i.test(msg)) {
    parts.push(`**Estratégia:** Consulta ao banco via Function Calling.\nVou acionar as ferramentas internas para buscar os dados mais recentes antes de responder. Isso garante que os números e status apresentados sejam precisos.`);
  } else if (/site|website|html|css|código|script/i.test(msg)) {
    parts.push(`**Estratégia:** Geração de código completo.\nVou estruturar o código antes de escrever — definindo componentes, responsividade e nomeação de arquivos. Entrega sem truncamentos ou placeholders.`);
  }
...
function extrairDadosOcultosDoHTML(html) {
  let dadosExtras = {
    vendas: "—",
    parcelamento: "—",
    envio: "Normal",
  ...
  try {
    const matchContext = html.match(/window\.__NORDIC_RENDERING_CTX__\s*=\s*({.+?});<\/script>/);
    if (matchContext) {
      const ctx = JSON.parse(matchContext[1]);
      const results = ctx?.appProps?.pageProps?.initialState?.results || [];
      const itemPrincipal = results[0];
      if (itemPrincipal && itemPrincipal.components) {
        itemPrincipal.components.forEach(comp => {
          if (comp.id === 'price' && comp.price) {
            if (comp.price.original_value) dadosExtras.precoOriginal = comp.price.original_value;
            if (comp.discount_label && comp.discount_label.value) {
              dadosExtras.desconto = `${comp.discount_label.value}% OFF`;
            }
          }
          if (comp.id === 'payment_summary' && comp.title) {
            let textoParcela = comp.title.text || "";
            if (comp.title.values && comp.title.values.price_installments) {
               const valorParc = comp.title.values.price_installments.value;
               textoParcela = textoParcela.replace('{price_installments}', `R$ ${valorParc}`);
            }
            dadosExtras.parcelamento = textoParcela;
          }
          if (comp.id === 'seller' && comp.subtitles) {
            const subtitleVendas = comp.subtitles.find(s => s.text && s.text.toLowerCase().includes('vendas'));
            if (subtitleVendas) {
              dadosExtras.vendas = subtitleVendas.text.replace(/\{.*?\}/g, '|').split('|').pop().trim();
            }
            if (comp.seller_info && comp.seller_info.power_seller_status) {
              dadosExtras.mercadoLider = comp.seller_info.power_seller_status.title;
            }
          }
          if (comp.id === 'shipping_summary' && comp.title) {
            const freteText = comp.title.text || "";
            const promessa = comp.title.values?.promise?.text || "";
            if (html.includes('full_icon') || html.includes('FULL_SUPER')) {
              dadosExtras.envio = "⚡ FULL";
            } else if (promessa.toLowerCase().includes('grátis')) {
              dadosExtras.envio = "Frete Grátis";
            } else {
              dadosExtras.envio = "Mercado Envios";
            }
          }
        });
      }
...
export async function buildAnswerStream(message, history = [], context = {}, onStep, attempt = 1) {
  ...
  if (isOwner && message) {
    ...
    if (ml.match(/iniciar aprendizado/)) {
      ...
      return { reply:'<b>🧠 Aprendizado Autônomo Iniciado.</b><br>Estou buscando dados simulados, testando minhas respostas e consolidando conhecimento no banco em segundo plano.', sources:[], reasoning:'', toolsExecutadas:[] };
    }
  ...
  try {
    ...
    const runResult = await runOrchestrator(message, history, context, onStep, async (msgToBuild, histRaw, ctxEnriquecido) => {
        ...
        if (!raw) return { reply:'⚠️ Resposta vazia. Tente novamente.', sources:[], reasoning:reasoningText, toolsExecutadas: toolsExecutadasMãe };
