// backend/src/iaService.js — v4 (Integração Multi-Agente e Pesquisa Web)
// Fixes aplicados v4:
//  1. Integração oficial com searchAgent.js para pesquisas na web em tempo real.
//  2. Regex needsWebSearch muito mais abrangente (cobre comparações, preços, etc).
//  3. Pipeline que injeta os resultados do agente web direto no prompt principal.
...
// IMPORTAÇÃO DO SEU AGENTE DE PESQUISA WEB
import { agentePesquisaProfunda } from './ia/agents/searchAgent.js';
...
Faça todo o raciocínio completo na sua cabeça, mas **NUNCA** inclua nenhum bloco de pensamento, skill, classificação, "Chain of Thought", "Analisando intenção" ou qualquer texto de raciocínio na mensagem enviada ao usuário.

Regras obrigatórias:
- O raciocínio é 100% interno.
- A resposta final deve ser APENAS a mensagem útil, clara e formatada em HTML (quando necessário).
- Nunca comece a resposta com "Pensando…", "Skill ativada", "Analisando intenção" ou qualquer texto de raciocínio.
...
function buildChainOfThoughtText(message, fileContexts, context, hasFiles) {
  ...
  switch (classification.type) {
    case 'WEBSITE':
      classText = `Identifiquei uma solicitação de desenvolvimento web. Skill selecionada: Desenvolvimento Web. Protocolo: Etapa A (Planejamento) — definir estrutura semântica HTML e arquitetura CSS; Etapa B (Geração) — criar código completo com HTML semântico, CSS moderno (variáveis, flex/grid, responsivo) e JavaScript quando necessário; Etapa C (Nomenclatura) — os arquivos serão nomeados com base no assunto do cliente.`;
      break;
    case 'CÓDIGO':
  ...
  if (needsWebSearch(msg)) {
    toolText = `A solicitação exige dados em tempo real da internet (pesquisa de mercado, comparação de modelos, preços ou tendências). Vou invocar o Agente de Pesquisa Web para trazer o contexto atualizado antes de formular a resposta final.`;
...
// FIX: A Regex agora é super agressiva para sempre notar quando você quer pesquisar algo
...
export async function buildAnswerStream(message, history = [], context = {}, onStep, attempt = 1) {
  ...
  try {
    ...
    // ─── INTEGRAÇÃO MULTI-AGENTE: ACIONAR O AGENTE DE PESQUISA ───
    let searchData = null;
    let searchSources = [];
    const isImageOnly = !!context.imageOnly;

    // Se precisar de internet e não for apenas uma imagem
    if (needsWebSearch(message) && !isImageOnly) {
      if (onStep) {
        await emitStep(onStep, '🌐 Acionando Agente de Pesquisa Web...', null, 300);
      }
      const contextForAgent = `Sessão do usuário: ${context.usuarioAtual?.nome || 'Desconhecido'}`;
      
      // Chamada real ao seu novo agente!
      ...
      if (searchResult.sucesso) {
        ...
        // Pega os links do agente para colocar na resposta final
        searchSources = searchResult.fontes.map(uri => {
          try {
            return { label: new URL(uri).hostname.replace('www.', ''), url: uri };
          } catch {
            return { label: uri, url: uri };
          }
        });
      }
      if (onStep) await emitDone(onStep, globalStepKey++, 200);
    }
    ...
    // Prompt com arquivos e com o RESULTADO DA PESQUISA injetados
    let injectPrompt = message || '[arquivo enviado sem texto]';
    if (fileContexts.length > 0) {
      injectPrompt += `\n\n[ARQUIVOS LIDOS]:\n${fileContexts.map(f => `[${f.name}]:\n${(f.context || '').substring(0, 6000)}`).join('\n\n')}`;
    }
    
    // INJEÇÃO DA PESQUISA DO AGENTE PARA A IA PRINCIPAL
    if (searchData) {
    ...
    // Agora o modelo principal SEMPRE usa as ferramentas do sistema (banco de dados, auditoria, etc).
    // A pesquisa web já foi resolvida pelo Agente Especializado!
    config.tools = buildTools(context.userRole);
    ...
    // Function Calling loop (Apenas para ferramentas internas do Prisma agora)
    ...
    // Pega as fontes se houver grounding local extra, e soma com as do agente
    const localChunks = response.candidates?.[0]?.groundingMetadata?.groundingChunks || [];
    const localSources = localChunks
      .filter(c => c.web?.uri)
      .map(c => ({ label: c.web.title || c.web.uri, url: c.web.uri }));

    // Junta as fontes do Agente + Fontes do Gemini (se houver) limitando a 5
    const finalSources = [...searchSources, ...localSources].slice(0, 5);

    return { reply, sources: finalSources, reasoning: reasoningText, toolsExecutadas };
