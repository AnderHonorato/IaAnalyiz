...
app.get('/api/divergencias/stats', async (req, res) => {
  try {
    ...
    res.json({ 
      pendente, corrigido, ignorado, reincidente, 
      total: pendente + corrigido + ignorado + reincidente 
    });
  } catch { res.status(500).json({ error: 'Erro ao buscar stats.' }); }
});us(500).json({ error: 'Erro ao buscar stats.' }); 
...
// DESVINCULAR ANÚNCIO
// ==========================================
app.put('/api/produtos/:id/desvincular', async (req, res) => {
  try {
    const id = parseInt(req.params.id);
    // Remove os itens do kit
    await prisma.kitItem.deleteMany({ where: { kitId: id } });
    // Volta o anúncio para a aba de PENDENTES
    const atualizado = await prisma.produto.update({
      where: { id },
      data: { plataforma: 'ML_PENDENTE', eKit: false, pesoGramas: 0 }
    });
    res.json(atualizado);
  } catch (e) {
...
// ==========================================
// BUSCAR DETALHES COMPLETOS DO ANÚNCIO NO ML
// ==========================================
app.get('/api/ml/item-details/:mlItemId', async (req, res) => {
  try {
    const { userId } = req.query;
    ...
    // Busca dados do item e descrição em paralelo
    const [itemRes, descRes] = await Promise.all([
      mlApi.get(`/items/${mlItemId}`),
      mlApi.get(`/items/${mlItemId}/description`).catch(() => ({ data: { plain_text: 'Descrição não disponível.' } }))
    ]);

    res.json({ ...itemRes.data, description_text: descRes.data.plain_text });
  } catch (e) {
    console.error('Erro ao buscar detalhes ML:', e.message);
