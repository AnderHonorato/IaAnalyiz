...
app.post('/api/ia/chat', async (req, res) => {
  ...
  try {
    ...
    // ── Monta sources (links detectados na resposta) ──
    const urlRegex = /https?:\/\/[^\s<>"]+/g;
    const foundUrls = [...new Set(reply.match(urlRegex) || [])];
    const sources = foundUrls.map(url => {
      let label = url;
      if (url.includes('anderhonorato.github.io')) label = 'Portfólio — Ander Honorato';
      else if (url.includes('mercadolivre'))       label = 'Mercado Livre';
      return { label, url };
    });
