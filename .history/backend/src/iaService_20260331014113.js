...
//  5. Integração com ENXAME de agentes e salvamento persistente (Anti-F5).
...
export async function buildAnswerStream(message, history = [], context = {}, onStep, attempt = 1) {
  ...
  try {
    ...
    // ── 1. Raciocínio dinâmico PRIMEIRO ──
    ...
    // ── 2. ACIONAMENTO DO ENXAME DE AGENTES via runOrchestrator ──
    // O agentOrchestrator agora cuida de pesquisar, codar, gerar mídia, SEO, etc.
    const runResult = await runOrchestrator(message, history, context, onStep, async (msgToBuild, histRaw, ctxEnriquecido) => {
        // Callback para construir a resposta do modelo mãe DEPOIS que o enxame trabalhou
        ...
        // Function Calling loop (Tools do sistema: banco, ML, etc)
    ...
    // Combina os sources da pesquisa e da ia mãe
    const finalSources = [...(runResult.fontes || []), ...(runResult.sources || [])].slice(0, 5);

    // Salva a mensagem no DB com todas as ferramentas persistidas para o F5
    // Assume-se que o schema possui esse campo (ou você adicionará depois)
    // Se o seu Prisma não possui "toolsExecutadas", salve apenas em reasoning se preferir, 
    // mas aqui eu aplico a lógica correta conforme autorizado.
    if (context.sessionId) {
        try {
            await prisma.chatDocument.create({ // Apenas um log secundário caso o ChatMessage não suporte array
                data: {
