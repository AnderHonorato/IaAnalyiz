...
const REASONING_CHUNK_DELAY = 2; // DEIXEI ULTRA RÁPIDO PARA NÃO ATRASAR RESPOSTAS SIMPLES
...
async function loopAprendizadoAutonomo() {
  ...
  try {
    ...
    // 1. Gerar uma pergunta/tópico inédito para a IA estudar
    ...
    // 2. IA tenta responder usando conhecimento local (Adivinhação/Previsão de contexto)
    ...
    // 3. Validação pelo "Professor" (Sistema Auditor)
    ...
    // 4. Salvar no banco vetorial se o acerto/aprendizado for satisfatório
    if (nota >= 65) {
  ...
  // Agenda o próximo ciclo. AUMENTADO PARA 45 SEG PARA NÃO ESTOURAR AS CONEXÕES DO NEON.
...
export function needsWebSearch(message) {
  ...
  // Regex expandida para capturar 'ver preço', 'celular', etc e forçar a web search
...
// ═══════════════════════════════════════════════════════════════════════════════
// EXTRAÇÃO DE DADOS OCULTOS DO HTML DO MERCADO LIVRE
// ─────────────────────────────────────────────────────────────────────────────
// Varre o HTML da página do ML e extrai dados que a API oficial não fornece,
// como vendas do vendedor, parcelamento, tipo de envio, Mercado Líder,
...
function extrairDadosOcultosDoHTML(html) {
  ...
  try {
    // O Mercado Livre esconde os dados nesta variável Nordic
    ...
    if (matchContext) {
      ...
      if (itemPrincipal && itemPrincipal.components) {
        itemPrincipal.components.forEach(comp => {
          ...
          // 5. Tipo de Envio (Verifica tags de Full e Frete Grátis)
    ...
    console.log("Erro ao tentar extrair __NORDIC_RENDERING_CTX__:", error.message);
  }
...
// ── Agente Validador (apenas para respostas longas/críticas) ───────────────
    ...
    if (deveValidar) {
      ...
      // TRAVA DE SEGURANÇA: Impede que o validador destrua o HTML
      if (validacao.respostaFinal) {
        // Verifica se o validador devolveu um relatório completo (tem tags HTML básicas)
        const isHtmlValido = validacao.respostaFinal.includes('<br') || validacao.respostaFinal.includes('<b');
        // Verifica se o texto devolvido não é apenas um comentário minúsculo (deve ter pelo menos 60% do tamanho original)
        const isTamanhoBom = validacao.respostaFinal.length >= (replyGerado.length * 0.6);

        if (isHtmlValido && isTamanhoBom) {
           replyGerado = validacao.respostaFinal; // Substituição segura pelo HTML corrigido
        } else {
           // O validador só cuspiu comentários textuais apontando o erro.
           // Mantemos o relatório original intacto para não quebrar a tela.
           emitAgentLog(onStep, 'Validador retornou apenas comentários. Layout original protegido.', 'warn');
        }
      }
    }
