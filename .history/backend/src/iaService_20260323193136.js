// backend/src/iaService.js — v2 FIXED
// Fixes aplicados:
//  1. extractExcel robusto: SheetJS (xlsx) → lê binário .xlsx, todas as abas → fallback Gemini
//  2. Imagens isoladas por mensagem: imageContext não vaza entre mensagens
//  3. buildGeminiHistory: não propaga base64 no histórico
//  4. extractCodeBlocksAndSave: nomes únicos por linguagem com incremento
...
// FIX: buildGeminiHistory não propaga base64 de imagens entre mensagens
// Apenas texto vai para o histórico — cada mensagem tem seu próprio contexto visual
function buildGeminiHistory(history) {
  ...
  while (i < msgs.length) {
    ...
    if (cur.role === 'user' && next?.role === 'ia') {
      pairs.push({
        // IMPORTANTE: apenas texto, nunca base64 de imagens no histórico
...
// Mapeamento completo de linguagem → extensão
const LANG_EXT = {
  html:'html', css:'css', js:'js', jsx:'jsx', ts:'ts', tsx:'tsx',
  py:'py', sql:'sql', json:'json', sh:'sh', bash:'sh', md:'md',
  java:'java', cs:'cs', cpp:'cpp', go:'go', rs:'rs', yaml:'yaml',
  yml:'yaml', xml:'xml', rb:'rb', php:'php', swift:'swift', kt:'kt',
  toml:'toml', dockerfile:'dockerfile', env:'env',
};
const LANG_NAMES = {
...
// FIX: nomes únicos com incremento por linguagem
async function extractCodeBlocksAndSave(sessionId, replyText, userMessage) {
  if (!sessionId || !replyText) return;

  // Regex robusta: aceita qualquer identificador de linguagem
...
// FIX: extractExcel robusto — SheetJS primeiro, fallback Gemini
async function extractExcel(base64, mimeType, fileName) {
  // ── CSV: processamento direto sem dependências ──────────────────────────────
  if (mimeType === 'text/csv') {
    try {
      ...
      const formatted = lines.map((line, idx) => {
        // Trata campos com vírgulas dentro de aspas
        const cols = [];
  ...
  try {
    ...
    if (XLSX) {
      ...
      for (const sheetName of sheetNames.slice(0, 8)) {
        ...
        // Determina número de colunas pelo maior array
  ...
  // ── Fallback: Gemini tenta ler como documento ──────────────────────────────
  try {
    const ai = getClient();

    // Gemini suporta xlsx como documento genérico em alguns contextos
    ...
    const r = await ai.models.generateContent({
      ...
      config: { temperature: 0, maxOutputTokens: 8000 },
      contents: [{ role: 'user', parts: [
        { text: `Extraia TODOS os dados desta planilha Excel "${fileName}". Liste cada linha no formato: COLUNA1 | COLUNA2 | COLUNA3. Inclua os cabeçalhos na primeira linha. Inclua todas as abas se houver mais de uma.` },
  ...
  // Último recurso: informa o problema ao usuário
...
export async function buildAnswerStream(message, history = [], context = {}, onStep, attempt = 1) {
  ...
  try {
    ...
    // FIX: imageContext é EXCLUSIVO desta mensagem — não vaza de mensagens anteriores
    // O contexto.imageContext vem da rota SSE apenas quando há uma imagem NO REQUEST ATUAL
    ...
    // Processa arquivos anexados nesta mensagem
    ...
    // FIX: histórico sem base64 de imagens — isolamento por mensagem garantido
