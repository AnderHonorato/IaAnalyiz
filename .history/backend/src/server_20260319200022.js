...
import { GoogleGenAI } from '@google/genai'; // Importação padrão para ES Modules
...
// ==========================================
// IA ANALYIZ (FIX: TYPEERROR MODEL)
// ==========================================
...
app.post('/api/ia/chat', async (req, res) => {
  ...
  try {
    if (userRole === 'BLOQUEADO') {
      return res.json({ reply: '🔒 **Acesso Restrito.** Atualmente seu perfil está sob análise do Criador. Por favor, utilize o botão "Solicitar Acesso" no menu superior.' });
    }
    ...
    const systemInstruction = `Você é a IA Analyiz. Ajude o Criador Ander com dados: ${produtos.length} produtos e ${divergencias.length} divergências.`;

    // CORREÇÃO DA INSTANCIAÇÃO
    ...
    // Envia a mensagem com a instrução de sistema no contexto do chat
    const result = await chat.sendMessage([
      { text: `INSTRUÇÃO DE SISTEMA: ${systemInstruction}` },
      { text: message }
    ]);
