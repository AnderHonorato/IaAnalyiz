...
// ── Funcionalidades de interface que NÃO são páginas — mas a IA deve conhecer ─
// Usadas para guiar o usuário quando ele pede algo que só funciona na UI
export const UI_FEATURES = {
  tema: {
    titulo:    'Seletor de Tema / Aparência',
    onde:      'Barra superior (header) do site — ícone de paleta 🎨 no canto superior direito',
    comoUsar:  'Clique no ícone de paleta na barra do topo para abrir o seletor de temas. Há 20 temas disponíveis (Dark, Light, Neon, etc). A preferência é salva automaticamente.',
    temFerram: false,
  },
  zoom: {
    titulo:    'Zoom / Escala da Interface',
    onde:      'Barra superior (header) — botões +/- ou slider de zoom ao lado do tema',
    comoUsar:  'Use os botões de zoom na barra do topo para ajustar o tamanho da interface (75% a 125%).',
    temFerram: false,
  },
  perfil: {
    titulo:    'Perfil do Usuário',
    onde:      'Menu no canto superior direito — avatar do usuário',
    comoUsar:  'Clique no avatar/nome no canto superior direito para acessar configurações de perfil, alterar nome e avatar.',
    temFerram: false,
  },
  ml_conectar: {
    titulo:    'Conectar Conta do Mercado Livre',
    onde:      'Página /ml (Dashboard ML) — botão "Conectar ML" ou ícone de configurações no header',
    comoUsar:  'Acesse /ml e clique em "Conectar Mercado Livre". Você será redirecionado para autorizar o acesso.',
    temFerram: false,
  },
};
...
=== FUNCIONALIDADES DE INTERFACE (guie o usuário — sem ferramenta disponível) ===
Para estas ações, VOCÊ não tem ferramenta. Explique CLARAMENTE onde o usuário faz na interface:
• Alterar TEMA/COR do site → ícone 🎨 na barra do topo (canto superior direito). São 20 temas.
• Ajustar ZOOM → botões +/- na barra do topo, ao lado do tema.
• Editar PERFIL/AVATAR → avatar/nome no canto superior direito.
• Conectar MERCADO LIVRE → página /ml → botão "Conectar ML".
NUNCA diga apenas "não consigo fazer isso". Sempre explique o caminho na interface!

=== SUAS FERRAMENTAS (FUNCTION CALLING) ===
Você tem acesso a ferramentas REAIS. EXECUTE sem pedir confirmação quando o contexto já pede ação:

GATILHOS AUTOMÁTICOS — execute a tool IMEDIATAMENTE:
• "aprova", "libera", "acesso" → chame listarUsuariosPendentes() depois aprovarUsuario(id)
• "corrige", "manda pra fila", "resolver divergências" → chame listarDivergenciasAtivas() depois enviarParaFilaDeCorrecao(id)
• "ignora essa divergência" → chame listarDivergenciasAtivas() depois ignorarDivergencia(id)
• "liga o agendador" / "ativa varredura" → chame ativarAgendador(intervalo)
• "desliga o agendador" → chame desativarAgendador()
• "corrigida" / "já corrigi" → chame marcarDivergenciaCorrigida(id)
• "corrige tudo" / "todas as divergências" → chame enviarLoteDivergencias()
• "quais são minhas divergências" → chame listarDivergenciasAtivas()
• "como está o sistema" / "resumo" → chame resumoGeral()
• "quais páginas" / "o que você tem" → chame listarPaginasDisponiveis()

REGRA SOBRE CERTEZA vs DÚVIDA:
• SE a intenção é CLARA (ex: "aprova o João", "liga o agendador em 60 minutos", "corrige tudo", "ignora MLB123") → EXECUTE A TOOL IMEDIATAMENTE sem perguntar.
• SE a intenção é AMBÍGUA (ex: "resolve isso", "faz aquilo", "corrige essa" sem contexto) → PERGUNTE primeiro: descreva a ação que pretende executar e peça confirmação com "Confirma?".
• SE o usuário responde "sim", "pode", "faz", "ok", "confirma", "vai lá" → EXECUTE A TOOL IMEDIATAMENTE. Não pergunte de novo.
• NUNCA execute ações irreversíveis (bloquear usuário) sem confirmar primeiro, independente da certeza.

FLUXO CORRETO para aprovar usuário:
1. Chame listarUsuariosPendentes() para obter a lista e IDs
2. Se só tem 1 → chame aprovarUsuario(id) direto
3. Se tem vários → mostre a lista e pergunte qual aprovar
4. Execute aprovarUsuario(id) com o ID correto

FLUXO CORRETO para corrigir divergência:
1. Chame listarDivergenciasAtivas() para obter IDs
2. Execute enviarParaFilaDeCorrecao(id) ou enviarLoteDivergencias() conforme pedido

REGRAS DE RESPOSTA:
1. CONCISA E DIRETA — máx 2-3 parágrafos. Chat é pequeno.
2. Use emojis contextualmente.
3. Se executou ação via Tool → confirme com entusiasmo e mostre o resultado!
...
7. Quando pedirem algo de interface (tema, zoom, perfil), explique o caminho exato de forma útil e visual.`;
}
...
// ── Sintetiza raciocínio a partir das tools chamadas ─────────────────────────
const TOOL_REASONING_MAP = {
  listarDivergenciasAtivas:   'Consultei as divergências ativas no banco de dados',
  enviarParaFilaDeCorrecao:   'Enviei a divergência para a fila de correção automática via API',
  enviarLoteDivergencias:     'Enviei todas as divergências pendentes para a fila de correção automática',
  marcarDivergenciaCorrigida: 'Marquei a divergência como corrigida manualmente',
  ignorarDivergencia:         'Marquei a divergência como ignorada',
  listarUsuariosPendentes:    'Consultei os usuários aguardando aprovação de acesso',
  aprovarUsuario:             'Aprovei o acesso do usuário no sistema',
  bloquearUsuario:            'Bloqueei o acesso do usuário no sistema',
  consultarAgendador:         'Consultei as configurações do agendador de varredura',
  ativarAgendador:            'Ativei a varredura automática de divergências',
  desativarAgendador:         'Desativei o agendador de varredura automática',
  listarProdutos:             'Consultei o catálogo de produtos',
  listarAvisosML:             'Verifiquei os avisos ativos do Mercado Livre',
  consultarStatusConexaoML:   'Verifiquei o status da conexão com o Mercado Livre',
  listarHistoricoPrecos:      'Consultei o histórico de alterações de preço',
  resumoGeral:                'Compilei um resumo geral do estado do sistema',
  listarPaginasDisponiveis:   'Listei as páginas e funcionalidades disponíveis',
  resumoGlobalPlataforma:     'Coletei métricas globais de todos os usuários da plataforma',
};

function sintetizarRaciocinio(message, toolsExecutadas) {
  if (!toolsExecutadas || toolsExecutadas.length === 0) {
    // Sem tools: resposta direta
    if (/tema|cor|aparência|visual/i.test(message))
      return 'Identifiquei que o usuário quer alterar a aparência do sistema. Como esta ação é feita na interface, vou indicar onde fica.';
    if (/zoom|escala|tamanho/i.test(message))
      return 'Identifiquei uma pergunta sobre ajuste de zoom da interface.';
    if (/divergen/i.test(message))
      return 'Analisei a pergunta sobre divergências e vou responder com base nos dados disponíveis.';
    if (/produto|sku|catálog/i.test(message))
      return 'Analisei a pergunta sobre o catálogo de produtos.';
    return 'Analisei a mensagem e vou responder diretamente com base no contexto.';
  }

  const desc = toolsExecutadas.map(t => TOOL_REASONING_MAP[t] || t).join(', depois ');
  const nomeAmigavel = {
    listarUsuariosPendentes: 'usuários pendentes',
    aprovarUsuario:          'aprovação de acesso',
    bloquearUsuario:         'bloqueio de acesso',
    listarDivergenciasAtivas:'divergências ativas',
    enviarParaFilaDeCorrecao:'fila de correção',
    enviarLoteDivergencias:  'lote de divergências',
    ativarAgendador:         'agendador',
    desativarAgendador:      'agendador',
    resumoGeral:             'estado do sistema',
  };

  const primeira = nomeAmigavel[toolsExecutadas[0]];
  if (toolsExecutadas.length === 1) {
    return `Entendi a solicitação. ${desc}.`;
  }
  return `Entendi que o usuário quer agir sobre ${primeira || 'o sistema'}. ${desc}.`;
}

// ── Mensagens de progresso para cada ferramenta ──────────────────────────────
export const TOOL_STEP_MESSAGES = {
  listarDivergenciasAtivas:   '🔍 Buscando suas divergências ativas...',
  ...
  marcarDivergenciaCorrigida: '✔️ Marcando divergência como corrigida manualmente...',
  ignorarDivergencia:         '👁️ Marcando divergência como ignorada...',
  listarUsuariosPendentes:    '👥 Verificando usuários aguardando aprovação...',
  aprovarUsuario:             '🔑 Acessando configurações do usuário... Aprovando acesso...',
  bloquearUsuario:            '🔒 Acessando configurações do usuário... Bloqueando acesso...',
  consultarAgendador:         '⏰ Consultando configurações do agendador de varredura...',
  ativarAgendador:            '⚡ Acessando configurações... Ativando varredura automática...',
  desativarAgendador:         '⏸️ Acessando configurações... Desativando agendador...',
  ...
  listarPaginasDisponiveis:   '🗺️ Listando páginas e recursos disponíveis...',
  resumoGlobalPlataforma:     '🌐 Coletando métricas globais da plataforma...',
};

// ═══════════════════════════════════════════════════════════════════════════════
// PIPELINE COM STREAMING — buildAnswerStream
// Igual ao buildAnswer mas chama onStep(msg) antes de cada tool call.
// Usado pelo endpoint SSE /api/ia/chat/stream
// ═══════════════════════════════════════════════════════════════════════════════
...
export async function buildAnswerStream(message, history = [], context = {}, onStep, attempt = 1) {
  ...
  try {
    ...
    // Loop de Function Calling com callbacks de progresso
    ...
    while (response.functionCalls && response.functionCalls.length > 0 && callCount < 5) {
      ...
      // Emite mensagem de progresso para o frontend
      const stepMsg = TOOL_STEP_MESSAGES[call.name] || `⚙️ Executando ${call.name}...`;
      if (onStep) onStep({ type: 'step', msg: stepMsg, tool: call.name, args: call.args });

      console.log(`\x1b[35m[IA-Tools-Stream]\x1b[0m 🤖 ${call.name}`, call.args);
      ...
      // Emite o resultado da ferramenta para o frontend ver
      const resultOk = !apiResult?.erro;
      if (onStep) onStep({
        type:   'tool_result',
        tool:   call.name,
        ok:     resultOk,
        msg:    resultOk
          ? `✅ ${call.name} concluído`
          : `❌ Erro: ${apiResult?.erro || 'falha desconhecida'}`,
      });

      console.log(`\x1b[35m[IA-Tools-Stream]\x1b[0m ${resultOk ? '✅' : '❌'} Resultado devolvido à IA`);
    ...
    const raw = response.text?.trim();
    if (!raw) return { reply: getFallback(), sources: [], reasoning: null, toolsExecutadas: [] };

    const reply  = cleanMetaText(ensureHTML(raw));
    const chunks = response.candidates?.[0]?.groundingMetadata?.groundingChunks || [];
    const sources = chunks.filter(c => c.web?.uri).map(c => ({
      label: c.web.title || c.web.uri, url: c.web.uri,
    })).slice(0, 3);

    const reasoning = sintetizarRaciocinio(message, toolsExecutadas);

    return { reply, sources, reasoning, toolsExecutadas };
