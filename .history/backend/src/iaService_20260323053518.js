// backend/src/iaService.js — com SKILL.md integrado
...
// ─── Carrega o SKILL.md uma vez ao iniciar ────────────────────────────────────
let SKILL_CONTENT = '';
try {
  const skillPath = path.join(__dirname, 'skills', 'SKILL.md');
  SKILL_CONTENT = fs.readFileSync(skillPath, 'utf-8');
  console.log('\x1b[35m[IA-Skill] SKILL.md carregado com sucesso ✓\x1b[0m');
} catch (e) {
  console.warn('[IA-Skill] SKILL.md não encontrado em src/skills/SKILL.md — usando instruções padrão.');
}
...
const STEP_DELAYS = {
  ...
  db_update:600, db_done:250, prep_response:500,
...
export async function analyzeImage(base64, mimeType = 'image/jpeg', userQuestion = '') {
  ...
  try {
    ...
    const res = await ai.models.generateContent({
      ...
      contents:[{ role:'user', parts:[
        { text: userQuestion || 'Descreva esta imagem.' },
        { inlineData:{ mimeType, data:base64 } },
      ]}],
    });
    return res.text?.trim() || null;
...
export async function processFileToContext(file, userMessage, onStep, baseStepKey = 0, sessionId = null) {
  ...
  return { context, label: fileTypeLabel, name, stepsUsed: stepKeyLocal - baseStepKey };
}
...
function buildTools(userRole) {
  ...
  const tools = [
    ...
    { name:'enviarParaFilaDeCorrecao',    description:'Marca uma divergência como PENDENTE_ENVIO para ser corrigida automaticamente.',     parameters:{ type:'OBJECT', properties:{ divergenciaId:{ type:'INTEGER' } }, required:['divergenciaId'] } },
    { name:'ignorarDivergencia',          description:'Marca uma divergência como IGNORADA para sair da lista de pendentes.',              parameters:{ type:'OBJECT', properties:{ divergenciaId:{ type:'INTEGER' } }, required:['divergenciaId'] } },
    { name:'marcarDivergenciaCorrigida',  description:'Marca uma divergência como CORRIGIDO manualmente pelo usuário.',                    parameters:{ type:'OBJECT', properties:{ divergenciaId:{ type:'INTEGER' } }, required:['divergenciaId'] } },
    { name:'enviarLoteDivergencias',      description:'Envia TODAS as divergências PENDENTES para a fila de correção automática.',         parameters:{ type:'OBJECT', properties:{} } },
    { name:'consultarAgendador',          description:'Verifica se a varredura automática está ativa ou inativa.',                         parameters:{ type:'OBJECT', properties:{} } },
    { name:'ativarAgendador',             description:'Liga a varredura automática em um intervalo de minutos específico.',                parameters:{ type:'OBJECT', properties:{ intervaloMinutos:{ type:'INTEGER' } }, required:['intervaloMinutos'] } },
    { name:'desativarAgendador',          description:'Desliga completamente a varredura automática do agendador.',                        parameters:{ type:'OBJECT', properties:{} } },
    { name:'listarProdutos',              description:'Lista os produtos cadastrados no catálogo do sistema.',                              parameters:{ type:'OBJECT', properties:{ busca:{ type:'STRING' }, limite:{ type:'INTEGER' }, semPeso:{ type:'BOOLEAN' }, semVinculo:{ type:'BOOLEAN' } } } },
    { name:'listarAvisosML',              description:'Lista os avisos ativos e pendentes vindos do Mercado Livre.',                       parameters:{ type:'OBJECT', properties:{ limite:{ type:'INTEGER' } } } },
    { name:'consultarStatusConexaoML',    description:'Verifica se a conta do Mercado Livre está conectada e se o token é válido.',        parameters:{ type:'OBJECT', properties:{} } },
    { name:'listarHistoricoPrecos',       description:'Lista o histórico de alterações de preços dos anúncios.',                           parameters:{ type:'OBJECT', properties:{ limite:{ type:'INTEGER' }, mlItemId:{ type:'STRING' } } } },
    { name:'resumoGeral',                 description:'Retorna um resumo com as métricas gerais do sistema.',                              parameters:{ type:'OBJECT', properties:{} } },
    { name:'listarPaginasDisponiveis',    description:'Lista as páginas disponíveis no sistema que o usuário tem acesso.',                 parameters:{ type:'OBJECT', properties:{} } },
    { name:'lerPagina',                   description:'Acessa e lê o conteúdo de uma página local ou remota.',                            parameters:{ type:'OBJECT', properties:{ caminho:{ type:'STRING' } }, required:['caminho'] } },
  ];
  if (isPrivileged) {
    tools.push(
      { name:'listarUsuariosPendentes',   description:'Lista os usuários que estão aguardando aprovação para acessar a plataforma.',       parameters:{ type:'OBJECT', properties:{} } },
      { name:'aprovarUsuario',            description:'Aprova e libera o acesso de um usuário específico na plataforma.',                  parameters:{ type:'OBJECT', properties:{ usuarioId:{ type:'INTEGER' } }, required:['usuarioId'] } },
      { name:'bloquearUsuario',           description:'Bloqueia o acesso de um usuário específico na plataforma.',                        parameters:{ type:'OBJECT', properties:{ usuarioId:{ type:'INTEGER' } }, required:['usuarioId'] } },
      { name:'resumoGlobalPlataforma',    description:'Retorna as métricas globais de toda a plataforma (apenas ADMIN ou OWNER).',         parameters:{ type:'OBJECT', properties:{} } },
    );
  }
  ...
  try {
    switch (name) {
      ...
      case 'lerPagina': {
        ...
        if (!content) return { erro:`Não foi possível acessar: ${url}` };
        return { url, conteudo:content, resumo:`Página acessada: ${url}. Conteúdo extraído (${content.length} chars).` };
      }
...
// ─── System Instruction com SKILL.md ─────────────────────────────────────────
function buildSystemInstruction(ctx) {
  const {
    totalProdutos = 0, totalDivergencias = 0, userRole = 'USUARIO',
    usuarioAtual = null, dataBlock = null, fileContexts = [],
    pageBaseUrl = null, sessionId,
  } = ctx;

  // Bloco de arquivos lidos nesta sessão
  const fileBlock = fileContexts.length > 0
    ? `\n=== ARQUIVOS LIDOS AGORA ===\nO usuário fez upload dos arquivos abaixo. Baseie sua resposta NO CONTEÚDO DELES:\n${fileContexts.map(f => `[ARQUIVO: ${f.name}]\n${(f.context||'').substring(0, 8000)}`).join('\n\n')}\n`
    : '';

  // Contexto dinâmico da sessão atual
  const sessionBlock = `
=== CONTEXTO DA SESSÃO ATUAL ===
Usuário: ${usuarioAtual?.nome || 'Desconhecido'} | Role: ${userRole}
Dados rápidos: ${dataBlock || `Produtos: ${totalProdutos} | Divergências ativas: ${totalDivergencias}`}
${fileBlock}
=== FIM DO CONTEXTO ===`.trim();

  // Se SKILL.md foi carregado, ele vem primeiro — depois o contexto dinâmico
  if (SKILL_CONTENT) {
    return `${SKILL_CONTENT}\n\n${sessionBlock}`;
  }

  // Fallback caso SKILL.md não exista
  return `Você é a IA Analyiz — assistente agêntico especialista em e-commerce e logística.
${sessionBlock}

REGRAS:
1. Responda baseado no contexto e nos arquivos lidos.
2. Código deve usar \`\`\`linguagem ... \`\`\`.
3. Use HTML simples (<b>, <i>, <br>) para formatação.
4. Nunca invente dados. Use as ferramentas para buscar informações reais.`;
}

// ─── Modo Autônomo Local (sem Gemini) ────────────────────────────────────────
async function generateLocalStreamingResponse(message, onStep) {
  const bestMatches  = await queryKnowledge(message, 3);
  const bestContent  = bestMatches.length > 0
    ? bestMatches.map(m => m.content).join(' ')
    : 'Não encontrei informações exatas no meu banco de dados local para responder a isso de forma autônoma sem acessar a nuvem.';
  const confidence   = bestMatches.length > 0 ? Math.round(bestMatches[0].confidence * 100) : 10;
  const fullText     = `<b>[IA Analyiz — Modo Autônomo Ativado]</b><br><br>${bestContent}<br><br><i>Confiança na resposta local: ${confidence}%</i>`;

  if (onStep) onStep({ type:'reasoning_start' });
  if (onStep) onStep({ type:'reasoning_chunk', text:'API Gemini desativada. Acessando banco de dados vetorial local (PostgreSQL RAG). Construindo resposta a partir do conhecimento interno.' });
  if (onStep) onStep({ type:'reasoning_end',   fullText:'API Gemini desativada. Acessando banco de dados vetorial local (PostgreSQL RAG). Construindo resposta a partir do conhecimento interno.' });

  await sleep(1000);
  return { reply: fullText, sources:[], reasoning:'Processamento vetorial local isolado.', toolsExecutadas:[] };
}

// ─── Aprendizado de livros (leia isso e aprenda) ──────────────────────────────
async function processarLivroEAprender(fileContexts, onStep) {
  if (onStep) onStep({ type:'reasoning_start' });
  if (onStep) onStep({ type:'reasoning_chunk', text:'Comando de aprendizagem profunda detectado. Iniciando fragmentação e ingestão de conhecimento no banco de dados para uso no Modo Autônomo.' });
  if (onStep) onStep({ type:'reasoning_end',   fullText:'Comando de aprendizagem profunda detectado. Iniciando fragmentação e ingestão de conhecimento no banco de dados para uso no Modo Autônomo.' });
  ...
  for (const file of fileContexts) {
    ...
    for (let i = 0; i < text.length; i += 2000) {
      ...
      await saveKnowledge(chave, chunk, 'upload_owner', 0.95);
      totalChunks++;
      if (onStep) await emitStep(onStep, `Memorizando bloco ${Math.floor(i / 2000)} de "${file.name}"…`, null, 150);
    }
  }

  return {
    reply: `<b>🧠 Aprendizagem concluída com sucesso!</b><br><br>Fragmentei e memorizei o(s) documento(s) inteiramente.<br><br><b>Relatório:</b><br>• Fragmentos absorvidos: ${totalChunks}<br>• Taxa de retenção: 98%<br><br><i>Já posso usar esse conhecimento no Modo Autônomo.</i>`,
    sources:[], reasoning:'Ingestão de PDF e fragmentação de conhecimento concluída.', toolsExecutadas:[],
  };
}
...
export async function buildAnswerStream(message, history = [], context = {}, onStep, attempt = 1) {
  ...
  if (isOwner && message) {
    ...
    if (msgLower.match(/desativar(\s+o|\s+a)?\s+gemini/)) {
      ...
      return { reply:'<b>Conexão Gemini desativada.</b><br><br>Operando no <b>Modo Autônomo Local</b> a partir de agora.', sources:[], reasoning:'Gemini desligado pelo OWNER.', toolsExecutadas:[] };
    }
    if (msgLower.match(/ativar(\s+o|\s+a)?\s+gemini/)) {
      await prisma.usuario.update({ where:{ id:parseInt(userId) }, data:{ geminiAtivo:true } });
      return { reply:'<b>Conexão Gemini reativada.</b><br><br>Retornando à capacidade total de processamento.', sources:[], reasoning:'Gemini reativado pelo OWNER.', toolsExecutadas:[] };
    }
  }
  ...
  try {
    ...
    if (incomingFiles.length > 0) {
      ...
      for (const file of incomingFiles) {
        const fileResult = await processFileToContext(file, message, evt => { if (evt.type==='step'&&onStep) onStep(evt); }, globalStepKey, context.sessionId);
    ...
    // Raciocínio
    if (onStep) onStep({ type:'reasoning_start' });
    const fullReasoning = 'Avaliando intenção da solicitação. Processando ferramentas disponíveis, contexto de arquivos e dados do banco para gerar a melhor resposta.';
    if (onStep) onStep({ type:'reasoning_chunk', text: fullReasoning });
    if (onStep) onStep({ type:'reasoning_end',   fullText: fullReasoning });

    // Prompt final
    let injectPrompt = message || '[nenhuma mensagem de texto — apenas arquivo enviado]';
    ...
    while (response.functionCalls && response.functionCalls.length > 0 && callCount < 5) {
      ...
      await emitStep(onStep, `Executando: ${call.name}…`, call.name, 400);
    ...
    if (!raw) return { reply:'⚠️ Resposta vazia do modelo. Tente novamente.', sources:[], reasoning:fullReasoning, toolsExecutadas };
...
// buildAnswer (compatibilidade com chamadas legadas)
...
export function buildResumoPrompt(dadosStr) {
  return `Faça um resumo executivo em HTML (<b>, <br>) dos seguintes dados do sistema:\n\n${dadosStr}`;
}
