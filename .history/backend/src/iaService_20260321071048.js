...
export function needsWebSearch(message) {
  ...
  return /mercado\s*livre|ml\s*api|taxa|tarifa|comissão|tendência|concorrência|tabela\s*de\s*frete|política\s*de\s*frete|regras?\s*do\s*ml|como\s+(vender|anunciar|calcular\s*frete)|o\s+que\s+(é|são)\s+(o\s+)?ml|novidades?\s*(do|no)\s*ml|hora|horas|hoje|agora|clima|câmbio|dólar|notícia|busca|pesquis/i.test(message);
}
