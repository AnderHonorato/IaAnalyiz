// backend/src/ia/agents/validationAgent.js — v2 (Logs Narrativos para SSE)
// Mudanças v2:
...
PERGUNTA DO USUÁRIO: "${perguntaOriginal.substring(0, 400)}"
DADOS DA PESQUISA WEB: "${dadosPesquisaWeb.substring(0, 800)}"
CONTEXTO DO SISTEMA: "${contextoInterno.substring(0, 600)}"

RESPOSTA PROPOSTA:
"${textoPuro.substring(0, 1000)}"
...
Se "aprovada" for false, preencha "respostaCorrigida" com a versão corrigida em HTML.
    `.trim();
    ...
    if (aprovada) {
      ...
      emit(`Inconsistência detectada — aplicando correção automática. Motivo: ${resultado.motivo}`, 'warn');
    }
