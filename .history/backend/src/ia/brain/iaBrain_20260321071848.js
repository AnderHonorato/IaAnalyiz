...
const GEMINI_EMBEDDING  = 'text-embedding-004'; // Modelo de vetorização
...
// ═══════════════════════════════════════════════════════════════════════════════
// NOVO: MOTOR VETORIAL (EMBEDDINGS)
// ═══════════════════════════════════════════════════════════════════════════════

// Transforma texto em coordenadas matemáticas
...
// Calcula a similaridade (distância) entre dois vetores. Retorna de -1 a 1 (1 = idêntico)
...
export async function salvarConhecimento({ categoria, chave, valor, confianca = 0.7, fonte = 'sistema' }) {
  try {
    // Agora geramos o vetor da lição que ela aprendeu
...
export async function buscarConhecimentoSemantico(pergunta = '', limite = 12) {
  if (!pergunta) return [];
  try {
    // 1. Cria a coordenada matemática da pergunta do usuário
    const vetorPergunta = await gerarEmbedding(pergunta);
    if (!vetorPergunta.length) return [];

    // 2. Busca conhecimentos recentes no banco
    const todos = await prisma.iaConhecimento.findMany({
      where: { confianca: { gte: CONFIANCA_MINIMA - 0.1 } },
      orderBy: [{ updatedAt: 'desc' }],
      take: 200, // Compara com as últimas 200 memórias
    });

    // 3. Calcula a distância matemática de cada memória com a pergunta
    const pontuados = todos.map(k => {
      const score = calcularSimilaridade(vetorPergunta, k.embedding);
      return { ...k, score };
    });

    // 4. Retorna as memórias com mais de 65% de relevância (score > 0.65)
    return pontuados
...
export async function buscarAprendizadosSimilares(pergunta, limite = 4) {
  try {
    ...
    const pontuados = todos.map(a => ({
      ...a, score: calcularSimilaridade(vetorPergunta, a.embedding)
    }));
...
export async function processarMensagemComAprendizado({ mensagem, userRole, userId, imageBase64 = null, imageMimeType = null, imageOnly = false, historyRaw = [], dataBlock = null }) {
  ...
  // BUSCA SEMÂNTICA USANDO EMBEDDINGS (MATEMÁTICA VETORIAL)
