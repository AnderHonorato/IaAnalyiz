...
async function gerarEmbedding(texto) {
  ...
  try {
    ...
    const response = await ai.models.embedContent({
      model:   'text-embedding-004', // Modelo atualizado
      contents: [texto.substring(0, 2048)], // Correção principal: O conteúdo DEVE estar em um array
    });
    // O SDK novo retorna .embeddings como array. O fallback mantido por precaução.
...
// ═══════════════════════════════════════════════════════════════════════════════
// PESQUISA DE MERCADO (LÓGICA DE NEGÓCIO)
// ═══════════════════════════════════════════════════════════════════════════════

export async function realizarPesquisaMercadoProfunda(userId, itensParaAnalisar, options = {}) {
  // Correção: Agora recebe options para pegar o onLog e as perguntas de follow-up
...
PERGUNTA DO USUÁRIO: ${perguntaFollowUp}
...
Responda DIRETAMENTE com o conteúdo HTML usando tags <b>, <br>, <ul>, <li>. Não envolva sua resposta em blocos de código Markdown (como \`\`\`html).`;
  ...
  // Limpa possíveis blocos markdown que a IA possa ter inserido acidentalmente
