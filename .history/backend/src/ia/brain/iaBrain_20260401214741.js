...
function logBrain(msg, type = 'info') {
  ...
  const level = { info: 'INFO', warn: 'WARN', success: 'OK', error: 'ERROR', learn: 'LEARN', think: 'THINK', mentor: 'MENTOR', analyze: 'ANALYZE' }[type] || 'INFO';
  console.log(`[iaBrain.js] [${t}] [${level}] ${msg}`);
}
...
// ─── Embedding via REST direto (Totalmente Silenciado em caso de erro) ────────
async function gerarEmbedding(texto) {
  ...
  try {
    ...
    // Retorno silencioso de array vazio. Isso impede que o terminal fique floodado
    // com mensagens de "NOT_FOUND" caso o modelo de embedding não esteja acessível.
    return [];
  }
}
