...
// ─── Embedding via REST direto (Correção do Erro 404 & Fallback) ──────────────
async function gerarEmbedding(texto) {
  if (!process.env.GEMINI_API_KEY || !texto) return [];
  try {
    const ai       = geminiClient();
    // Passando o contents como string direta para maior compatibilidade
    ...
    // Se der 404 no text-embedding-004, tenta o modelo anterior universal
    if (e.message.includes('NOT_FOUND') || e.message.includes('404')) {
      try {
        ...
        console.error('[Embedding Fallback Error]', fallbackErr.message);
        return [];
      }
    }
...
Se a "PERGUNTA/COMANDO DO USUÁRIO" for uma pergunta direta ou dúvida específica, RESPONDA APENAS À PERGUNTA. Seja cirúrgico, detalhado na resposta, mas NÃO recrie o relatório inteiro com os 4 tópicos. Vá direto ao ponto.

CENÁRIO B (Pesquisa Inicial Completa): 
Se o comando for genérico para realizar uma análise de mercado, GERE UM RELATÓRIO COMPLETO, EXTREMAMENTE DETALHADO E VISUAL. NÃO FAÇA RESUMOS.
...
// ─── Helper: Extração Segura de JSON blindada (Corrigido para truncamentos) ───
function extrairJSONSeguro(texto) {
  ...
  // Se o JSON foi cortado no meio (truncado), pega até o final da string disponível
  ...
  try { 
    ...
    if (e.message.includes('Unexpected end') || e.message.includes('Unterminated string')) {
...
// ═══════════════════════════════════════════════════════════════════════════════
// SEÇÃO 5 — VALIDAÇÃO COM GEMINI (Mudado para FLASH para não truncar)
// ═══════════════════════════════════════════════════════════════════════════════
...
Avalie e responda com JSON PURO:
{
  ...
  "aprendizados": ["fato 1 aprendido"]
    ...
    const response = await ai.models.generateContent({
      model:    GEMINI_FLASH, // Alterado de LITE para FLASH para evitar o truncation de JSON
      contents: [{ role: 'user', parts: [{ text: prompt }] }],
      config:   {
        temperature:      0.1,
