...
// Sem apiVersion = usa v1beta por padrão, que suporta tools, responseMimeType, etc.
...
// ─── Embedding via REST direto (Correção do Erro Array Path requests[]) ───────
async function gerarEmbedding(texto) {
  ...
  try {
    ...
    const response = await ai.models.embedContent({
      ...
      contents: [texto.substring(0, 2048)],
    });
 ...
 */
export async function agentePesquisaProfunda(pergunta, contextoAdicional = "", onLog = () => {}) {
  try {
    const ai = geminiClient();
    onLog('Iniciando varredura na internet usando Google Search...', 'info');

    const prompt = `Você é o Agente de Pesquisa Profunda da Analyiz.
Você TEM permissão e ACESSO À INTERNET através da ferramenta de busca (googleSearch).
Sua missão é OBRIGATORIAMENTE realizar uma varredura na internet agora mesmo sobre: ${pergunta}
Contexto atual do sistema: ${contextoAdicional}

Traga tendências de preço, opiniões de consumidores em fóruns/vídeos, lançamentos recentes e estratégias de concorrentes.
Seja técnico, direto e use dados reais da sua pesquisa.`;
    ...
    onLog('Varredura na web concluída com sucesso.', 'success');
    return response.text || 'Sem resultado da pesquisa.';
  } catch (e) {
    onLog(`Falha na pesquisa externa: ${e.message}`, 'error');
...
// ═══════════════════════════════════════════════════════════════════════════════
// PESQUISA DE MERCADO (LÓGICA DE NEGÓCIO MELHORADA VISUALMENTE)
// ═══════════════════════════════════════════════════════════════════════════════

export async function realizarPesquisaMercadoProfunda(userId, itensParaAnalisar, options = {}) {
  const { onLog = () => {}, perguntaFollowUp = "", contextoAnterior = "" } = options;
  const titulos = itensParaAnalisar.map(i => i.titulo).join(', ');
  const busca = perguntaFollowUp ? perguntaFollowUp : `Tendências de mercado e preços para: ${titulos}`;

  onLog('Preparando parâmetros de pesquisa de mercado...', 'info');

  // 1. Pesquisa Externa (Internet)
  const insightsWeb = await agentePesquisaProfunda(busca, contextoAnterior, onLog);

  onLog('Compilando dados e gerando Relatório de Inteligência em HTML...', 'info');

  // 2. Análise Comparativa (Dados Internos)
  const ai = geminiClient();

  const prompt = `Você é um Analista de Inteligência de Mercado Sênior de E-commerce.
DADOS INTERNOS DO SISTEMA: ${JSON.stringify(itensParaAnalisar)}
DADOS EXTERNOS (WEB/CONCORRÊNCIA): ${insightsWeb}
PERGUNTA/COMANDO DO USUÁRIO: "${perguntaFollowUp}"

INSTRUÇÃO DE RESPOSTA (MUITO IMPORTANTE):
Avalie cuidadosamente a "PERGUNTA/COMANDO DO USUÁRIO":

CENÁRIO A (Pergunta específica / Follow-up): 
Se a "PERGUNTA/COMANDO DO USUÁRIO" for uma pergunta direta ou dúvida específica (ex: "Qual vendedor é minha ameaça?", "Devo baixar o preço em R$20?"), RESPONDA APENAS À PERGUNTA. Seja cirúrgico, detalhado na resposta, mas NÃO recrie o relatório inteiro com os 4 tópicos. Vá direto ao ponto.

CENÁRIO B (Pesquisa Inicial Completa): 
Se o comando for genérico para realizar uma análise de mercado (ex: "pesquisa de mercado do Motor...", "analise esse produto"), GERE UM RELATÓRIO COMPLETO, EXTREMAMENTE DETALHADO E VISUAL. NÃO FAÇA RESUMOS.

DIRETRIZES VISUAIS E DE CONTEÚDO (Para ambos os cenários):
- NÃO envolva sua resposta em blocos de código Markdown (como \`\`\`html). Retorne HTML puro.
- Traga DADOS REAIS da pesquisa: cite nomes dos vendedores, preços exatos (R$), métricas, diferenças de frete e características onde aplicável.

SE FOR GERAR O RELATÓRIO COMPLETO (CENÁRIO B), USE ESTA ESTRUTURA OBRIGATÓRIA:
<h3 style="color: #38bdf8; margin-top: 20px; font-weight: bold;">📊 1. Mapeamento de Concorrentes</h3>
(Faça uma análise de quem domina. E OBRIGATORIAMENTE crie uma tabela HTML para comparar os principais concorrentes)
Use este HTML para a tabela:
<table style="width:100%; border-collapse: collapse; margin: 15px 0; font-size: 13px; text-align: left;">
  <tr style="background-color: rgba(138,180,248,0.1); border-bottom: 2px solid #8ab4f8;">
    <th style="padding: 10px;">Concorrente</th><th style="padding: 10px;">Preço (R$)</th><th style="padding: 10px;">Estratégia/Diferencial</th>
  </tr>
  <tr><td style="padding: 10px; border-bottom: 1px solid #e2e8f0;">...</td><td style="padding: 10px; border-bottom: 1px solid #e2e8f0;">...</td><td style="padding: 10px; border-bottom: 1px solid #e2e8f0;">...</td></tr>
</table>

<h3 style="color: #10b981; margin-top: 20px; font-weight: bold;">💰 2. Análise de Preço e Margem</h3>
(Onde posicionar o preço, viabilidade de frete grátis, é possível subir o preço? Vale a pena baixar?)

<h3 style="color: #a78bfa; margin-top: 20px; font-weight: bold;">🚀 3. Otimização de Anúncio e SEO</h3>
(O que melhorar nas fotos, título, e descrição técnica com base no que os concorrentes não estão fazendo bem)

<h3 style="color: #f59e0b; margin-top: 20px; font-weight: bold;">🎯 4. Plano de Ação Imediato</h3>
(Use esta caixa de destaque abaixo para dar o veredito final passo a passo do que fazer hoje)
<div style="background-color: rgba(245, 158, 11, 0.08); padding: 14px; border-left: 4px solid #f59e0b; border-radius: 6px; margin: 15px 0;">
  ...
  const result = await ai.models.generateContent({
    model:    GEMINI_FLASH,
    contents: [{ role: 'user', parts: [{ text: prompt }] }],
    config:   { temperature: 0.3 }, // Baixei a temperatura para focar em dados reais e não alucinar na tabela
  });

  // Limpa possíveis blocos markdown
  let relatorio = result.text || '';
  relatorio = relatorio.replace(/```html|```/g, '').trim();

  onLog('Validando informações e estatísticas do relatório...', 'info');

  // 3. Validação
  const validacao = await agenteValidador(relatorio, itensParaAnalisar);
  if (!validacao.aprovada && validacao.correcao) {
    onLog('Pequena correção aplicada na validação de dados cruzados.', 'warn');
    relatorio = validacao.correcao;
  }

  onLog('Relatório finalizado com sucesso!', 'success');
  ...
  return {
    ...
    precoMedio: itensParaAnalisar.reduce((acc, i) => acc + i.preco, 0) / (itensParaAnalisar.length || 1),
  };
}
...
export async function buscarEstudosRecentes(limite = 40) {
  try {
    return await prisma.iaEstudoTerminal.findMany({ orderBy: { createdAt: 'desc' }, take: limite });
  } catch { return []; }
}
...
export async function analisarSistemaAutonomamente(userId = null) {
  ...
  try {
    ...
    const [
      totalProdutos, totalDivs, divsPorStatus, produtosComFicha,
      totalAvisos, ultimasDivs, agendadores, totalUsuarios, sessaoStats,
    ] = await Promise.all([
    ...
    if (pendentes === 0 && totalDivs > 0) {
      conhecimentos.push({ categoria: 'saude_logistica', chave: 'sem_pendencias', valor: `Logística 100% saudável: nenhuma divergência pendente. Todas as ${totalDivs} resolvidas ou ignoradas.`, confianca: 0.95 });
    }
    ...
    if (totalAvisos > 0) {
      conhecimentos.push({ categoria: 'avisos_ml', chave: 'avisos_ativos', valor: `⚠️ ${totalAvisos} aviso(s) ativo(s) do Mercado Livre sobre anúncios com peso ou dimensões incorretos.`, confianca: 1.0 });
    }
    agendadores.forEach(ag => {
      conhecimentos.push({ categoria: 'configuracao_agendador', chave: `agendador_user_${ag.usuarioId}`, valor: `Agendador usuário ${ag.usuarioId}: ${ag.ativo ? 'ATIVO' : 'INATIVO'}.`, confianca: 0.98 });
    });
    if (totalUsuarios > 0) {
      conhecimentos.push({ categoria: 'metricas_usuarios', chave: 'usuarios_plataforma', valor: `Plataforma tem ${totalUsuarios} usuário(s) cadastrados. ${sessaoStats} ativos agora.`, confianca: 0.9 });
    }
    ...
    if (Array.isArray(mentoria.novosConhecimentos)) {
      for (const nc of mentoria.novosConhecimentos) {
        if (nc.chave && nc.valor) {
          await salvarConhecimento({ categoria: 'mentoria_gemini', chave: nc.chave.substring(0, 100), valor: nc.valor, confianca: 0.88, fonte: 'gemini_mentoria' });
        }
      }
    }
...
export async function processarMensagemComAprendizado({ mensagem, userRole, userId, imageBase64 = null, imageMimeType = null, imageOnly = false, historyRaw = [], dataBlock = null }) {
  ...
  const { reply, sources } = await buildAnswer(msgEfetiva, historyRaw, {
    totalProdutos, totalDivergencias, userRole, usuarioAtual, pendentes,
    imageContext: imageDesc, imageOnly: !!imageOnly, dataBlock: dataBlockFinal || null,
  });
    ...
    for (const p of padroes) {
      if (p.regex.test(pergunta) || p.regex.test(respLimpa)) {
        ...
        if (chaveNorm.length > 5) {
          await salvarConhecimento({ categoria: p.cat, chave: chaveNorm, valor: `Q: ${pergunta.substring(0, 180)} | A: ${respLimpa.substring(0, 260)}`, confianca: 0.60, fonte: 'extracao_conversa' });
        }
        break;
      }
    }
...
export async function iniciarLoopAprendizado() {
  ...
  const ciclo = async () => {
    ...
    try {
      ...
      logBrain(`Erro ciclo #${_cicloContador}: ${e.message}`, 'error');
    }
...
export function pararLoopAprendizado() {
  _loopAtivo = false;
  logBrain('Loop parado', 'warn');
}

async function limparAprendizadosAntigos() {
  try {
    const lim = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
    const rem = await prisma.iaAprendizado.deleteMany({
      where: { aprovada: false, confianca: { lt: 0.25 }, createdAt: { lt: lim } },
    });
...
export async function getEstatisticasAprendizado() {
  try {
    ...
    return {
      totalConhecimentos: totC,
      totalAprendizados:  totA,
      taxaAcerto:         totA > 0 ? Math.round((ap / totA) * 100) : 0,
      aprovados:          ap,
      reprovados:         rp,
      ultimoEstudo:       ultE?.createdAt || null,
      loopAtivo:          _loopAtivo,
      cicloAtual:         _cicloContador,
      categorias:         cats.map(c => ({ categoria: c.categoria, total: c._count })),
    };
  } catch {
    return { totalConhecimentos: 0, totalAprendizados: 0, taxaAcerto: 0, aprovados: 0, reprovados: 0, loopAtivo: _loopAtivo, cicloAtual: _cicloContador, categorias: [] };
  }
}