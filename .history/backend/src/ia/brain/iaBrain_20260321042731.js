...
function extrairJSONSeguro(texto) {
  ...
  let jsonStr = limpo.slice(start, end + 1);
  
  // Remove trailing commas (vírgulas extras antes de chaves de fechamento)
  jsonStr = jsonStr.replace(/,\s*([\}\]])/g, '$1');
  
  try {
    return JSON.parse(jsonStr);
  } catch (e) {
    throw new Error(`Erro ao parsear JSON limpo: ${e.message}. JSON: ${jsonStr.substring(0, 150)}...`);
  }
}
...
export async function iniciarLoopAprendizado() {
  ...
  logBrain(`   Ciclos: análise a cada 1min | Mentoria a cada 5min`, 'info');
  ...
  const ciclo = async () => {
    ...
    try {
      ...
      // Mentoria com Gemini a cada 5min
