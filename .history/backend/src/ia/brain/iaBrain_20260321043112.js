...
function extrairJSONSeguro(texto) {
  ...
  if (start === -1) {
    throw new Error(`JSON não encontrado. Retorno cru da IA: ${texto.substring(0, 150)}...`);
  }

  // Se a IA cortou o texto e não fechou o JSON, forçamos o fechamento.
  let jsonStr = '';
  if (end === -1 || end < start) {
    jsonStr = texto.slice(start) + '}';
  } else {
    jsonStr = texto.slice(start, end + 1);
  }
  
  // Remove trailing commas problemáticas
  jsonStr = jsonStr.replace(/,\s*([\}\]])/g, '$1');
  
  try {
    return JSON.parse(jsonStr);
  } catch (e) {
    // Tentativa final: forçar aspas extras ou fechar array/objeto quebrado
  ...
  try {
    ...
    if (existente) {
      ...
      await prisma.iaConhecimento.update({
        ...
        data: { valor: valor.substring(0, MAX_CHARS_VALOR), confianca: novaConfianca, fonte, atualizadoEm: new Date(), usos: { increment: 1 } },
      });
    } else {
      await prisma.iaConhecimento.create({
        data: { categoria, chave: chave.substring(0, 120), valor: valor.substring(0, MAX_CHARS_VALOR), confianca, fonte },
      });
    }
...
export async function buscarConhecimento(categoria, limite = 20) {
  try {
    return await prisma.iaConhecimento.findMany({ where: { categoria, confianca: { gte: CONFIANCA_MINIMA } }, orderBy: [{ confianca: 'desc' }, { usos: 'desc' }], take: limite });
  } catch { return []; }
}
...
export async function buscarConhecimentoSemantico(palavras = [], limite = 12) {
  ...
  try {
    const condicoes = palavras.slice(0, 5).map(p => [ { chave: { contains: p, mode: 'insensitive' } }, { valor: { contains: p, mode: 'insensitive' } } ]).flat();
    const items = await prisma.iaConhecimento.findMany({ where: { OR: condicoes, confianca: { gte: CONFIANCA_MINIMA - 0.1 } }, orderBy: [{ confianca: 'desc' }, { usos: 'desc' }], take: limite });
...
// ═══════════════════════════════════════════════════════════════════════════════
// SEÇÃO 2 E 3 — APRENDIZADOS E TERMINAL
// ═══════════════════════════════════════════════════════════════════════════════
...
export async function salvarAprendizado({ pergunta, respostaTentativa, respostaFinal, aprovada, confianca = 0.5, motivo = '', userId = null }) {
  try {
    await prisma.iaAprendizado.create({
      data: { pergunta: pergunta.substring(0, 500), respostaTentativa: respostaTentativa.substring(0, 2000), respostaFinal: respostaFinal.substring(0, 2000), aprovada, confianca, motivo: motivo.substring(0, 200), usuarioId: userId ? parseInt(userId) : null },
    });
  } catch (e) {
...
export async function buscarAprendizadosSimilares(pergunta, limite = 4) {
  try {
    ...
    if (!palavras.length) return [];
    const condicoes = palavras.map(p => ({ pergunta: { contains: p, mode: 'insensitive' } }));
    return await prisma.iaAprendizado.findMany({ where: { OR: condicoes, aprovada: true, confianca: { gte: 0.6 } }, orderBy: [{ confianca: 'desc' }, { createdAt: 'desc' }], take: limite });
  } catch { return []; }
}
...
// ═══════════════════════════════════════════════════════════════════════════════
// SEÇÃO 5 — VALIDAÇÃO COM GEMINI (AUMENTADO MAX OUTPUT TOKENS)
// ═══════════════════════════════════════════════════════════════════════════════
...
Avalie e responda com JSON PURO:
{
  ...
  "aprendizados": ["fato 1 aprendido"]
    ...
    const response = await ai.models.generateContent({
        ...
        config:   { 
            temperature: 0.1, 
            maxOutputTokens: 3000, // Limite grandão para não cortar o JSON
...
// ═══════════════════════════════════════════════════════════════════════════════
// SEÇÃO 6 — MENTORIA AUTÔNOMA COM GEMINI (AUMENTADO MAX OUTPUT TOKENS)
// ═══════════════════════════════════════════════════════════════════════════════
...
Oriente a IA com JSON PURO:
{
  ...
  "acaoImediata": "acao"
    ...
    const response = await ai.models.generateContent({
      ...
      config:   { 
        temperature: 0.35, 
        maxOutputTokens: 4000, // Limite enorme para aguentar JSONs grandes
        responseMimeType: "application/json"
      },
      contents: [{ role: 'user', parts: [{ text: prompt }] }],
    });
...
export async function processarMensagemComAprendizado({ mensagem, userRole, userId, imageBase64 = null, imageMimeType = null, imageOnly = false, historyRaw = [], dataBlock = null }) {
  ...
  const palavrasChave = (mensagem || '').toLowerCase().split(/\s+/).filter(w => w.length > 3).slice(0, 6);
  const [conhecimentos, aprendizadosAnteriores] = await Promise.all([ buscarConhecimentoSemantico(palavrasChave, 12), buscarAprendizadosSimilares(mensagem || '', 3) ]);
