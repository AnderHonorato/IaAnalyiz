...
function buildDynamicReasoning(message, fileContexts, context, hasFiles) {
  ...
  if (hasFiles && fileContexts.length > 0) {
    ...
    const total = fileContexts.reduce((s, f) => s + (f.context?.length || 0), 0);
    lines.push(`📎 ${fileContexts.length} arquivo(s): ${nomes} — ${total.toLocaleString('pt-BR')} chars extraídos`);
  } else if (msg.length > 0) {
    lines.push(`💬 ${nome}: "${msg.substring(0, 90)}${msg.length > 90 ? '…' : ''}"`);
  }

  if (/site|website|landing page|html.*css|criar.*página/i.test(msg)) {
    const assunto = msg.match(/(petshop|restaurante|portfolio|clinica|blog|hotel|academia|loja|dashboard|login)/i)?.[1] || 'web';
    lines.push(`🖥️ Criação de página — ${assunto}`);
  } else if (/divergen|peso|frete|auditoria|reincidente/i.test(msg)) {
    lines.push(`📦 Logística ML — ${context.totalDivergencias || 0} divergência(s) ativa(s)`);
  } else if (/produto|sku|catálogo|estoque|kit/i.test(msg)) {
    lines.push(`🗂️ Catálogo — ${context.totalProdutos || 0} produto(s)`);
  } else if (/usuário|acesso|aprovar|bloquear|permissão/i.test(msg)) {
    const pend = (context.pendentes || []).length;
    lines.push(`👤 Gestão de usuários (${role})${pend > 0 ? ` — ${pend} pendente(s)` : ''}`);
  } else if (/preço|precific|valor|custo/i.test(msg)) {
    lines.push(`💰 Consulta de preços`);
  } else if (/resumo|relatório|métricas|dashboard/i.test(msg)) {
    lines.push(`📊 Visão geral — ${context.totalProdutos || 0} produtos, ${context.totalDivergencias || 0} divergências`);
  } else if (hasFiles) {
    lines.push(`📄 Análise de ${fileContexts[0]?.group || 'arquivo'}`);
  } else {
    lines.push(`💡 Resposta direta`);
  }

  if (needsWebSearch(msg)) {
    lines.push(`🌐 Pesquisa web ativada`);
  } else if (context.dataBlock) {
    lines.push(`🗄️ Contexto do sistema: ${context.dataBlock.length.toLocaleString('pt-BR')} chars`);
  } else if (/divergen|produto|usuário|preço|agendador/i.test(msg)) {
    lines.push(`⚙️ Consultando banco de dados via Function Calling`);
  } else {
    lines.push(`⚡ Respondendo com contexto da sessão`);
  }
