...
REGRA CRÍTICA DE PRECISÃO — NUNCA CONTRADIGA OS DADOS:
- Os dados na seção "DADOS DO SISTEMA" acima são a FONTE DA VERDADE absoluta
- Se os dados dizem que há X usuários bloqueados, você diz X — mesmo que o usuário insista no contrário
- Se os dados dizem "nenhum", você diz "nenhum" — mas explique o que você vê
- NUNCA invente ou contradiga os dados do banco
- Se o usuário disser "tem sim", responda: "Verificando os dados atuais do banco, [cite o que os dados mostram]. Pode ser que a informação tenha mudado recentemente."
- Respostas devem sempre ser baseadas nos dados acima, não em suposições`;
}
...
function ensureHTML(text) {
  ...
  // ETAPA 1: converte markdown [texto](url) -> tag <a>
  let result = text.replace(/\[([^\]]+)\]\((https?:\/\/[^)]+)\)/g,
    '<a href="$2" target="_blank">$1</a>');

  // ETAPA 2: remove artefatos soltos ]. ]( que o modelo gera
  result = result
    .replace(/\]\./g, '.')
    .replace(/\]\)/g, ')')
    .replace(/\]\s/g, ' ');

  // ETAPA 3: converte URLs nuas fora de tags HTML (sem reprocessar atributos)
  result = processURLsOutsideTags(result);

  // ETAPA 4: markdown residual
  result = result
    .replace(/\*\*(.+?)\*\*/g, '<b>$1</b>')
    .replace(/\*([^*\n<]+)\*/g, '<b>$1</b>')
...
function processURLsOutsideTags(html) {
  ...
  while (i < html.length) {
    if (html[i] === '<') {
      ...
      out += segment.replace(urlRe, (url) => {
        const clean = url.replace(/[.,;:!?]+$/, '');
        const trail = url.slice(clean.length);
        return '<a href="' + clean + '" target="_blank">' + urlLabel(clean) + '</a>' + trail;
      });
  ...
  try {
    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        ...
        temperature:       0.1,  // baixo para máxima precisão nos dados
        maxOutputTokens:   800,
        topP:              0.85,
        safetySettings: [
